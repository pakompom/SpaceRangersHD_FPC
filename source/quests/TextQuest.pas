...
type
  ...
  TTextQuest = class(TObjectEx)
    ...
    ChangeLogText: TTextField;
    EditorScreenWidth: Integer;
    EditorScreenHeight: Integer;
    EditorGridWidth: Integer;
    ...
    QuestSuccessGovMessageText: TTextField;
    ToStarText: TTextField;
    ToPlanetText: TTextField;
    DateText: TTextField;
    MoneyText: TTextField;
    FromPlanetText: TTextField;
    FromStarText: TTextField;
    ...
    procedure LoadFromReader(Reader: TBufEC; HeaderOnly: Boolean);
    procedure ResetEventIndices;
    function FindLocationIndex(LocationId: Integer): Integer;
    function FindPathIndex(PathId: Integer): Integer;
    procedure BuildLegacySequences;
    procedure FreeLegacySequences;
    procedure InferLegacyVisitLimits;

    function ExpandText(Text: WideString; Colorize: Boolean): WideString;
    function CheckCriticalParameters: Boolean;
    procedure Start(Money: Integer; PreserveExternalParameters: Boolean);
    procedure EnterLocation(LocationId: Integer);
    procedure FollowPath(PathId: Integer);
    procedure ShowEvent(Event: TEvent);
    procedure ShowOutcome;
...
begin
  ...
  QuestSuccessGovMessageText.Text :=
      TQuestMessages(MessageText.QuestMessages).GetText('GameContent.QuestSuccessGovMessage');
  QuestDescriptionText.Text :=
      TQuestMessages(MessageText.QuestMessages).GetText('GameContent.QuestDecription');
  ...
  FromPlanetText.Text :=
      TQuestMessages(MessageText.QuestMessages).GetText('GameContent.RFromPLanet');
  ...
  for i := GetLocationCount downto 1 do
    GetLocation(i).Free;
  Locations.Clear;
  for i := GetPathCount downto 1 do
    GetPath(i).Free;
  Paths.Clear;
  for i := GetParameterCount downto 1 do
    GetParameter(i).Free;
  Parameters.Clear;
...
begin
  ...
  if FormatVersion < 1111111125 then
    Reader.GetInt32;
  if FormatVersion >= 1111111120 then
    DefaultTraversalLimit := Reader.GetInt32;
  if FormatVersion >= 1111111121 then
    Difficulty := Reader.GetInt32;
  if FormatVersion >= 1111111125 then
    ParameterCount := Reader.GetInt32
  ...
  for i := 1 to ParameterCount do
    Parameters.Add(TParameter.Create(i));
  if FormatVersion >= 1111111125 then
    for i := 1 to ParameterCount do
  ...
  begin
    ...
    for i := 1 to PathCount do
      Paths.Add(TPath.Create);
    for i := 1 to LocationCount do
      Locations.Add(TLocation.Create);
...
var
  ...
  procedure CountLegacySequenceConnections(
      Location:
          TLocation; // @addr 0x4EA778 @ida "void __userpurge $name(TLocation *Location@<eax>, int *IncomingCount@<edx>, int *OutgoingCount@<ecx>, TPath **OutgoingPath@<^0>, TPath **IncomingPath@<^4>, void *ParentFrame@<^8>);" @stackpop 0x8 @calls "0x4EA8A1 0x4EA921 0x4EA997" @note "Returns zero counts for start, success, failure, or already grouped locations. Callee pops 8 bytes; caller pops ParentFrame."
      var IncomingCount, OutgoingCount: Integer;
      var IncomingPath, OutgoingPath: TPath
  );
  var
    i: Integer;
    Path: TPath;
  begin
    ...
    IncomingPath := nil;
    OutgoingPath := nil;
    if Location.IsStart then
      Exit;
    if Location.IsSuccess then
      Exit;
    if Location.IsFailure then
      Exit;
    if Location.Sequence <> nil then
      Exit;
    for i := 1 to GetPathCount do
    begin
      Path := GetPath(i);
  ...
  procedure PrependLegacySequencePaths(
      Path: TPath
  ); // @addr 0x4EA854 @ida "void __usercall $name(TPath *Path@<eax>, void *ParentFrame@<^0>);"
  var
  ...
  begin
    ...
    CountLegacySequenceConnections(
        Location,
        IncomingCount,
        OutgoingCount,
        IncomingPath,
    ...
    begin
      ...
      if IncomingCount = 1 then
        PrependLegacySequencePaths(IncomingPath);
    end;
  end;
  procedure AppendLegacySequencePaths(
      Path: TPath
  ); // @addr 0x4EA8D4 @ida "void __usercall $name(TPath *Path@<eax>, void *ParentFrame@<^0>);"
  var
    IncomingCount, OutgoingCount: Integer;
    Location: TLocation;
    IncomingPath, OutgoingPath: TPath;
  begin
    ...
    begin
      Sequence.AddLocation(Location);
      if OutgoingCount = 1 then
        AppendLegacySequencePaths(OutgoingPath);
    end;
  end;
begin
  for i := 1 to GetLocationCount do
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Limit := Limit + Path.TraversalLimit;
        end;
      end;
...
begin
  ...
  begin
    Text :=
        ReplaceAllWideString(
            Text,
            '[' + TrimWideString(GetParameter(i).NameText.Text) + ']',
            '[p' + IntToWideString(i) + ']'
        );
    Text :=
        ReplaceAllWideString(
  ...
  begin
    ...
    begin
      ...
      begin
        if Text[ContentStart + ContentLength] = ']' then
          Dec(Depth);
        if Text[ContentStart + ContentLength] = '[' then
          Inc(Depth);
        if Depth = 0 then
      ...
      if ContentStart >= Length(Text) then
        Break;
  ...
  begin
    Text :=
        ReplaceAllWideString(
            Text,
            '[p' + IntToWideString(i) + ']',
            ColorStart + IntToWideString(GetParameter(i).Value) + ColorEnd
        );
...
begin
  ...
  begin
    ...
    if Parameter.Enabled
        and (Parameter.CriticalOutcome <> qoNone)
        and (not Parameter.CriticalAtMinimum or (Parameter.Value <= Parameter.MinValue))
        and (Parameter.CriticalAtMinimum or (Parameter.Value >= Parameter.MaxValue)) then
      if Outcomes[Integer(Parameter.CriticalOutcome)] < 0 then
        Outcomes[Integer(Parameter.CriticalOutcome)] := i;
  end;
  if Outcomes[3] >= 0 then
    Selected := Outcomes[3]
  else if Outcomes[1] >= 0 then
    Selected := Outcomes[1]
  else if Outcomes[2] >= 0 then
    Selected := Outcomes[2]
  else
    Exit;
  Result := True;
  ...
  begin
    ...
    if TrimWideString(Event.Picture.Text) <> '' then
      OutcomeEvent.Picture.Text := Event.Picture.Text;
    if TrimWideString(Event.Sound.Text) <> '' then
      OutcomeEvent.Sound.Text := Event.Sound.Text;
    if TrimWideString(Event.Music.Text) <> '' then
  ...
  begin
    ...
    for i := 1 to GetParameterCount do
      ...
      begin
        ...
        if GetParameter(i).IsMoney and (Money >= 0) then
          GetParameter(i).Value := Money
        else if not PreserveExternalParameters
            or (FindTextPosW('ext_', GetParameter(i).NameText.Text) <> 1) then
...
begin
  ...
  begin
    ...
    begin
      ...
      TTextQuestInterface(Pointer(PtrUInt(PlayerInterface) + 0))
        ...
        begin
          ...
          begin
            ...
            if Path.Available then
              ...
              TTextQuestInterface(Pointer(PtrUInt(PlayerInterface) + 0)).AddDisabledPath(Caption);
          end;
        end;
        Chosen.Free;
        Group.Free;
...
begin
  ...
  begin
    ...
    if TrimWideString(Event.Picture.Text) <> '' then
      PlayerInterface.ShowPicture(TrimWideString(Event.Picture.Text));
    if TrimWideString(Event.Music.Text) <> '' then
      PlayerInterface.PlayMusic(TrimWideString(Event.Music.Text));
    if TrimWideString(Event.Sound.Text) <> '' then
      PlayerInterface.PlaySound(TrimWideString(Event.Sound.Text));
  end;
end;
  ...
  begin
    ...
    for i := 1 to GetParameterCount do
      if GetParameter(i).Enabled
          and not GetParameter(i).Hidden
          and ((GetParameter(i).Value <> 0) or GetParameter(i).ShowWhenZero) then
      begin
        ValueText := GetParameter(i).GetValueText(GetParameter(i).Value);
        ValueText := ReplaceAllWideString(ValueText, '<>', IntToWideString(GetParameter(i).Value));
