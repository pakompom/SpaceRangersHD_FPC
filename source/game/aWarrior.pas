...
type
  ...
  TWarrior = class(TNormalShip)
    ...
    procedure AssignFlagshipWeaponTargets;
    function FindNearestFriendlyFlagship: TShip;
    procedure ManeuverFlagship;
    procedure ReassignFlagshipHomePlanet;
    procedure MoveToRandomPlanetOrbit;
    function IsHomePatrolTurn: Boolean;
    procedure ConsumeNodes(Amount: Integer);
  end;
const
...
begin
  ...
  if Index >= 0 then
    HomePlanet.Warriors.Delete(Index);
  inherited;
end;
...
var
  ...
  Config: TBlockParEC;
  Weapon: TWeapon;
  function SelectTechLevel(
      Minimum,
      Maximum: Integer
  ): Integer; // @addr $517754 @ida "int __usercall $name@<eax>(int Minimum@<eax>, int Maximum@<edx>, void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link."
  begin
...
begin
  ...
  if WarriorType = wtFlagship then
    Dec(Galaxy.RangerSpawnQuotas[PilotRace]);
  Name := '';
  if (ModShipNameConfig <> nil) and (ModShipNameConfig.CountBlocks('Warrior') > 0) then
  begin
    Config := ModShipNameConfig.GetBlock('Warrior');
    if RaceToOwner(PilotRace) = OwnerId then
      Config := Config.GetBlock(OwnerToSys(OwnerId))
    else
    begin
      if Config.CountBlocks(OwnerToSys(OwnerId)) > 0 then
        Config := Config.GetBlock(OwnerToSys(OwnerId));
      if Config.CountBlocks(OwnerToSys(RaceToOwner(PilotRace))) > 0 then
        Config := Config.GetBlock(OwnerToSys(RaceToOwner(PilotRace)));
    end;
    LastNameIndex := 0;
    FirstNameIndex := Config.GetParamCount - 1;
    Name :=
        Config.GetParamValue(NextRandomIntRange(LastNameIndex, FirstNameIndex, RandomState))
            + ' '
            + '-'
  ...
  begin
    CreateAndEquipHull(
        Round(3 * HullBaseSize * EquipmentSizeFactors[5]),
        SelectTechLevel(3, 8),
  ...
  begin
    CreateAndEquipHull(
        ...
        Min(TechLevel, SelectTechLevel(1, 6)),
        RaceToOwner(PilotRace),
...
begin
  ...
  try
    ...
    if WarriorType = wtFlagship then
      SetMoney(Money + Max(1000, Min(5000, Galaxy.MaxRangerWealth div 15)));
    NextDayLogic;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if (CurrentPlanet <> HomePlanet) or (LiberationGroup <> nil) or IsHomePatrolTurn then
            OrderTakeoff;
        end
        else
          OrderTakeoff;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              else if LiberationGroup = nil then
              begin
                Stage := 15;
        ...
        begin
          ...
          begin
            ...
            Stage := 19;
            if Order = soNone then
              if IsHomePatrolTurn then
                MoveToRandomPlanetOrbit
              else if not NavigateToHomePlanet then
                MoveToRandomPatrolPoint;
            Stage := 20;
          end;
        end;
        ...
        begin
          ...
          begin
            ...
            SelectEnemyShipInStar;
            EngageEnemyShip;
            if (Order = soNone) and not NavigateToHomePlanet then
              MoveToRandomPatrolPoint;
          end;
        end;
      end;
      ...
      begin
        ...
        raise Exception.Create(
            'Error in procedure TWarrior.NextDayLogic '
                + GetFullName(' ')
                + ' label = '
                + IntToStr(Stage));
      end;
...
var
  ...
  var
    Needed, Available, Restored: Integer;
    Fraction: Single;
  ...
  begin
    ...
    begin
      ...
      begin
        Needed :=
            Ceil(
                (GetHull.Weight - GetHull.HullPoints)
                    * (GetCombatStatusStrength(cseBWRepairDebuff) * 0.002 + 1)
                    / 10
                    / 2
      ...
      begin
        ...
        begin
          ...
          if (not (Equipment is TWeapon)
              ...
              and (Equipment.ItemType <> t_Hull)
              and (Equipment.EquippedFlag <> 0) then
          begin
            if Threshold >= Equipment.ConditionPercent then
              NeedsRepair := True;
        ...
        begin
          ...
          begin
            ...
            if (not (Equipment is TWeapon)
                ...
                and (Equipment.EquippedFlag <> 0) then
            begin
              Equipment.ConditionPercent :=
                  RemapClamped(Fraction, 0, 1, Equipment.ConditionPercent, 100);
              if Equipment.ConditionPercent > 0 then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Order = soNone then
          begin
            if NextRandomIntRange(1, 100, RandomState) > 80 then
            begin
              Station := nil;
              ...
              begin
                ...
                begin
                  if PointDistanceSquared(Position, Ship.Position) >= Sqr(CalculateSpeed) then
                    Stations.Add(Ship);
                end;
              end;
              if Stations.Count > 0 then
                Station := Stations[NextRandomIntRange(0, Stations.Count - 1, RandomState)];
              Stations.Free;
              if Station <> nil then
            ...
            if (Order = soNone) and IsHomePatrolTurn then
              MoveToRandomPlanetOrbit;
            if (Order = soNone) and not NavigateToHomePlanet then
              MoveToRandomPatrolPoint;
          end;
      ...
      begin
        ...
        begin
          ...
          if Order = soNone then
          begin
            if NextRandomIntRange(1, 100, RandomState) > 40 then
            begin
...
begin
  if HomePlanet.CurrentStar = CurrentStar then
  begin
    OrderLanding(HomePlanet, False);
    Result := True;
  end
  else if (GetFuelTanks.Fuel = GetFuelTanks.Capacity)
      or (JumpRange * JumpRange
          >= PointDistanceSquared(HomePlanet.CurrentStar.Position, CurrentStar.Position)) then
  begin
...
begin
  ...
  begin
    Equipment := Inventory[I];
    if (Equipment.BrokenFlag <> 0) or (Equipment.ConditionPercent < 30) then
      Equipment.Repair;
  end;
...
begin
  ...
  begin
    if WarriorType = wtFlagship then
      Result :=
          LocalizedText(
                  'ShipType.' + OwnerToSys(RaceToOwner(PilotRace)) + '.' + GetTypeNameKey + 'Big')
              + Separator
              + Name
    else
      Result :=
...
begin
  Result := Byte(HomePlanet.RangerRelations[Galaxy.Rangers.IndexOf(TObject(Ranger) as TRanger)]);
end;
...
begin
  ...
  begin
    Result :=
        ((GetHull.HullPoints < GetHull.Weight * 0.3) and (GetHull.HullPoints < 200))
            or (GetHull.HullPoints < 130)
            or ((GetHull.Weight * 0.5 * OwnerInfo[OwnerId].FearThresholdScale > GetHull.HullPoints)
                and (EnemyShip <> nil)
...
begin
  Result :=
      (GetHull.Weight * 0.5 * OwnerInfo[OwnerId].FearThresholdScale > GetHull.HullPoints)
          and (ChanceToWin(Ship) - OwnerInfo[OwnerId].FearThresholdScale / 2 < 0);
end;
...
var
  ...
  Asteroid: TAsteroid;
  Missile: TMissile;
  SameRacePlanet: Boolean;
  Stage: Integer;
  IgnoreRanger: Boolean;
begin
  Stage := 0;
  if WarriorType = wtFlagship then
    ...
    try
          ...
          begin
            ...
            begin
              ...
              if (not (Weapon.GetWeaponInfo.ShotType in [wstTorpedo..wstRocket])
                ...
                if GetWeaponRange(Weapon) >= Distance then
                begin
                  Weapon.Target := Ship;
                  Inc(Assigned);
      ...
      if (EnemyShip <> nil)
          ...
          and EnemyShip.InNormalSpace then
      begin
        SameRacePlanet :=
            (EnemyShip.OrderTarget is TPlanet)
                and ((EnemyShip.OrderTarget as TPlanet).RaceId = HomePlanet.RaceId)
                and ((EnemyShip.OrderTarget as TPlanet).RelationToShip(EnemyShip)
                    <= RelationToShip(EnemyShip));
        IgnoreRanger :=
            (EnemyShip.TypeId = stRanger)
                and ((EnemyShip.OrderTarget = HomePlanet) or SameRacePlanet)
                and not EnemyShip.IsAttackingShip(Self);
        if IgnoreRanger then
          for I := 1 to EnemyShip.WeaponCount do
            if (EnemyShip.Weapons[I].Target <> nil)
                and (EnemyShip.Weapons[I].Target is TShip)
                and not (EnemyShip.Weapons[I].Target is TPirate) then
              if GetRelationLevelToShip(EnemyShip.Weapons[I].Target as TShip) > rlHostile then
              begin
                IgnoreRanger := False;
                Break;
              end;
                ...
                begin
                  ...
                  begin
                    ...
                    if IsEquipmentUsable(Weapon) then
                      if PointDistanceSquared(Position, Ship.Position)
                          <= Sqr(GetWeaponRange(Weapon)) then
                      begin
                        Stage := 38;
        ...
        if (Missile.Target = Self) and (Missile.OwnerShip <> Self) then
          ...
          begin
            ...
            if not (Weapon.GetWeaponInfo^.ShotType in [wstTorpedo..wstRocket])
                and (Weapon.Target = nil)
                and IsEquipmentUsable(Weapon) then
              if PointDistanceSquared(Position, Missile.Position)
                  <= Sqr(GetWeaponRange(Weapon)) then
              begin
              ...
              if CanSafelyDetonateItem(Item) then
                ...
                begin
                  ...
                  if not (Weapon.GetWeaponInfo^.ShotType in [wstTorpedo..wstRocket])
                      and (Weapon.Target = nil)
                      and IsEquipmentUsable(Weapon) then
                    if PointDistanceSquared(Position, Item.Position)
                        <= Sqr(GetWeaponRange(Weapon)) then
                    begin
                      Weapon.Target := Item;
                      Inc(Assigned);
      ...
      begin
        AppendLogLineThreadSafe(E.ClassName + ' ' + E.Message);
        raise Exception.Create(
            'Error in procedure TWarrior.ArmsToTarget '
                + GetFullName(' ')
                + ' label = '
                + IntToStr(Stage));
      end;
...
var
  Scores: array[1..5] of Single;
  Assigned, I, J: Integer;
  Ship: TShip;
  Weapon: TWeapon;
  Distance: Single;
  Item: TItem;
  ...
  Stage: Integer;
  IgnoreRanger: Boolean;
  procedure ScoreTarget(
      Ship: TShip
  ); // @addr $51B618 @ida "void __usercall $name(TShip *Ship@<eax>, void *ParentFrame@<^0>);"
  var
    J, Range: Integer;
    Score, Distance: Single;
    Weapon: TWeapon;
  begin
    Score := 0;
    for J := 1 to WeaponCount do
    begin
      ...
      begin
        ...
        begin
          if Score = 0 then
            Score :=
                Ship.CalculateAttackStrength
                    / Ship.CalculateDefenseStrength
                    * RemapClamped(Distance, 0, Range, 1.5, 1);
          if (Score > Scores[J]) or (Weapon.Target = nil) then
          begin
...
begin
  ...
  begin
    Ship := CurrentStar.Ships[I];
    if (Ship <> Self)
        and (Ship is TWarrior)
        and ((Ship as TWarrior).WarriorType = wtFlagship)
        and Ship.InNormalSpace then
      if Ship.GetRelationLevelToShip(Self) > rlHostile then
      begin
...
var
  I: Integer;
  Ship, BestShip: TShip;
  Chance, BestChance, Distance, TravelTime, BestDistance: Double;
  InFlagshipRange: Boolean;
  Range, MinRange, MaxRange: Integer;
  Score, BestScore, OwnAttack: Double;
  Weapon: TWeapon;
begin
  if WarriorType = wtFlagship then
  ...
  begin
    ...
    InFlagshipRange :=
        (Ship <> nil) and (PointDistanceSquared(Position, Ship.Position) < Sqr(Ship.GetRadarRange));
  end;
  if InFlagshipRange then
  begin
    BestScore := -1;
    BestShip := nil;
    MinRange := -1;
    ...
    begin
      ...
      begin
        Range := GetWeaponRange(Weapon);
        if (Range < MinRange) or (MinRange < 0) then
          MinRange := Range;
        if (Range > MaxRange) or (MaxRange < 0) then
          MaxRange := Range;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        if MaxRange > Distance then
          Score := Score * RemapClamped(Distance, MinRange, MaxRange, 1.5, 1)
        else
          Score :=
              Score
                  * Distance
                  / (Speed + 1)
                  * RemapClamped((Speed + 1) / (Ship.Speed + 1), 0.5, 2, 0.25, 1);
        if Ship is TKling then
        begin
          if (Ship as TKling).KlingType = ktBertor then
            Score := Score * 5;
          if (Ship as TKling).KlingType = ktKlig then
            Score := Score * 0.25;
        end;
    ...
    if BestShip <> nil then
      ...
      or (EnemyShip.ConsecutiveDockedDays > 3) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            EnemyShip := Ship;
            if ChanceToWin(Ship) > 1 then
              Exit;
          end
          else if (LiberationGroup = nil)
              or ((GetPlayer = Ship) and (Ship.CurrentStanding = ssPirateActive)) then
          begin
            Chance := ChanceToWin(Ship);
            TravelTime := PointDistance(Position, Ship.Position) / (Speed + 1) + 0.1;
            if Chance / TravelTime > BestChance / BestDistance then
            begin
              EnemyShip := Ship;
              BestChance := Chance;
  ...
  begin
    ...
    RetreatToFlagship :=
        (Ship <> nil)
            ...
            and not EnemyShip.HasNoUsableWeapons
            and (EnemyShip.EnemyShip = Self)
            and (EnemyShip.OrderTarget = Self)
            and (EnemyShip.Speed > Speed)
            and (Ship.GetRelationLevelToShip(EnemyShip) <= rlHostile);
    ...
    else if ChanceToWin(EnemyShip) > 0.5 then
    begin
...
var
  Enemies: TList;
  SupportWeight: Single;
  OwnMinRange, OwnMaxRange: Integer;
  OwnAttack: Single;
  Allies: TList;
  EnemiesInRange: Integer;
  AttackWeight, OwnDefense, FearWeight: Single;
  I: Integer;
  Ship: TShip;
  Candidate, Destination: TPointF;
  BestShip: TShip;
  BestShipScore, BestPositionScore, Score: Single;
  RandomDistance, Angle, AllyAreaScore, EnemyAreaScore: Integer;
  AllyRepair, EnemyRepair: Single;
  procedure GetWeaponRangeBounds(
      Ship: TShip;
      var Minimum, Maximum: Integer
  ); // @addr $51C9D4 @ida "void __usercall $name(TShip *Ship@<eax>, int *Minimum@<edx>, int *Maximum@<ecx>, void *ParentFrame@<^0>);"
  var
    I, Range: Integer;
    Weapon: TWeapon;
  begin
    ...
    begin
      ...
      begin
        ...
        if (Minimum > Range) or (Minimum < 0) then
          Minimum := Range;
        if (Maximum < Range) or (Maximum < 0) then
          Maximum := Range;
      end;
    end;
  end;
  function AreaWeaponScore(
      Ship: TShip
  ): Integer; // @addr $51CAA0 @ida "int __usercall $name@<eax>(TShip *Ship@<eax>, void *ParentFrame@<^0>);"
  var
    I: Integer;
    Weapon: TWeapon;
  begin
  ...
  function ProjectOutsideStar(
      Point: TPointF
  ): TPointF; // @addr $51CB78 @ida "void __usercall $name(TPointF *Point@<eax>, TPointF *Result@<edx>, void *ParentFrame@<^0>);"
  var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Sqr(Result.X) + Sqr(Result.Y) > Sqr(Radius) then
          Break;
        Inc(Attempts);
    ...
    else if Sqr(Radius) >= SquaredDistance then
    begin
  ...
  function EvaluatePosition(
      Point: TPointF
  ): Single; // @addr $51CCFC @ida "float __usercall $name@<st0>(TPointF *Point@<eax>, void *ParentFrame@<^0>);"
  var
    Ship: TShip;
    AttackPotential,
    SupportPotential,
    ...
    Risk,
    Threat,
    BestEnemyPotential: Single;
    Distance, HookRange: Single;
    Minimum, Maximum: Integer;
    ShipDistance, Attack: Single;
    I: Integer;
    Item: TItem;
  begin
    AttackPotential := 0;
    ...
    begin
      ...
      Attack :=
          Ship.CalculateAttackStrength
              * (1 + AreaWeaponScore(Ship) * (0.1 * RemapClamped(SupportWeight, 0, 2, 0.1, 1)));
      Threat := Attack * RemapClamped(ShipDistance, Minimum, Maximum, 1, 0);
      if Attack * 0.99 > Threat then
        Threat := Threat + (Attack - Threat) * Ship.Speed / (Ship.Speed + ShipDistance - Minimum);
      if Ship.OrderTarget = Self then
        Threat := Threat * RemapClamped(Speed / Max(1, Ship.Speed), 0.5, 1, 0, 1);
      IncomingStrength := IncomingStrength + Threat;
      Threat := Attack * RemapClamped(ShipDistance, OwnMinRange, OwnMaxRange, 1, 0);
      if Attack * 0.99 > Threat then
        Threat := Threat + (Attack - Threat) * Speed / (Speed + ShipDistance - OwnMinRange);
      Threat :=
          Threat
              / (Max(OwnAttack, Ship.CalculateDefenseStrength)
                  * (1
                      + Ship.GetRepairStrengthFactor
                          * (0.1 * RemapClamped(SupportWeight, 0, 2, 1, 0.1))));
      if (Ship is TKling) and ((Ship as TKling).KlingType = ktBertor) then
        Threat := 1.5 * Threat;
      if Threat > BestEnemyPotential then
        BestEnemyPotential := Threat;
      AttackPotential := AttackPotential + 0.1 * Threat;
    end;
    AttackPotential := AttackPotential + BestEnemyPotential;
    for I := 0 to Allies.Count - 1 do
    begin
      Ship := Allies[I];
      GetWeaponRangeBounds(Ship, Minimum, Maximum);
      ShipDistance := PointDistance(Point, Ship.Position);
      ...
      SupportPotential :=
          SupportPotential
              + Attack
                  * RemapClamped(ShipDistance, OwnMinRange, OwnMaxRange, 1, 0)
                  / Max(OwnAttack, Ship.CalculateDefenseStrength);
      Threat := Attack * RemapClamped(ShipDistance, Minimum, Maximum, 1, 0);
      Attack := Attack - Threat;
      if Attack > Threat * 0.01 then
        Threat := Threat + Attack * Ship.Speed / (Ship.Speed + ShipDistance - Minimum);
    ...
    if GetCargoHook <> nil then
      ...
      begin
        Item := CurrentStar.Items[I];
        if (Item is TProtoplasm)
            and CanCargoHookHandleItem(Item, Self)
            and not IsItemInPickupRange(Item) then
        begin
          Distance := Sqrt(Sqr(Point.X - Item.Position.X) + Sqr(Point.Y - Item.Position.Y));
          HookRange := GetCargoHookRange;
          if Distance <= HookRange then
            NodeValue := NodeValue + Min((Item as TProtoplasm).StackCount, CargoFreeSpace)
          else
            NodeValue :=
                NodeValue
                    + Min((Item as TProtoplasm).StackCount, CargoFreeSpace)
    ...
    begin
      Benefit :=
          AttackPotential * AttackWeight + 0.05 * NodeValue + SupportPotential * SupportWeight;
    ...
    Benefit :=
        Benefit
            * RemapClamped(
                Distance,
                CurrentStar.MapDiameter * 0.7,
                CurrentStar.MapDiameter * 1.2,
                1,
                0.5);
    if Distance > CurrentStar.MapDiameter * 1.2 then
      Benefit := CurrentStar.MapDiameter * Benefit * 1.2 / Distance;
    Result := Benefit - Risk;
  end;
  ...
  begin
    ...
    begin
      ...
      while (50 - 90 * Factor > GetCombatStatusStrength(cseBWBuff))
          and (GetCarriedNodeCount >= Int64(50)) do
      begin
        ConsumeNodes(50);
...
begin
  ...
  begin
    ...
    if (Ship <> Self) and Ship.InNormalSpace then
      if GetRelationLevelToShip(Ship) <= rlHostile then
        Enemies.Add(Ship)
      else if (GetRelationLevelToShip(Ship) > rlNormal)
          and ((Ship is TWarrior)
              or ((Ship.EnemyShip <> nil)
                  and (GetRelationLevelToShip(Ship.EnemyShip) <= rlHostile))) then
        Allies.Add(Ship);
  end;
  GetWeaponRangeBounds(Self, OwnMinRange, OwnMaxRange);
  OwnAttack := CalculateAttackStrength;
  OwnDefense := CalculateDefenseStrength;
  if (Enemies.Count <= 2) or (Allies.Count <= 0) or (OwnMaxRange < 300) then
  begin
    Enemies.Free;
    Allies.Free;
    EngageEnemyShip;
  ...
  begin
    ...
    if PointDistanceSquared(Position, Ship.Position) < Sqr(OwnMaxRange) then
      Inc(EnemiesInRange);
  end;
  for I := 0 to Allies.Count - 1 do
  begin
  ...
  AttackWeight := 1;
  SupportWeight :=
      RemapClamped(AllyAreaScore * EnemyRepair - EnemyAreaScore * AllyRepair, -100, 100, -2, 2);
  ...
  begin
    Ship := Allies[I];
    Score :=
        EvaluatePosition(Ship.Position)
            / (Trunc(PointDistance(Ship.Position, Position) / (Speed + 1)) + 1);
    if (Score > BestShipScore) or (BestShip = nil) then
    begin
      BestShipScore := Score;
      BestShip := Ship;
    end;
  ...
  begin
    ...
    Score :=
        EvaluatePosition(Candidate) / (Trunc(PointDistance(Candidate, Position) / (Speed + 1)) + 1);
...
var
  ...
  Star, NearbyStar, BestStar: TStar;
  I, J, K: Integer;
  FlagshipFactor, GarrisonStrength, NearbyThreat: Single;
  Ship: TShip;
  Warrior: TWarrior;
...
begin
  ...
  begin
    ...
    if (Star.ControlFaction = sfCoalition)
        ...
        and (Star.ConstellationGraphIndex <> 20) then
    begin
      FlagshipFactor := 1;
      GarrisonStrength := 0;
      RacePlanetCount := 0;
      ...
      if (GarrisonStrength <> 0) and (RacePlanetCount <> 0) then
      begin
        NearbyThreat := 0;
        for J := 1 to Galaxy.Stars.Count div 5 do
        begin
          NearbyStar := TObject(Star.StarDistances[J].Star) as TStar;
          if NearbyStar.ConstellationGraphIndex <> 20 then
            if NearbyStar.Status.CustomFaction <> '' then
              NearbyThreat := NearbyThreat + 5 / (J + 5)
            else
              case NearbyStar.ControlFaction of
                sfCoalition:;
        ...
        Score :=
            RacePlanetCount * GarrisonStrength * NearbyThreat * FlagshipFactor * (StationCount + 3);
        if HomePlanet.CurrentStar = Star then
          Score := Score * 1.3;
        if (Score > BestScore) or (BestStar = nil) then
        begin
          BestScore := Score;
          BestStar := Star;
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            Inc(WarriorCount);
            if Warrior.WarriorType = wtFlagship then
              FlagshipFactor := FlagshipFactor * 0.1;
          end;
        end;
        Score := WarriorCount * FlagshipFactor;
        if HomePlanet = Planet then
          Score := Score * 1.3;
        if (Score > BestScore) or (BestPlanet = nil) then
        begin
          BestScore := Score;
          BestPlanet := Planet;
        end;
...
function TWarrior.BuildMoneyExtortionResponse(
    OtherShip: TShip;
    var Response: WideString;
    DemandedAmount: Integer
): Boolean;
begin
  ...
  if (GetPlayer <> OtherShip) and ((EnemyShip = nil) or (EnemyShip.CurrentStar <> CurrentStar)) then
    EnemyShip := OtherShip;
...
function TWarrior.BuildTrucePaymentResponse(
    OtherShip: TShip;
    var Response: WideString;
    OfferedAmount: Integer
): Boolean;
var
  ...
  begin
    ...
    if OtherShip is TNormalShip then
      TNormalShip(OtherShip).CurrentSystemKills.Normal := 0;
    if OtherShip is TRanger then
    begin
      ...
      begin
        Ship := CurrentStar.Ships[I];
        if (Ship is TWarrior) and ((Ship as TWarrior).HomePlanet = Planet) and (Self <> Ship) then
        begin
          if OtherShip = Ship.EnemyShip then
            Ship.EnemyShip := nil;
          for J := 1 to Ship.WeaponCount do
          begin
            Weapon := Ship.Weapons[J];
            if OtherShip = Weapon.Target then
              Weapon.Target := nil;
          end;
          // Native repeats this check after clearing weapon targets.
          if OtherShip = Ship.EnemyShip then
            Ship.EnemyShip := nil;
          if OtherShip.EnemyShip = Ship then
            OtherShip.EnemyShip := nil;
          Ship.ChangeRelationToRanger(OtherShip, 30);
          if (Ship.Order = soFollowShip) and ((Ship.OrderTarget as TShip) = OtherShip) then
          begin
...
begin
  ...
  else if Wealth * (1 / 15) < OfferedAmount then
  begin
    Response := LookupVisibleTalkText('Talk.Truce.' + GetTypeNameKey + 'Ok', OtherShip);
    AcceptPayment;
    Result := True;
...
function TWarrior.BuildAttackRequestResponse(
    Requester: TShip;
    var Response: WideString;
    Target: TShip
): Boolean;
var
  I, HostileCount: Integer;
  Ship: TShip;
...
begin
  ...
  if (OrderTarget = Target)
      and (GetRelationLevelToShip(Target) = rlHostile)
      and (HostileCount <= 1) then
    AcceptRequest
  else if (LiberationGroup <> nil) and (Requester.LiberationGroup <> LiberationGroup) then
  ...
  begin
    ...
    Response := LookupVisibleTalkText('Talk.Attack.' + GetTypeNameKey + 'Suspect', Requester)
      ...
      and (Requester is TNormalShip)
      and ((Requester as TNormalShip).Rank < Rank) then
    Response := LookupVisibleTalkText('Talk.Attack.' + GetTypeNameKey + 'HaveBusiness', Requester)
  else if HostileCount > 0 then
    Response := LookupVisibleTalkText('Talk.Attack.' + GetTypeNameKey + 'HaveBusiness', Requester)
  else
    AcceptRequest;
end;

function TWarrior.AcceptPartnershipOffer(
    OtherShip: TShip;
    var Response: WideString;
    PaymentAmount: Integer
): Boolean;
...
function TWarrior.BuildPartnershipOfferResponse(
    OtherShip: TShip;
    var Response: WideString;
    PaymentAmount: Integer
): Boolean;
...
begin
  ...
  FragilityScale := 1.5;
  if Item.ItemType in [t_FuelTanks, t_Radar, t_CargoHook] then
  ...
  case BonusKind of
    ...
    bonDroid:
      Result :=
          Value
              * 10
              / Max(0.1, GetHull.GetFragilityFactor(NoFlags))
              * (1 + ShortInt(WarriorType = wtFlagship) * 0.5);
    bonHook:
      Result :=
          (Min(Value, HullBaseSize * EquipmentSizeFactors[5]) + Value * 0.1)
              * 1.3
              * ShortInt(WarriorType = wtFlagship);
    ...
    bonWRadius:
      if WarriorType = wtFlagship then
        ...
        Result := Value * 1.5 * Sqr(Max(100, SmoothedEnemySpeed) / Max(100, SmoothedSpeed));
...
function TWarrior.EvaluateWeaponDamage(
    Weapon: TWeapon;
    IncludeAdditiveBonuses: Boolean;
    BaseDamage: Single
): Single;
...
begin
  ...
  if WarriorType = wtFlagship then
  begin
    if (Item.ItemType = t_Protoplasm) and (CargoFreeSpace > 0) then
    begin
      Result := True;
      Exit;
    end
