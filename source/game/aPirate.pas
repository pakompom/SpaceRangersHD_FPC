...
type
  TPirate = class;
  TPirate = class(TNormalShip)
    PrisonTermRemaining: Cardinal;
    PirateType: Byte;
    Gap515: array[0..2] of Byte;
    RaidPressure: Single;
    ...
    function NavigateToServicePlanet(Absolute: Boolean): Boolean;
    function SelectServicePlanet: TPlanet;
    procedure TryJumpToNearbyBattle(UnusedMode: Byte);
    function TryDockAtStation(Types: TShipTypeMask): Boolean;
    procedure SelectNearestReachableDestination;
    procedure SellAllCargoGoods;
    procedure ProcessUnseenProgression;
    procedure TryOfferRansomToPursuer;
    function ProcessImprisonment: Boolean;
    procedure SelectIncidentalEnemy;
    procedure ReviewPartnership;
    procedure MoveToRandomPlanetOrbit;
    function TryRetreatFromSystem: Boolean;
  end;
const
  PirateSkillBonusWeights: array[22..27] of Integer = (90, 90, 90, 90, 30, 5);
  PirateSlotBonusWeights: array[13..20] of Integer = (100, 150, 150, 400, 150, 100, 20, 100);
...
var
  ...
  procedure SelectUniqueName(
      Config: TBlockParEC
  ); // @addr $50CC78 @ida "void __usercall $name(TBlockParEC *Config@<eax>, void *ParentFrame@<^0>);" @note "Nested helper with caller-popped static link."
  var
    ...
    Block: TBlockParEC;
  begin
    if (Config <> nil) and (Config.CountBlocks('Pirate') <> 0) then
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Ship := Star.Ships[J];
            if (Self <> Ship) and (Ship.TypeId = stPirate) and ((Ship as TPirate).Name = Name) then
            begin
              Duplicate := True;
              Break;
            end;
...
begin
  HasHull := GetHull <> nil;
  if HasHull or (Planet.OwnerId = Byte(oiPirate)) then
  ...
  begin
    ...
    if GetHull.OwnerId in TOwnerMask(PlanetOwnerMasks.Coalition) then
      PilotRace := OwnerToRace(GetHull.OwnerId)
    else if HomePlanet.IsMainPiratePlanet then
      PilotRace := OwnerToRace(PickRandomEquipmentOwner(HomePlanet.RandomState))
    else
      PilotRace := HomePlanet.RaceId;
  end
  else
  begin
    if HomePlanet.IsMainPiratePlanet then
      PilotRace := OwnerToRace(PickRandomEquipmentOwner(HomePlanet.RandomState))
    else
      PilotRace := HomePlanet.RaceId;
    if HomePlanet.OwnerId = Byte(oiPirate) then
      OwnerId := Byte(oiPirate)
    else
  ...
  begin
    ...
    begin
      ...
      if PirateRank > 3 then
        PirateRank := 3;
      AddPirateRankPoints(
          NextRandomIntRange(0, PirateRankPointThresholds[PirateRank] div 2, RandomState)
      );
    end;
  end;
...
begin
  ...
  if LoadedSaveVersion >= 79 then
    RaidPressure := Buffer.GetSingle
  else
    RaidPressure := 0;
end;
...
begin
  ...
  try
    ...
    begin
      Stage := 1;
      if not (CurrentPlanet.OwnerId in [Ord(oiMaloc)..Ord(oiGaal), Ord(oiPirate)])
          or (ClanShip
              and not (CurrentPlanet.OwnerId in TOwnerMask(PlanetOwnerMasks.PirateClan))) then
      begin
        OrderTakeoff;
        Exit;
      end;
      begin
        Stage := 2;
        if not ClanShip then
          if ProcessImprisonment then
            Exit;
    ...
    begin
      ...
      begin
        ...
        ReloadWeaponAmmo;
        TrainSkillsAutomatically;
        if RepairHullAtLocation and not ClanShip then
          Exit;
        Stage := 7;
        BuyEquipmentAtLocation(False);
        RestoreEssentialEquipment;
        ...
        if DockedTo.InNormalSpace
            and ((TRuins(DockedTo).FlyToStar = nil)
                or (TRuins(DockedTo).FlyToStar = CurrentStar)) then
          OrderTakeoff
        else
        begin
          if ((TRuins(DockedTo).FlyToStar = nil)
                  or (TRuins(DockedTo).FlyToStar.Dominion <> DockedTo))
              and ((DockedTo.OrderTarget = nil)
                  or not (DockedTo.OrderTarget is TStar)
                  or (TStar(DockedTo.OrderTarget).Dominion <> DockedTo)) then
            RaidPressure := 0.05 + RaidPressure;
          OrderNone(False);
        end;
      end;
      Exit;
    ...
    begin
      ...
      Stage := 11;
      if TryRetreatFromSystem then
        Exit;
      if InFear and ((CurrentStar.ControlFaction = sfPirates) or not ClanShip) then
      begin
        Stage := 12;
        if not (Order in [soLand, soJump]) and not OrderAbsolute then
        begin
          Stage := 13;
          if ClanShip then
          begin
            Planet := SelectNearestQueuedPlanet;
            if (Planet <> nil) and (Planet.CurrentStar = CurrentStar) then
              OrderLanding(Planet, True)
            else
            begin
              Station :=
                  FindNearestDockableStation(
                      TStationStandingMask(NonTargetableStationStandingMasks[Ord(sfPirates)])
                  );
              if Station <> nil then
          ...
          begin
            if NextRandomUnitFloat(RandomState) > 0.8 then
              TryDockAtStation([6..13]);
            if Order <> soLand then
              if NextRandomUnitFloat(RandomState) > 0.66 then
            ...
            if (Order in [soLand, soJump])
              ...
              if JettisonCargoGoodsTowardTargetValue(
                  Max(200, Galaxy.ComputeScaledMiniMoney(OwnerId))) then
                NotifyFearCargoDrop(EnemyShip);
          end;
        end;
        Stage := 14;
        if Order in [soLand, soJump] then
          UpdateAfterburnerState
        else if ClanShip then
          EngageEnemyShip
        else if Order = soNone then
        begin
          SelectEnemyShipInStar;
          EngageEnemyShip;
        end;
      ...
      begin
        ...
        begin
          ...
          if (EnemyShip <> nil)
            ...
            OrderNone(False);
          if not OrderAbsolute and not Collecting then
          begin
            Stage := 20;
            SelectEnemyShipInStar;
            EngageEnemyShip;
      ...
      if ClanShip then
      begin
        if (Order = soNone) and HasHullDamageOrBrokenEquippedItems then
          NavigateToServicePlanet(False);
        Stage := 22;
        if (Order = soNone)
            and not TryCollectBestFloatingItem(50)
            and ((GetDesiredCargoFreeSpace > CargoFreeSpace)
                or (NextRandomIntRange(1, 10, RandomState) <= 2)) then
          NavigateToQueuedPlanet(True);
        if (Order = soMove)
            and (PickupTargets = nil)
            and (NextRandomIntRange(1, 10, RandomState) <= 3) then
          TryCollectBestFloatingItem(50);
        Stage := 23;
        if (Order = soNone) and (NextRandomIntRange(1, 10, RandomState) <= 2) then
        begin
          Station :=
              FindNearestDockableStation(
                  TStationStandingMask(NonTargetableStationStandingMasks[Ord(sfPirates)])
      ...
      Stage := 25;
      if (CurrentStar.Dominion <> nil)
          and TShip(CurrentStar.Dominion).CanDock(Self)
          and (TRuins(CurrentStar.Dominion).FlyToStar <> nil)
          and (CurrentStar.Battle = 0) then
      begin
        Count := 0;
        ...
        begin
          ...
          if not Ship.InHyperspace
              ...
              and (Ship <> Self)
              and (Ship.DockedTo <> CurrentStar.Dominion) then
            Inc(Count);
        end;
        Stage := 26;
        if Count >= 3 then
        begin
          ...
          if Galaxy.CurrentTurn + EstimateOrderTravelTurns + 1
              > TRuins(CurrentStar.Dominion).FlyDate then
            UpdateAfterburnerState;
        end;
      end;
      Stage := 27;
      if (CurrentStar.Dominion <> nil)
          ...
          and (Order in [soNone, soMove]) then
        for I := 0 to CurrentStar.Ships.Count - 1 do
        begin
          Ship := CurrentStar.Ships[I];
    ...
    begin
      ...
      raise Exception.Create(
          'Error in procedure TPirate.NextDayLogic '
              + GetFullName(' ')
              + ' label = '
              + IntToStr(Stage));
    end;
      ...
      begin
        ...
        begin
          ...
          if (Planet.OwnerId in [Ord(oiMaloc)..Ord(oiGaal), Ord(oiPirate)])
              and CanQueueReachablePlanet(Planet) then
            PlanetQueue.Add(Planet);
        end;
      end;
    end;
...
begin
  Result :=
      (Planet.OwnerId <> Byte(oiDominator)) and ((PirateType = 0) or (Planet.OwnerId = OwnerId));
end;
...
begin
  ...
  begin
    ...
    if (Star.Battle <> 0)
        ...
        and (Star.Ships.Count - 3 >= Star.ShipTypeCounts[stKling])
        and (Star.Ships.Count < 13) then
    begin
...
begin
  ...
  begin
    Ship := CurrentStar.Ships[I];
    if (Ship.TypeId in Types) and Ship.InNormalSpace and Ship.CanDock(Self) then
    begin
...
begin
  ...
  begin
    ...
    else if (PartnerShip.Order = soJump)
  ...
  begin
    ...
    begin
      ...
      if BestTurns > CandidateTurns then
      begin
  ...
  if (Target <> nil) and (Target <> OrderTarget) then
    if Target is TPlanet then
      ...
      OrderJump(Target as TStar, False);
end;
  ...
  begin
    ...
    begin
      ...
      if (not (Equipment is TWeapon)
          ...
          and ((Equipment.BrokenFlag <> 0) or (Equipment.ConditionPercent < 30))
...
begin
  ...
  begin
    ...
    Result := LocalizedText('ShipType.' + RaceName + '.' + GetTypeNameKey) + Separator + Name;
end;

function TPirate.GetGreetingShipCategory: Byte;
begin
  if OwnerId = Byte(oiPirate) then
    Result := gscPirateClan
  else
    Result := gscPirate;
end;
  ...
  if PirateType <> 0 then
    Result :=
        Trunc(
            RemapClamped(
                GetHull.Weight,
                HullBaseSize * EquipmentSizeFactors[5],
...
var
  Award: Byte;
  ProgressFactor, StrengthFactor: Double;
begin
  if (DaysSincePlayerSeen >= 60) and (GetPlayer <> nil) then
    ...
    begin
      ...
      if (0.1 * ProgressFactor > NextRandomUnitFloat(RandomState))
          and ((0.4 * StrengthFactor > StrengthInBestRanger)
              or (NextRandomUnitFloat(RandomState) < 0.1)) then
      ...
      if (0.05 * ProgressFactor > NextRandomUnitFloat(RandomState))
          and ((0.3 * StrengthFactor > StrengthInBestRanger)
              or (0.7 * StrengthFactor > StrengthInAverageRanger)
              or (0.001 * ProgressFactor > NextRandomUnitFloat(RandomState))) then
        GenerateExtraWeapon;
      if 0.05 * ProgressFactor > NextRandomUnitFloat(RandomState) then
        GainExperience(SeededRandomIntRange(100, 500, RandomState), 0);
      if (GetPlayer.PirateRank > PirateRank)
          and (NextRandomUnitFloat(RandomState) < 0.01)
          and (PirateRank < 4) then
      begin
      ...
      if (NextRandomUnitFloat(RandomState) < 0.02)
          ...
          and ((AwardIds = nil) or (2 * (Rank + 1) > AwardIds.Count)) then
      begin
        Award :=
            SelectAward(
                RaceToOwner(CurrentPlanet.RaceId),
                [atAccomplishment, atSecretMission, atPerfidy],
    ...
    begin
      if (NextRandomUnitFloat(RandomState) < 0.2)
          and ((StrengthInBestRanger < 0.7) or (NextRandomUnitFloat(RandomState) < 0.3)) then
        if (NextRandomFloatRange(0, 0.7, RandomState) > WealthInBestRanger) and (Money < 25000) then
          SetMoney(Money + Galaxy.ComputeScaledBigMoney(2))
        else
          ImproveRandomEquipment(True);
      if (NextRandomUnitFloat(RandomState) < 0.05)
          and ((StrengthInBestRanger < 0.5)
              or (StrengthInAverageRanger < 1)
              or (NextRandomUnitFloat(RandomState) < 0.001)) then
        GenerateExtraWeapon;
      if NextRandomUnitFloat(RandomState) < 0.2 then
        GainExperience(NextRandomIntRange(250, 1000, RandomState), 0);
      if (OwnerId = Byte(oiPirate))
          and (GetPlayer.PirateRank > PirateRank)
          and (NextRandomUnitFloat(RandomState) < 0.01)
          and (PirateRank < 4) then
      begin
        AddRankPoints(NextRandomIntRange(16, 32, RandomState));
      ...
      if (GetPlayer.Rank > Rank) and (NextRandomUnitFloat(RandomState) < 0.1) and (Rank < 4) then
      begin
        AddRankPoints(NextRandomIntRange(10, 20, RandomState));
        TryPromoteRank;
      end;
      if (NextRandomUnitFloat(RandomState) < 0.03)
          and (CurrentPlanet <> nil)
          and ((AwardIds = nil) or (2 * (Rank + 1) > AwardIds.Count)) then
      begin
        Award :=
...
begin
    ...
    case Ship.TypeId of
      stTransport:
        if (CurrentStar.ControlFaction = sfCoalition)
            and (CurrentStar.Status.CustomFaction = '')
            and (CurrentStar = Ship.CurrentStar)
            and (TruceShip <> Ship)
            and (Ship.TruceShip <> Self)
            and not IsHullDestroyed
            and not AcceptsRansomDemandFrom(Ship) then
          Result := 0
        else
          Result := Min(50, Max(20, OwnerRelations[OwnerId, Ship.OwnerId] - 20));
      stPirate:
        if Ship.OwnerId = Byte(oiPirate) then
          Result := Min(100, 100 + SeededRandomIntRange(-20, 20, Seed + Ship.Seed))
        else
          Result := Max(50, OwnerRelations[OwnerId, Ship.OwnerId]);
      stWarrior:
        Result :=
            Min(
                50,
  ...
  begin
    if (CurrentStanding = ssPirateActive)
        and (Ship.CurrentStar.ControlFaction = sfPirates)
        and (Ship.CurrentStar.Status.CustomFaction = '')
    ...
    case Ship.TypeId of
      ...
      stKling, stTranclucator: Result := 50;
      Ord(rstRangerCenter)..Ord(rstCustomStation):
        if (Ship.CurrentStanding in [ssCoalitionMilitary, ssCoalitionActive])
...
begin
  if (Attacker is TTranclucator)
    ...
    ReactToAttack(TTranclucator(Attacker).OwnerShip);
  EnemyShip := Attacker;
  if Attacker is TRanger then
  begin
    ChangeRelationToRanger(Attacker, -10);
    if (CurrentStanding in [ssPirateActive, ssPirateMilitary])
        and (Attacker.CurrentStanding in [ssCoalitionMilitary, ssCoalitionActive])
        and (MainPiratePlanet <> nil)
        and (MainPiratePlanet.OwnerId = Byte(oiPirate)) then
    begin
  ...
  if Attacker.PartnerShip <> nil then
    if Attacker.PartnerShip.TypeId = stRanger then
    begin
      ChangeRelationToRanger(Attacker.PartnerShip, -5);
      if (CurrentStanding in [ssPirateActive, ssPirateMilitary])
          and (Attacker.PartnerShip.CurrentStanding in [ssCoalitionMilitary, ssCoalitionActive])
          and (MainPiratePlanet <> nil)
          and (MainPiratePlanet.OwnerId = Byte(oiPirate)) then
      begin
...
begin
  ...
  if (OwnerId = Byte(oiPirate)) and (PirateType <> 0) then
    Result :=
        ((GetHull.HullPoints < GetHull.Weight * 0.15) and (GetHull.HullPoints < 100))
            or (GetHull.HullPoints < 65)
            or ((GetHull.Weight * 0.25 * OwnerInfo[OwnerId].FearThresholdScale > GetHull.HullPoints)
                and (EnemyShip <> nil)
                and (EnemyShip.OrderTarget = Self)
                and (ChanceToWin(EnemyShip) - OwnerInfo[OwnerId].FearThresholdScale / 2 < 0))
  else
  begin
    Result :=
    ...
    begin
      ...
      if (EnemyCount - 1) * Threat * 0.33 + Threat > Tolerance then
        Result := True;
    end;
  end;
  InFear := Result;
  if InFear then
    RaidPressure := 0.5 * RaidPressure;
end;
...
begin
  if InNormalSpace
      ...
      and not NoTalk
      and not EnemyShip.NoTalk
      and ((OwnerId <> Byte(oiPirate))
          or (PirateType = 0)
          or not (EnemyShip.CurrentStanding in [ssCoalitionMilitary, ssCoalitionActive])) then
    if (EnemyShip.GetMaxWeaponRange * EnemyShip.GetMaxWeaponRange
            >= PointDistanceSquared(Position, EnemyShip.Position))
        and (((Galaxy.CurrentTurn * EnemyShip.Id mod 3 = 0)
                and (NextRandomUnitFloat(RandomState) > 0.2))
            or (GetHullIntegrityPercent < 20)) then
    begin
      LowOffer := Min(Money, GetWealthScaledAmount(1));
      HighOffer :=
          Min(Money, (GetWealthScaledAmount(3) + EnemyShip.GetWealthScaledAmount(4)) * 0.5);
      Amount :=
...
begin
  Result :=
      (GetHull.Weight * 0.6 * OwnerInfo[OwnerId].FearThresholdScale > GetHull.HullPoints)
          and (OwnerInfo[OwnerId].FearThresholdScale / 2 > ChanceToWin(Ship) * (1 + RaidPressure));
  if Result and (EnemyShip = Ship) then
    RaidPressure := 0;
end;
...
var
  ...
  begin
    ...
    ReplaceTextToken(
        Text,
        '<Month>',
        IntToStr(Int64(PrisonTermRemaining div 30)),
        '<color=255,240,100>'
    );
...
begin
  ...
  begin
    if ((CurrentStar.Battle <> 0)
            and (CurrentStar.LastDominatorPresenceTurn >= Galaxy.CurrentTurn - 1))
        or (CurrentPlanet.OwnerId = Byte(oiPirate)) then
    begin
      PrisonTermRemaining := 0;
      RefreshCurrentStanding;
      Result := False;
  ...
  else if (CurrentPlanet.OwnerId <> Byte(oiPirate))
      and ((CurrentStar.Battle = 0)
          or (CurrentStar.LastDominatorPresenceTurn < Galaxy.CurrentTurn - 1)) then
  begin
    for I := 0 to CurrentPlanet.Warriors.Count - 1 do
    begin
      Ship := CurrentPlanet.Warriors[I];
    ...
    if (NextRandomUnitFloat(RandomState) < 0.01)
        or (CurrentPlanet.GetRelationLevelToShip(Self) = rlHostile) then
    begin
      Imprison;
      RefreshCurrentStanding;
    end;
...
begin
  ...
  if Galaxy.GetAIJunkToleranceLevel < CurrentStar.Items.Count then
    ...
    begin
      ...
      if ((Item.ItemType = t_Minerals)
              or not (Byte(Item.ItemType) in [Ord(t_Food)..Ord(t_Narcotics)]))
          and ((Item.ItemType <> t_Minerals) or not IsRecentlyDroppedItem(Item))
          and ((Item.ScriptItem = nil) or (TScriptItem(Item.ScriptItem).Name = ''))
          and not CanCargoHookHandleItem(Item, Self) then
        if (GetPlayer.CurrentStar <> CurrentStar)
            or (GetRelationLevelToShip(GetPlayer) <= rlBad)
            or (PointDistance(GetPlayer.Position, Item.Position) >= 600)
            or ((NextRandomUnitFloat(RandomState) <= 0.2)
...
var
  ...
  Chance, BestChance, Distance, BestDistance: Double;
  PriorityTargetFound: Boolean;
begin
  if ((GetCargoHook <> nil) or (PirateType <> 0))
      and ((EnemyShip = nil)
          or (EnemyShip.CurrentStar <> CurrentStar)
          or ((EnemyShip.ConsecutiveDockedDays > 3) and (PirateType <> 0)))
      and (UsableWeaponCount <> 0) then
  begin
    ...
    begin
      ...
      begin
        Ship := CurrentStar.Ships[I];
        if PriorityTargetFound and (Ship.TargetingRestriction <> 6) then
          Continue;
        if (Ship.TargetingRestriction in [1..3, 5])
            or (Ship = Self)
            or not Ship.InNormalSpace
            or (TruceShip = Ship)
            or (Ship.TruceShip = Self)
            or ((GetPlayer = Ship) and (GetPlayer.TruceShip = Self))
            or (Ship.LiberationGroup <> nil)
            or (GetPlayer.QuestTargetKillShip = Ship)
            or ((Ship.TypeId in [Ord(rstRangerCenter)..Ord(rstCustomStation)])
                and ((OwnerId <> Byte(oiPirate))
                    or (CurrentStar.Battle = 0)
                    or (Ship.CurrentStanding in [ssNeutral..ssPirateMilitary])))
            or (Ship.TypeId = stTranclucator) then
          Continue;
        if (CurrentStar.ControlFaction = sfDominators)
            or (CurrentStar.Status.CustomFaction <> '')
            or ((CurrentStar.ControlFaction = sfPirates)
                and (OwnerId = Byte(oiPirate))
                and (Galaxy.CoalitionDefeatedTurn = 0)) then
        begin
          if not (Ship is TNormalShip)
              or ((OwnerId = Byte(oiPirate)) and (Ship.OwnerId <> Byte(oiPirate))) then
          begin
            EnemyShip := Ship;
            if ChanceToWin(Ship) > 0.5 then
              Exit;
          end;
        end
        else
        begin
          if (Ship.TargetingRestriction = 6) and not PriorityTargetFound then
          begin
          ...
          begin
            if (NextRandomIntRange(0, 30, RandomState) + 60
                    < (Integer(RelationToShip(Ship)) and $7F))
                and not Ship.AbductedByPirateClan then
              Continue;
            ...
            begin
              if (Galaxy.CurrentTurn
                      < 100
                          ...
                          + 300)
                  or (NextRandomIntRange(0, 100, RandomState)
                          * GalaxyDifficultyTuning[Galaxy.DifficultyLevels[7]]
                              .GoodsEventDurationFactor
                      < 40) then
                Continue;
            ...
            else if GetPlayer.QuestTargetDefendShip = Ship then
              Chance := Chance * 5;
            if (Chance < 0.3) and (StrengthInAverageRanger < 0.7) then
              Continue;
            if (Chance < 1.5)
                and (Ship.TypeId = stRanger)
                and (Ship.GetDominantCareer <> rcPirate) then
              Continue;
            if (Chance < 3.5)
                and (Ship.TypeId = stRanger)
                and (Ship.GetDominantCareer = rcPirate) then
              Continue;
            if (Chance < 3.5) and (Ship.TypeId = stPirate) then
              Continue;
            if not IsTargetStillPursuable(Ship)
                and (GetPlayer <> Ship)
                and (GetPlayer.QuestTargetDefendShip <> Ship) then
              Continue;
            if Chance > BestChance then
            begin
              EnemyShip := Ship;
              BestChance := Chance;
              if GetPlayer = EnemyShip then
                Break;
            end;
          end;
        end;
      ...
      begin
        if (EnemyShip.TargetingRestriction = 6) or not CanContactShip(EnemyShip) then
        begin
          AssignWeaponTargetsInStar;
          Exit;
        end;
    ...
    else if UsableWeaponCount <> 0 then
    begin
      EnemyShip := nil;
      BestDistance := 100000;
      for I := 0 to CurrentStar.Ships.Count - 1 do
      begin
        ...
        if Ship.InNormalSpace
            ...
            and (not (Ship.TypeId in [Ord(rstRangerCenter)..Ord(rstCustomStation)])
                or not (Ship.CurrentStanding in [ssNeutral..ssPirateMilitary]))
            and ((RelationToShip(Ship) < 10) or Ship.AbductedByPirateClan) then
        begin
...
begin
  ...
  if NextRandomIntRange(1, 100, RandomState)
      < RemapClamped(CurrentStar.CountPirateShips(True), 30, 60, 0, 100)
          * (1 + RaidPressure * 0.1) then
    Aggression := 100;
  for I := 0 to CurrentStar.Ships.Count - 1 do
  begin
    Ship := CurrentStar.Ships[I];
    if Ship.InNormalSpace
        and (TruceShip <> Ship)
        and not (Ship.TypeId in [Ord(rstRangerCenter)..Ord(rstCustomStation)])
        and (Ship.TypeId <> stTranclucator)
        and (Ship <> Self)
        and not (Ship.TargetingRestriction in [1..3, 5])
        and not AcceptsRansomDemandFrom(Ship)
        and ((RelationToShip(Ship) < 80)
            or (NextRandomIntRange(1, 100, RandomState) <= Aggression - 1))
        and ((RelationToShip(Ship) < 60)
            or (NextRandomIntRange(1, 100, RandomState) <= Aggression + 33))
        and ((RelationToShip(Ship) < 30)
            or (NextRandomIntRange(1, 100, RandomState) <= Aggression + 66))
        and ((RelationToShip(Ship) < 10)
            or (NextRandomIntRange(1, 100, RandomState) <= Aggression + 100))
        and ((not (Ship is TPirate)
                and (GetPlayer <> Ship)
                and ((Ship.OwnerId <> Byte(oiPirate)) or not (Ship is TNormalShip)))
            or (((Ship.OwnerId <> Byte(oiPirate))
                    or (Ship.PilotRace <> PilotRace)
                    or (NextRandomIntRange(1, 100, RandomState) <= 50))
                and ((ShortInt((Ship as TNormalShip).PirateRank) <= ShortInt(PirateRank + Byte(0)))
                    or (NextRandomIntRange(1, 100, RandomState) <= 70))
                and (((Ship as TNormalShip).PirateRank <= Integer(PirateRank) + 1)
                    or (NextRandomIntRange(1, 100, RandomState) <= 70))
                and (((Ship as TNormalShip).PirateRank <= Integer(PirateRank) + 2)
                    or (NextRandomIntRange(1, 100, RandomState) <= 70))
                and (((Ship as TNormalShip).PirateRank <> 7)
                    or (NextRandomIntRange(1, 100, RandomState) <= 50)))) then
    begin
      Distance := PointDistance(Position, Ship.Position);
      if NextRandomFloatRange(0.3, 3, RandomState) * BestDistance > Distance then
      begin
  ...
  if EnemyShip = nil then
    RaidPressure := Max(Aggression, 0) * 0.001 + RaidPressure;
end;
...
begin
  ...
  if (EnemyShip <> nil) and (EnemyShip.CurrentStar = CurrentStar) then
    ...
    else if (ChanceToWin(EnemyShip) > 2)
...
begin
  if (EnemyShip <> nil)
      and (OrderTarget = EnemyShip)
      and (Integer(Seed + Cardinal(Galaxy.CurrentTurn)) mod 5 = 0)
      and not AcceptsRansomDemandFrom(EnemyShip) then
    TryExtortShip(EnemyShip);
  if (EnemyShip <> nil)
      and (OrderTarget = EnemyShip)
      and (Integer(Seed + Cardinal(Galaxy.CurrentTurn)) mod 4 = 0)
      and (((ChanceToWin(EnemyShip) < 1.1) and (GetHullIntegrityPercent > 30))
          or ((OwnerId = Byte(oiPirate))
              and (CurrentStar.ControlFaction = sfPirates)
              and (CurrentStar.Status.CustomFaction = '')
              and (ChanceToWin(EnemyShip) < 2))) then
    RequestAlliesAttackShip(EnemyShip);
end;
...
function TPirate.BuildMoneyExtortionResponse(
    OtherShip: TShip;
    var Response: WideString;
    DemandedAmount: Integer
): Boolean;
...
begin
  ...
  if OtherShip.TruceShip = Self then
    ...
    Response := LookupVisibleTalkText('Talk.Money.WeAlreadyHavePact', OtherShip)
  else if UnknownVirtualC0(OtherShip) or not AcceptsRansomDemandFrom(OtherShip) then
...
begin
  ...
  if OtherShip.TruceShip = Self then
    ...
    Response := LookupVisibleTalkText('Talk.Goods.WeAlreadyHavePact', OtherShip)
  else if UnknownVirtualC0(OtherShip) or not (AcceptsRansomDemandFrom(OtherShip) or Forced) then
...
function TPirate.BuildTrucePaymentResponse(
    OtherShip: TShip;
    var Response: WideString;
    OfferedAmount: Integer
): Boolean;
var
  ...
  begin
    ...
    if (OwnerId = Byte(oiPirate))
        and (CurrentStar.ControlFaction = sfPirates)
        and (CurrentStar.Status.CustomFaction = '')
        and (OtherShip is TNormalShip) then
      if TNormalShip(OtherShip).CurrentSystemKills.Pirate > 0 then
      begin
        TNormalShip(OtherShip).CurrentSystemKills.Pirate := 0;
        for I := 0 to CurrentStar.Ships.Count - 1 do
        begin
          Ship := CurrentStar.Ships[I];
          if (Ship is TNormalShip) and (Ship.OwnerId = Byte(oiPirate)) then
    ...
    begin
      if (MainPiratePlanet <> nil) and (MainPiratePlanet.OwnerId = Byte(oiPirate)) then
        MainPiratePlanet.ChangeRelationToRanger(OtherShip, 10);
...
begin
  ...
  if (GetPlayer = PartnerShip) and (GetPlayer.CurrentStar = CurrentStar) then
  begin
  ...
  if OtherShip.TruceShip = Self then
    ...
    Response := LookupVisibleTalkText('Talk.Truce.WeAlreadyHavePact', OtherShip)
  else if (OwnerId = Byte(oiPirate))
      and (OtherShip is TNormalShip)
      and (OtherShip.OwnerId <> Byte(oiPirate))
      and (OtherShip.CurrentStar.ControlFaction = sfPirates)
      and (OtherShip.CurrentStar.Status.CustomFaction = '')
      and (TNormalShip(OtherShip).CurrentSystemKills.Pirate > 0) then
  begin
    if 2 * Wealth * (1 / 15) < OfferedAmount then
    begin
      ...
      or ((ChanceToWin(OtherShip) < 1)
          and (GetHullIntegrityPercent < 40)
          and AcceptsRansomDemandFrom(OtherShip))
      or ((ChanceToWin(OtherShip) < 0.2) and AcceptsRansomDemandFrom(OtherShip))
      or (OfferedAmount
          > RemapClamped(
              Integer(GetWinChancePercent(OtherShip)) and $7F,
              0,
...
function TPirate.BuildAttackRequestResponse(
    Requester: TShip;
    var Response: WideString;
    Target: TShip
): Boolean;
  ...
  var
    UnusedLocal: Integer; // Native gap before the floating-point temporaries.
  begin
    Result :=
        (Cardinal(RelationToShip(Target)) and $7F)
            >= Round(
                (Integer(RelationToShip(Requester)) and $7F)
...
begin
  ...
  begin
    ...
    if (Target.OwnerId = Byte(oiDominator)) or (Target.TypeId = stPirate) then
      ...
      (Requester as TRanger).AddPirateCareerActivity(1);
  end;
  ...
  else if (RelationToShip(Target) >= 60) and FriendsPreferred then
  begin
...
function TPirate.AcceptPartnershipOffer(
    OtherShip: TShip;
    var Response: WideString;
    PaymentAmount: Integer
): Boolean;
begin
  ...
  begin
    ...
    Response :=
        FormatText2(
            LookupVisibleTalkText('Talk.Pirate.Ok', OtherShip),
            '<color=255,240,100>',
            '<Month>',
...
function TPirate.BuildPartnershipOfferResponse(
    OtherShip: TShip;
    var Response: WideString;
    PaymentAmount: Integer
): Boolean;
begin
  if PartnerShip = OtherShip then
  begin
    Result := True;
    Exit;
  end;
  ...
  if RelationToShip(OtherShip) < 45 then
    Response := LookupVisibleTalkText('Talk.Pirate.Suspect', OtherShip)
  else if PartnerShip <> nil then
    Response :=
        FormatText1(
            LookupVisibleTalkText('Talk.Pirate.AlreadyHavePartner', OtherShip),
            '<color=255,240,100>',
            '<Partner>',
            (PartnerShip as TRanger).Name
        )
  else if ((OtherShip is TPlayer)
          and (TPlayer(OtherShip).GetMaxPiratePartners <= TPlayer(OtherShip).PiratePartners.Count))
      or ((OwnerId = Byte(oiPirate)) and (GetPlayer.PirateRank < PirateRank)) then
    Response := LookupVisibleTalkText('Talk.Pirate.NeedPirate', OtherShip)
  else if (OtherShip is TRanger)
      and (OtherShip.GetEffectiveSkillLevel(psLeadership)
    ...
    Response := LookupVisibleTalkText('Talk.Pirate.SmallMoney', OtherShip)
  else
    Result := True;
  ...
  if PartnerShip <> nil then
    ...
    begin
      ...
      begin
        ...
        and (EnemyShip.PartnerShip = PartnerShip) then
    begin
      if CanNotifyPartner then
      begin
        Leader := PartnerShip;
      ...
      begin
        ...
        for I := 0 to GetPlayer.PiratePartners.Count - 1 do
          if TShip(GetPlayer.PiratePartners[I]).PartnershipDaysRemaining > Longest then
            Longest := TShip(GetPlayer.PiratePartners[I]).PartnershipDaysRemaining;
        if Longest >= PartnershipDaysRemaining then
        begin
          Leader := PartnerShip;
          if (Order = soFollowShip) and (OrderTarget = PartnerShip) then
            OrderNone(False);
...
begin
  Result := False;
  if (PirateType <> 0)
      and (TShip(Argument).CurrentStanding in [ssCoalitionMilitary, ssCoalitionActive])
      and ((CurrentStar.ControlFaction <> sfPirates)
          or (CurrentStar.Status.CustomFaction <> '')
          or (MainPiratePlanet = nil)
          or (MainPiratePlanet.OwnerId <> Byte(oiPirate))
          or (TShip(Argument).CurrentStar = MainPiratePlanet.CurrentStar)) then
    Result := True;
end;
...
var
  I: Integer;
  Ship: TShip;
  DominatorAndCustomStrength, CoalitionStrength, PirateStrength, RetreatFactor, StrengthScale:
      Single;
  Star, PirateStar, AnyStar: TStar;
  Angle, PirateAngle, AnyAngle: Single;
  Station: TRuins;
  Urgent, PirateSystem, HostileSystem: Boolean;
begin
  Result := False;
  ...
  HostileSystem :=
      (CurrentStar.ControlFaction = sfDominators) or (CurrentStar.Status.CustomFaction <> '');
  Station := nil;
  if not PirateSystem then
    ...
    begin
      ...
      if (Ship is TRuins)
          ...
          and (CurrentStar.Dominion <> Ship)
          and Ship.InNormalSpace
          and Ship.CanDock(Self)
          and ((TRuins(Ship).FlyToStar = nil) or (TRuins(Ship).FlyToStar = CurrentStar)) then
      begin
        Station := TRuins(Ship);
        Break;
      end;
  ...
  if (EnemyShip = nil)
      or (EnemyShip.OrderTarget <> Self)
      or (EnemyShip.EstimateOrderTravelTurns > 2) then
    RetreatFactor := RetreatFactor * 0.5;
  if Station = nil then
    RetreatFactor := RemapClamped(RaidPressure, 0, 15, 1, 0.1) * RetreatFactor;
  if InFear and not PirateSystem then
    RetreatFactor := 1.4 * RetreatFactor;
  if (OwnerId = Byte(oiPirate)) or (HostileSystem and (Station <> nil)) then
  begin
    if (OwnerId = Byte(oiPirate)) and not PirateSystem then
      RetreatFactor := RetreatFactor * 0.25;
    ...
    if TransitOriginStar <> nil then
      ...
      begin
        Ship := CurrentStar.Ships[I];
        if (Ship.CurrentStanding in [ssPirateActive, ssPirateMilitary])
            and (Ship.OrderTarget = CurrentStar) then
          PirateStrength :=
              Min(10, Max(0.1, Ship.Strength * StrengthScale)) * 0.75 + PirateStrength;
      end;
    if (Abs(CoalitionStrength - 2.5 * DominatorAndCustomStrength) * RetreatFactor >= PirateStrength)
        or (HasNoUsableWeapons and not PirateSystem) then
    begin
      Urgent :=
          InFear
              and (EnemyShip <> nil)
              and (EnemyShip.OrderTarget = Self)
              and (EnemyShip.EstimateOrderTravelTurns <= 2);
      ...
      else if Station <> nil then
      begin
        OrderLanding(Station, False);
        if Urgent or (Galaxy.CurrentTurn + EstimateOrderTravelTurns + 1 > Station.FlyDate) then
          UpdateAfterburnerState;
        Result := True;
        Exit;
      end
      ...
      begin
        ...
        AnyStar := nil;
        AnyAngle := 0;
        while (Galaxy.Stars.Count > I) and (CurrentStar.StarDistances[I].Distance <= JumpRange) do
        begin
          Star := CurrentStar.StarDistances[I].Star;
          Inc(I);
          Angle :=
              PointBearingDegrees(CurrentStar.Position, Star.Position)
                  - PointBearingDegrees(MakeFloatPoint(0, 0), Position);
          Angle := WrapSignedHeadingDegrees(Angle);
          if (AnyStar = nil) or (Abs(Angle) < AnyAngle) then
          begin
            AnyStar := Star;
            AnyAngle := Abs(Angle);
          end;
          if (Star.ControlFaction = sfPirates)
              ...
              and ((PirateStar = nil) or (Abs(Angle) < PirateAngle)) then
          begin
            PirateStar := Star;
        ...
        if InFear
            and (2 * RetreatFactor * PirateStrength
                < Abs(CoalitionStrength - DominatorAndCustomStrength))
            and (NextRandomIntRange(0, 9, RandomState) < 5) then
          PirateStar := AnyStar;
        if PirateStar <> nil then
        begin
          if OrderTarget <> PirateStar then
...
begin
  ...
  EffectivenessScale :=
      EffectivenessScale * 0.01 * (100 + SeededRandomIntRange(-25, 25, Seed + 3 * Id));
...
begin
  ...
  case BonusKind of
    ...
    bonScan:
      Result :=
          Value * 10 + Value * 35 * (Integer(CountWeaponsByDamageFlags(ScannerFlags)) and $7F);
    ...
    bonSlotRadar:
      if (GetSlotCount(sskRadar) = 0) and (Value > 0) then
        Result := PirateSlotBonusWeights[Ord(BonusKind)] * 0.3
      else if (GetRadar <> nil) and (Value < 0) then
        Result :=
            -PirateSlotBonusWeights[Ord(BonusKind)]
                - PirateSlotBonusWeights[18] * (Integer(CountMissileWeapons) and $7F)
    ...
    bonSlotDroid:
      if (GetSlotCount(sskRepairRobot) = 0) and (Value > 0) then
        ...
        Result := -PirateSlotBonusWeights[Ord(BonusKind)]
    ...
    begin
      ...
      if Value < 0 then
        Result := Max(Value, -GetSlotCount(sskWeapon)) * PirateSlotBonusWeights[Ord(BonusKind)];
      if (Integer(CountEquippedWeapons) and $7F) > Max(Value + GetSlotCount(sskWeapon), 1) then
        Result :=
            Result
                - (PirateSlotBonusWeights[Ord(BonusKind)] * 0.6)
      ...
      if (GetSlotCount(sskArtefact) < DefaultHullSlotCounts[8]) and (Value > 0) then
        Result :=
            Min(Value, DefaultHullSlotCounts[8] - GetSlotCount(sskArtefact))
                * PirateSlotBonusWeights[Ord(BonusKind)];
      if Value < 0 then
        Result := Max(Value, -GetSlotCount(sskArtefact)) * PirateSlotBonusWeights[Ord(BonusKind)];
    ...
    begin
            ...
            Min(
                    6
                        - (Integer(
                            ...
                            and $7F),
                    Value)
                * PirateSkillBonusWeights[Ord(BonusKind)];
      if (Value > 0)
          and (Value
                  + (Integer(
                          GetEffectiveSkillLevel(
        ...
        Result :=
            Result
                + (PirateSkillBonusWeights[Ord(BonusKind)] * 0.05)
                    * (Value
                        + (Integer(
                                GetEffectiveSkillLevel(
                                    TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22])
      ...
      if (Value < 0)
        ...
        Result :=
            Result
                + (PirateSkillBonusWeights[Ord(BonusKind)] * 0.03)
                    * (Value
                        + (Integer(
                                GetEffectiveSkillLevel(
                                    TPilotSkill(EquipmentBonusSkills[Ord(BonusKind) - 22])
  ...
  if (PirateType = 0) and (BonusKind in [bonSlotForsage, bonMass]) then
    Result := Result * 1.3;
  if (PirateType = 0) and (BonusKind in [bonSpeed]) then
    Result := Result * 1.2 * Max(100, SmoothedEnemySpeed) / Max(100, SmoothedSpeed);
  if (PirateType = 1) and (BonusKind in [bonHull, bonSpeed, bonDroid, bonDef..bonWMissile]) then
    Result := Result * 1.3;
  if (PirateType = 2) and (BonusKind in [bonScan, bonWEnergy..bonWRadius]) then
    Result := Result * 1.3;
  if (PirateType = 3) and (BonusKind in [bonSpeed, bonRadar, bonDroid, bonWRadius, bonMass]) then
    Result := Result * 1.3;
  if (PirateType = 3) and (BonusKind in [bonWEnergy..bonWMissile]) then
    Result := Result * 0.7;
  if BonusKind in [bonSkill1..bonSkill6] then
    Result :=
        Result
            * 0.01
            * (100 + SeededRandomIntRange(-75, 75, Seed + 131 * Ord(BonusKind)))
            * RaceSkillEvaluationFactors[PilotRace, EquipmentBonusSkills[Ord(BonusKind) - 22]]
  else
    Result := Result * 0.01 * (100 + SeededRandomIntRange(-25, 25, Seed + 131 * Ord(BonusKind)));
end;

function TPirate.EvaluateWeaponDamage(
    Weapon: TWeapon;
    IncludeAdditiveBonuses: Boolean;
    BaseDamage: Single
): Single;
...
begin
  ...
  if dkShock in Flags then
    case PirateType of
      2: Result := Result * (1.3 + (Integer(CountWeaponsByDamageFlags(ShockFlags)) and $7F) * 0.15);
    else
      Result := Result * (1.2 + (Integer(CountWeaponsByDamageFlags(ShockFlags)) and $7F) * 0.05);
    end;
  ...
  if dkReduceEngine in Flags then
    SlowFactor := 1 + 2 * SlowFactor;
  DisruptionFactor := 0;
  if dkDecelerate in Flags then
    DisruptionFactor := DisruptionFactor + 1;
  if (CountActiveArtefacts(Ord(t_ArtDecelerate)) > 0) and (dkSplinter in Flags) then
    DisruptionFactor :=
        DisruptionFactor
            + CountActiveArtefacts(Ord(t_ArtDecelerate))
                * ((Ord(CanBoostArtefact(Ord(t_ArtDecelerate), Weapon, False)) * 1) + 1);
  TotalDisruption := DisruptionFactor;
  for I := 1 to Integer(CountEquippedWeapons) and $7F do
  begin
    if dkDecelerate in Weapons[I].GetDamageFlags then
      TotalDisruption := TotalDisruption + 1;
    if (CountActiveArtefacts(Ord(t_ArtDecelerate)) > 0)
        and (dkSplinter in Weapons[I].GetDamageFlags) then
  ...
  begin
    if DisruptionFactor > 0.01 then
      case PirateType of
        ...
        Result := Result + 10 * DisruptionFactor;
      end;
    if SlowFactor > 0.01 then
      case PirateType of
    ...
    begin
      ...
      case PirateType of
        1: Result := Result + ShotTotal * 5;
      else
        Result := Result + ShotTotal * 3;
      end;
    end;
    if dkMoreDrop in Flags then
      Result := Result + ScannerFactor * 15;
  ...
  case Byte(Weapon.GetWeaponInfo.ShotType) of
    ...
    Ord(wstMissile):
      Result :=
          Result
              * (0.9 + Weapon.GetWeaponInfo.SecondaryDamageRadius * 0.2 * 0.01 + StatusFactor)
              * Weapon.GetShotCount;
    Ord(wstTorpedo):
      Result :=
    ...
    Ord(wstSplash):
      Result :=
          Result
              * (1
                  + Weapon.GetWeaponInfo.SecondaryDamageRadius * 0.4 * 0.01 * SpeedFactor
                  + StatusFactor);
    Ord(wstAreaDamage):
      Result := Result * (1 + Weapon.Range * 0.3 * 0.01 * SpeedFactor + StatusFactor);
  else
  ...
  if (PirateType = 3)
      and (Flags * DisablingFlags <> [])
      and (CountWeaponsByDamageFlags(Flags * DisablingFlags) = 0) then
    Result := Result * 4
  else if dkMissile in Flags then
    Result := Result * 1.2;
end;
...
begin
  ...
  Result :=
      (Byte(Item.ItemType) in [Ord(t_Technics), Ord(t_Luxury), Ord(t_Alcohol)..Ord(t_Narcotics)])
          or (2 * Speed >= Distance)
          or (Item.Cost >= RemapClamped(Distance / Speed, 1, 10, 0.01, 0.05) * Wealth);
end;
...
begin
  ...
  begin
    if (GetPlayer <> nil) and (GetPlayer = PartnerShip) then
      ...
      Owner := OwnerId;
    if IsInPrison or ((PirateType = 0) and (Owner <> 7)) then
      CurrentStanding := ssNeutral
    else if (Owner = 7) and (PirateType <> 0) then
      CurrentStanding := ssPirateMilitary
    else if (Owner = 7)
        and ((CurrentSystemKills.Normal > 0) or (CurrentStar.ControlFaction = sfPirates)) then
