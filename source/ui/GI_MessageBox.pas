...
type
  TMessageBoxGI = class;
  TMessageBoxGI = class(TMessageLoopGI)
    MessageText: WideString;
    Options: Cardinal;
    UnusedOption: Integer;
    OffsetX: Integer;
    OffsetY: Integer;
    procedure OnOpen; override;
    procedure ProcessCallbackTimers; override;
    procedure AcceptClick(Sender: TObjectGI);
    procedure CancelClick(Sender: TObjectGI);
    procedure DialogKeyDown(Sender: TObjectGI; VirtualKey: Cardinal);
  end;
function ShowMessageBoxGI(
    Parent: TMessageLoopGI;
    const Text: WideString;
    Options: Cardinal;
    UnusedOption: Integer = 0;
    OffsetX: Integer = 0;
    OffsetY: Integer = 0
): Cardinal;
implementation
...
begin
  ...
  if Options and mbgWarning = mbgWarning then
    Light.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'LightRed')
  else if Options and mbgQuestion = mbgQuestion then
    Light.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'LightBlue')
  else if Options and mbgError = mbgError then
    Light.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'LightRed')
  else
    Light.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'LightBlue');
  ...
  if Options and mbgWarning = mbgWarning then
    Icon.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'Warning')
  else if Options and mbgQuestion = mbgQuestion then
    Icon.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'Question')
  else if Options and mbgError = mbgError then
    Icon.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'Error')
  else
    Icon.SetImagePath('GI,Bm.FormMessageBox.' + GiResourceSuffix + 'Info');
  ...
  begin
    ...
    TextLabel.SetSize(
        Classes.Point(
            CandidateSize.X - Borders.Left - Borders.Right - Icon.ClientSize.X - GiScalePixels(10),
            1
        )
    );
    ...
    Window.SetSize(
        Classes.Point(
            TextSize.X + Borders.Left + Borders.Right + Icon.ClientSize.X + GiScalePixels(10),
    ...
    if ActualRatio - TargetRatio < -0.1 then
      Inc(CandidateSize.X, 10)
    else if ActualRatio - TargetRatio > 0.1 then
      Inc(CandidateSize.Y, 10)
    else
      Break;
    Dec(Attempts);
  end;
  ...
  begin
    ...
    if Options and mbgCancel <> mbgCancel then
      AcceptButton.SetPosition(
          Classes.Point(
              WindowSize.X div 2 - AcceptButton.ClientSize.X div 2,
      ...
      AcceptButton.SetPosition(
          Classes.Point(
              WindowSize.X div 2 - AcceptButton.ClientSize.X - GiScalePixels(5),
              WindowSize.Y - Borders.Bottom - AcceptButton.ClientSize.Y
          )
      );
  ...
  begin
    ...
    if Options and mbgOK <> mbgOK then
      CancelButton.SetPosition(
          Classes.Point(
              WindowSize.X div 2 - CancelButton.ClientSize.X div 2,
      ...
      CancelButton.SetPosition(
          Classes.Point(
              WindowSize.X div 2 + GiScalePixels(5),
              WindowSize.Y - Borders.Bottom - CancelButton.ClientSize.Y
          )
      );
  ...
  if Options and mbgLeftAlign = mbgLeftAlign then
    TextLabel.SetTextAlignX(taxLeft)
  else
    TextLabel.SetTextAlignX(taxCenter);
  TextLabel.SetTextAlignY(tayAuto);
  TextLabel.SetPosition(
      Classes.Point(
          Icon.ClientSize.X + Borders.Left + GiScalePixels(10),
          Integer(
                  Round(
                      (WindowSize.Y
                              - Borders.Top
  ...
  if Icon.LocalPosition.Y + Icon.ClientSize.Y
      > TextLabel.LocalPosition.Y + TextLabel.ClientSize.Y then
  begin
    if TextLabel.LocalPosition.Y - (Icon.ClientSize.Y div 2 - TextLabel.ClientSize.Y div 2)
        < Borders.Top then
      IconTop := Borders.Top
    else
      IconTop :=
          TextLabel.LocalPosition.Y - (Icon.ClientSize.Y div 2 - TextLabel.ClientSize.Y div 2);
...
begin
  if (Options and mbgCancel = mbgCancel)
      and ((VirtualKey = VK_ESCAPE)
          or (VirtualKey = Ord('N'))
          or ((VirtualKey = VK_RETURN) and (Options and mbgOK <> mbgOK))) then
    CancelClick(Sender)
...
function ShowMessageBoxGI(
    ...
    const Text: WideString;
    Options: Cardinal;
    UnusedOption, OffsetX, OffsetY: Integer
): Cardinal;
var
