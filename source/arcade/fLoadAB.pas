...
type
  ...
  TfLoadAB = class(TMessageLoopGI)
    Entries: array of TfLoadABSlot;
    ...
    procedure ArenaListKeyDown(Sender: TObjectGI; VirtualKey: Cardinal);
    function AppendEntry(IgnoredAccess: Integer): Integer;
    procedure RebuildArenaList;
    procedure InitializeArenaRow(Row: TObjectGI);
    procedure SelectArena(Index: Integer);
    procedure UpdateArenaRow(Index: Integer);
    procedure ArenaRowMouseEnter(Sender: TObjectGI);
    procedure ArenaRowMouseLeave(Sender: TObjectGI);
    procedure ArenaRowMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure ArenaRowDoubleClick(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
...
var
  ...
  List: TBlockParEC;
  Name: WideString;
  function ReadArcadeDescription(
      const Path: WideString
  ): WideString; // @addr $557E9C @ida "void __usercall $name(unsigned __int16 *Path@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x558168"
  var
    J, LineCount: Integer;
  begin
    Result := '';
...
begin
  ...
  begin
    ...
    DifficultyImage.SetPosition(
        Classes.Point(
            Row.ClientSize.X - DifficultyImage.ClientSize.X - 10,
...
begin
  ...
  if SelectedIndex = Index then
    Image.SetImagePath('GI,Bm.FormLoadAB.SlotActive')
  else if HoveredIndex = Index then
    Image.SetImagePath('GI,Bm.FormLoadAB.SlotOnMouse')
  else
    Image.SetImagePath('GI,Bm.FormLoadAB.SlotNormal');
end;
