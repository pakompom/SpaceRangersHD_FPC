...
uses
  ...
  GR_DX;
type
  TRotateImage5GI = class;
  TRotateImage5GI = class(TObjectGI)
    ImageCache: TCHSAIControlEC;
    ...
    procedure SetFrameIndex(Value: Cardinal);
    ...
    procedure QueueImagePath(PendingLoads: TList; Path: WideString);
  end;
var
  RotateImageConstructionStage: Integer = -1;
implementation
uses
  GlobalsV,
  SysUtils,
...
begin
  ...
  begin
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Result :=
                  Buffer.GetPixel32(Point.X - HitTestBounds.Left, Point.Y - HitTestBounds.Top)
                      > $FFFFFF;
              Buffer.Free;
            end;
            Texture := nil;
      ...
      Result :=
          Byte(
                  RotatedImage.GetPaletteColor(
                          RotatedImage.GetPixelIndex(
...
var
  ...
  OldClip: TRect;
begin
  // The supplied clip is in the same coordinate space as the sprite. Observer
  // world coordinates may be negative or far beyond the physical window.
    ...
    begin
      ...
      try
        ...
        Ex_OKGR_RotateBuf_Draw_BYTE(
            ...
            Data.GetFrameIndexPlane(RenderedFrameIndex),
            Data.GetSourcePitchBytes,
            OriginPoint.X,
            OriginPoint.Y,
            Angle,
            Rotation.Buffer
