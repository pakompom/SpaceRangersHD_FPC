...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script SF_GCntRun');
  Index := FindScriptTemplateIndex(av[1].GetString);
  if Index < 0 then
    raise Exception.Create('Error.Script.NotFound SF_GCntRun');
...
begin
  ...
  Index := FindScriptTemplateIndex(av[1].GetString);
  if Index < 0 then
    raise Exception.Create('Error.Script.NotFound SF_GLastTurnRun');
...
begin
  ...
  begin
    ...
    for I := 0 to Galaxy.Scripts.Count - 1 do
      if TScript(Galaxy.Scripts[I]).ClassId = ClassId then
        Inc(Count);
    av[0].SetInt(Count);
  end
  else
    av[0].SetInt(Galaxy.Scripts.Count);
end;
procedure SF_IsScriptActive(av: array of TVarEC; code: TCodeEC);
var
  Index: Integer;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script IsScriptActive');
  av[0].SetInt(0);
  Index := FindScriptTemplateIndex(av[1].GetString);
  if Index >= 0 then
...
begin
  LastArg := High(av);
  if LastArg < 2 then
    raise Exception.Create('Error.Script GetValueFromScript');
  if av[1].GetString = '' then
  begin
    Value := ScriptFunctionScope.GetVarNE(av[2].GetString);
    if Value = nil then
      raise Exception.Create(
          'Error.Script GetValueFromScript - var ' + av[2].GetString + ' not found');
  end
  else
  begin
    Index := FindScriptTemplateIndex(av[1].GetString);
    ...
    if Value = nil then
      Value := Script.TurnCode.LocalVar.GetVarNE(av[2].GetString);
    if Value = nil then
      Value := ScriptFunctionScope.GetVarNE(av[2].GetString);
    if Value = nil then
      raise Exception.Create(
          'Error.Script GetValueFromScript - var '
              + av[2].GetString
              + ' not found in script '
              + av[1].GetString);
  end;
...
begin
  LastArg := High(av);
  if LastArg < 2 then
    raise Exception.Create('Error.Script RunFunctionFromScript');
  Index := FindScriptTemplateIndex(av[1].GetString);
  if Index < 0 then
    Exit;
  Index := TScriptTemplUnit(ScriptTemplates[Index]).ActiveScriptIndex;
  ...
  if Value = nil then
    Value := Script.TurnCode.LocalVar.GetVarNE(Name);
  if (Value = nil) or (Value.RealVType <> vkFunction) then
    raise Exception.Create(
        'Error.Script RunFunctionFromScript - function '
            + Name
            + ' not found in script '
            + av[1].GetString);
  ...
  try
    ...
    on E: Exception do
      raise ExceptionExpressionEC.Create(
          'Error in function ' + Value.Name + ' (' + E.ClassName + ' ' + E.Message + ')');
  end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetVariableName');
  av[0].SetString(av[1].Name);
end;
procedure SF_GetVariableType(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetVariableType');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script AddJournalRecord');
  if High(av) = 1 then
    GetPlayer.AddJournalRecord(av[1].GetString)
  else
  begin
    Turn := av[2].GetInt;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script AutoBattle');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_GiveRewardByNom');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_CountReward');
  Ship := TShip(av[1].GetDword);
  if High(av) = 1 then
  begin
    ...
    for Index := 0 to Ship.AwardIds.Count - 1 do
      if SysToReward(
              LookupLocalizedTextByKey(
                  'Reward.' + SysUtils.IntToStr(Integer(Ship.AwardIds[Index])) + '.Type'
              ))
          = Kind then
        Inc(Count);
    av[0].SetInt(Count);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_CountRewardByNom');
  Ship := TShip(av[1].GetDword);
  Award := av[2].GetInt;
  if Ship.AwardIds = nil then
  begin
  ...
  Count := 0;
  for Index := 0 to Ship.AwardIds.Count - 1 do
    if Integer(Ship.AwardIds[Index]) = Award then
      Inc(Count);
  av[0].SetInt(Count);
end;
procedure SF_DeleteRewardByNom(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_DeleteRewardByNom');
  Ship := TShip(av[1].GetDword);
  Award := av[2].GetInt;
  if High(av) > 2 then
    Count := av[3].GetInt
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_ShipPicksItem');
  Ship := TShip(av[1].GetDword);
  Item := TItem(av[2].GetDword);
  if High(av) = 2 then
  begin
    if Ship.HasPickupTarget(Item) then
      av[0].SetInt(1)
    else if Ship.IsRecentlyDroppedItem(Item) then
      av[0].SetInt(-1)
    else
      av[0].SetInt(0);
  end
  ...
  begin
    ...
    if Ship.IsRecentlyDroppedItem(Item) then
      Ship.RecentlyDroppedItemIds.Delete(Ship.RecentlyDroppedItemIds.IndexOf(Item));
  end
  else if av[3].GetInt < 0 then
  begin
...
begin
  Found := False;
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_DropItem');
  Ship := TShip(av[1].GetDword);
  ...
  begin
    ...
    begin
      ...
      if Binding.Item <> nil then
        Binding.Item.ScriptItem := nil;
      Binding.Item := Item;
    ...
    if Ship.CurrentStar.Items.IndexOf(Item) >= 0 then
      Item.NoDropFlag := SavedNoDrop;
    Found := True;
  end
  else
  begin
  ...
  Done:
  if not Found then
    raise Exception.Create('Error.Script SF_DropItem - item not found');
end;
procedure SF_DropScriptItem(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script DropScriptItem');
  Ship := TShip(av[1].GetDword);
  Binding := TScriptItem(av[2].GetDword);
  if (Binding.Item <> nil)
      and ((Ship.Inventory.IndexOf(Binding.Item) >= 0)
          or (Ship.Artefacts.IndexOf(Binding.Item) >= 0))
  ...
  begin
    ...
    if Binding.Item <> nil then
      Binding.Item.NoDropFlag := SavedNoDrop;
  end;
end;
procedure SF_DeleteEquipment(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script SF_DeleteEquipment');
  Ship := TShip(av[1].GetDword);
  Index := 0;
  Kind := av[2].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script Rnd');
  if High(av) > 2 then
  begin
    Seed := av[3].GetDword;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script GameDateTxtByTurn');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script SF_Id');
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    av[0].SetDword(TShip(Obj).Id)
  else if Obj is TPlanet then
    av[0].SetDword(TPlanet(Obj).Id)
  else if Obj is TStar then
    av[0].SetDword(TStar(Obj).Id)
  else if Obj is TConstellation then
    av[0].SetDword(TConstellation(Obj).Id)
  else if Obj is TItem then
    av[0].SetDword(TItem(Obj).Id)
  else if Obj is THole then
    av[0].SetDword(THole(Obj).Id)
  else if Obj is TMissile then
    av[0].SetDword(TMissile(Obj).Id)
  else if Obj is TAsteroid then
    av[0].SetDword(TAsteroid(Obj).Id)
  else
    raise Exception.Create('Error.Script SF_Id 2');
end;
procedure SF_GalaxyMoney(av: array of TVarEC; code: TCodeEC);
var
  ScaleIndex: Byte;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script SF_SetName');
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    TShip(Obj).Name := av[2].GetString
  else if Obj is TPlanet then
    TPlanet(Obj).Name := av[2].GetString
  else if Obj is TStar then
    TStar(Obj).Name := av[2].GetString
  else if Obj is TItem then
    TItem(Obj).NameOverride := av[2].GetString;
end;
procedure SF_UseTranclucator(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_UseTranclucator');
  av[0].SetDword(0);
  Ship := TShip(av[1].GetDword);
  ...
  begin
    ...
    if High(av) > 1 then
      ...
      begin
        ...
        if (Obj is TArtefactTranclucator) and (TArtefactTranclucator(Obj).BrokenFlag = 0) then
          Break;
      end;
      ...
      begin
        Angle :=
            SeededRandomIntRange(
                    0,
                    360,
                    Drone.Id * (Ship.CurrentStar.GenerationSeed * Cardinal(Galaxy.CurrentTurn)))
                * Pi
                / 180;
        Drone.Position.X := Sin(Angle) * 100 + Ship.Position.X;
        Drone.Position.Y := Ship.Position.Y - Cos(Angle) * 100;
      end
      else if Ship.CurrentPlanet <> nil then
        Drone.CurrentPlanet := Ship.CurrentPlanet
      else if Ship.DockedTo <> nil then
        Drone.DockedTo := Ship.DockedTo;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script HullDamage');
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
  else if Obj is TScriptItem then
    Hull := THull(TScriptItem(Obj).Item)
  else
    Hull := THull(Obj);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script Hitpoints');
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
  else if Obj is TScriptItem then
...
begin
  ...
  begin
    ...
    begin
      ...
      if High(av) > 2 then
        Binding.HitPlayer := av[3].GetInt <> 0;
    end
    else
    begin
      av[0].SetInt(Ord(Binding.Hit));
      if High(av) > 2 then
        Binding.Hit := av[3].GetInt <> 0;
    end;
  end;
end;
procedure SF_ChangeGlobalRelationsShips(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) <> 3 then
    raise Exception.Create('Error.Script SetRelationGroup');
  CurrentScript.SetGroupRelation(av[1].GetInt, av[2].GetInt, TRelationLevel(av[3].GetInt));
end;
procedure SF_SetRelationPlanet(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 3 then
    raise Exception.Create('Error.Script SetRelationPlanet');
  CurrentScript
      .SetPlanetRelation(av[2].GetInt, TPlanet(av[1].GetDword), TRelationLevel(av[3].GetInt));
end;
procedure SF_GetRelationPlanet(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GetRelationPlanet');
...
begin
  ...
  if High(av) > 0 then
    Galaxy.CurrentTurn := av[1].GetInt;
end;
procedure SF_ShipType(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipType');
  Ship := TShip(av[1].GetDword);
  if GetPlayer.RuinsProxy = Ship then
    av[0].SetString('PlayerBridge')
  else if Ship.TypeNameOverrideKey <> '' then
    av[0].SetString(Ship.TypeNameOverrideKey)
  else if (Ship.ScriptShip <> nil)
      and (TScriptShip(Ship.ScriptShip).Script.ScriptFileName = 'Script.PC_fem_rangers')
      and (TScriptShip(Ship.ScriptShip).GetGroup.Name = 'GroupFem') then
    av[0].SetString('FemRanger')
  else
    av[0].SetString(Ship.GetTypeNameKey);
  if High(av) > 1 then
  begin
    if av[2].GetString = Ship.GetTypeNameKey then
      Ship.TypeNameOverrideKey := ''
    else
      Ship.TypeNameOverrideKey := av[2].GetString;
  end;
end;
procedure SF_ConName(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ConName');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script StarName');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarMapLabel');
  Star := TStar(av[1].GetDword);
  av[0].SetString(Star.MapLabel);
  if High(av) > 1 then
    Star.MapLabel := av[2].GetString;
end;
procedure SF_PlanetName(av: array of TVarEC; code: TCodeEC);
var
  Obj: TPlanet;
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script PlanetName');
...
begin
  if High(av) <> 3 then
    raise Exception.Create('Error.Script PlanetSetGoods');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipName');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipRank');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipRankPoints');
  Ship := TObject(av[1].GetDword) as TNormalShip;
  av[0].SetInt(Ship.RankPoints);
  if High(av) > 1 then
    Ship.RankPoints := av[2].GetInt;
end;
procedure SF_ShipNextRankPoints(av: array of TVarEC; code: TCodeEC);
var
  Ship: TNormalShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipNextRankPoints');
  Ship := TObject(av[1].GetDword) as TNormalShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipRaiseRank');
  Ship := TObject(av[1].GetDword) as TNormalShip;
  if High(av) > 1 then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ConNear');
  Constellation := TConstellation(av[1].GetDword);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ConStars');
  Constellation := TConstellation(av[1].GetDword);
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ConStar');
  Constellation := TConstellation(av[1].GetDword);
  Index := av[2].GetInt;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script GalaxyStar');
  Index := av[1].GetInt;
...
begin
  if High(av) <> 5 then
    raise Exception.Create('Error.Script StarAngleBetween');
  ...
  Angle :=
      Abs(
          HeadingDifferenceDegrees(
              PointBearingDegrees(CenterStar.Position, Star1.Position),
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script FindPlanet');
  Star := TStar(av[1].GetDword);
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script CanJump');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(0);
  if (Ship.GetFuelTanks = nil) or (Ship.GetEngine = nil) then
    Exit;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ShipInStar');
  if (av[1].GetDword <> 0) and (TShip(av[1].GetDword).CurrentStar = TStar(av[2].GetDword)) then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ShipInPlanet');
  if TShip(av[1].GetDword).CurrentPlanet = TPlanet(av[2].GetDword) then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  ...
  else if (Obj is TShip) and (av[2].GetInt in [10, 11]) then
    Ship := TShip(Obj)
  else
    raise Exception.Create('Error.Script ShipStatistic - illegal object type');
  case av[2].GetInt of
    0:
    begin
      av[0].SetInt(NormalShip.TotalShipKillCount);
      if High(av) > 2 then
        NormalShip.TotalShipKillCount := av[3].GetInt;
    end;
    1:
    begin
      av[0].SetInt(NormalShip.PirateKillCount);
      if High(av) > 2 then
        NormalShip.PirateKillCount := av[3].GetInt;
    end;
    2:
    begin
      av[0].SetInt(NormalShip.DominatorKillCount);
      if High(av) > 2 then
        NormalShip.DominatorKillCount := av[3].GetInt;
    end;
    3:
    begin
      av[0].SetInt(NormalShip.LiberatedSystemCount);
      if High(av) > 2 then
        NormalShip.LiberatedSystemCount := av[3].GetInt;
    end;
    4:
    begin
      av[0].SetInt(NormalShip.CivilianKillCount);
      if High(av) > 2 then
        NormalShip.CivilianKillCount := av[3].GetInt;
    end;
    5:
    begin
      av[0].SetInt(NormalShip.MilitaryKillCount);
      if High(av) > 2 then
        NormalShip.MilitaryKillCount := av[3].GetInt;
    end;
    6:
    begin
      av[0].SetInt(NormalShip.RangerKillCount);
      if High(av) > 2 then
        NormalShip.RangerKillCount := av[3].GetInt;
    end;
    7:
      ...
      begin
        ...
        if High(av) > 2 then
          GetPlayer.BlackHoleKillCount := av[3].GetInt;
      end;
    8:
      if GetPlayer = NormalShip then
      begin
        ...
        if High(av) > 2 then
          GetPlayer.HyperspaceKillCount := av[3].GetInt;
      end;
    9:
    begin
      av[0].SetInt(NormalShip.TradeLossBalance);
      if High(av) > 2 then
        NormalShip.TradeLossBalance := av[3].GetInt;
    end;
    10:
    begin
      ...
      begin
        ...
        begin
          ...
          if I >= 0 then
            Ship.HomePlanet.Warriors.Delete(I);
    ...
    begin
      ...
      if High(av) > 2 then
        NormalShip.ContrabandProfit := av[3].GetInt;
    end;
  else
    raise Exception.Create('Error.Script ShipStatistic');
  end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipMoney');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(Ship.Money);
  if High(av) >= 2 then
    Ship.SetMoney(Max(0, av[2].GetInt));
end;
procedure SF_ShipFuel(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipFuel');
  Ship := TShip(av[1].GetDword);
  if (Ship = nil) or (Ship.GetFuelTanks = nil) then
    av[0].SetInt(0)
  else
    av[0].SetInt(Ship.GetFuelTanks.Fuel);
  if (High(av) >= 2) and (Ship.GetFuelTanks <> nil) then
  begin
    Ship.GetFuelTanks.Fuel := av[2].GetInt;
    if Ship.GetFuelTanks.Fuel < 0 then
      Ship.GetFuelTanks.Fuel := 0;
  end;
end;
procedure SF_ShipFuelLow(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipFuelLow');
  Ship := TShip(av[1].GetDword);
  if Ship.GetFuelTanks = nil then
    av[0].SetInt(1)
  else if Ship.GetFuelTanks.Fuel < Ship.GetFuelTanks.Capacity then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipStrengthInBestRanger');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipStrength');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ChanceToWin');
  Ship := TShip(av[1].GetDword);
  Target := TShip(av[2].GetDword);
  if (High(av) > 2) and (av[3].GetInt <> 0) then
    av[0].SetFloat(Ship.ChanceToWin(Target))
  else
    av[0].SetInt(Ship.GetWinChancePercent(Target));
end;
procedure SF_RangerStatus(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script RangerStatus');
  Obj := TObject(av[1].GetDword);
  if High(av) > 1 then
  begin
  ...
  begin
    if not (Obj is TRanger) then
      raise Exception.Create('Error.Script RangerStatus 2');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script RangerPlaceInRating');
  Obj := TObject(av[1].GetDword);
  if not (Obj is TRanger) then
    raise Exception.Create('Error.Script RangerPlaceInRating 2');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script RangerExcludedFromRating');
  Obj := TObject(av[1].GetDword);
  if not (Obj is TRanger) then
    raise Exception.Create('Error.Script RangerExcludedFromRating 2');
  Ranger := TRanger(Obj);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipFind');
  av[0].SetDword(0);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipDestroy');
  if av[1].GetDword = 0 then
  begin
    av[0].SetInt(0);
    Exit;
  ...
  if High(av) <= 1 then
    TShip(av[1].GetDword).DestroyQueued := True
  else if av[2].GetInt >= 0 then
    TShip(av[1].GetDword).DestroyQueued := Boolean(av[2].GetInt);
end;
procedure SF_ShipDestroyType(av: array of TVarEC; code: TCodeEC);
var
  I, J: Integer;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipDestroyType');
  Kind := av[1].GetInt;
  ...
  begin
    ...
    begin
      ...
      if (Kind = 1)
          ...
          and (TKling(Ship).DominatorSeries = dsBlazer)
          and (TKling(Ship).KlingType <> ktBoss)
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemDestroy');
  Obj := TObject(av[1].GetDword);
  Item := nil;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script GroupToShip');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script OrderJump');
  Absolute := False;
  if High(av) >= 3 then
    Absolute := Boolean(av[3].GetInt);
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
      ...
      AppendLogLineThreadSafe(
          AnsiString(
              'Warning! '
                  ...
                  + ' orders ship '
                  + Ship.GetFullName(' ')
                  + ' to jump while landed on planet!'
          )
      );
    ...
    if (GetPlayer = Ship)
        and (GetInnermostScreenLoop = StarMapScreen)
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script OrderLanding');
  Absolute := False;
  if High(av) >= 3 then
    Absolute := Boolean(av[3].GetInt);
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script IsPlayer');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script GroupCount');
  GroupIndex := av[1].GetInt;
  Count := 0;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GroupIn');
  GroupIndex := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CountIn');
  GroupIndex := av[1].GetInt;
  ...
  begin
    ...
    begin
      ...
      begin
        Binding := TScriptShip(CurrentScript.Ships[I]);
        if ExcludeHyperspace and Binding.Ship.InHyperspace then
          Continue;
        if (Binding.GroupIndex = GroupIndex)
            and (Binding.Ship.CurrentStar.Constellation = Location) then
          Inc(FoundCount);
      end;
    end
    else if Location is TStar then
    begin
      ...
      begin
        ...
        if ExcludeHyperspace and Binding.Ship.InHyperspace then
          Continue;
        if ExcludeHyperspace
            and (Binding.Ship.DockedTo <> nil)
            and Binding.Ship.DockedTo.InHyperspace then
          Continue;
    ...
    begin
      ...
      begin
        Binding := TScriptShip(CurrentScript.Ships[I]);
        if (Binding.GroupIndex = GroupIndex)
            and (Binding.Ship = GetPlayer)
            and (GetPlayer.RuinsMode <> 0)
            and (GetPlayer.RuinsSavedPlanet = Location) then
          Inc(FoundCount)
        else if (Binding.GroupIndex = GroupIndex)
            and Binding.Ship.IsOnPlanet
            and (Binding.Ship.CurrentPlanet = Location) then
          Inc(FoundCount);
      end;
    end
    else if Location is TShip then
    begin
      ...
      begin
        ...
        if (Binding.GroupIndex = GroupIndex)
            and (Binding.Ship = GetPlayer)
            and (GetPlayer.RuinsMode <> 0)
            and (GetPlayer.RuinsSavedDockedTo = Location) then
          Inc(FoundCount)
        else if (Binding.GroupIndex = GroupIndex)
            and Binding.Ship.IsDockedToShip
            and (Binding.Ship.DockedTo = Location) then
          Inc(FoundCount);
      end;
    end
    else if Location is TScriptPlace then
    begin
      ...
      begin
        Binding := TScriptShip(CurrentScript.Ships[I]);
        if (Binding.GroupIndex = GroupIndex)
            and TScriptPlace(Location).ShipInPlace(Binding.Ship) then
          Inc(FoundCount);
      end;
    end;
  end;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script NearestGroup');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ChangeState');
  if av[1].RealVType <> vkString then
    Index := av[1].GetInt
  else
  begin
    Index := 0;
    while Index < CurrentScript.States.Count do
    begin
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script StarAngle');
...
begin
    ...
    begin
      if TShip(Obj).InNormalSpace then
        if GetPlayer.InNormalSpace then
          InPlayerSystem := GetPlayer.CurrentStar = TShip(Obj).CurrentStar;
    end
    else if Obj is TPlanet then
    begin
      if GetPlayer.InNormalSpace then
        InPlayerSystem := GetPlayer.CurrentStar = TPlanet(Obj).CurrentStar;
    end
  ...
  begin
    ...
    begin
      if Obj is TShip then
        MessageEntry.Targets[0].ShipId := TShip(Obj).Id
      else
        MessageEntry.Targets[0].PlanetId := TPlanet(Obj).Id;
    end;
    if High(av) >= 5 then
    ...
    begin
      if Obj is TShip then
        MessageEntry.Targets[1].ShipId := TShip(Obj).Id
      else
        MessageEntry.Targets[1].PlanetId := TPlanet(Obj).Id;
    end;
    if High(av) >= 6 then
    ...
    begin
      if Obj is TShip then
        MessageEntry.Targets[2].ShipId := TShip(Obj).Id
      else
        MessageEntry.Targets[2].PlanetId := TPlanet(Obj).Id;
    end;
    av[0].SetInt(1);
  end;
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script EtherDelete');
  RemovePlayerBubbleByKey(av[1].GetString);
end;
procedure SF_EtherIdAdd(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script EtherIdAdd');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script EtherIdDelete');
  while True do
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EtherState');
...
begin
  if High(av) <> 3 then
    raise Exception.Create('Error.Script ConChangeRelationToRanger');
...
begin
  ...
  begin
    Ship := TShip(av[2].GetDword);
    if (Ship = GetPlayer) and (CurrentScript = nil) then
      Exit;
  end
  else
    Ship := CurrentScript.CurrentShip;
  if Ship = GetPlayer then
    ...
    av[0].SetDword(TScriptShip(Ship.ScriptShip).Data[Index]);
end;
procedure SF_SetData(av: array of TVarEC; code: TCodeEC);
var
  Index: Integer;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SetData');
  Index := 0;
  if High(av) >= 2 then
    Index := av[2].GetInt;
  if High(av) >= 3 then
  ...
  if Ship = GetPlayer then
    ...
    TScriptShip(Ship.ScriptShip).Data[Index] := av[1].GetDword;
end;
procedure SF_ShipData(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetDword(GetScriptShipBindingForContext(CurrentScript.CurrentShip, CurrentScript).Data[0]);
  if High(av) >= 1 then
    GetScriptShipBindingForContext(CurrentScript.CurrentShip, CurrentScript).Data[0] :=
        av[1].GetDword;
end;
procedure SF_Format(av: array of TVarEC; code: TCodeEC);
var
  Text, Color: WideString;
  I, Count: Integer;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script Format');
  if High(av) = 1 then
  begin
    Text := av[1].GetString;
    ...
    for I := 0 to Count - 1 do
      Text :=
          ReplaceAllWideString(
              Text,
              av[2 + I * 2].GetString,
              WrapTextInColor(av[2 + I * 2 + 1].GetString, Color)
          );
    av[0].SetString(Text);
  end;
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DeleteTags');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script Dialog');
  av[0].SetInt(0);
  if ExitScreenLoop then
    Exit;
  if GetPlayer = nil then
    Exit;
  if (High(av) = 1) and (av[1].GetDword = 0) then
    Exit;
  if not GetPlayer.InNormalSpace then
    Exit;
  if CurrentScreenId <> screenStarMap then
    Exit;
  if TalkDialogActive then
    Exit;
  Immediate :=
      TurnCalculationPhase
  ...
  begin
    ...
    else if TShip(av[1].GetDword).OpenPlayerConversation(False) then
      av[0].SetInt(1);
    ScriptUnSnap(Snapshot);
    Exit;
  end;
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else if Binding.Ship.OpenPlayerConversation(False) then
              av[0].SetInt(1);
            ScriptUnSnap(Snapshot);
            Exit;
          end;
    ...
    begin
      ...
      begin
        ...
        else if TShip(Obj).OpenPlayerConversation(False) then
          av[0].SetInt(1);
        ScriptUnSnap(Snapshot);
        Exit;
      end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DText');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DAddText');
  if GetPlayer.IsDockedToShip then
    RuinsTalkScreen.DialogText :=
        RuinsTalkScreen.DialogText
            + ReplaceAllWideString(
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script DAdd');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DAnswer');
  if GetPlayer.IsDockedToShip then
  begin
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptTakeoffChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
        RuinsTalkScreen.AddScriptTakeoffChoice('');
    end
    else if (ExtractDelimitedPartW(LowerCase(AnsiString(av[1].GetString)), 0, '~') = 'exit')
        or (ExtractDelimitedPartW(LowerCase(AnsiString(av[1].GetString)), 0, '~') = 'fastexit') then
    begin
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptNewsExitChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptGameEndChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptHangarChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptRestartChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen
            .AddScriptGoodsChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      Count := CountDelimitedPartsW(av[1].GetString, '~');
      if Count > 1 then
        RuinsTalkScreen.AddChoice(
            '- ' + ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'),
            0,
    ...
    begin
      ...
      if Count > 1 then
        RuinsTalkScreen.AddChoice(
            ...
            RuinsTalkScreen.RunScriptAnswerKeepingScroll
        )
      else
        RuinsTalkScreen.AddChoice(
            '',
      ...
      RuinsTalkScreen.AddChoice(
          '- ' + av[1].GetString,
          CurrentScript.CurrentAnswer,
          RuinsTalkScreen.RunScriptAnswer
      );
  end
  else if not GetPlayer.IsOnPlanet then
  begin
    if (ExtractDelimitedPartW(LowerCase(AnsiString(av[1].GetString)), 0, '~') = 'exit')
        or (ExtractDelimitedPartW(LowerCase(AnsiString(av[1].GetString)), 0, '~') = 'takeoff') then
    begin
      Count := CountDelimitedPartsW(av[1].GetString, '~');
      if Count > 1 then
    ...
    begin
      ...
      if Count > 1 then
        TalkScreen.AddChoice(
            '- ' + ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'),
            CurrentScript.CurrentAnswer,
            TalkScreen.RunScriptAnswerKeepingScroll,
            0
        )
      else
        TalkScreen
            .AddChoice('', CurrentScript.CurrentAnswer, TalkScreen.RunScriptAnswerKeepingScroll, 0);
    end
    else
      TalkScreen.AddChoice(
          '- ' + av[1].GetString,
          CurrentScript.CurrentAnswer,
          TalkScreen.RunScriptAnswer,
          0
      );
  end
  else
  begin
    ...
    begin
      ...
      if Count > 1 then
        GovernmentScreen
            .AddScriptPlanetChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        GovernmentScreen
            .AddScriptShopChoice(ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'))
      else
    ...
    begin
      ...
      if Count > 1 then
        GovernmentScreen.AddChoice(
            '- ' + ExtractDelimitedRangeW(av[1].GetString, 1, Count - 1, '~'),
            CurrentScript.CurrentAnswer,
            GovernmentScreen.RunScriptAnswerKeepingScroll
        )
      else
        GovernmentScreen.AddChoice(
            '',
      ...
      GovernmentScreen.AddChoice(
          '- ' + av[1].GetString,
          CurrentScript.CurrentAnswer,
          GovernmentScreen.RunScriptAnswer
      );
  end;
end;
procedure SF_DChange(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script DChange');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ItemExist');
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ItemInStar');
  Obj := TObject(av[1].GetDword);
  if Obj is TScriptItem then
    Item := TScriptItem(Obj).Item
  else
  ...
  begin
    ...
    begin
      ...
      if (Binding.GroupIndex = Integer(Obj))
          and ((Binding.Ship.Inventory.IndexOf(Item) > 0)
              or (Binding.Ship.Artefacts.IndexOf(Item) > 0)) then
      begin
        av[0].SetInt(1);
        Exit;
      end;
  ...
  begin
    Star := Obj as TStar;
    if Star.Items.IndexOf(Item) < 0 then
      av[0].SetInt(0)
    else
      av[0].SetInt(1);
  end
  ...
  begin
    Ship := Obj as TShip;
    if (Ship.Inventory.IndexOf(Item) < 0) and (Ship.Artefacts.IndexOf(Item) < 0) then
      av[0].SetInt(0)
    else
      av[0].SetInt(1);
  end
  ...
  begin
    ...
    begin
      if FindShopSlotByItem(Item) = nil then
        av[0].SetInt(0)
      else
        av[0].SetInt(1);
    end
    else if Planet.EquipmentShop.IndexOf(Item) < 0 then
      av[0].SetInt(0)
    else
      av[0].SetInt(1);
  end;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script SF_ItemCount');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
  Count := 0;
  for I := 0 to Ship.Inventory.Count - 1 do
    if TItem(Ship.Inventory[I]).ItemType = TItemType(Kind) then
      Inc(Count);
  for I := 0 to Ship.Artefacts.Count - 1 do
    if TItem(Ship.Artefacts[I]).ItemType = TItemType(Kind) then
      Inc(Count);
  av[0].SetInt(Count);
end;
procedure SF_DecayGoods(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script DecayGoods');
  Kind := av[2].GetInt;
  Obj := TObject(av[1].GetDword);
  if Obj is TPlanet then
    (Obj as TPlanet).ForceGoodsScarcity(True, [Kind]);
end;
procedure SF_UpsurgeGoods(av: array of TVarEC; code: TCodeEC);
var
  Kind: Byte;
...
begin
  if High(av) <> 3 then
    raise Exception.Create('Error.Script GoodsAdd');
  Kind := av[2].GetInt;
  Obj := TObject(av[1].GetDword);
  if Obj is TPlanet then
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GoodsCount');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GoodsCost');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GoodsRuinsForBuy');
  Kind := av[2].GetInt;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShipGoods');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
  ...
  if High(av) > 3 then
    ...
    Cost := Quantity * GoodsMarket[Kind].AveragePrice
  else
    Cost := Round(Quantity * Ship.CargoGoods[Kind].TotalCost / Ship.CargoGoods[Kind].Count);
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShipGoodsIllegalOnPlanet');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script GoodsDrop');
  Ship := TShip(av[1].GetDword);
  Kind := av[2].GetInt;
  ...
  begin
    ...
    Item :=
        PMovingDropItemEntry(
                    Ship.CurrentStar.MovingDropItems[Ship.CurrentStar.MovingDropItems.Count - 1])
                .Payload
            as TItem;
  end
  else
  begin
    ...
    Angle :=
        SeededRandomIntRange(
                0,
                360,
                Kind * (Cardinal(Galaxy.CurrentTurn) * GetPlayer.CurrentStar.GenerationSeed))
            * Pi
            / 180;
  ...
  begin
    ...
    if ScriptItem.Item <> nil then
      ScriptItem.Item.ScriptItem := nil;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script UselessItemCreate');
  ...
  if not (Place.PlaceKind
      in [spkPolar, spkPlanetPosition, spkStarDirection, spkGroupCentroid, spkCoordinates]) then
    RaiseWideMessage('Error.Script UselessItemCreate place');
  Place.OriginStar.Items.Add(Item);
  Item.Position := Place.GetPoint;
  Angle :=
      HeadingDegreesToRadians(
          SeededRandomIntRange(0, 359, Item.Id * Place.OriginStar.GenerationSeed)
      );
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GoodsSellPrice');
  Obj := TObject(av[1].GetDword);
  Kind := av[2].GetInt;
  if Obj is TRuins then
    av[0].SetInt(TRuins(Obj).ShopGoods[Kind].BaseSalePrice)
  else
    av[0].SetInt(TPlanet(Obj).Goods[Kind].BaseSalePrice);
  if High(av) > 2 then
  begin
    if Obj is TRuins then
      TRuins(Obj).ShopGoods[Kind].BaseSalePrice := av[3].GetInt
    else
      TPlanet(Obj).Goods[Kind].BaseSalePrice := av[3].GetInt;
  end;
end;
procedure SF_GoodsBuyPrice(av: array of TVarEC; code: TCodeEC);
var
...
begin
  ...
  Kind := av[2].GetInt;
  if Obj is TRuins then
    av[0].SetInt(TRuins(Obj).ShopGoods[Kind].PurchasePrice)
  else
    av[0].SetInt(TPlanet(Obj).Goods[Kind].PurchasePrice);
  if High(av) > 2 then
  begin
    if Obj is TRuins then
      TRuins(Obj).ShopGoods[Kind].PurchasePrice := av[3].GetInt
    else
      TPlanet(Obj).Goods[Kind].PurchasePrice := av[3].GetInt;
  end;
end;
procedure SF_CountTurn(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script CountTurn');
  Turns := 0;
  Ship := TShip(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  ...
  begin
    ...
    while OtherShip.DockedTo <> nil do
      OtherShip := OtherShip.DockedTo;
    ...
    begin
      ...
      if OtherShip.InHyperspace then
        Position := OtherShip.GetArrivalPosition(OtherShip.TransitOriginStar);
    end;
  end
  else
    raise Exception.Create('Error.Script CountTurn 1');
  ...
  begin
    if Ship.CurrentPlanet = nil then
      Inc(Turns, Ceil(PointDistance(Position, Ship.Position) / Max(1, Ship.Speed)))
    else
      Inc(
          Turns,
          Ceil(PointDistance(Position, Ship.CurrentPlanet.GetPosition) / Max(1, Ship.Speed))
      );
  end
  else
  begin
    if Ship.CurrentPlanet = nil then
      Inc(
          Turns,
          Ceil(PointDistance(Ship.GetJumpDeparturePoint(Star), Ship.Position) / Max(1, Ship.Speed))
      )
    else
      Inc(
          Turns,
          Ceil(
              PointDistance(Ship.GetJumpDeparturePoint(Star), Ship.CurrentPlanet.GetPosition)
                  / Max(1, Ship.Speed)
          )
      );
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ShipSetBad');
  if av[1].GetDword <> 0 then
    TShip(av[1].GetDword).EnemyShip := TShip(av[2].GetDword);
end;
procedure SF_GroupSetBad(av: array of TVarEC; code: TCodeEC);
var
  GroupIndex, I, Count: Integer;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script GroupSetBad');
  GroupIndex := av[1].GetInt;
  Count := CurrentScript.Ships.Count;
  for I := 0 to Count - 1 do
  begin
    Binding := TScriptShip(CurrentScript.Ships[I]);
    if Binding.GroupIndex = GroupIndex then
      Binding.Ship.EnemyShip := TShip(av[2].GetDword);
  end;
end;
procedure SF_ShipSetPartner(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipJoin');
  Ship := TShip(av[2].GetDword);
  if Ship = nil then
    Exit;
  ...
  begin
    if TScriptShip(Ship.ScriptShip).Script <> CurrentScript then
      AppendLogLineThreadSafe(
          AnsiString(
              'Warning.Script ShipJoin ('
                  + CurrentScript.ScriptFileName
                  + '): ship '
                  + Ship.GetFullName(' ')
                  + ' was already in another script ('
  ...
  { A nonstring third argument suppresses the state change; it is not a state index. }
    ...
    StateIndex := TScriptGroup(CurrentScript.Groups[Binding.GroupIndex]).InitialStateIndex;
  if High(av) >= 4 then
    Binding.Data[0] := av[4].GetDword;
  if High(av) >= 5 then
    Binding.Data[1] := av[5].GetDword;
  if High(av) >= 6 then
    Binding.Data[2] := av[6].GetDword;
  if High(av) >= 7 then
    Binding.Data[3] := av[7].GetDword;
  if StateIndex >= 0 then
    CurrentScript.ChangeState(Binding, StateIndex);
  ScriptUnSnap(Snapshot);
end;
procedure SF_ShipOut(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInScript');
  if TShip(av[1].GetDword).HasScriptBindings
      or (((High(av) <= 1) or (av[2].GetInt <> 0))
          and (TShip(av[1].GetDword).AbsoluteScriptOrder > 0)) then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipInGameEvent');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(0);
  if (Ship is TWarrior) and ((Ship as TWarrior).LiberationGroup <> nil) then
    av[0].SetInt(1);
  if (Ship is TRuins) and ((Ship as TRuins).FlyToStar <> nil) then
    av[0].SetInt(1);
end;
procedure SF_ShipInCurScript(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipInCurScript');
  if TShip(av[1].GetDword).HasScriptBindings
      and (GetScriptShipBindingForContext(TShip(av[1].GetDword), CurrentScript).Script
          = CurrentScript) then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipInNormalSpace');
  if (av[1].GetDword <> 0) and TShip(av[1].GetDword).InNormalSpace then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInHole');
  Ship := TShip(av[1].GetDword);
  if (High(av) > 1) and (av[2].GetInt <> 0) then
    while Ship.DockedTo <> nil do
      Ship := Ship.DockedTo;
  if Ship.InHyperspace and (Ship.Order = soJumpHole) then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipIsTakeoff');
  Ship := TShip(av[1].GetDword);
  if Ship.Order = soTakeoff then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipCntWeapon');
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script ShipWeapon');
  Ship := TShip(av[1].GetDword);
  I := av[2].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ArtefactTypeInUse');
  Ship := TShip(av[1].GetDword);
  Item := nil;
  if (av[2].RealVType = vkDword) and (av[2].GetDword > $FFFF) then
    Item := TArtefact(av[2].GetDword);
  Name := '';
  if (Item <> nil) and (Item.GetEffectiveType in [t_Artefact, t_Artefact2]) then
    Name := Item.ConfigBlockName
  else if av[2].RealVType = vkString then
    Name := av[2].GetString;
  if Name <> '' then
  begin
    Count := 0;
    ...
    begin
      ...
      if (Item.ItemType in [t_Artefact, t_Artefact2])
          and (Item.ConfigBlockName = Name)
          and (Item.BrokenFlag = 0)
          and (Item.EquippedFlag <> 0) then
        Inc(Count);
    end;
    av[0].SetInt(Count);
  end
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipGroup');
  Binding := GetScriptShipBindingForContext(TShip(av[1].GetDword), CurrentScript);
  if Binding <> nil then
    av[0].SetInt(Binding.GroupIndex)
  else
    av[0].SetInt(-1);
end;
procedure SF_ShipSpeed(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipSpeed');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EnginePower');
  Obj := TObject(av[1].GetDword);
  Engine := nil;
  if Obj is TEngine then
    Engine := TEngine(Obj)
  else if (Obj is TScriptItem)
      and ((Obj as TScriptItem).Item <> nil)
      and ((Obj as TScriptItem).Item is TEngine) then
    Engine := TEngine((Obj as TScriptItem).Item)
  else if Obj is TShip then
    Engine := TShip(Obj).GetEngine;
  if Engine <> nil then
  begin
    av[0].SetInt(Engine.OutputPercent);
    if High(av) > 1 then
      Engine.OutputPercent := av[2].GetInt;
  end
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipJump');
  av[0].SetInt(TShip(av[1].GetDword).GetJumpRange);
end;
procedure SF_ShipArmor(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipArmor');
  av[0].SetInt(TShip(av[1].GetDword).GetArmor);
end;
procedure SF_ShipProtectability(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipProtectability');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipDroidRepair');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if Ship.IsEquipmentUsable(Ship.GetRepairRobot) then
    av[0].SetInt(Ship.CalculateRepairPoints(Ship.GetRepairRobot));
end;
procedure SF_ShipRadarRange(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipRadarRange');
  av[0].SetInt(TShip(av[1].GetDword).GetRadarRange);
end;
procedure SF_ShipScanerPower(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipScanerPower');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipHookPower');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if Ship.IsEquipmentUsable(Ship.GetCargoHook) then
    av[0].SetInt(Ship.CalculateCargoHookPower(Ship.GetCargoHook));
end;
procedure SF_ShipHookRange(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipHookRange');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if Ship.IsEquipmentUsable(Ship.GetCargoHook) then
    av[0].SetInt(Ship.GetCargoHookRange);
end;
procedure SF_ShipAverageDamage(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipAverageDamage');
  Ship := TShip(av[1].GetDword);
  MinDamage := 0;
  MaxDamage := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        MinDamage :=
            MinDamage
                + Ship.GetWeaponMinDamage(Weapon) * Weapon.GetAttackCount * Weapon.GetShotCount;
        MaxDamage :=
            MaxDamage
                + Ship.GetWeaponMaxDamage(Weapon) * Weapon.GetAttackCount * Weapon.GetShotCount;
      end
      else
      begin
  ...
  if High(av) > 1 then
  begin
    if av[2].GetInt = 0 then
      av[0].SetInt(MinDamage)
    else if av[2].GetInt = 1 then
      av[0].SetInt(MaxDamage)
    else
      av[0].SetInt(Round((MinDamage + MaxDamage) * 0.5));
  end
  else
    av[0].SetInt(Round((MinDamage + MaxDamage) * 0.5));
end;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipHealthFactor');
  Ship := TShip(av[1].GetDword);
  Index := av[2].GetInt;
  Remaining := 0;
  if Ship = nil then
  ...
  if Index = 0 then
  begin
    if Ship.HasRadiationSickness then
      Remaining := Ship.RadiationHealth[1].ExpireTurn - Galaxy.CurrentTurn;
    if High(av) > 2 then
    begin
      Duration := av[3].GetInt;
      if Duration = -1 then
        Duration := RadiationHealthDefinitions[1].Duration;
      if Duration = 0 then
      begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipHealthFactorStatus');
  Ship := TShip(av[1].GetDword);
  Index := av[2].GetInt;
  if Index = 0 then
  begin
...
begin
  ...
  if High(av) >= 1 then
    GetPlayer.DiseaseImmunity := av[1].GetInt;
end;
procedure SF_ShipStatusEffect(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipStatusEffect');
  Ship := TShip(av[1].GetDword);
  ...
  if Index < 0 then
    ...
    av[0].SetFloat(PCombatStatusEffect(Ship.CombatStatusEffects[Index]).Strength);
  if High(av) > 2 then
  begin
    Strength := av[3].GetFloat;
    if High(av) > 3 then
      Source := TShip(av[4].GetDword)
    else
      Source := nil;
    if Strength > 0 then
      Ship.AddCombatStatusStrength(EffectType, Strength, Source)
    else if Strength < 0 then
      Ship.ReduceCombatStatusStrength(EffectType, -Strength);
  end;
end;
procedure SF_GroupIs(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GroupIs');
  Binding := GetScriptShipBindingForContext(TShip(av[1].GetDword), CurrentScript);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StateIs');
  Binding := GetScriptShipBindingForContext(TShip(av[1].GetDword), CurrentScript);
  if Binding = nil then
  begin
    av[0].SetInt(0);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script Dist');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj <> nil then
  begin
    if Obj is TShip then
      Point1 := (Obj as TShip).Position
    else if Obj is TItem then
      Point1 := (Obj as TItem).Position
    else if Obj is TScriptItem then
    begin
      if (Obj as TScriptItem).Item <> nil then
        Point1 := (Obj as TScriptItem).Item.Position
      else
        raise Exception.Create('Error.Script Dist 1');
    end
    else if Obj is TScriptPlace then
      Point1 := (Obj as TScriptPlace).GetPoint
    else if Obj is TPlanet then
      Point1 := (Obj as TPlanet).GetPosition
    else if Obj is TStar then
      Point1 := (Obj as TStar).Position
    else if Obj is TAsteroid then
      Point1 := (Obj as TAsteroid).Position
    else if Obj is TMissile then
      Point1 := (Obj as TMissile).Position
    else
      raise Exception.Create('Error.Script Dist 2');
    Obj := TObject(av[2].GetDword);
    if Obj <> nil then
    begin
      if Obj is TShip then
        Point2 := (Obj as TShip).Position
      else if Obj is TItem then
        Point2 := (Obj as TItem).Position
      else if Obj is TScriptItem then
      begin
        if (Obj as TScriptItem).Item <> nil then
          Point2 := (Obj as TScriptItem).Item.Position
        else
          raise Exception.Create('Error.Script Dist 3');
      end
      else if Obj is TScriptPlace then
        Point2 := (Obj as TScriptPlace).GetPoint
      else if Obj is TPlanet then
        Point2 := (Obj as TPlanet).GetPosition
      else if Obj is TStar then
        Point2 := (Obj as TStar).Position
      else if Obj is TAsteroid then
        Point2 := (Obj as TAsteroid).Position
      else if Obj is TMissile then
        Point2 := (Obj as TMissile).Position
      else
        raise Exception.Create('Error.Script Dist 4');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script Angle');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj <> nil then
  begin
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script DistStar');
  av[0]
      .SetFloat(
          PointDistanceSquared(TStar(av[1].GetDword).Position, TStar(av[2].GetDword).Position));
end;
procedure SF_BuyPirate(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyPirate');
  MoneyPercent := 100;
  if High(av) > 1 then
    MoneyPercent := av[2].GetInt;
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyTransport');
  Planet := TPlanet(av[1].GetDword);
  MoneyPercent := 100;
  if High(av) > 2 then
    MoneyPercent := av[3].GetInt;
  if Planet <> nil then
  begin
    if High(av) > 1 then
      case av[2].GetInt of
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script Name');
  Obj := TObject(av[1].GetDword);
  if Obj is TStar then
    av[0].SetString(TStar(Obj).Name)
  else if Obj is TConstellation then
    av[0].SetString(TConstellation(Obj).GetName)
  else if Obj is TPlanet then
    av[0].SetString(TPlanet(Obj).Name)
  else if Obj is TShip then
    av[0].SetString(TShip(Obj).GetFullName(' '))
  else if Obj is TScriptItem then
  begin
    if TScriptItem(Obj).Item = nil then
      av[0].SetString('NameError')
    else
      av[0].SetString(TScriptItem(Obj).Item.GetDisplayName);
  end
  else if Obj is TItem then
    av[0].SetString(TItem(Obj).GetDisplayName)
  else
    av[0].SetString('NameError');
end;
procedure SF_ShortName(av: array of TVarEC; code: TCodeEC);
...
begin
  ...
  if Obj is TStar then
    ...
    av[0].SetString(TShip(Obj).GetName)
  else if Obj is TScriptItem then
  begin
    if TScriptItem(Obj).Item = nil then
      av[0].SetString('NameError')
    else
      av[0].SetString(TScriptItem(Obj).Item.GetShortName);
  end
  else if Obj is TItem then
    av[0].SetString(TItem(Obj).GetShortName)
  else
    av[0].SetString('NameError');
end;
procedure SF_FirstGiveMoney(av: array of TVarEC; code: TCodeEC);
    ...
    case ProgramId of
      ...
      begin
        if High(av) > 2 then
          ...
          Count := SeededRandomIntRange(1, 3, Galaxy.CurrentTurn);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DomikProgrammDate');
  Ship := TKling(av[1].GetDword);
  av[0].SetInt(Ship.ActiveProgramAppliedTurn);
  if High(av) > 1 then
    Ship.ActiveProgramAppliedTurn := av[2].GetInt;
end;
procedure SF_HoleMamaCreate(av: array of TVarEC; code: TCodeEC);
var
  Angle, Scale: Single;
...
begin
  if High(av) <> 2 then
    raise Exception.Create('Error.Script HoleCreate');
  Hole := THole.Create;
  Hole.InitializeGraphic('');
  THoleSE(Hole.Graphic).SetState(1);
  Hole.CreatedTurn := Galaxy.CurrentTurn;
  ...
  if not (Place.PlaceKind
      in [spkPolar, spkPlanetPosition, spkStarDirection, spkGroupCentroid, spkCoordinates]) then
    RaiseWideMessage('Error.Script HoleCreate place 1');
  Hole.Star1 := Place.OriginStar;
  Angle :=
      HeadingDegreesToRadians(SeededRandomIntRange(0, 359, Hole.Id * Hole.Star1.GenerationSeed));
  ...
  if not (Place.PlaceKind
      in [spkPolar, spkPlanetPosition, spkStarDirection, spkGroupCentroid, spkCoordinates]) then
    RaiseWideMessage('Error.Script HoleCreate place 2');
  Hole.Star2 := Place.OriginStar;
  Angle :=
      HeadingDegreesToRadians(SeededRandomIntRange(0, 359, Hole.Id * Hole.Star2.GenerationSeed));
  ...
  begin
    ...
    PrimaryFilm.AttachObject(0, Film);
      ...
      and ((GetPlayer.CurrentStar = Hole.Star1) or (GetPlayer.CurrentStar = Hole.Star2)) then
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script BlazerLanding');
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script NearCivilPlanet');
  Ship := TShip(av[1].GetDword);
  av[0].SetDword(0);
  BestDistance := 1E30;
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script Sound');
  SoundManager.PlaySound(av[1].GetString);
end;
procedure SF_Tips(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script Tips');
  ShowPlayerTipOnce(av[1].GetInt);
end;
procedure SF_TipsState(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script TipsState');
...
var
  Text: WideString;
  Block: TBlockParEC;
  procedure AddConfigBlock(
      Block: TBlockParEC;
      Dest: TVarEC
  ); // @addr 0x616034 @ida "void __usercall $name(TBlockParEC *Block@<eax>, TVarEC *Dest@<edx>, void *ParentFrame@<^0>);" @note "Nested in SF_CT. Appends to Dest's existing array; empty child blocks contain one vkEmpty placeholder."
  var
    I: Integer;
    Value: TVarEC;
  begin
    ...
    begin
      ...
      if Value.GetArray.Count = 0 then
        Value.GetArray.AddItem(TVarEC.Create(vkEmpty));
      Dest.GetArray.AddItem(Value);
    end;
  end;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CT');
  if High(av) > 1 then
  begin
    ...
    else if av[2].RealVType <> vkArray then
      raise Exception.Create('Error.Script CT - 2nd argument is not an array');
    av[2].GetArray.Clear;
    if av[1].GetString = '' then
      Block := LanguageDataConfig
    else
      Block := LanguageDataConfig.FindBlockByPath(av[1].GetString);
    if Block <> nil then
      AddConfigBlock(Block, av[2]);
    av[0].SetInt(av[2].GetArray.Count);
    if av[2].GetArray.Count = 0 then
      av[2].GetArray.AddItem(TVarEC.Create(vkEmpty));
  end
  else
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BlockExist');
  Path := av[1].GetString;
  Count := CountDelimitedPartsW(Path, '.');
  if High(av) < 2 then
    Block := LanguageDataConfig
  else if av[2].GetString = 'Lang' then
    Block := LanguageDataConfig
  else if av[2].GetString = 'Main' then
    Block := MainDataConfig
  else if av[2].GetString = 'Config' then
    Block := UserSettingsConfig
  else
    raise Exception.Create('Error.Script BlockExist - unknown type ' + av[2].GetString);
  av[0].SetInt(0);
  for I := 0 to Count - 1 do
  begin
    if Block.CountBlocks(ExtractDelimitedPartW(Path, I, '.')) = 0 then
      Exit;
...
var
  Block: TBlockParEC;
  procedure AddMainDataBlock(
      Block: TBlockParEC;
      Dest: TVarEC
  ); // @addr 0x6167B0 @ida "void __usercall $name(TBlockParEC *Block@<eax>, TVarEC *Dest@<edx>, void *ParentFrame@<^0>);" @note "Nested in SF_GetMainData. Appends to Dest's existing array; empty child blocks contain one vkEmpty placeholder."
  var
    I: Integer;
    Value: TVarEC;
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetMainData');
  if High(av) > 1 then
  begin
    if av[2].RealVType = vkEmpty then
    begin
      ...
      raise Exception.Create('Error.Script GetMainData - 2nd argument is not an array');
    av[2].GetArray.Clear;
    if av[1].GetString = '' then
      Block := MainDataConfig
    else
      Block := MainDataConfig.FindBlockByPath(av[1].GetString);
    if Block <> nil then
      AddMainDataBlock(Block, av[2]);
    av[0].SetInt(av[2].GetArray.Count);
    if av[2].GetArray.Count = 0 then
      av[2].GetArray.AddItem(TVarEC.Create(vkEmpty));
  end
  else if MainDataConfig.CountParamsByPath(av[1].GetString) > 0 then
    av[0].SetString(MainDataConfig.GetParamByPath(av[1].GetString))
  else
    av[0].SetString('');
end;
procedure SF_GetGameOptions(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script GetGameOptions');
  if av[1].GetString = 'ResolutionX' then
    av[0].SetInt(GameScreenWidth)
  else if av[1].GetString = 'ResolutionY' then
    av[0].SetInt(GameScreenHeight)
  else
    try
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ResourceExist');
  av[0].SetInt(Ord(CacheDataRoot.FileExistsByPath(av[1].GetString)));
end;
procedure SF_CurrentMods(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    av[0].SetInt(CountDelimitedPartsW(SelectedMods, ','))
  else if av[1].RealVType = vkString then
    av[0].SetInt(Ord(FindTextOffsetW(SelectedModsDisplaySuffix, ', ' + av[1].GetString + ',') >= 0))
  else if av[1].GetInt = -1 then
    av[0].SetString(SelectedMods)
  else
    av[0].SetString(ExtractDelimitedPartW(SelectedMods, av[1].GetInt, ','));
end;
procedure SF_RobotSupport(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(Ord(RobotAutoWin or ((RobotInterface <> nil) and (RobotInterface.Support() = 0))));
end;
procedure SF_StarShips(av: array of TVarEC; code: TCodeEC);
var
  Star: TStar;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarShips');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Star.Ships.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarPlanets');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Star.Planets.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarMissiles');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Star.Missiles.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarAsteroids');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Star.Asteroids.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GroupShip');
  GroupIndex := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ShipItems');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Ship.Inventory.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ShipArts');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Ship.Artefacts.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ArtTranclucatorToShip');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_TranclucatorData');
  Ship := TTranclucator(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
    if av[2].GetString = 'Proprietor' then
      Kind := 0
    else if av[2].GetString = 'ArtSize' then
      Kind := 1
    else if av[2].GetString = 'ArtSysName' then
      Kind := 2
    else if av[2].GetString = 'Docking' then
      Kind := 3
    else if av[2].GetString = 'SeekItems' then
      Kind := 4
    else if av[2].GetString = 'AutoArrange' then
      Kind := 5
    else if av[2].GetString = 'LandStorage' then
      Kind := 6
    else if av[2].GetString = 'LandPermitPlanets' then
      Kind := 7
    else if av[2].GetString = 'LandPermitRuins' then
      Kind := 8
    else if av[2].GetString = 'SeekPermitNone' then
      Kind := 9
    else if av[2].GetString = 'SeekPermitArtefact' then
      Kind := 10
    else if av[2].GetString = 'SeekPermitMicromodule' then
      Kind := 11
    else if av[2].GetString = 'SeekPermitEquipment' then
      Kind := 12
    else if av[2].GetString = 'SeekPermitUseless' then
      Kind := 13
    else if av[2].GetString = 'SeekPermitGoods' then
      Kind := 14
    else if av[2].GetString = 'SeekPermitNode' then
      Kind := 15
    else
      raise Exception.Create(
          'Error.Script SF_TranclucatorData - unknown property ' + av[2].GetString);
  end
  else
    Kind := av[2].GetInt;
  ...
  case Kind of
    ...
    begin
      ...
      if WriteValue then
        Ship.ArtefactSize := av[3].GetInt;
    end;
    2:
    begin
      av[0].SetString(Ship.ArtefactSystemName);
      if WriteValue then
        Ship.ArtefactSystemName := av[3].GetString;
    end;
    3:
    begin
      av[0].SetInt(Ord(Ship.FollowOwner));
      if WriteValue then
        Ship.FollowOwner := av[3].GetInt <> 0;
    end;
    4:
    begin
      av[0].SetInt(Ord(Ship.SeekItems));
      if WriteValue then
        Ship.SeekItems := av[3].GetInt <> 0;
    end;
    5:
    begin
      av[0].SetInt(Ord(Ship.AutoArrange));
      if WriteValue then
        Ship.AutoArrange := av[3].GetInt <> 0;
    end;
    6:
    begin
      av[0].SetInt(Ord(Ship.StoreOnLanding));
      if WriteValue then
        Ship.StoreOnLanding := av[3].GetInt <> 0;
    end;
    7:
    begin
    ...
    raise Exception.Create(
        'Error.Script SF_TranclucatorData - unknown property ' + IntToWideString(Kind));
  end;
end;
procedure SF_LinkItemToScript(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_LinkItemToScript');
  Item := TItem(av[1].GetDword);
  if Item.ScriptItem <> nil then
    (Item.ScriptItem as TScriptItem).Item := nil;
  if High(av) < 2 then
  begin
    if CurrentScript = nil then
      raise Exception.Create('Error.Script SF_LinkItemToScript - no script');
    Binding := nil;
    for I := 0 to CurrentScript.Items.Count - 1 do
    begin
      Binding := TScriptItem(CurrentScript.Items[I]);
      if (Binding.Name = '') and (Binding.Item = nil) then
        Break;
      Binding := nil;
    end;
    if Binding = nil then
    ...
    if Binding.ActionCode <> nil then
      Binding.ActionCode.Free;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ReleaseItemFromScript');
  Obj := TObject(av[1].GetDword);
  Binding := nil;
  if Obj is TItem then
    Binding := TScriptItem(TItem(Obj).ScriptItem);
  if Obj is TScriptItem then
    Binding := TScriptItem(Obj);
  if Binding.Item <> nil then
    Binding.Item.ScriptItem := nil;
  Binding.Item := nil;
end;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ScriptItemData');
  Obj := TObject(av[1].GetDword);
  Binding := nil;
  if Obj is TItem then
    Binding := TScriptItem(TItem(Obj).ScriptItem);
  ...
  case Index of
    ...
    raise Exception.Create('Error.Script ScriptItemData no');
  end;
  if High(av) > 2 then
    case Index of
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ScriptItemTextData');
  Obj := TObject(av[1].GetDword);
  Binding := nil;
  if Obj is TItem then
    Binding := TScriptItem(TItem(Obj).ScriptItem);
  ...
  case Index of
    ...
    raise Exception.Create('Error.Script ScriptItemTextData no');
  end;
  if High(av) > 2 then
    case Index of
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ScriptItemToItem');
  Obj := TObject(av[1].GetDword);
  av[0].SetDword(0);
  if Obj <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetShipPirateRank');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipPirateRankPoints');
  Ship := TObject(av[1].GetDword) as TNormalShip;
  av[0].SetDword(Ship.PirateRankPoints);
  if High(av) > 1 then
    Ship.PirateRankPoints := av[2].GetDword;
end;
procedure SF_ShipNextPirateRankPoints(av: array of TVarEC; code: TCodeEC);
var
  Ship: TNormalShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipNextPirateRankPoints');
  Ship := TObject(av[1].GetDword) as TNormalShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInPirateClan');
  Ship := TShip(av[1].GetDword);
  if GetPlayer = Ship then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script RaisePirateRank');
  Ship := TNormalShip(av[1].GetDword);
  if High(av) > 1 then
    Ship.PirateRank := av[2].GetInt
  else
  begin
    Rank := Ship.PirateRank;
    if Rank < 7 then
      Ship.PirateRank := Rank + 1;
  end;
  if GetPlayer = Ship then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemType');
  Value := 0;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomWeaponType');
  av[0].SetString('');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
  ...
  if Obj is TScriptItem then
    Item := TScriptItem(Obj).Item;
  if Item <> nil then
    if Item is TCustomWeapon then
      av[0].SetString((Item as TCustomWeapon).GetWeaponInfo.ConfigName);
end;
procedure SF_ItemName(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemName');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  Text := '';
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemFullName');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  Text := '';
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemSize');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
  ...
  begin
    ...
    if (IsCountable or IsGoods)
        and (High(av) > 1)
        and (av[2].RealVType = vkString)
        and (av[2].GetString = 'Count') then
    begin
      if IsCountable then
        av[0].SetDword(TCountableItem(Item).StackCount)
      else
        av[0].SetDword(TGoods(Item).Quantity);
    end
    else
      av[0].SetDword(Item.Weight);
  ...
  begin
    ...
    if High(av) > 1 then
      TTranclucator(Obj).ArtefactSize := av[2].GetInt;
  end;
end;
procedure SF_ItemOwner(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemOwner');
  Value := 0;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemSubrace');
  Value := 0;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TEquipment(Obj);
  if Obj is TScriptItem then
    Item := TScriptItem(Obj).Item as TEquipment;
  if Item <> nil then
  begin
    Value := Ord(Item.DominatorSeries);
    if High(av) > 1 then
      Item.DominatorSeries := TDominatorSeries(av[2].GetDword);
  end;
  av[0].SetDword(Value);
end;
procedure SF_ItemCost(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemCost');
  Value := 0;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemIsInUse');
  Obj := TObject(av[1].GetDword);
  av[0].SetInt(0);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemIsInSet');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  av[0].SetInt(0);
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    av[0].SetInt(GetPlayer.SelectedEquipmentConfiguration + 1)
  else
  begin
    Index := av[1].GetInt - 1;
    if GetPlayer.SelectedEquipmentConfiguration = Index then
      av[0].SetInt(2)
    else if GetPlayer.HasEquipmentConfiguration(Index) then
      av[0].SetInt(1)
    else
      av[0].SetInt(0);
    if High(av) > 1 then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemIsBroken');
  Obj := TObject(av[1].GetDword);
  Broken := 0;
  Item := nil;
  if Obj is TItem then
  ...
  begin
    if Item is TEquipment then
      Broken := (Item as TEquipment).BrokenFlag;
    if Item is TArtefact then
      Broken := (Item as TArtefact).BrokenFlag;
  end;
  if Broken <> 0 then
    av[0].SetInt(1)
  else
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipCanUseEq');
  Ship := TShip(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  if Obj is TItem then
    Item := TItem(Obj)
  ...
  if Item <> nil then
    if Item is TEquipment then
      av[0].SetInt(Ord(Ship.CanUseEquipmentTech(TEquipment(Item))));
end;
procedure SF_ShipCanRepairEq(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipCanRepairEq');
  Ship := TShip(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  if Obj is TItem then
    Item := TItem(Obj)
  ...
  if Item <> nil then
    if Item is TEquipment then
      av[0].SetInt(Ord(Ship.CanRepairEquipmentTech(TEquipment(Item))));
end;
procedure SF_ShipTechLevelKnowledge(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipTechLevelKnowledge');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(Ship.TechKnowledge);
  if High(av) > 1 then
    Ship.TechKnowledge := av[2].GetInt;
end;
procedure SF_WeaponTarget(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script WeaponTarget');
  Obj := TObject(av[1].GetDword);
  av[0].SetDword(0);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetEquipmentStats');
  Obj := TObject(av[1].GetDword);
  Stat := 0;
  Item := nil;
  if Obj is TItem then
  ...
  begin
    ...
    if Item.ItemType in [t_Weapon1..t_CustomWeapon] then
      case Stat of
        ...
        3:
          av[0].SetInt(Ord(ClassifyWeaponDamageFlags((Item as TWeapon).GetWeaponInfo.DamageFlags)));
    ...
    if av[0].RealVType = vkEmpty then
      av[0].SetInt(0);
  end;
end;
procedure SF_SetEquipmentStats(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SetEquipmentStats');
  Obj := TObject(av[1].GetDword);
  Value := av[2].GetDword;
  Stat := 0;
  if High(av) > 2 then
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script CreateEquipment');
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script CreateCustomArt');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script CustomArtData');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
  ...
  begin
    ...
    case Index of
      ...
      raise Exception.Create('Error.Script CustomArtData no');
    end;
    if High(av) > 2 then
      case Index of
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script CustomArtTextData');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
  ...
  begin
    ...
    case Index of
      ...
      raise Exception.Create('Error.Script CustomArtTextData no');
    end;
    if High(av) > 2 then
      case Index of
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreateMM');
  Index := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreateNodes');
  Count := av[1].GetInt;
  DropFlag := False;
  Series := dsBlazer;
  if High(av) > 1 then
    Series := TDominatorSeries(av[2].GetInt);
  if High(av) > 2 then
    DropFlag := av[3].GetInt <> 0;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreateCustomCountableItem');
  ConfigName := av[1].GetString;
  if High(av) > 1 then
    Count := av[2].GetInt
  else
...
begin
  if High(av) < 1 then
    av[0].SetInt(Galaxy.CountExistingSatellites)
  else
  begin
    Index := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FreeItem');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script AddItemToShip');
  Ship := TShip(av[1].GetDword);
  Item := TItem(av[2].GetDword);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetItemFromShip');
  Ship := TShip(av[1].GetDword);
  av[0].SetDword(0);
  if Ship <> nil then
  begin
    ...
    begin
      Index := av[2].GetInt;
      if (Index < 0) or (Index >= Ship.Inventory.Count) then
        Exit;
      Item := TItem(Ship.Inventory[Index]);
      if (av[2].RealVType = vkDword) and (av[2].GetDword = 0) and (Item is THull) then
        AppendLogLineThreadSafe(
            'Warning, GetItemFromShip received 0 from dword-type variable and will remove hull from ship '
                + Ship.GetFullName(' ')
                + ' ('
                + GetScriptContextDescription
                + ')'
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetArtFromShip');
  Ship := TShip(av[1].GetDword);
  av[0].SetDword(0);
  if Ship <> nil then
  begin
    ...
    begin
      Index := av[2].GetInt;
      if (Index < 0) or (Index >= Ship.Artefacts.Count) then
        Exit;
      Item := TEquipment(Ship.Artefacts[Index]);
    end;
    Item.Unequip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ArrangeItems');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script AddItemToPlanet');
  Planet := TPlanet(av[1].GetDword);
  Item := TItem(av[2].GetDword);
  av[0].SetInt(0);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetItemFromPlanet');
  Planet := TPlanet(av[1].GetDword);
  Index := av[2].GetInt;
  av[0].SetDword(0);
  if (Planet <> nil)
      and (Planet.SurfaceLootEntries <> nil)
      and (Index >= 0)
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script AddItemToShop');
  Shop := TObject(av[1].GetDword);
  Item := TItem(av[2].GetDword);
  av[0].SetInt(0);
  Refresh :=
      (TemporaryShopSlots <> nil)
          and ((TemporaryShopPlanet = Shop) or (TemporaryShopStation = Shop));
  if Item <> nil then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetItemFromShop');
  Shop := TObject(av[1].GetDword);
  av[0].SetDword(0);
  if Shop <> nil then
  begin
    ...
    if Refresh then
      RestoreTemporaryShopStock;
    if Shop is TPlanet then
      Items := TPlanet(Shop).EquipmentShop
    else if Shop is TRuins then
      Items := TRuins(Shop).EquipmentShop
    else
      Exit;
    if (av[2].RealVType = vkDword) and (av[2].GetDword > $10000) then
    begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script AddItemToStorage');
  ...
  if Item is TGoods then
    ...
    begin
      ...
      if Entry.LocationOwner <> Location then
        Continue;
      if not (Entry.Item is TGoods) then
        Continue;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetItemFromStorage');
  Index := av[1].GetInt;
  av[0].SetDword(0);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindItemInStorage');
  Item := TItem(av[1].GetDword);
  av[0].SetInt(-1);
  for Index := 0 to GetPlayer.StorageEntries.Count - 1 do
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script PutItemInVault');
  ...
  begin
    ...
    Galaxy.StoreItem(av[1].GetString, TObject(av[2].GetDword));
end;
procedure SF_GetItemFromVault(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script DropItemInSystem');
  Star := TStar(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  if Obj is TScriptItem then
    Item := TScriptItem(Obj).Item
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StopMovingItem');
  Star := TStar(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  if Obj is TScriptItem then
    Item := TScriptItem(Obj).Item
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarItems');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if High(av) = 1 then
      av[0].SetInt(Star.Items.Count)
    else
    begin
      Index := av[2].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetItemFromStar');
  Star := TStar(av[1].GetDword);
  av[0].SetDword(0);
  if Star <> nil then
  begin
    ...
    begin
      Index := av[2].GetInt;
      if (Index < 0) or (Index >= Star.Items.Count) then
        Exit;
      Item := TItem(Star.Items[Index]);
    end;
    if not Star.RecordingTurnFilm then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetItems');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    if Planet.SurfaceLootEntries <> nil then
      Count := Planet.SurfaceLootEntries.Count
    else
      Count := 0;
    if High(av) = 1 then
      av[0].SetInt(Count)
...
begin
  if High(av) < 1 then
    av[0].SetInt(GetPlayer.StorageEntries.Count)
  else
  begin
    Index := av[1].GetInt;
    if (Index >= 0) and (Index < GetPlayer.StorageEntries.Count) then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StorageItemLocation');
  Index := av[1].GetInt;
  av[0].SetDword(0);
  if (Index >= 0) and (Index < GetPlayer.StorageEntries.Count) then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShopItems');
  Shop := TObject(av[1].GetDword);
  if Shop = nil then
    raise Exception.Create('Error.Script ShopItems shop=nil');
  if Shop is TPlanet then
    Items := TPlanet(Shop).EquipmentShop
  else if Shop is TRuins then
    Items := TRuins(Shop).EquipmentShop
  else
    raise Exception.Create('Error.Script ShopItems obj not a shop');
  if High(av) = 1 then
  begin
    Count := 0;
    if (TemporaryShopSlots <> nil)
        and ((GetPlayer.CurrentPlanet = Shop) or (GetPlayer.DockedTo = Shop)) then
    begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script AddDialogOverride');
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script AddDialogInject');
  DialogName := av[1].GetString;
  if CountDelimitedPartsW(DialogName, ':') > 1 then
  begin
    Index := FindScriptTemplateIndex(ExtractDelimitedPartW(DialogName, 0, ':'));
  ...
  if High(av) > 4 then
    Entry.ReplaceGreeting := av[5].GetInt <> 0
  else
    Entry.ReplaceGreeting := False;
  if High(av) > 5 then
    Entry.AnswerData := av[6].GetDword
  else
    Entry.AnswerData := 0;
  if High(av) > 6 then
  begin
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script InjectAnswer');
  Text := av[1].GetString;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script AddDialogBlock');
...
begin
  ...
  if High(av) >= 1 then
    GetPlayer.PendingDockDialogue := av[1].GetInt;
end;
procedure SF_GetShipPlanet(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ScriptRun');
  Star := TStar(av[1].GetDword);
  ...
  begin
    ...
    begin
      if Script.Ships.Count > 0 then
        Exit;
      if TryRestartScript(Script, Star, Planet) then
        av[0].SetInt(1);
      Exit;
    end;
  end;
  if TryStartScriptByName(Star, Planet, Name) then
    av[0].SetInt(1);
end;
procedure SF_SFT(av: array of TVarEC; code: TCodeEC);
var
  ...
  procedure LogArray(
      Prefix: WideString;
      Value: TVarEC
  ); // @addr 0x61EC8C @ida "void __usercall $name(unsigned __int16 *Prefix@<eax>, TVarEC *Value@<edx>, void *ParentFrame@<^0>);" @note "Nested in SF_SFT; Value must resolve to an array."
  var
    I: Integer;
    Item: TVarEC;
    Path: WideString;
  begin
    ...
    begin
      ...
      if Item.Name <> '' then
        ...
        Path := Prefix + IntToWideString(I);
      if Item.RealVType <> vkArray then
        AppendLogLineThreadSafe(AnsiString(Path + ']=' + Item.GetString))
      else
        LogArray(Path + ',', Item);
    end;
  end;
begin
  ...
  for I := 1 to High(av) do
    if av[I].RealVType = vkArray then
      LogArray(av[I].Name + '[', av[I])
    else
      AppendLogLineThreadSafe(AnsiString(av[I].GetString));
end;
procedure SF_StartTextQuest(av: array of TVarEC; code: TCodeEC);
var
...
begin
  ...
  if High(av) > 1 then
    Request.SuccessCaption := av[2].GetString
  else
    Request.SuccessCaption := '';
  if High(av) > 2 then
    Request.FailureCaption := av[3].GetString
  else
    Request.FailureCaption := '';
  Request.Script := CurrentScript;
  QueuedTextQuests.Add(Request);
  if CurrentScript <> nil then
    CurrentScript.InitCode.LocalVar.GetVar('GQuestStatus').SetInt(1);
  av[0].SetInt(1);
end;
procedure SF_CreateABShip(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreateABShip');
  if CurrentScript <> StagedArcadeShipScript then
  ...
  if High(av) > 1 then
    Ship.Team := av[2].GetInt
  else
    Ship.Team := 0;
  if High(av) > 2 then
    Ship.HealthScalePercent := av[3].GetInt
  else
    Ship.HealthScalePercent := 100;
  if High(av) > 3 then
    Ship.DamageScalePercent := av[4].GetInt
  else
    Ship.DamageScalePercent := 100;
  if High(av) > 4 then
    Ship.ScriptLabel := av[5].GetString
  else
    Ship.ScriptLabel := '';
  if High(av) > 5 then
  begin
    Obj := TObject(av[6].GetDword);
    if av[6].GetInt = -1 then
      Ship.RandomRewardsDisabled := False
    else if Obj is TScriptItem then
      Ship.RewardObject := TScriptItem(Obj).Item
    else
      Ship.RewardObject := Obj;
  end
  else
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ConvertToABShip');
  if CurrentScript <> StagedArcadeShipScript then
    StagedArcadeShips.Clear;
  StagedArcadeShipScript := CurrentScript;
  ...
  begin
    ...
    if Source.Graphic is TRuinsSE then
      Ship.CreateRuinsVisual(
          Source.Graphic.GraphKey,
          Max(Source.Graphic.Size.X, Source.Graphic.Size.Y)
      )
    else
      Ship.CreateShipVisual(Source.Graphic.GraphKey, Source.Graphic.Size.X);
    ...
    begin
      ...
      begin
        if Weapon.ItemType <> t_CustomWeapon then
          ab_Weapon_Initialize(@Ship.Weapons[I], Ord(Weapon.ItemType))
        else
          ab_Weapon_Initialize(@Ship.Weapons[I], Weapon.GetWeaponInfo.ArcadeWeaponType);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script PlayerABDamageMod');
  if TPlayer(av[1].GetDword) = GetPlayer then
    Ship := PlayerArcadeShip
  else
    Ship := TabShip(av[1].GetDword);
  if Ship = nil then
  begin
    av[0].SetInt(0);
    Exit;
  ...
  if Name = 'damage' then
    Value := @Ship.WeaponDamageScale
  else if Name = 'recharge' then
    Value := @Ship.AmmoRechargeScale
  else if Name = 'speed' then
    Value := @Ship.MovementScale
  else if Name = 'gravity' then
    Value := @Ship.GravityScale
  else if Name = 'regen' then
    Value := @Ship.RegenerationRate
  else if Name = 'fragility' then
    Value := @Ship.DamageTakenScale
  else if Name = 'luck' then
    Value := @Ship.LuckScale
  else
    Exit;
  av[0].SetInt(Round(100 * Value^));
  if High(av) > 2 then
    Value^ := av[3].GetInt * 0.01;
end;
procedure SF_StartAB(av: array of TVarEC; code: TCodeEC);
var
...
begin
  ...
  if High(av) > 1 then
  begin
    if av[2].RealVType = vkInt then
      Request.BackgroundId := av[2].GetInt
    else if av[2].RealVType = vkString then
      Request.BackgroundMapName := av[2].GetString;
  end;
  QueuedArcadeBattles.Add(Request);
  if CurrentScript <> nil then
    CurrentScript.InitCode.LocalVar.GetVar('GABStatus').SetInt(1);
  av[0].SetInt(1);
end;
procedure SF_StartRobots(av: array of TVarEC; code: TCodeEC);
var
...
begin
  ...
  if High(av) < 5 then
    raise Exception.Create('Error.Script StartRobots');
  New(Request);
  Request.MapName := av[1].GetString;
  ...
  if High(av) > 5 then
    Request.StartText := av[6].GetString + Request.StartText
  else
    Request.StartText := '621' + Request.StartText;
  Request.Script := CurrentScript;
  QueuedPlanetaryBattles.Add(Request);
  if CurrentScript <> nil then
    CurrentScript.InitCode.LocalVar.GetVar('GRobotStatus').SetInt(1);
end;
procedure SF_MarkRobotsMapAsUsed(av: array of TVarEC; code: TCodeEC);
var
  I: Integer;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MarkRobotsMapAsUsed');
  Name := av[1].GetString;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipOwner');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipPilotRace');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipSkill');
  Ship := TShip(av[1].GetDword);
  if Ship = nil then
    Exit;
  Current := False;
  if av[2].RealVType = vkString then
  begin
    if av[2].GetString = 'Accuracy' then
      Skill := 0
    else if av[2].GetString = 'AccuracyCur' then
    begin
      Skill := 0;
      Current := True;
    end
    else if av[2].GetString = 'Mobility' then
      Skill := 1
    else if av[2].GetString = 'MobilityCur' then
    begin
      Skill := 1;
      Current := True;
    end
    else if av[2].GetString = 'Technical' then
      Skill := 2
    else if av[2].GetString = 'TechnicalCur' then
    begin
      Skill := 2;
      Current := True;
    end
    else if av[2].GetString = 'Trader' then
      Skill := 3
    else if av[2].GetString = 'TraderCur' then
    begin
      Skill := 3;
      Current := True;
    end
    else if av[2].GetString = 'Charm' then
      Skill := 4
    else if av[2].GetString = 'CharmCur' then
    begin
      Skill := 4;
      Current := True;
    end
    else if av[2].GetString = 'Leadership' then
      Skill := 5
    else if av[2].GetString = 'LeadershipCur' then
    begin
      Skill := 5;
      Current := True;
    end
    else
      raise Exception.Create('Error.Script ShipSkill unknown query ' + av[2].GetString);
  end
  else
  begin
    Skill := av[2].GetInt;
    if (Skill < 0) or (Skill > 5) then
      raise Exception.Create('Error.Script ShipSkill - invalid skill index');
  end;
  if Current then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipFace');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if High(av) > 1 then
    begin
      ...
      begin
        ...
        else if av[2].GetString = 'Path' then
          av[0].SetString(Ship.GetCaptainPortraitResourceBase)
        else
          raise Exception.Create('Error.Script ShipFace unknown keyword - ' + av[2].GetString);
      end
      else
      begin
        av[0].SetInt(Ship.PortraitFaceId);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipFreeExp');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    av[0].SetInt(Ship.FreeExperience);
    if High(av) > 1 then
      Ship.FreeExperience := av[2].GetInt;
  end;
end;
procedure SF_GetShipExpByType(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetShipExpByType');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if (High(av) = 1) or (GetPlayer <> Ship) then
      av[0].SetInt(Ship.TotalExperience)
    else
      case av[2].GetInt of
        ...
        5:
          av[0]
              .SetInt(
                  GetPlayer.TotalExperience
                      - GetPlayer.ExperienceByDominators
                      - GetPlayer.ExperienceByPirates
                      - GetPlayer.ExperienceByNormals
                      - GetPlayer.ExperienceByTraderCareer);
      else
        av[0].SetInt(Ship.TotalExperience);
      end;
  end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CoordX');
  Obj := TObject(av[1].GetDword);
  av[0].SetInt(0);
  if Obj <> nil then
  begin
    if Obj is TShip then
      Position := (Obj as TShip).Position
    else if Obj is TItem then
      Position := (Obj as TItem).Position
    else if Obj is TScriptItem then
    begin
      if (Obj as TScriptItem).Item <> nil then
        Position := (Obj as TScriptItem).Item.Position
      else
        raise Exception.Create('Error.Script CoordX - script item does not exist');
    end
    else if Obj is TScriptPlace then
      Position := (Obj as TScriptPlace).GetPoint
    else if Obj is TPlanet then
      Position := (Obj as TPlanet).GetPosition
    else if Obj is TStar then
      Position := (Obj as TStar).Position
    else if Obj is TAsteroid then
      Position := (Obj as TAsteroid).Position
    else if Obj is TMissile then
      Position := (Obj as TMissile).Position
    else
      raise Exception.Create('Error.Script CoordX - Object type not supported');
    av[0].SetInt(Round(Position.X));
    if High(av) > 1 then
    begin
      Position.X := av[2].GetInt;
      if Obj is TShip then
        (Obj as TShip).Position := Position
      else if Obj is TItem then
        (Obj as TItem).Position := Position
      else if Obj is TScriptItem then
        (Obj as TScriptItem).Item.Position := Position
      else if Obj is TScriptPlace then
      begin
      ...
      begin
        (Obj as TPlanet).Orbit :=
            TPolarPoint(IntegerPointToPolar(Classes.Point(Round(Position.X), Round(Position.Y))));
      ...
      begin
        with TStar(Obj).Constellation.StarLinks do
          ...
          begin
            ...
            if (Link.StartPoint.X = TStar(Obj).Position.X)
                and (Link.StartPoint.Y = TStar(Obj).Position.Y) then
            begin
              Link.StartPoint.X := Position.X;
              Link.StartPoint.Y := Position.Y;
            end;
            if (Link.EndPoint.X = TStar(Obj).Position.X)
                and (Link.EndPoint.Y = TStar(Obj).Position.Y) then
            begin
        ...
        (Obj as TAsteroid).Position := Position
      else if Obj is TMissile then
        (Obj as TMissile).Position := Position;
    end;
  end;
end;
procedure SF_CoordY(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CoordY');
  Obj := TObject(av[1].GetDword);
  av[0].SetInt(0);
  if Obj <> nil then
  begin
    ...
    begin
      if (Obj as TScriptItem).Item <> nil then
        ...
        raise Exception.Create('Error.Script CoordY - script item does not exist');
    end
    else if Obj is TScriptPlace then
      Position := (Obj as TScriptPlace).GetPoint
    else if Obj is TPlanet then
      ...
      raise Exception.Create('Error.Script CoordY - Object type not supported');
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShipSetCoords');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipAngle');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ObjectType');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TStar then
    av[0].SetInt(1)
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInHyperSpace');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyRanger');
  MoneyPercent := 100;
  if High(av) > 1 then
    MoneyPercent := av[2].GetInt;
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyWarrior');
  MoneyPercent := 100;
  if High(av) > 1 then
    MoneyPercent := av[2].GetInt;
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyBigWarrior');
  MoneyPercent := 100;
  if High(av) > 1 then
    MoneyPercent := av[2].GetInt;
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyDomik');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyDomikExtremal');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    if High(av) = 1 then
      Ship := TShip(Planet.SpawnWeightedDominatorShip)
    else if High(av) = 2 then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BuyTrank');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    BasicEquipment := True;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script TransferShip');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    ...
    begin
      ...
      begin
        if not (Destination is TStar)
            and not (Destination is TPlanet)
            and not (Destination is TShip) then
        ...
        begin
          ...
          NewStar := TStar(Destination);
        Index := OldStar.Ships.IndexOf(Ship);
        if (Index >= 0) and (OldStar.Ships.Count > Index) then
          OldStar.Ships.Delete(Index);
        NewStar.Ships.Add(Ship);
        Ship.CurrentStar := NewStar;
        if Ship.CurrentPlanet <> nil then
        begin
          if Ship is TNormalShip then
            (Ship as TNormalShip).LastDockedPlanet := Ship.CurrentPlanet;
        end
        else if (Ship.DockedTo <> nil) and (Ship is TRanger) then
          (Ship as TRanger).LastDockedNonPlanetLocation := Ship.DockedTo;
        if (GetPlayer = Ship) and (TemporaryShopSlots <> nil) then
          RestoreTemporaryShopStock;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script OrderLock');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    av[0].SetInt(Ship.AbsoluteScriptOrder);
    if High(av) > 1 then
      Ship.AbsoluteScriptOrder := av[2].GetInt;
  end;
end;
procedure SF_OrderForsage(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script OrderForsage');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(0);
  if Ship <> nil then
  begin
    ...
    begin
      if av[2].GetInt <> 0 then
        Ship.AfterburnerActive :=
            (Ship.GetSlotCount(sskAfterburner) > 0) and Ship.IsEquipmentUsable(Ship.GetEngine)
      else
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script OrderNone');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    SavedOrderLock := Ship.AbsoluteScriptOrder;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script OrderMove');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    SavedOrderLock := Ship.AbsoluteScriptOrder;
    ...
    begin
      ...
      begin
        Absolute := False;
        if High(av) > 2 then
          Absolute := av[3].GetInt <> 0;
        Ship.OrderMove(
            TScriptPlace(av[2].GetDword)
                .GetRandomPoint(
                    (Ship.Seed + Ship.CurrentStar.GenerationSeed) * Cardinal(Galaxy.CurrentTurn)),
            Absolute
        );
      end
      else
        raise Exception.Create('Error.Script OrderMove invalid place');
    end;
    Ship.AbsoluteScriptOrder := SavedOrderLock;
    if GetPlayer = Ship then
      PendingPlayerFollowTarget := nil;
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script OrderTeleport');
  Absolute := False;
  if High(av) > 4 then
    Absolute := av[5].GetInt <> 0;
  Destination.X := av[3].GetInt;
  Destination.Y := av[4].GetInt;
  TransitionData := 10;
  if High(av) > 5 then
    TransitionData := av[6].GetInt;
  if av[1].GetDword <> 0 then
    if av[2].GetDword <> 0 then
    begin
      Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script OrderTakeOff');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    SavedOrderLock := Ship.AbsoluteScriptOrder;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script OrderJumpHole');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script RelationToRanger');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  Ranger := TRanger(av[2].GetDword);
  if Ranger <> nil then
  begin
    ...
    begin
      ...
      begin
        if av[3].GetInt < 0 then
          Relation := 0
        else if av[3].GetInt > 100 then
          Relation := 100
        else
          Relation := av[3].GetInt;
    ...
    begin
      if (TShip(Obj).RangerRelations <> nil) and (TShip(Obj).RangerRelations.Count > 0) then
        ...
        av[0].SetInt(TShip(Obj).RelationToShip(Ranger));
      if (High(av) > 2)
          and (TShip(Obj).RangerRelations <> nil)
          and (TShip(Obj).RangerRelations.Count > 0) then
      begin
        if av[3].GetInt < 0 then
          Relation := 0
        else if av[3].GetInt > 100 then
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script RelationToShip');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  Ship := TShip(av[2].GetDword);
  if Ship <> nil then
  begin
    if Obj is TPlanet then
      av[0].SetInt(TPlanet(Obj).RelationToShip(Ship))
    else if Obj is TShip then
      av[0].SetInt(TShip(Obj).RelationToShip(Ship));
  end;
end;
procedure SF_StarOwner(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarOwner');
  Star := TStar(av[1].GetDword);
  av[0].SetInt(0);
  if Star <> nil then
  begin
    ...
    if High(av) > 1 then
      Star.ControlFaction := TStarFaction(av[2].GetInt);
  end;
end;
procedure SF_StarBattle(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarBattle');
  Star := TStar(av[1].GetDword);
  av[0].SetInt(0);
  if Star <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarSeries');
  Star := TStar(av[1].GetDword);
  av[0].SetInt(0);
  if Star <> nil then
  begin
    ...
    if High(av) > 1 then
      Star.DominatorSeries := TDominatorSeries(av[2].GetInt);
  end;
end;
procedure SF_StarHoles(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarNearbyStars');
  av[0].SetDword(0);
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarNearbyStarsDist');
  av[0].SetDword(0);
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    Index := av[2].GetInt;
    if (Index >= 0) and (Index < Galaxy.Stars.Count) then
      av[0].SetInt(Star.StarDistances[Index].Distance);
  end;
end;
procedure SF_StarSetGraph(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarSetGraph');
  Star := TStar(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreatePlanet');
  Star := TStar(av[1].GetDword);
  ...
  begin
    ...
    if TPlanet(Star.Planets.Last).Orbit.Radius < Planet.Orbit.Radius then
      Star.Planets.Add(Planet)
    else
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script PlanetSetGraph');
  Planet := TPlanet(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
    ...
    RetainSpaceObject(
        TObjectSE(Planet.Graphic),
        CreateSpaceObjectByName('Planet', Planet.GraphName, Point(0, 0))
    );
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetGetGraph');
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetPopulation');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    av[0].SetInt(Planet.Population);
    if High(av) > 1 then
      Planet.Population := av[2].GetInt;
  end;
end;
procedure SF_PlanetOwner(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetOwner');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetRace');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetGov');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    av[0].SetInt(Ord(Planet.Government));
    if High(av) > 1 then
      Planet.Government := TPlanetGovernment(av[2].GetInt);
  end;
end;
procedure SF_PlanetEco(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetEco');
  Planet := TPlanet(av[1].GetDword);
  if Planet <> nil then
  begin
    av[0].SetInt(Ord(Planet.Economy));
    if High(av) > 1 then
      Planet.Economy := TPlanetEconomy(av[2].GetInt);
  end;
end;
procedure SF_PlanetTerrain(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script PlanetTerrain');
  Planet := TPlanet(av[1].GetDword);
  ...
  case Terrain of
    ...
    raise Exception.Create('Error.Script PlanetTerrain ttype');
  end;
  if High(av) <= 2 then
    Exit;
  Value := av[3].GetInt;
  ...
  if Planet.SurfaceLootEntries <> nil then
    ...
    begin
      ...
      if Entry.TerrainKind = Terrain then
        Entry.SurfaceTileIndex := Min(Entry.SurfaceTileIndex, Value);
    end;
end;
procedure SF_PlanetTerrainExplored(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script PlanetTerrainExplored');
  Planet := TPlanet(av[1].GetDword);
  Terrain := TPlanetTerrainKind(av[2].GetInt);
  case Terrain of
    ...
    raise Exception.Create('Error.Script PlanetTerrainExplored ttype');
  end;
  if High(av) > 2 then
  begin
    case Terrain of
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetOrbitRadius');
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetOrbitalVelocity');
  Planet := TPlanet(av[1].GetDword);
  av[0].SetInt(Round(10 * Planet.OrbitalVelocity));
  if High(av) > 1 then
    Planet.OrbitalVelocity := av[2].GetInt * 0.1;
end;
procedure SF_PlanetSize(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetSize');
  Planet := TPlanet(av[1].GetDword);
  av[0].SetInt(Planet.Radius);
  if High(av) > 1 then
    Planet.Radius := av[2].GetInt;
end;
procedure SF_PlanetCurInvention(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetCurInventionPoints');
  Planet := TPlanet(av[1].GetDword);
  av[0].SetFloat(Planet.CurrentInventionPoints);
  if High(av) > 1 then
    Planet.CurrentInventionPoints := av[2].GetFloat;
end;
procedure SF_PlanetInventionLevel(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetBoostInventions');
  Planet := TPlanet(av[1].GetDword);
  if High(av) > 1 then
    Count := av[2].GetInt
  else
    Count := 1;
  for I := 1 to Count do
    Planet.AdvanceInventionProgress;
end;
procedure SF_PlanetWarriors(av: array of TVarEC; code: TCodeEC);
var
  Planet: TPlanet;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetWarriors');
  Planet := TPlanet(av[1].GetDword);
  if High(av) = 1 then
    av[0].SetInt(Planet.Warriors.Count)
  else
  begin
    Index := av[2].GetInt;
...
begin
  if High(av) = 0 then
    av[0].SetInt(Galaxy.Constellations.Count)
  else
  begin
    Index := av[1].GetInt;
...
begin
  ...
  begin
    Total := 0;
    for I := 0 to 7 do
      Inc(Total, (Galaxy.DifficultyLevels[Byte(I)] + 1) * 50);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SectorVisible');
  Sector := TConstellation(av[1].GetDword);
  if Sector <> nil then
  begin
    av[0].SetInt(Ord(Sector.Visible));
    if High(av) > 1 then
      Sector.Visible := av[2].GetInt <> 0;
  end;
  if (High(av) > 1)
      and (av[2].GetInt = 1)
      and (MainPiratePlanet <> nil)
      and (MainPiratePlanet.CurrentStar.Constellation = Sector) then
    for I := 0 to Galaxy.Constellations.Count - 1 do
      TConstellation(Galaxy.Constellations[I]).RestoreHiddenForm;
end;
procedure SF_HullHP(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HullHP');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
  else if Obj is THull then
    Hull := THull(Obj)
  else if (Obj is TScriptItem) and ((Obj as TScriptItem).Item is THull) then
  ...
  begin
    ...
    begin
      ...
      if Pos('UpTo', av[2].GetString) = 1 then
        Hull.HullPoints := Max(Hull.HullPoints, Amount)
      else if Pos('DownTo', av[2].GetString) = 1 then
        Hull.HullPoints := Min(Hull.HullPoints, Amount)
      else if Pos('To', av[2].GetString) = 1 then
        Hull.HullPoints := Amount
      else if Pos('Plus', av[2].GetString) = 1 then
        Hull.HullPoints := Max(Hull.HullPoints, Min(Hull.Weight, Hull.HullPoints + Amount))
      else if Pos('Minus', av[2].GetString) = 1 then
        Dec(Hull.HullPoints, Amount)
      else
        raise Exception.Create('Error.Script HullHP - unknown key ' + av[2].GetString);
    end
    else
      Hull.HullPoints := av[2].GetInt;
    if Obj is TShip then
      Ship := TShip(Obj)
    else
      Ship := TShip(Hull.OwnerShip);
    if (Ship <> nil) and Ship.IsHullDestroyed and not Ship.DestroyQueued then
    begin
      if av[0].GetInt > 0 then
        Ship.ScriptItemsAct(satOnDeath, nil, nil, 0);
      if (Ship.CurrentStar <> nil) and Ship.CurrentStar.RecordingTurnFilm then
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script HullDamageSuspectibility');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HullType');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HullSpecial');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HullSeries');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TShip then
    Hull := TShip(Obj).GetHull
...
begin
  if High(av) = 0 then
    av[0].SetInt(Galaxy.Holes.Count)
  else
  begin
    Index := av[1].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script HoleCreate2');
  Star1 := TStar(av[1].GetDword);
  ...
  if High(av) < 3 then
    ...
    Hole.InitializeGraphic(av[3].GetString);
  THoleSE(Hole.Graphic).SetState(1);
  Hole.CreatedTurn := Galaxy.CurrentTurn;
  Hole.HoleType := 1;
  Hole.Star1 := Star1;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleStar1');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleStar2');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleX1');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    av[0].SetInt(Round(Hole.Position1.X));
    if High(av) > 1 then
      Hole.Position1.X := av[2].GetInt;
  end;
end;
procedure SF_HoleY1(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleY1');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    av[0].SetInt(Round(Hole.Position1.Y));
    if High(av) > 1 then
      Hole.Position1.Y := av[2].GetInt;
  end;
end;
procedure SF_HoleX2(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleX2');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    av[0].SetInt(Round(Hole.Position2.X));
    if High(av) > 1 then
      Hole.Position2.X := av[2].GetInt;
  end;
end;
procedure SF_HoleY2(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleY2');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    av[0].SetInt(Round(Hole.Position2.Y));
    if High(av) > 1 then
      Hole.Position2.Y := av[2].GetInt;
  end;
end;
procedure SF_HoleTurnCreate(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleTurnCreate');
  Hole := THole(av[1].GetDword);
  av[0].SetInt(0);
  if Hole <> nil then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleMap');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    av[0].SetString(Hole.ArcadeMapName);
    if High(av) > 1 then
      Hole.ArcadeMapName := av[2].GetString;
  end
  else
    av[0].SetString('');
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script HoleGraph');
  Hole := THole(av[1].GetDword);
  if Hole <> nil then
  begin
    ...
    if High(av) > 1 then
    begin
      if Hole.Graphic <> nil then
        ReleaseSpaceObject(Hole.Graphic);
      RetainSpaceObject(
          Hole.Graphic,
          CreateSpaceObjectByName('Hole', av[2].GetString, Point(0, 0))
      );
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarRuins');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    Found := -1;
    ...
    begin
      ...
      begin
        ...
        if Ship is TRuins then
        begin
          if ((Ship.TypeNameOverrideKey <> '') and (Ship.TypeNameOverrideKey = av[2].GetString))
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CreateQuestItem');
  Item := TUselessItem.Create;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipTurnBeforeEndOrder');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipOrder');
  Ship := TShip(av[1].GetDword);
  if (High(av) > 1) and (av[2].RealVType = vkString) then
  begin
    ...
    begin
      ...
      if High(av) > 2 then
        Ship.OrderDestination.X := av[3].GetFloat;
    end
    else if av[2].GetString = 'Y' then
    begin
      av[0].SetFloat(Ship.OrderDestination.Y);
      if High(av) > 2 then
        Ship.OrderDestination.Y := av[3].GetFloat;
    end
    else
      raise Exception.Create('Error.Script ShipOrder - unknown key ' + av[2].GetString);
  end
  else
  begin
    if (GetPlayer = Ship) and (PendingPlayerFollowTarget <> nil) then
      ...
      av[0].SetInt(Ord(Ship.Order));
    if High(av) > 1 then
      Ship.Order := TShipOrder(av[2].GetDword);
  end;
end;
procedure SF_ShipOrderData1(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipOrderData1');
  Ship := TShip(av[1].GetDword);
  HighWord := Ship.OrderStateData shr 16;
  LowWord := Ship.OrderStateData and $FFFF;
  av[0].SetInt(LowWord);
  if High(av) > 1 then
    Ship.OrderStateData := (Integer(HighWord) shl 16) + (av[2].GetInt and $FFFF);
end;
procedure SF_ShipOrderData2(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipOrderData2');
  Ship := TShip(av[1].GetDword);
  HighWord := Ship.OrderStateData shr 16;
  LowWord := Ship.OrderStateData and $FFFF;
  av[0].SetInt(HighWord);
  if High(av) > 1 then
    Ship.OrderStateData := Integer(LowWord) + ((av[2].GetInt and $FFFF) shl 16);
end;
procedure SF_ShipOrderObj(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipOrderObj');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if (GetPlayer = Ship) and (PendingPlayerFollowTarget <> nil) then
    ...
    begin
      if (GetPlayer = Ship) and (PendingPlayerFollowTarget <> nil) then
        PendingPlayerFollowTarget := TShip(av[2].GetDword)
      else
        Ship.OrderTarget := TObject(av[2].GetDword);
    end;
  end;
end;
procedure SF_ShipDestination(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipDestination');
  Obj := TObject(av[1].GetDword);
  if Obj <> nil then
  begin
    ...
    begin
      ...
      begin
        ...
        if High(av) > 2 then
          TRuins(Obj).FlyDate := av[3].GetInt;
      end
      else
      begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script BuildRuins');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    Station := nil;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script BuildCustomRuins');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    Station := TRuins.Create;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipStanding');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipSlots');
  Ship := TShip(av[1].GetDword);
  av[0].SetInt(0);
  if Ship <> nil then
    case av[2].GetInt of
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomMissileType');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissileMaxDamage');
  av[0].SetInt(TMissile(av[1].GetDword).MaxDamage);
  if High(av) > 1 then
    TMissile(av[1].GetDword).MaxDamage := av[2].GetInt;
end;
procedure SF_MissileMinDamage(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissileMinDamage');
  av[0].SetInt(TMissile(av[1].GetDword).MinDamage);
  if High(av) > 1 then
    TMissile(av[1].GetDword).MinDamage := av[2].GetInt;
end;
procedure SF_MissileLive(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissileLive');
  av[0].SetInt(TMissile(av[1].GetDword).FlightTicks);
  if High(av) > 1 then
    TMissile(av[1].GetDword).FlightTicks := av[2].GetInt;
end;
procedure SF_MissileSpeed(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissileSpeed');
  av[0].SetInt(Round(TMissile(av[1].GetDword).Speed));
  if High(av) > 1 then
    TMissile(av[1].GetDword).Speed := av[2].GetInt;
end;
procedure SF_MissileAngle(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissileAngle');
  av[0].SetInt(Round(TMissile(av[1].GetDword).Direction));
  if High(av) > 1 then
    TMissile(av[1].GetDword).Direction := av[2].GetInt;
end;
procedure SF_AsteroidMinerals(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script AsteroidMinerals');
  av[0].SetInt(TAsteroid(av[1].GetDword).MineralCount);
  if High(av) > 1 then
    TAsteroid(av[1].GetDword).MineralCount := av[2].GetInt;
end;
procedure SF_AsteroidGraph(av: array of TVarEC; code: TCodeEC);
var
  Asteroid: TAsteroid;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script AsteroidGraph');
  ...
  begin
    ...
    RetainSpaceObject(
        Asteroid.GraphObject,
        CreateSpaceObjectByName(
            ExtractDelimitedPartW(av[2].GetString, 0, '.'),
            av[2].GetString,
            Point(0, 0)
        )
    );
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script AsteroidRespawn');
  Asteroid := TAsteroid(av[1].GetDword);
...
begin
  if High(Args) < 2 then
    raise Exception.Create('Error.Script ArrayAdd');
  ...
  if High(Args) > 2 then
    NewValue.Name := Args[3].GetString;
...
begin
  if High(Args) < 2 then
    raise Exception.Create('Error.Script ArrayDelete');
  if Args[1].RealVType <> vkArray then
    raise Exception.Create('Error.Script ArrayDelete - not array');
  if Args[2].RealVType = vkString then
    Args[1].GetArray.DeleteByName(Args[2].GetString)
  else
  begin
    ElementIndex := Args[2].GetInt;
    if (ElementIndex >= 0) and (ElementIndex < Args[1].GetArray.Count) then
      Args[1].GetArray.Delete(ElementIndex);
  end;
  Args[0].SetInt(Args[1].GetArray.Count);
end;
procedure SF_ArrayClear(Args: array of TVarEC; Code: TCodeEC);
begin
  if High(Args) < 1 then
    raise Exception.Create('Error.Script ArrayClear');
  if Args[1].RealVType <> vkArray then
...
begin
  if High(Args) < 1 then
    raise Exception.Create('Error.Script ArrayDim');
  if Args[1].RealVType <> vkArray then
...
begin
  ...
  if ArrayCount < 1 then
    raise Exception.Create('Error.Script ArraySort');
  if Args[1].RealVType <> vkArray then
  ...
  for I := 1 to ElementCount - 1 do
    for ElementIndex := ElementCount - 1 downto I do
      if Args[1].GetArray.GetItem(ElementIndex).GetInt
          < Args[1].GetArray.GetItem(ElementIndex - 1).GetInt then
        for ArrayIndex := 1 to ArrayCount do
        begin
          SwapValue := Args[ArrayIndex].GetArray.GetItem(ElementIndex);
          Args[ArrayIndex]
              .GetArray
              .SetItem(ElementIndex, Args[ArrayIndex].GetArray.GetItem(ElementIndex - 1));
...
begin
  ...
  if ArrayCount < 1 then
    raise Exception.Create('Error.Script ArraySortPartial');
  ...
  for ElementIndex := ElementCount - 1 downto MinIndex do
    if Args[2].GetArray.GetItem(ElementIndex).GetInt
        < Args[2].GetArray.GetItem(ElementIndex - 1).GetInt then
      for ArrayIndex := 1 to ArrayCount do
      begin
        SwapValue := Args[ArrayIndex + 1].GetArray.GetItem(ElementIndex);
        Args[ArrayIndex + 1]
            .GetArray
            .SetItem(ElementIndex, Args[ArrayIndex + 1].GetArray.GetItem(ElementIndex - 1));
...
begin
  ...
  if LastArgIndex < 2 then
    raise Exception.Create('Error.Script ArrayRandomize');
  if Args[2].RealVType <> vkArray then
  ...
  begin
    ...
    if SwapIndex1 <> SwapIndex2 then
      ...
      begin
        ...
        Args[ArrayArgIndex]
            .GetArray
            .SetItem(SwapIndex1, Args[ArrayArgIndex].GetArray.GetItem(SwapIndex2));
...
begin
  if High(Args) < 2 then
    raise Exception.Create('Error.Script ArrayFind');
  if Args[1].RealVType <> vkArray then
...
begin
  if High(Args) < 2 then
    raise Exception.Create('Error.Script ArrayFindInSorted');
  if Args[1].RealVType <> vkArray then
  ...
  begin
    ProbeIndex :=
        Round(
            (TargetValue - LowValue) * (HighIndex - LowIndex) / (HighValue - LowValue) + LowIndex
        );
  ...
  if ProbeIndex = HighIndex then
    ScanDirection := -1
  else
    ScanDirection := 1;
  Inc(ProbeIndex, ScanDirection);
  ProbeValue := Args[1].GetArray.GetItem(ProbeIndex).GetInt;
  while (ProbeValue <> TargetValue)
      and (ProbeIndex < HighIndex)
      and (ProbeIndex > LowIndex)
      and ((TargetValue - ProbeValue) * ScanDirection > 0) do
  begin
    Inc(ProbeIndex, ScanDirection);
    ProbeValue := Args[1].GetArray.GetItem(ProbeIndex).GetInt;
  end;
  if ProbeValue = TargetValue then
    Args[0].SetInt(ProbeIndex)
  else
    Args[0].SetInt(-1);
end;
procedure SF_DistToNearestEnemySystem(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DistToNearestEnemySystem');
  Star := TStar(av[1].GetDword);
  I := 0;
  while I < Galaxy.Stars.Count do
  begin
    Other := TObject(Star.StarDistances[I].Star) as TStar;
    if (Other.ControlFaction = sfDominators) or (Other.Status.CustomFaction <> '') then
      Break;
    Inc(I);
  end;
  if I = Galaxy.Stars.Count then
    av[0].SetInt(-1)
  else
    av[0].SetInt(Star.StarDistances[I].Distance);
end;
procedure SF_StarEnemyThreatLevel(av: array of TVarEC; code: TCodeEC);
var
  Star, Other: TStar;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarEnemyThreatLeve');
  Star := TStar(av[1].GetDword);
  IncludePirates := (High(av) > 1) and (av[2].GetInt = 1);
  if Star = nil then
    raise Exception.Create('Error.Script StarEnemyThreatLevel');
  if (Star.ControlFaction = sfDominators)
      or (Star.Status.CustomFaction <> '')
      or (IncludePirates and (Star.ControlFaction = sfPirates)) then
  begin
    av[0].SetInt(3);
    Exit;
  end;
  ...
  if IncludePirates then
    Standings := Standings + [ssPirateActive, ssPirateMilitary];
  for J := 0 to Star.Ships.Count - 1 do
  begin
    Ship := TShip(Star.Ships[J]);
  ...
  begin
    ...
    if (Other.ControlFaction = sfDominators)
        or (Star.Status.CustomFaction <> '')
        or (IncludePirates and (Star.ControlFaction = sfPirates)) then
      ...
      begin
        ...
        if (Ship is TRuins)
            and (TRuins(Ship).FlyToStar = Star)
            and (Ship.CurrentStanding in Standings) then
        begin
          av[0].SetInt(1);
          Exit;
...
procedure SF_BuildListOfQuestPossibleLocations(av: array of TVarEC; code: TCodeEC);
var
  I, J, K, MinDistance, MaxDistance, Count: Integer;
  Star, Candidate: TStar;
  Allowed: Boolean;
  Value: TVarEC;
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script BuildListOfQuestPossibleLocations');
  if av[1].RealVType = vkEmpty then
  begin
    av[1].ConvertToKind(vkArray);
    av[1].SetArray(TVarArrayEC.Create);
  end
  else if av[1].RealVType <> vkArray then
    raise Exception.Create('Error.Script BuildListOfQuestPossibleLocations - not array');
  ...
  begin
    ...
    if (Star.StarDistances[I].Distance >= MinDistance)
        and (Candidate.ControlFaction = sfCoalition)
        and (Candidate.Status.CustomFaction = '')
        and (Candidate.Battle = 0)
        and (Galaxy.KellerTargetStar <> Candidate)
        and Candidate.IsConstellationVisible then
    begin
      Allowed := True;
      if High(av) > 4 then
        for J := 5 to High(av) do
          if TStar(av[J].GetDword) = Candidate then
            Allowed := False;
      if Allowed then
      begin
  ...
  begin
    ...
    if Star.ControlFaction = sfDominators then
      for J := 0 to Star.Ships.Count - 1 do
        if (TShip(Star.Ships[J]).TypeId = stKling)
            and (TShip(Star.Ships[J]).OrderTarget is TStar) then
        begin
  ...
  av[0].SetInt(Count);
  if Count = 0 then
    av[1].GetArray.AddItem(TVarEC.Create(vkEmpty));
end;
procedure SF_FindItemInShip(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script FindItemInShip');
  Ship := TShip(av[1].GetDword);
  Obj := TObject(av[2].GetDword);
  Item := nil;
  if Obj is TItem then
  ...
  begin
    ...
    if av[0].GetInt < 0 then
      av[0].SetInt(Ship.Artefacts.IndexOf(Item));
  end;
end;
procedure SF_GalaxyRangers(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) = 0 then
    av[0].SetInt(Galaxy.Rangers.Count)
  else
  begin
    Index := av[1].GetInt;
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script MakeShipEnterStar');
  Ship := TShip(av[1].GetDword);
  Star := TStar(av[2].GetDword);
  ...
  if I >= 0 then
    Ship.CurrentStar.Ships.Delete(I);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipAddDropItem');
  Ship := TShip(av[1].GetDword);
  Item := TItem(av[2].GetDword);
  if (Ship <> nil) and (Item <> nil) then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusPriority');
  if (av[1].GetInt < 0) or (av[1].GetInt >= MicroModuleTemplateCount) then
    raise Exception.Create('Error.Script BonusPriority - number out of range');
  av[0].SetInt(MicroModuleTemplates[av[1].GetInt].Priority);
end;
procedure SF_BonusIsSpecial(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusIsSpecial');
  if (av[1].GetInt < 0) or (av[1].GetInt >= MicroModuleTemplateCount) then
    raise Exception.Create('Error.Script BonusIsSpecial - number out of range');
  av[0].SetInt(Ord(MicroModuleTemplates[av[1].GetInt].SpecialOnly));
end;
procedure SF_BonusName(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusName');
  if (av[1].GetInt < 0) or (av[1].GetInt >= MicroModuleTemplateCount) then
    raise Exception.Create('Error.Script BonusName - number out of range');
  av[0].SetString(MicroModuleTemplates[av[1].GetInt].Name);
end;
procedure SF_BonusNumInCfg(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusNumInCfg');
  if av[1].GetInt = -1 then
    av[0].SetString('')
  else
  begin
    if (av[1].GetInt < 0) or (av[1].GetInt >= MicroModuleTemplateCount) then
      raise Exception.Create('Error.Script BonusNumInCfg - number out of range');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SeriesNumInCfg');
  if av[1].GetInt = -1 then
    av[0].SetString('')
  else
  begin
    if (av[1].GetInt < 0) or (av[1].GetInt >= HullSeriesCount) then
      raise Exception.Create('Error.Script SeriesNumInCfg - number out of range');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindBonusByName');
  av[0].SetInt(-1);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindSeriesByName');
  av[0].SetInt(-1);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindBonusByCustomTag');
  av[0].SetInt(-1);
  Occurrence := 0;
  if High(av) > 1 then
    Occurrence := av[2].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindBonusByNameInCfg');
  av[0].SetInt(-1);
  for Index := 0 to High(MicroModuleTemplates) do
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusCustomTag');
  Block :=
      LanguageDataConfig
          .GetBlockByPath('MicroModuls.' + MicroModuleTemplates[av[1].GetInt].ConfigName);
  if Block.CountParams('CustomTag') <= 0 then
    av[0].SetString('')
  else
    av[0].SetString(Block.GetParamByPath('CustomTag'));
end;
procedure SF_CreateEquipmentWithSpecial(av: array of TVarEC; code: TCodeEC);
var
  Item: TEquipment;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SpecialToEquipment');
  ...
  if Obj is TItem then
    RawItem := TItem(Obj);
  if Obj is TScriptItem then
    RawItem := TScriptItem(Obj).Item;
  if (RawItem <> nil) and (RawItem is TEquipment) then
    Item := TEquipment(RawItem)
  else
    raise Exception.Create('Error.Script SpecialToEquipment - not eq');
  if RandomMode then
  begin
    ...
    begin
      if Template.SpecialOnly
          ...
          and (not IsHull or IsBonusCompatibleWithHull(Index, THull(Item)))
          and (not IsOther or IsBonusCompatibleWithEquipment(Index, Item))
          and (Template.Priority <= PriorityLimit) then
      begin
        if Count = 0 then
          BestPriority := Template.Priority
        else
          BestPriority := Max(BestPriority, Template.Priority);
    ...
    begin
      ...
      begin
        ...
        if MicroModuleTemplates[MicroModuleCandidateIndices[Chosen]].Priority
            >= MinimumPriority then
          Break;
      end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ModuleToEquipment');
  RandomMode := (High(av) = 1) or ((av[1].RealVType = vkDword) and (av[1].GetDword > $FFFF));
  if RandomMode then
    Obj := TObject(av[1].GetDword)
  else
  ...
  if (RawItem <> nil) and (RawItem is TEquipment) then
    ...
    raise Exception.Create('Error.Script ModuleToEquipment - not eq');
  if RandomMode then
  begin
    if High(av) > 1 then
      Obj := TObject(av[2].GetDword)
    ...
    repeat
      ...
      Index :=
          Galaxy.SelectMicroModuleForEquipment(
              BasePriority + 5 * Step,
              Min(BasePriority + 40 + 20 * Step, 100),
              AdvanceRandomSeed(Galaxy.RandomState),
              Obj,
              Item
          );
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EqSpecial');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  ...
  begin
    ...
    if High(av) > 1 then
      Missile.SpecialModuleIndex := av[2].GetInt + 1;
  end;
  if Obj is TEquipment then
    Item := TEquipment(Obj)
  else if (Obj is TScriptItem) and ((Obj as TScriptItem).Item is TEquipment) then
    Item := (Obj as TScriptItem).Item as TEquipment
  else
    Exit;
  av[0].SetInt(Item.SpecialModuleIndex - 1);
  if High(av) > 1 then
    Item.SpecialModuleIndex := av[2].GetInt + 1;
end;
procedure SF_EqModule(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EqModule');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TMissile then
  begin
    ...
    if High(av) > 1 then
      Missile.MicroModuleIndex := av[2].GetInt + 1;
  end;
  if Obj is TEquipment then
    Item := TEquipment(Obj)
  else if (Obj is TScriptItem) and ((Obj as TScriptItem).Item is TEquipment) then
  ...
  if High(av) > 1 then
    Item.MicroModuleIndex := av[2].GetInt + 1;
end;
procedure SF_MayAddBonusToEq(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script MayAddModuleToEq');
  av[0].SetInt(0);
  Index := av[1].GetInt;
  Obj := TObject(av[2].GetDword);
  if Obj is TEquipment then
  ...
  if (Item.SpecialModuleIndex = 0)
      or not MicroModuleTemplates[Item.SpecialModuleIndex - 1].BlocksMicroModuleSlot then
  begin
    if Item is TWeapon then
      av[0].SetInt(Ord(IsBonusCompatibleWithWeapon(Index, TWeapon(Item))))
    else if Item is THull then
      av[0].SetInt(Ord(IsBonusCompatibleWithHull(Index, THull(Item))))
    else
      av[0].SetInt(Ord(IsBonusCompatibleWithEquipment(Index, Item)));
  end;
end;
procedure SF_BuildListOfMMByPriority(av: array of TVarEC; code: TCodeEC);
var
  I, LowPriority, HighPriority, Count: Integer;
  Value: TVarEC;
  ExcludeRacial: Boolean;
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script BuildListOfMMByPriority');
  if av[1].RealVType = vkEmpty then
  begin
    av[1].ConvertToKind(vkArray);
    av[1].SetArray(TVarArrayEC.Create);
    ...
    raise Exception.Create('Error.Script BuildListOfMMByPriority - not array');
  if High(av) > 3 then
    ExcludeRacial := av[4].GetInt <> 0
  else
    ExcludeRacial := True;
  ...
  for I := 0 to MicroModuleTemplateCount - 1 do
    if not MicroModuleTemplates[I].SpecialOnly
        and (MicroModuleTemplates[I].Priority <= HighPriority)
        and (MicroModuleTemplates[I].Priority >= LowPriority)
        and ((Byte(ExcludeRacial) and Byte(MicroModuleTemplates[I].RacialRestriction)) = 0) then
    begin
...
var
  ...
  Factions: array of WideString;
  TypeCount: Integer;
  CustomTypes: array of WideString;
  Text: WideString;
  Value: TVarEC;
  ...
  procedure CheckShip(
      Ship: TShip
  ); // @addr 0x62B728 @ida "void __usercall $name(TShip *Ship@<eax>, void *ParentFrame@<^0>);" @note "Requires SF_BuildListOfNewShips' live frame. The custom-type filter is applied only when the faction filter is enabled."
  var
    Found: Boolean;
    I: Integer;
  begin
    if Cardinal(Ship.Id) < MinimumId then
      Exit;
    if not (Ship.TypeId in Mask) then
      Exit;
    if ((Ship.ScriptShip <> nil) or (Ship.AbsoluteScriptOrder <> 0)) and not IncludeScripted then
      Exit;
    if FactionCount >= 0 then
    begin
      if Ship.ScriptShip <> nil then
        Text := TScriptShip(Ship.ScriptShip).StateText
      else
        Text := '';
      if (FactionCount = 0) and (Text <> '') then
        Exit;
      Found := FactionCount = 0;
      for I := 0 to FactionCount - 1 do
        if Text = Factions[I] then
        begin
          Found := True;
          Break;
        end;
      ...
      begin
        ...
        if (TypeCount = 0) and (Text <> '') then
          Exit;
        Found := TypeCount = 0;
        for I := 0 to TypeCount - 1 do
          if Text = CustomTypes[I] then
          begin
            Found := True;
            Break;
          end;
  ...
  procedure CheckItem(
      Item: TItem
  ); // @addr 0x62B8F0 @ida "void __usercall $name(TItem *Item@<eax>, void *ParentFrame@<^0>);" @note "Requires SF_BuildListOfNewShips' live frame. Ignores nil and non-Tranclucator artefacts."
  begin
    if Item <> nil then
      if Item is TArtefactTranclucator then
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script BuildListOfNewShips');
  if av[1].RealVType = vkEmpty then
  begin
    av[1].ConvertToKind(vkArray);
    av[1].SetArray(TVarArrayEC.Create);
    ...
    raise Exception.Create('Error.Script BuildListOfNewShips - not array');
  av[1].GetArray.Clear;
  ...
  begin
    ...
    begin
      ...
      for I := 0 to FactionCount - 1 do
        Factions[I] := ExtractDelimitedPartW(Text, I, ',');
    end;
    if High(av) > 6 then
    begin
      Text := av[7].GetString;
      ...
      begin
        ...
        for I := 0 to TypeCount - 1 do
          CustomTypes[I] := ExtractDelimitedPartW(Text, I, ',');
      end;
    end
    else
    begin
      ...
      begin
        ...
        if Ship.TypeId <> stWarrior then
          for K := 0 to Ship.Artefacts.Count - 1 do
            CheckItem(TItem(Ship.Artefacts[K]));
      end;
      for J := 0 to Star.Planets.Count - 1 do
      begin
        Planet := TPlanet(Star.Planets[J]);
        ...
        begin
          Ship := TShip(Planet.Warriors[K]);
          for L := 0 to Ship.Artefacts.Count - 1 do
            CheckItem(TItem(Ship.Artefacts[L]));
        end;
      end;
      for J := 0 to Star.Items.Count - 1 do
        CheckItem(TItem(Star.Items[J]));
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script Chameleon');
  Ship := TShip(av[1].GetDword);
  if Ship = nil then
    Ship := GetPlayer;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script IsChameleon');
  Ship := TShip(av[1].GetDword);
  if Ship = nil then
    Ship := GetPlayer;
  if Ship.ScriptChameleon then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
procedure SF_PlayerChameleonCurType(av: array of TVarEC; code: TCodeEC);
begin
  if not GetPlayer.ChameleonActive then
    av[0].SetInt(-1)
  else
    av[0].SetInt(Ord(GetPlayer.ChameleonSeries));
  if High(av) > 0 then
    if av[1].GetInt < 0 then
      GetPlayer.ChameleonActive := False
    else
    begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SwitchToMirrorImage');
  Ship := TShip(av[1].GetDword);
  if Ship = nil then
    Ship := GetPlayer;
  ...
  begin
    Hull := Ship.GetHull;
    if Hull.HullType <> htSpecial then
      raise Exception.Create('Error.Script SwitchToMirrorImage: Not a special hull');
    Hull.SpecialModuleIndex := av[2].GetInt + 1;
    if not Ship.ScriptChameleon and not Ship.ChameleonActive and not Ship.GraphDominator then
    begin
      Mirror :=
          CreateSpaceObjectByName('Ship2', 'Ship.Akrin.' + Hull.GetSpecialKindGraph, Point(0, 0))
              as TShip2SE;
      Mirror.CopyDataFromMirrorImage(Ship.Graphic);
      Mirror.Free;
      Ship.GraphName := Ship.Graphic.GraphKey;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EquipmentImageName');
  if (av[1].RealVType = vkString) and (av[1].GetString = 'GetImagePath') then
  begin
    if High(av) < 2 then
      raise Exception.Create('Error.Script EquipmentImageName 2');
    Item := TItem(av[2].GetDword);
  ...
  begin
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            if Item is TUselessItem then
              TUselessItem(Item).CheckIfWeDisplayAsArtefact;
            if Item is TArtefactCustom then
              TArtefactCustom(Item).LoadConfig(False);
          end;
        end;
      end
      else if Obj is TTranclucator then
      begin
        ...
        if High(av) > 1 then
          (Obj as TTranclucator).ArtefactSystemName := av[2].GetString;
      end;
    end;
  end;
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarFonImage');
  Star := TStar(av[1].GetDword);
  av[0].SetInt(Star.BackgroundImage);
  if High(av) > 1 then
    Star.BackgroundImage := av[2].GetInt;
end;
procedure SF_ExtremalTakeOff(av: array of TVarEC; code: TCodeEC);
begin
...
procedure SF_StarListToPlanetList(av: array of TVarEC; code: TCodeEC);
var
  I, J, Priority, Count, EnemyDistance: Integer;
  Score, Penalty: Double;
  Star: TStar;
...
begin
  ...
  if av[1].RealVType <> vkArray then
    raise Exception.Create('Error.Script StarListToPlanetList - not array');
  if (High(av) > 6) and (av[7].RealVType <> vkArray) then
    raise Exception.Create('Error.Script StarListToPlanetList - not array');
  Penalty := 0;
  if High(av) > 7 then
  ...
  begin
    ...
    begin
      ...
      Star := Planet.CurrentStar;
      J := 0;
      while (J < Galaxy.Stars.Count)
          and ((TObject(Star.StarDistances[J].Star) as TStar).ControlFaction <> sfDominators)
          and ((TObject(Star.StarDistances[J].Star) as TStar).Status.CustomFaction = '') do
        Inc(J);
      if J = Galaxy.Stars.Count then
        EnemyDistance := 1
      else
        EnemyDistance := Star.StarDistances[J].Distance;
      Score := EnemyDistance * Weights[Planet.OwnerId];
      if High(av) > 6 then
        for J := 0 to av[7].GetArray.Count - 1 do
          if (TStar(av[7].GetArray.GetItem(J).GetDword) = Star)
              or (av[7].GetArray.GetItem(J).GetDword = Star.Id) then
            Score := Score * Penalty;
      New(ScorePtr);
      ScorePtr^ := Score;
      Scores.Add(ScorePtr);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomWin');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomLose');
...
begin
  ...
  if High(av) < 2 then
    raise Exception.Create('Error.Script StartVideo');
  ...
  begin
    ...
    ShipScreen.ScriptVideoTimer :=
        ShipScreen.ScheduleCallbackTimer(5, 5, ShipScreen.AdvanceScriptVideo);
  end
  else
  begin
    New(Request);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StartMusic');
  if MusicEnabled then
  begin
    ...
    while MusicManager.IsPlaying do
      SysUtils.Sleep(1);
    if CountDelimitedPartsW(av[1].GetString, '.') > 1 then
      MusicManager.PlayFile(av[1].GetString)
    else
    begin
      SavedCategory := MusicManager.CategoryOverride;
      MusicManager.CategoryOverride := '';
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MusicControls');
  Command := av[1].GetString;
  if Command = 'SetPlayListOverride' then
  begin
    if High(av) < 2 then
      raise Exception.Create('Error.Script MusicControls 2');
    MusicManager.CategoryOverride := av[2].GetString;
    if MusicEnabled and (MusicManager.CategoryOverride <> '') then
      MusicManager.PlayCategory('');
  end
  else if Command = 'GetPlayListOverride' then
    av[0].SetString(MusicManager.CategoryOverride)
  else if Command = 'GetCurFile' then
    av[0].SetString(MusicManager.GetCurrentFileName)
  else if Command = 'StopCurFile' then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoComeKlingToStar');
  Star := TStar(av[1].GetDword);
  if Star <> nil then
  begin
    if Star.NoComeKling then
      ...
      av[0].SetInt(0);
    if High(av) > 1 then
      Star.NoComeKling := av[2].GetInt <> 0;
  end;
end;
procedure SF_NoDropToShip(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoDropToShip');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if Ship.NoDrop then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoTargetToShip');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    av[0].SetInt(Ship.TargetingRestriction);
    if High(av) > 1 then
      Ship.TargetingRestriction := av[2].GetInt;
  end;
end;
procedure SF_NoTalkToShip(av: array of TVarEC; code: TCodeEC);
var
  ...
  begin
    ...
    begin
      if Ship.NoScan then
        ...
        av[0].SetInt(0);
      if High(av) > 1 then
        Ship.NoScan := av[2].GetInt <> 0;
    end;
  end;
end;
procedure SF_NoJump(av: array of TVarEC; code: TCodeEC);
begin
  if GetPlayer.NoJump then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
  if High(av) > 0 then
    GetPlayer.NoJump := av[1].GetInt <> 0;
end;
procedure SF_NoLanding(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoLanding');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TPlanet then
  begin
    if (Obj as TPlanet).NoLanding then
      av[0].SetInt(1);
    if High(av) > 1 then
      (Obj as TPlanet).NoLanding := av[2].GetInt <> 0;
  end
  else if Obj is TRuins then
  begin
    if (Obj as TRuins).NoLanding then
      av[0].SetInt(1);
    if High(av) > 1 then
      (Obj as TRuins).NoLanding := av[2].GetInt <> 0;
  end;
end;
procedure SF_NoShopUpdate(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoShopUpdate');
  av[0].SetInt(0);
  Obj := TObject(av[1].GetDword);
  if Obj is TPlanet then
  begin
    ...
    if High(av) > 1 then
      (Obj as TPlanet).ShopUpdateMode := av[2].GetInt;
  end
  else if Obj is TRuins then
  begin
    av[0].SetInt(Ord((Obj as TRuins).ShopUpdateMode));
    if High(av) > 1 then
      (Obj as TRuins).ShopUpdateMode := TShopUpdateMode(av[2].GetInt);
  end;
end;
procedure SF_PlanetExtraFlags(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetExtraFlags');
  av[0].SetInt(0);
  Planet := TPlanet(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script NoDropItem');
  av[0].SetInt(0);
  Item := nil;
  Obj := TObject(av[1].GetDword);
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CanSellItem');
  av[0].SetInt(1);
  Binding := nil;
  Obj := TObject(av[1].GetDword);
  if Obj is TItem then
  ...
  if Binding <> nil then
  begin
    if not Binding.CanSell then
      av[0].SetInt(0);
    if High(av) > 1 then
      Binding.CanSell := av[2].GetInt <> 0;
  end;
end;
procedure SF_TruceBetweenShips(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script TruceBetweenShips');
  ...
  begin
    ...
    if (Partner1 <> nil)
        and (Partner2 <> nil)
        and ((Partner1.EnemyShip = Partner2) or (Partner2.EnemyShip = Partner1)) then
      Partner1.TruceWithShip(Partner2);
    if (Partner1 <> nil) and ((Partner1.EnemyShip = Ship2) or (Ship2.EnemyShip = Partner1)) then
      Partner1.TruceWithShip(Ship2);
    if (Partner2 <> nil) and ((Ship1.EnemyShip = Partner2) or (Partner2.EnemyShip = Ship1)) then
      Ship1.TruceWithShip(Partner2);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInPrison');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if Ship.CurrentPlanet <> nil then
  begin
    ...
    begin
      ...
      case Ship.TypeId of
        ...
        begin
          ...
          if High(av) > 1 then
            (Ship as TRanger).PrisonTermRemaining := av[2].GetInt;
        end;
        stPirate:
        begin
          av[0].SetInt((Ship as TPirate).PrisonTermRemaining);
          if High(av) > 1 then
            (Ship as TPirate).PrisonTermRemaining := av[2].GetInt;
        end;
      end;
  end;
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipPartners');
  Ship := TShip(av[1].GetDword);
  Wanted := -1;
  Found := -1;
  av[0].SetDword(0);
...
var
  Index, Count: Integer;
begin
  if GetPlayer.PiratePartners = nil then
    Count := 0
  else
    Count := GetPlayer.PiratePartners.Count;
  if High(av) < 1 then
    av[0].SetInt(Count)
  else
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipIsPartner');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
  begin
    if (High(av) > 1) and (av[2].GetInt <> 0) then
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipFreeSpace');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
    av[0].SetInt(Ship.CargoFreeSpace);
end;
procedure SF_ShipWealth(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script ShipWealth');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
    av[0].SetInt(Ship.Wealth);
end;
procedure SF_DomiksDefeated(av: array of TVarEC; code: TCodeEC);
var
  Kind: Byte;
...
begin
  ...
  begin
    if av[1].GetInt = 1 then
      Galaxy.CoalitionDefeatedTurn := Galaxy.CurrentTurn
    else
      Galaxy.CoalitionDefeatedTurn := 0;
  end;
end;
procedure SF_ShipRefuel(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipRefuel');
  Ship := TShip(av[1].GetDword);
  if Ship <> nil then
    if Ship.GetFuelTanks <> nil then
      Ship.GetFuelTanks.Fuel := Ship.GetFuelTanks.Capacity;
end;
procedure SF_ShipRepairEq(av: array of TVarEC; code: TCodeEC);
var
  Index: Integer;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipRepairEq');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemInScript');
  Item := TItem(av[1].GetDword);
...
var
  Score: Integer;
  Planet: TPlanet;
  MaxScore, MinScore, TargetScore, BestDistance, Percent, I, J, Faction: Integer;
  Star: TStar;
  BestPlanet: TPlanet;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FindPlanetByAdvancement');
  Faction := 0;
  if High(av) > 1 then
    Faction := av[2].GetInt;
  ...
  begin
    Star := TStar(Galaxy.Stars[I]);
    if (Faction < 0)
        or ((TStarFaction(Faction) = Star.ControlFaction) and (Star.Status.CustomFaction = '')) then
      for J := 0 to Star.Planets.Count - 1 do
      begin
        Planet := TPlanet(Star.Planets[J]);
        if Planet.OwnerId <> Byte(oiUninhabited) then
...
procedure SF_StarListToTransitPlanetList(av: array of TVarEC; code: TCodeEC);
var
  I, J, K, Unused, Count, DirectDistance, Detour, Limit, BaseDistance: Integer;
  Score, BestScore, Penalty: Double;
  Star, Origin, Destination: TStar;
  Planet, BestPlanet: TPlanet;
  Temp: TVarEC;
  ScorePtr: PDouble;
  Scores: TList;
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script StarListToTransitPlanetList');
  BaseDistance := 10;
  ...
  if av[1].RealVType <> vkArray then
    raise Exception.Create('Error.Script StarListToTransitPlanetList - not array');
  if High(av) > 8 then
    for I := 0 to 4 do
      Weights[I] := av[I + 5].GetInt
  else
    for I := 0 to 4 do
      Weights[I] := 100;
  if (High(av) > 10) and (av[10].RealVType <> vkArray) then
    raise Exception.Create('Error.Script StarListToTransitPlanetList - not array2');
  Penalty := 0;
  if High(av) > 10 then
    Penalty := av[11].GetInt / 100;
  Scores := TList.Create;
  I := 0;
  ...
  begin
    Star := TStar(av[1].GetArray.GetItem(I).GetDword);
    Detour :=
        Round(PointDistance(Star.Position, Destination.Position))
            + Round(PointDistance(Star.Position, Origin.Position))
            - DirectDistance
            + BaseDistance;
    Unused := 0;
    BestPlanet := nil;
        ...
        if High(av) > 10 then
          for K := 0 to av[10].GetArray.Count - 1 do
            if (TPlanet(av[10].GetArray.GetItem(K).GetDword) = Planet)
                or (av[10].GetArray.GetItem(K).GetDword = Planet.Id) then
              Score := Score * Penalty;
        if ((BestPlanet = nil) and (Limit >= Score) and (Score > 0))
            or ((Score < BestScore) and (Score > 0)) then
        begin
          BestPlanet := Planet;
          BestScore := Score;
        end;
    ...
    if (Star = Origin) or (Star = Destination) or (BestPlanet = nil) then
      av[1].GetArray.Delete(I)
    else
    begin
      av[1].GetArray.GetItem(I).SetDword(PtrUInt(BestPlanet));
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GalaxyEventDate');
  av[0].SetInt(0);
  Index := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GalaxyEventType');
  av[0].SetString('');
  Index := av[1].GetInt;
  if (Index >= 0) and (Index < Galaxy.GalaxyEvents.Count) then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GalaxyEventData');
  av[0].SetInt(0);
  Index := av[1].GetInt;
  if (Index >= 0) and (Index < Galaxy.GalaxyEvents.Count) then
  begin
    if High(av) > 1 then
      av[0].SetInt(TGalaxyEvent(Galaxy.GalaxyEvents[Index]).GetData(av[2].GetInt))
    else
      av[0].SetInt(TGalaxyEvent(Galaxy.GalaxyEvents[Index]).Data.Count);
  end;
end;
procedure SF_GalaxyEventsTextData(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GalaxyEventTextData');
  Index := av[1].GetInt;
  if (Index >= 0) and (Index < Galaxy.GalaxyEvents.Count) then
  begin
    if High(av) > 1 then
      av[0].SetString(TGalaxyEvent(Galaxy.GalaxyEvents[Index]).GetTextData(av[2].GetInt))
    else
      av[0].SetInt(TGalaxyEvent(Galaxy.GalaxyEvents[Index]).TextData.Count);
  end
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetNewsDate');
  av[0].SetInt(0);
  Index := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetNewsType');
  av[0].SetInt(0);
  Index := av[1].GetInt;
  if (Index >= 0) and (Index < Galaxy.PlanetNews.Count) then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetNewsText');
  av[0].SetString('');
  Index := av[1].GetInt;
  if (Index >= 0) and (Index < Galaxy.PlanetNews.Count) then
  begin
    ...
    if High(av) > 1 then
      PPlanetNewsEntry(Galaxy.PlanetNews[Index]).Text := av[2].GetString;
  end;
end;
procedure SF_ControlledSystems(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ControlledSystems');
  Faction := TStarFaction(av[1].GetInt);
  Count := 0;
  for Index := 0 to Galaxy.Stars.Count - 1 do
  begin
    Star := TStar(Galaxy.Stars[Index]);
    if (Star.Status.CustomFaction = '') and (Star.ControlFaction = Faction) then
      Inc(Count);
  end;
  av[0].SetInt(Count);
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script DeltaWin');
  Faction := av[1].GetInt;
  av[0].SetInt(Galaxy.WarDeltaWin[Faction]);
  if High(av) > 1 then
    Galaxy.WarDeltaWin[Faction] := av[2].GetInt;
end;
procedure SF_ShipInFear(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipInFear');
  Ship := TShip(av[1].GetDword);
  if (Ship <> nil) and Ship.InFear then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetNodesFromShip');
  Ship := TShip(av[1].GetDword);
  Remaining := 0;
  Total := 0;
  Series := -1;
  if High(av) > 1 then
    Remaining := av[2].GetInt;
  if High(av) > 2 then
    Series := av[3].GetInt;
  for Index := Ship.Inventory.Count - 1 downto 0 do
  begin
    Item := TItem(Ship.Inventory[Index]);
    if not (Item is TProtoplasm) then
      Continue;
...
begin
  ...
  begin
    Entry := GetPlayer.StorageEntries[Index];
    if ((Entry.LocationOwner = Location) or (Location = nil))
        and (Byte(Entry.Item.ItemType) = Byte(t_Protoplasm)) then
    begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script RangerBaseNodes');
  av[0].SetInt(TRanger(av[1].GetDword).BaseNodes);
  if High(av) > 1 then
    TRanger(av[1].GetDword).BaseNodes := av[2].GetInt;
end;
procedure SF_RuinsAllowModernization(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script RuinsAllowModernization');
  if TRuins(av[1].GetDword).ModernizationSponsor then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
  if High(av) > 1 then
    TRuins(av[1].GetDword).ModernizationSponsor := av[2].GetInt <> 0;
end;
procedure SF_RuinsMicromoduleChain(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script RuinsMicromoduleChain');
  Obj := TObject(av[1].GetDword);
  if not (Obj is TRuins) then
    raise Exception.Create('Error.Script RuinsMicromoduleChain - not a ruins');
  Station := TRuins(Obj);
  Kind := av[2].GetInt;
  if High(av) = 2 then
  begin
    ...
    begin
      ...
      if GetPlayer.NeedsMicroModule(av[0].GetInt + 1) then
        Break;
    end;
  end
  else
...
begin
  if High(av) < 1 then
    av[0].SetInt(GetPlayer.CurrentSystemKills.Dominator)
  else
    av[0].SetInt(TNormalShip(av[1].GetDword).CurrentSystemKills.Dominator);
end;
procedure SF_ShipTypeN(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipSubType');
  Ship := TShip(av[1].GetDword);
  if Ship is TKling then
    av[0].SetInt(Ord(TKling(Ship).KlingType))
  else if Ship is TTransport then
    ...
    av[0].SetInt(Ord(TRanger(Ship).PreferredCareer))
  else
    av[0].SetInt(0);
  if High(av) > 1 then
  begin
    if Ship is TKling then
      TKling(Ship).KlingType := TKlingType(av[2].GetInt)
    else if Ship is TTransport then
      ...
      TRanger(Ship).PreferredCareer := TRangerCareer(av[2].GetInt);
  end;
end;
procedure SF_ShipChangeStar(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipChangeStar');
  if (TShip(av[1].GetDword) = GetPlayer) and (TemporaryShopSlots <> nil) then
    RestoreTemporaryShopStock;
...
procedure SF_IsFilm(av: array of TVarEC; code: TCodeEC);
begin
  if StarMapScreen.Mode = smmTurnFilm then
    av[0].SetInt(1)
  else
    av[0].SetInt(0);
  SysUtils.Sleep(1);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script FilmFlags');
  ...
  case Kind of
    ...
    begin
      ...
      if High(av) >= Arg then
        Star.PlayerCombatOccurred := av[Arg].GetInt <> 0;
    end;
    2:
    begin
      av[0].SetInt(Ord(Star.InterruptLongTravel));
      if High(av) >= Arg then
        Star.InterruptLongTravel := av[Arg].GetInt <> 0;
    end;
    3:
    begin
      av[0].SetInt(Ord(Star.KeepFilmRunning));
      if High(av) >= Arg then
        Star.KeepFilmRunning := av[Arg].GetInt <> 0;
    end;
  end;
end;
procedure SF_ShowEffect(av: array of TVarEC; code: TCodeEC);
var
  ...
  TargetGraphic, SourceGraphic: TObjectSE;
  TargetFilm, SourceFilm: TEFilmObj;
  Name: WideString;
  Target, Source: TObject;
  Item: TItem;
  Damage, ShotVisual, Variant, Destruction: Integer;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShowEffect');
  Name := av[1].GetString;
  ShotVisual := av[2].GetInt;
  Target := TObject(av[3].GetDword);
  Source := nil;
  if High(av) > 3 then
    Source := TObject(av[4].GetDword);
  Damage := 0;
  if High(av) > 4 then
    Damage := av[5].GetInt;
  Destroyed := False;
  if High(av) > 5 then
    Destroyed := av[6].GetInt <> 0;
  PlaySound := True;
  if High(av) > 6 then
    PlaySound := av[7].GetInt <> 0;
  if High(av) >= 10 then
    Color := CurrentPixelFormat.PackRgb(av[8].GetInt, av[9].GetInt, av[10].GetInt)
  else if Target is TShip then
  begin
    if GetPlayer = Target then
      Color := OwnerToFilmColor(RaceToOwner(GetPlayer.PilotRace))
    else if TShip(Target).HasNamedScriptFaction then
      Color := CustomFactionToFilmColor(TScriptShip(TShip(Target).ScriptShip).StateText)
    else
      Color := OwnerToFilmColor(TShip(Target).OwnerId);
  end
  else
    Color := 0;
  Variant := -1;
  if High(av) >= 11 then
    Variant := av[11].GetInt;
  ...
  begin
    ...
    begin
      ...
      if Target is TShip then
        TargetFilm := TShip(Target).FilmObject
      else if Target is TPlanet then
        TargetFilm := TPlanet(Target).FilmObject
      else if Target is TStar then
        TargetFilm :=
            PrimaryFilm.FindObject(
                ClassSEtoName(TStar(Target).Graphic),
                TStar(Target).Graphic.GraphKey,
                TStar(Target).Id
            )
      else if Target is THole then
        TargetFilm := TEFilmObj(THole(Target).FilmObjectId)
      else if Target is TMissile then
        TargetFilm := TMissile(Target).FilmObject
      else if Target is TAsteroid then
        TargetFilm := TAsteroid(Target).FilmObject
      else
      begin
        Item := nil;
        if Target is TItem then
          Item := Target as TItem
        else if Target is TScriptItem then
          Item := TScriptItem(Target).Item;
        if Item <> nil then
          TargetFilm := Item.FilmObject;
      end;
      SourceFilm := nil;
      if Source <> nil then
      begin
        if Source is TShip then
          SourceFilm := TShip(Source).FilmObject
        else if Source is TPlanet then
          SourceFilm := TPlanet(Source).FilmObject
        else if Source is TStar then
          SourceFilm :=
              PrimaryFilm.FindObject(
                  ClassSEtoName(TStar(Source).Graphic),
                  TStar(Source).Graphic.GraphKey,
                  TStar(Source).Id
              )
        else if Source is THole then
          SourceFilm := TEFilmObj(THole(Source).FilmObjectId)
        else if Source is TMissile then
          SourceFilm := TMissile(Source).FilmObject
        else if Source is TAsteroid then
          SourceFilm := TAsteroid(Source).FilmObject
        else
        begin
          Item := nil;
          if Source is TItem then
            Item := Source as TItem
          else if Source is TScriptItem then
            Item := TScriptItem(Source).Item;
          if Item <> nil then
            SourceFilm := Item.FilmObject;
        end;
      end;
      ...
      begin
        ...
        if Destruction = 5 then
          PrimaryFilm.SetObjectAlpha(Step, TargetFilm, 0);
      end;
  ...
  begin
    TargetGraphic := nil;
    if Target is TShip then
      TargetGraphic := TShip(Target).Graphic
    else if Target is TPlanet then
      TargetGraphic := TPlanet(Target).Graphic
    else if Target is TStar then
      TargetGraphic := TStar(Target).Graphic
    else if Target is THole then
      TargetGraphic := THole(Target).Graphic
    else if Target is TMissile then
      TargetGraphic := TMissile(Target).Graphic
    else if Target is TAsteroid then
      TargetGraphic := TAsteroid(Target).GraphObject
    else
    begin
      Item := nil;
      if Target is TItem then
        ...
        Item := TScriptItem(Target).Item;
      if Item <> nil then
        TargetGraphic := Item.GetGraphObject;
    end;
    ...
    begin
      ...
      begin
        if Source is TShip then
          SourceGraphic := TShip(Source).Graphic
        else if Source is TPlanet then
          SourceGraphic := TPlanet(Source).Graphic
        else if Source is TStar then
          SourceGraphic := TStar(Source).Graphic
        else if Source is THole then
          SourceGraphic := THole(Source).Graphic
        else if Source is TMissile then
          SourceGraphic := TMissile(Source).Graphic
        else if Source is TAsteroid then
          SourceGraphic := TAsteroid(Source).GraphObject
        else
        begin
          Item := nil;
          if Source is TItem then
            ...
            Item := TScriptItem(Source).Item;
          if Item <> nil then
            SourceGraphic := Item.GetGraphObject;
        end;
      end;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShowStaticEffect');
  Name := av[1].GetString;
  X := av[2].GetInt;
  Y := av[3].GetInt;
  if High(av) > 3 then
    Scale := av[4].GetInt * 0.01
  else
    Scale := 1;
  if (PlayerStar <> nil) and PlayerStar.RecordingTurnFilm then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipConnect');
  Ship := TShip(av[1].GetDword);
  if Ship = nil then
    raise Exception.Create('Error.Script ShipConnect ship=nil');
  Star := Ship.CurrentStar;
  if Star = nil then
    raise Exception.Create('Error.Script ShipConnect ship.FCurStar=nil');
  Ship.PrepareTurnMovement(Star.CurrentStepIndex, True);
  Ship.FilmAlpha := 0;
  if Star.MovementStepCount - 1 <= Star.CurrentStepIndex then
    Ship.FilmAlpha := 255
  else
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script FilmSound');
  ...
  begin
    ...
    FilmObject := nil;
    if Obj is TShip then
      FilmObject := TShip(Obj).FilmObject
    else if Obj is TPlanet then
      FilmObject := TPlanet(Obj).FilmObject
    else if Obj is TStar then
      FilmObject :=
          PrimaryFilm.FindObject(
              ClassSEtoName(TStar(Obj).Graphic),
              TStar(Obj).Graphic.GraphKey,
              TStar(Obj).Id
          )
    else if Obj is THole then
      FilmObject := TEFilmObj(THole(Obj).FilmObjectId)
    else if Obj is TMissile then
      FilmObject := TMissile(Obj).FilmObject
    else if Obj is TAsteroid then
      FilmObject := TAsteroid(Obj).FilmObject
    else
    begin
      Item := nil;
      if Obj is TItem then
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script FireWeapon');
  Ship := TShip(av[1].GetDword);
  ...
  if Target is TMissile then
    Ship.FireWeaponAtMissile(Weapon, Target, RecordFilm)
  else if Target is TShip then
    Ship.FireWeaponAtShip(Weapon, TShip(Target), RecordFilm)
  else if Target is TItem then
    Ship.FireWeaponAtItem(Weapon, TItem(Target), RecordFilm)
  else if Target is TAsteroid then
    Ship.FireWeaponAtAsteroid(Weapon, Target, RecordFilm);
end;
procedure SF_WeaponHit(av: array of TVarEC; code: TCodeEC);
var
  Target: TObject;
...
begin
  ...
  if High(av) > 3 then
    Range := av[4].GetInt;
  if Target is TShip then
    av[0].SetInt(TShip(Target).ApplyWeaponHit(Ship, Weapon, Range, Color, Flags, 1, 0))
  else if Target is TMissile then
  begin
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script DealDamageToShip');
  Ship := TShip(av[1].GetDword);
  Source := TObject(av[2].GetDword);
  Damage := av[3].GetInt;
  if (Source <> nil) and (Source is TItem) then
    av[0].SetInt(Ship.ApplyExplosionDamage(nil, Source, Damage, nil))
  else
  begin
    if High(av) > 3 then
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script LaunchMissile');
  Ship := TShip(av[1].GetDword);
  Target := TObject(av[2].GetDword);
  Obj := TObject(av[3].GetDword);
  if Obj is TScriptItem then
    Weapon := TWeapon(TScriptItem(Obj).Item)
  else
    Weapon := TWeapon(Obj);
  Shot := 0;
  if High(av) > 3 then
...
begin
  if High(av) < 9 then
    raise Exception.Create('Error.Script SpawnMissile');
  Star := TStar(av[1].GetDword);
  Target := TObject(av[2].GetDword);
  X := av[3].GetInt;
  Y := av[4].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script BonusText');
  Index := av[1].GetInt;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PirateType');
  Ship := TShip(av[1].GetDword);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script QuestsStatusByNom');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipCalcParam');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipRefit');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if High(av) > 2 then
    if av[3].GetInt > Ship.Money then
      Ship.SetMoney(av[3].GetInt);
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipImproveItems');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  Count := 1;
  if High(av) > 1 then
  ...
  begin
    Ship.GetHull.Weight :=
        Ship.GetHull.Weight - Ship.CargoFreeSpace + Ship.GetDesiredCargoFreeSpace;
...
var
  ...
  Item: TItem;
  Equipment: TEquipment;
  SavedId, Kind: Integer;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ItemImprovement');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
  ...
  begin
    ...
    if not Equipment.HasStandardStats then
      av[0].SetInt(1);
    if High(av) > 1 then
    begin
      Kind := av[2].GetInt;
      if (Kind < 0) or (Kind > 3) then
        Kind := 3;
      if High(av) > 2 then
        Equipment.DetailImprovement := av[3].GetInt
      else
        Equipment.DetailImprovement := 0;
      if High(av) > 3 then
      begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ShipFreeFlight');
  Ship := TShip(av[1].GetDword);
  ScriptSnap(Snapshot);
  if (High(av) < 2) or (av[2].GetInt = 0) then
    Ship.NextDayLogic
  else
    case av[2].GetInt of
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_ShipKillFactionInCurSystem');
  Obj := TObject(av[1].GetDword);
  ...
  begin
    ...
    if High(av) > 2 then
      (Obj as TNormalShip).CurrentSystemKills.Custom := av[3].GetInt;
  end
  else
  begin
...
begin
  av[0].SetInt(0);
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_CapitalShipStats');
  Obj := TObject(av[1].GetDword);
  Ship := nil;
  Hull := nil;
  if Obj is TScriptShip then
    Ship := (Obj as TScriptShip).Ship;
  if Obj is TShip then
    Ship := TShip(Obj);
  if Ship <> nil then
    Hull := Ship.GetHull;
  if Obj is THull then
    Hull := THull(Obj);
  if Hull <> nil then
  begin
...
begin
  ...
  begin
    Mode := av[1].GetInt;
    if High(av) > 1 then
      GetPlayer.RuinsStatusText := av[2].GetString
    else
      GetPlayer.RuinsStatusText := '';
    if Mode > 0 then
      GetPlayer.EnterRuinsMode(Mode)
    else
      GetPlayer.CloseRuinsModeScreen;
  end;
end;
...
begin
  ...
  if High(av) > 0 then
    GetPlayer.DebtAmount := av[1].GetInt;
end;
procedure SF_PlayerDebtDate(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.DebtDueTurn);
  if High(av) > 0 then
    GetPlayer.DebtDueTurn := av[1].GetInt;
end;
procedure SF_PlayerDebtCnt(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.DebtDefaultCount);
  if High(av) > 0 then
    GetPlayer.DebtDefaultCount := av[1].GetInt;
end;
procedure SF_PlayerDeposit(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.DepositAmount);
  if High(av) > 0 then
    GetPlayer.DepositAmount := av[1].GetInt;
end;
procedure SF_PlayerDepositDate(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.DepositStartTurn);
  if High(av) > 0 then
    GetPlayer.DepositStartTurn := av[1].GetInt;
end;
procedure SF_PlayerDepositDay(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.DepositDayCount);
  if High(av) > 0 then
    GetPlayer.DepositDayCount := av[1].GetInt;
end;
procedure SF_PlayerDepositPercent(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(Round(GetPlayer.DepositInterestRate * 100));
  if High(av) > 0 then
    GetPlayer.DepositInterestRate := av[1].GetInt * 0.01;
end;
procedure SF_PlayerMedPolicy(av: array of TVarEC; code: TCodeEC);
begin
  av[0].SetInt(GetPlayer.MedicalPolicyTicks);
  if High(av) > 0 then
    GetPlayer.MedicalPolicyTicks := av[1].GetInt;
end;
procedure SF_ShipCustomShipInfosCount(av: array of TVarEC; code: TCodeEC);
var
  I, Count: Integer;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipCustomShipInfosCount');
  Ship := TShip(av[1].GetDword);
  Count := 0;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipAddCustomShipInfo');
  New(Info);
  ...
  if High(av) > 2 then
    Info.Description := av[3].GetString
  else
    Info.Description := '';
  if High(av) > 3 then
    Info.Data[1] := av[4].GetInt
  else
    Info.Data[1] := 0;
  if High(av) > 4 then
    Info.Data[2] := av[5].GetInt
  else
    Info.Data[2] := 0;
  if High(av) > 5 then
    Info.Data[3] := av[6].GetInt
  else
    Info.Data[3] := 0;
  if High(av) > 6 then
    Info.TextData1 := av[7].GetString
  else
    Info.TextData1 := '';
  if High(av) > 7 then
    Info.TextData2 := av[8].GetString
  else
    Info.TextData2 := '';
  if High(av) > 8 then
    Info.TextData3 := av[9].GetString
  else
    Info.TextData3 := '';
  Info.StatusEffect :=
      LanguageDataConfig
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipDeleteCustomShipInfo');
  Ship := TShip(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
    for I := 0 to Ship.CustomShipInfos.Count - 1 do
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ShipFindCustomShipInfoByType');
  Ship := TShip(av[1].GetDword);
  Index := 0;
  for I := 0 to Ship.CustomShipInfos.Count - 1 do
  begin
...
begin
  ...
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipCustomShipInfoDescription');
  av[0].SetString('');
  Ship := TShip(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
    ...
    if (Info = nil) or (Info.TypeName <> av[2].GetString) then
      Exit;
  end
  else if (av[2].GetDword > 10000) and (av[2].GetInt <> -1) then
  begin
    Info := PCustomShipInfo(av[2].GetDword);
    if (Ship <> nil) and (Ship.CustomShipInfos.IndexOf(Info) < 0) then
      Exit;
  end
  else
  begin
  ...
  begin
    Block :=
        LanguageDataConfig
            ...
            .GetBlock('CustomInfos')
            .GetBlock(Info.TypeName);
    if Block.CountParams('Description') <> 0 then
      av[0].SetString(Block.GetParam('Description'));
  end;
  if High(av) > 2 then
    Info.Description := av[3].GetString;
end;
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShipCustomShipInfoData');
  av[0].SetInt(0);
  Ship := TShip(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
  ...
  case DataIndex of
    ...
    raise Exception.Create('Error.Script ShipCustomShipInfoData ind');
  end;
  if High(av) > 3 then
    case DataIndex of
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script ShipCustomShipInfoTextData');
  av[0].SetString('');
  Ship := TShip(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarCustomStarInfosCount');
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script StarAddCustomStarInfo');
  Star := TStar(av[1].GetDword);
  ...
  if High(av) > 3 then
    Info.Distance := av[4].GetInt
  else
    Info.Distance := 0;
  if High(av) > 4 then
    Info.Icon := av[5].GetString
  else
    Info.Icon := '';
  if High(av) > 5 then
    Info.Info := av[6].GetString
  else
    Info.Info := '';
end;
procedure SF_StarDeleteCustomStarInfo(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarDeleteCustomStarInfo');
  Star := TStar(av[1].GetDword);
  if av[2].RealVType = vkString then
  begin
  ...
  if I < 0 then
    Exit;
  if Star.CustomSystemInfos.Count <= I then
    Exit;
  Info := TCustomSystemInfo(Star.CustomSystemInfos[I]);
  Star.CustomSystemInfos.Delete(I);
  Info.Free;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarFindCustomStarInfoByType');
  Star := TStar(av[1].GetDword);
  for I := 0 to Star.CustomSystemInfos.Count - 1 do
  begin
    Info := TCustomSystemInfo(Star.CustomSystemInfos[I]);
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script StarCustomStarInfoData');
  av[0].SetString('');
  Star := TStar(av[1].GetDword);
  I := av[2].GetInt;
  ...
  begin
    ...
    begin
      ...
      if High(av) > 3 then
        Info.Icon := av[4].GetString;
    end
    else if av[3].GetString = 'Info' then
    begin
      av[0].SetString(Info.Info);
      if High(av) > 3 then
        Info.Info := av[4].GetString;
    end
    else
      raise Exception.Create('Error.Script StarCustomStarInfoData 2');
  end;
end;
procedure SF_ItemCanBeBroken(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemCanBeBroken');
  CanBreak := False;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemFragility');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
  ...
  if Item <> nil then
    if Item is TEquipment then
      av[0].SetFloat(TEquipment(Item).GetFragilityFactor([]));
end;
procedure SF_ItemDurability(av: array of TVarEC; code: TCodeEC);
var
  Obj: TObject;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemDurability');
  OldValue := 100;
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemLevel');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  OldLevel := 0;
  if Obj is TItem then
      ...
      begin
        ...
        case Item.ItemType of
          ...
          begin
            ...
            TFuelTanks(Item).Capacity :=
                TFuelTanks(Item).Capacity
                    + TFuelTanks(NewBase).Capacity
                    - TFuelTanks(OldBase).Capacity;
          ...
          begin
            ...
            TEngine(Item).Speed :=
                TEngine(Item).Speed + TEngine(NewBase).Speed - TEngine(OldBase).Speed;
            TEngine(Item).JumpRange :=
                TEngine(Item).JumpRange + TEngine(NewBase).JumpRange - TEngine(OldBase).JumpRange;
          end;
          t_Radar:
          begin
            TRadar(Item).TechLevel := NewLevel;
            TRadar(Item).Range :=
                TRadar(Item).Range + TRadar(NewBase).Range - TRadar(OldBase).Range;
          end;
          t_Scaner:
          begin
            TScaner(Item).TechLevel := NewLevel;
            TScaner(Item).ScanPower :=
                TScaner(Item).ScanPower + TScaner(NewBase).ScanPower - TScaner(OldBase).ScanPower;
          end;
          t_RepairRobot:
          begin
            TRepairRobot(Item).TechLevel := NewLevel;
            TRepairRobot(Item).RepairPoints :=
                TRepairRobot(Item).RepairPoints
                    + TRepairRobot(NewBase).RepairPoints
                    - TRepairRobot(OldBase).RepairPoints;
          end;
          t_CargoHook:
          begin
            TCargoHook(Item).TechLevel := NewLevel;
            TCargoHook(Item).PickupPower :=
                TCargoHook(Item).PickupPower
                    + TCargoHook(NewBase).PickupPower
                    - TCargoHook(OldBase).PickupPower;
            TCargoHook(Item).Range :=
                TCargoHook(Item).Range + TCargoHook(NewBase).Range - TCargoHook(OldBase).Range;
            TCargoHook(Item).MinPullSpeed :=
                TCargoHook(Item).MinPullSpeed
                    + TCargoHook(NewBase).MinPullSpeed
                    - TCargoHook(OldBase).MinPullSpeed;
            TCargoHook(Item).MaxPullSpeed :=
                TCargoHook(Item).MaxPullSpeed
                    + TCargoHook(NewBase).MaxPullSpeed
                    - TCargoHook(OldBase).MaxPullSpeed;
          end;
          t_DefGenerator:
          begin
            TDefGenerator(Item).TechLevel := NewLevel;
            TDefGenerator(Item).DamageFactor :=
                TDefGenerator(Item).DamageFactor
                    + TDefGenerator(NewBase).DamageFactor
          ...
          begin
            ...
            TWeapon(Item).Range :=
                TWeapon(Item).Range + TWeapon(NewBase).Range - TWeapon(OldBase).Range;
            TWeapon(Item).MinDamage :=
                TWeapon(Item).MinDamage + TWeapon(NewBase).MinDamage - TWeapon(OldBase).MinDamage;
            TWeapon(Item).MaxDamage :=
                TWeapon(Item).MaxDamage + TWeapon(NewBase).MaxDamage - TWeapon(OldBase).MaxDamage;
            TWeapon(Item).AmmoCapacity :=
                TWeapon(Item).AmmoCapacity
                    + TWeapon(NewBase).AmmoCapacity
                    - TWeapon(OldBase).AmmoCapacity;
            TWeapon(Item).Ammo :=
                Max(0, TWeapon(Item).Ammo + TWeapon(NewBase).Ammo - TWeapon(OldBase).Ammo);
          end;
        end;
        OldBase.Free;
        NewBase.Free;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemCharge');
  Obj := TObject(av[1].GetDword);
  if Obj is TArtefactTransmitter then
  begin
    av[0].SetInt(TArtefactTransmitter(Obj).Power);
    if High(av) > 1 then
      TArtefactTransmitter(Obj).Power := av[2].GetInt;
  end
  else
    av[0].SetInt(0);
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MissilesToRearm');
  Obj := TObject(av[1].GetDword);
  Needed := 0;
  Item := nil;
  if Obj is TItem then
  ...
  if Item <> nil then
    ...
    begin
      ...
      begin
        Count := av[2].GetInt;
        TWeapon(Item as TEquipment).Ammo :=
            Max(
                0,
                Min(
                    TWeapon(Item as TEquipment).AmmoCapacity,
                    TWeapon(Item as TEquipment).Ammo + Count
                )
            );
      end;
    end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script WeaponAmmunition');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  if Obj is TItem then
    Item := TItem(Obj);
...
begin
  ...
  if High(av) > 1 then
    Weapon.AmmoCapacity := av[2].GetInt;
end;
procedure SF_ShipSpecialBonuses(av: array of TVarEC; code: TCodeEC);
var
  Ship: TShip;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_ShipSpecialBonuses');
  Ship := TShip(av[1].GetDword);
...
begin
  ...
  if Item.ExtraSpecials = nil then
    ...
    Count := Item.ExtraSpecials.Count;
  if High(av) < 2 then
    av[0].SetInt(Count)
  else
  begin
    ...
    if (Index < 0) or (Index >= Count) then
      ...
      av[0].SetInt(PExtraSpecial(Item.ExtraSpecials[Index]).ModuleIndexPlusOne - 1);
  end;
end;
procedure SF_ItemExtraSpecialsCountByType(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_ItemExtraSpecialsAddByType');
  Item := TEquipment(av[1].GetDword);
  ModuleIndex := av[2].GetInt + 1;
  if ModuleIndex <= 0 then
    raise Exception.Create('Error.Script SF_ItemExtraSpecialsAddByType invalid bonus');
  if High(av) > 2 then
    Count := av[3].GetInt
  else
    Count := 1;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SF_ItemExtraSpecialsDeleteByType');
  Item := TEquipment(av[1].GetDword);
  ModuleIndex := av[2].GetInt + 1;
  if High(av) > 2 then
    Count := av[3].GetInt
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_ExecuteCodeFromString');
  SourceText := av[1].GetString;
  if High(av) = 1 then
  begin
    if code <> nil then
      ExecuteScriptText(SourceText, code.LocalVar)
    else
      ExecuteScriptText(SourceText, nil);
  end
  else
  begin
    SavedScript := CurrentScript;
    ...
    if code <> nil then
      RunCode.LinkAll(code.LocalVar, False);
    Count := (High(av) - 1) div 2;
    for I := 1 to Count do
      RunCode
          .LocalVar
          .Add(av[I * 2].GetString, av[I * 2 + 1].RealVType)
          .Assume(av[I * 2 + 1], True);
    try
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script SF_GenerateCodeStringFromBlock');
  av[0].SetString('');
  Path := av[1].GetString;
  Count := CountDelimitedPartsW(Path, '.');
  if (High(av) > 1) and (av[2].GetInt <> 0) then
  ...
  begin
    ...
    begin
      AppendLogLineThreadSafe(
          AnsiString(
              'Warning.Script SF_GenerateCodeStringFromBlock - cant find block '
                  + av[1].GetString
                  + ' ['
                  + IntToStr(I)
                  + ']'
  ...
  if av[0].GetString = '' then
    AppendLogLineThreadSafe(
        AnsiString('Warning.Script SF_GenerateCodeStringFromBlock - no code at ' + av[1].GetString)
    );
end;
procedure SF_ItemOnUseCode(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemOnUseCode');
  Obj := TObject(av[1].GetDword);
  Binding := nil;
  if Obj is TItem then
    Binding := TScriptItem(TItem(Obj).ScriptItem);
  ...
  begin
    ...
    if High(av) > 1 then
      Binding.OnUseText := av[2].GetString;
  end
  else
    av[0].SetString('');
end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ItemOnActCode');
  Obj := TObject(av[1].GetDword);
  Binding := nil;
  if Obj is TItem then
    Binding := TScriptItem(TItem(Obj).ScriptItem);
  ...
  begin
    ...
    begin
      ...
      if High(av) > 3 then
        Binding.OnActionText :=
            '[' + av[3].GetString + '|' + av[4].GetString + ']' + av[2].GetString
      else if High(av) = 3 then
        Binding.OnActionText := '[' + av[3].GetString + '|]' + av[2].GetString
      else
        Binding.OnActionText := av[2].GetString;
    end;
  end
  else
    av[0].SetString('');
...
begin
  ...
  begin
    if High(av) < 3 then
      raise Exception.Create('Error.Script CreateActCodeEvent cant call info without ship');
    Obj := TObject(av[3].GetDword);
    if not (Obj is TShip) then
      raise Exception.Create('Error.Script CreateActCodeEvent cant call info - not a ship');
    Ship := TShip(Obj);
    for I := 0 to Ship.CustomShipInfos.Count - 1 do
    begin
      Info := PCustomShipInfo(Ship.CustomShipInfos[I]);
      ...
      begin
        if High(av) >= 4 then
          Object1 := TObject(av[4].GetDword)
        else
          Object1 := nil;
        if High(av) >= 5 then
          Object2 := TObject(av[5].GetDword)
        else
          Object2 := nil;
        if High(av) >= 6 then
          Param := av[6].GetInt
  ...
  begin
    ...
    begin
      if High(av) >= 3 then
        Object1 := TObject(av[3].GetDword)
      else
        Object1 := nil;
      if High(av) >= 4 then
        Object2 := TObject(av[4].GetDword)
      else
        Object2 := nil;
      if High(av) >= 5 then
        Param := av[5].GetInt
    ...
    begin
      ...
      if TItem(Obj).ScriptItem <> nil then
        av[0]
            .SetInt(
                TScriptItem(TItem(Obj).ScriptItem)
                    .RunActionCode(ActionType, Ship, Object1, Object2, Param));
      if Obj is TEquipmentWithActCode then
        av[0]
            .SetInt(RunItemConfigActionCode(TItem(Obj), ActionType, Ship, Object1, Object2, Param));
    end;
  end;
end;
procedure SF_CurItem(av: array of TVarEC; code: TCodeEC);
...
var
  Matches: Boolean;
begin
  if ScriptActionTypeStack.Count < 1 then
    av[0].SetDword(0)
  else if High(av) <= 0 then
    av[0].SetDword(PtrUInt(ScriptActionTypeStack[ScriptActionTypeStack.Count - 1]))
  else
  begin
...
begin
  if ScriptActionParamStack.Count < 1 then
    av[0]
        .SetInt(0)
        // Purchase action 43 carries the destination object in the parameter slot.
        // Keep numeric actions signed 32-bit, but expose this object as a native-width
        // script DWORD, like ActObject1/ActObject2, rather than truncating its address.
  ...
  begin
    ...
    if High(av) > 0 then
      ScriptActionParamStack[ScriptActionParamStack.Count - 1] := Pointer(av[1].GetInt);
  end;
end;
procedure SF_OnUseCodeTranclucator(av: array of TVarEC; code: TCodeEC);
var
  Item: TArtefactTranclucator;
  Ship: TTranclucator;
  Index: Integer;
  Angle: Single;
  Entry: PStorageEntry;
  ...
  Other: TObject;
begin
  if (ScriptUseItem <> nil) and (ScriptUseItem is TArtefactTranclucator) then
    Item := TArtefactTranclucator(ScriptUseItem)
  else
  begin
    if High(av) < 1 then
      Exit;
    if av[1].GetDword = 0 then
      Exit;
    if not (TObject(av[1].GetDword) is TArtefactTranclucator) then
      Exit;
  ...
  begin
    ...
    begin
      ...
      Angle :=
          SeededRandomIntRange(
                  ...
                  360,
                  Cardinal(Ship.Id)
                      * (Cardinal(Galaxy.CurrentTurn) * GetPlayer.CurrentStar.GenerationSeed))
              * Pi
              / 180;
      ...
      if Index >= 0 then
        GetPlayer.Artefacts.Delete(Index);
      Index := GetPlayer.Inventory.IndexOf(Item);
      if Index >= 0 then
        GetPlayer.Inventory.Delete(Index);
      Item.Free;
      if ScriptUseItem = Item then
        ScriptUseItem := nil;
      ...
      if GetPlayer.ChameleonActive then
        ...
        begin
          ...
          if (Other is TKling) and not TShip(Other).HasIndependentScriptFaction then
            TKling(Other).DetectAttackingPlayer(GetPlayer);
        end;
    end
    ...
    begin
      ...
      ShowMessageBoxGI(
          ShipScreen,
          LocalizedColorText('FormShip.UseTranclucator'),
    ...
    begin
      ...
      ShowMessageBoxGI(
          ShipScreen,
          LocalizedColorText('FormShip.NotUseTranclucator'),
...
procedure SF_OnUseCodeTransmitter(av: array of TVarEC; code: TCodeEC);
var
  Artefact: TArtefactTransmitter;
  ShowFeedback: Boolean;
begin
  if (ScriptUseItem <> nil) and (ScriptUseItem is TArtefactTransmitter) then
    Artefact := TArtefactTransmitter(ScriptUseItem)
  else
  begin
    if High(av) < 1 then
      Exit;
    if av[1].GetDword = 0 then
      Exit;
    if not (TObject(av[1].GetDword) is TArtefactTransmitter) then
      Exit;
    Artefact := TArtefactTransmitter(av[1].GetDword);
  end;
  if High(av) > 1 then
    ShowFeedback := av[2].GetInt <> 0
  else
    ShowFeedback := True;
  ...
  begin
    ...
    ShowMessageBoxGI(
        ShipScreen,
        LocalizedColorText('FormShip.UseOnlyInSpace'),
        mbgCancel or mbgUnused04
    );
  end
...
begin
  av[0].SetDword(0);
  if (ScriptUseItem <> nil) and (ScriptUseItem is TArtefact) then
    Item := TArtefact(ScriptUseItem)
  else
  begin
    if High(av) < 1 then
      Exit;
    if av[1].GetDword = 0 then
      Exit;
    if not (TObject(av[1].GetDword) is TArtefact) then
      Exit;
        ...
        begin
          Distance :=
              PointDistanceSquared(
                  ...
                  TStar(Galaxy.Stars[Index]).Position
              );
          if (Distance > 400) and (Distance < 2500) then
            ...
            begin
              ...
              if NextRandomIntRange(0, 100, GetPlayer.RandomState) < 20 then
                Break;
            end;
        end;
      if Hole.Star2 = nil then
    ...
    begin
      for I := 0 to GetPlayer.CurrentStar.Missiles.Count - 1 do
        with TMissile(GetPlayer.CurrentStar.Missiles[I]) do
          if GetPlayer = OwnerShip then
            FlightTicks := 10000;
      ShowMessageBoxGI(
          ShipScreen,
          LocalizedColorText('FormShip.UseMissileDef'),
          mbgCancel or mbgUnused04
      );
    end;
  end;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MessageBox');
  if High(av) < 2 then
    Options := mbgCancel or mbgUnused04
  else
    Options := av[2].GetInt;
  if High(av) < 3 then
    OffsetX := 0
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script MessageBoxYesNo');
  if High(av) < 2 then
    Options := mbgOK or mbgCancel or mbgQuestion
  else
    Options := av[2].GetInt or mbgOK or mbgCancel;
  if High(av) < 3 then
    OffsetX := 0
  else
    OffsetX := av[3].GetInt;
  if High(av) < 4 then
    ...
    OffsetY := av[4].GetInt;
  av[0].SetInt(0);
  if ShowMessageBoxGI(GetInnermostScreenLoop, av[1].GetString, Options, 0, OffsetX, OffsetY)
      = mbgResultOK then
    av[0].SetInt(1);
end;
procedure SF_CountBox(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script CountBox');
  ...
  if High(av) > 6 then
    ...
    Limit := Maximum;
  if High(av) > 7 then
    Available := av[8].GetInt
  else
    Available := Maximum;
  if High(av) > 8 then
    TotalLimit := av[9].GetInt
  else
...
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script NumberBox');
  ...
  if High(av) > 4 then
    ...
    Limit := Maximum;
  if (High(av) > 5) and (av[6].GetString <> '') then
    AlternateCaption := 'GI,' + av[6].GetString
  else
    AlternateCaption := '';
  if High(av) > 6 then
    Value := av[7].GetInt
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script TextBox');
  Caption := av[1].GetString;
  Value := '';
  if High(av) > 1 then
    Value := av[2].GetString;
  MaximumLength := 30;
  if High(av) > 2 then
    MaximumLength := av[3].GetInt;
  if High(av) < 4 then
    OffsetX := 0
  else
    OffsetX := av[4].GetInt;
  ...
  if ShowTextInputDialog(GetInnermostScreenLoop, Caption, Value, MaximumLength, OffsetX, OffsetY)
      = 1 then
    av[0].SetString(Value);
end;
procedure SF_ListBox(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ListBox');
  Caption := av[1].GetString;
  Choices := TList.Create;
  Index := -1;
  ...
  if High(av) < 4 then
    ...
    OffsetY := av[4].GetInt;
  if ShowListDialog(GetInnermostScreenLoop, Index, Caption, Choices, OffsetX, OffsetY) = 1 then
    av[0].SetInt(Index)
  else
    av[0].SetInt(-1);
  while Choices.Count > 0 do
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script UselessItemText');
  Obj := TObject(av[1].GetDword);
  Item := nil;
  Text := '';
  if Obj is TItem then
  ...
  if Item <> nil then
    ...
    begin
      ...
      if High(av) > 1 then
        (Item as TUselessItem).CustomText := av[2].GetString;
    end;
  av[0].SetString(Text);
end;
procedure SF_UselessItemData(av: array of TVarEC; code: TCodeEC);
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script UselessItemData');
  Obj := TObject(av[1].GetDword);
  Index := av[2].GetInt;
  Item := nil;
  Value := 0;
  ...
  if Item <> nil then
    ...
    begin
      ...
      if High(av) > 2 then
        (Item as TUselessItem).Data[Index - 1] := av[3].GetInt;
    end;
  av[0].SetInt(Value);
end;
procedure SF_GetAchievementSHU(av: array of TVarEC; code: TCodeEC);
...
procedure SF_UICheckElement(av: array of TVarEC; code: TCodeEC);
var
  FormName, Path, Query: WideString;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script UICheckElement');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  Control := nil;
  if FormName = '' then
    Form := GetInnermostScreenLoop
  else
  ...
  if Control = nil then
    raise Exception.Create(
        'Error.Script UICheckElement - cant find ' + Path + ' (' + FormName + ')');
  Query := av[3].GetString;
  if Query = 'IsActive' then
    av[0].SetInt(Ord(Control.Active))
  else if Query = 'PosX' then
    av[0].SetInt(Control.LocalPosition.X)
  else if Query = 'PosY' then
    av[0].SetInt(Control.LocalPosition.Y)
  else if Query = 'SizeX' then
    av[0].SetInt(Control.ClientSize.X)
  else if Query = 'SizeY' then
    av[0].SetInt(Control.ClientSize.Y)
  else if Query = 'IsDisable' then
  begin
    if not (Control is TGraphButtonGI) then
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a button');
  ...
  begin
    ...
    begin
      ...
      if High(av) > 3 then
        TEditGI(Control).SetText(av[4].GetString);
    end
    else if Control is TLabelGI then
      av[0].SetString(TLabelGI(Control).GetText)
    else
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a label or edit');
  end
  else if Query = 'Help' then
  begin
    av[0].SetString(Control.HelpText);
    if High(av) > 3 then
      Control.HelpText := av[4].GetString;
  end
  ...
  begin
    ...
    begin
      ...
      if High(av) > 3 then
        TEditGI(Control).SetCaretPosition(av[4].GetInt);
    end
    else
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not an edit');
  end
  else if Query = 'IsFocused' then
    av[0].SetInt(Ord(Form.FocusedControl = Control))
  ...
  begin
    ...
    begin
      ...
      av[0].SetString(TgiGI(Control).GetImagePath)
    else if Control is TImageGI then
      av[0].SetString(TImageGI(Control).GetImagePath)
    else
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a image');
  end
  else if Query = 'CurFrame' then
  begin
    if not (Control is TgaiGI) then
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a gai');
    av[0].SetInt(TgaiGI(Control).SequenceFrame);
    if High(av) > 3 then
      TgaiGI(Control).SetSequenceFrame(av[4].GetInt);
  end
  ...
  begin
    ...
    begin
      ...
      if High(av) > 3 then
        TGraphButtonGI(Control).SetDown(av[4].GetInt <> 0);
    end
    else
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a button');
  ...
  begin
    ...
    begin
      if High(av) < 4 then
        raise Exception.Create(
            'Error.Script UICheckElement - Rescale option can not be used without image path');
      if (High(av) > 4) and (av[5].GetString = 'GI') then
        TGraphBufGI(Control).LoadScaledGiPath(av[4].GetString)
      else
        TGraphBufGI(Control).LoadScaledBitmapPathAsRgba(av[4].GetString);
    end
    else
      raise Exception.Create(
          'Error.Script UICheckElement - ' + Path + ' (' + FormName + ') is not a graph buffer');
  end
  else
    raise Exception.Create('Error.Script UICheckElement - query not recognized - ' + Query);
end;
procedure SF_InterfaceState(av: array of TVarEC; code: TCodeEC);
var
  FormName, Path: WideString;
  I: Integer;
  Entry: TInterfaceStateOverride;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script InterfaceState');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  if Galaxy = nil then
  begin
    Form := FindMessageLoop(FormName);
    if Form = nil then
      raise Exception.Create('ML not found - ' + FormName);
    Control := Form.FindControlByPath(Path);
    if Control = nil then
      AppendLogLineThreadSafe(AnsiString('Object not found - ' + Path))
    else
    begin
      av[0].SetInt(Ord(Control.Active));
      if (Control is TGraphButtonGI) and TGraphButtonGI(Control).Disabled and Control.Active then
        av[0].SetInt(2);
      if High(av) > 2 then
      begin
        Control.SetActive(av[3].GetInt > 0);
        if (av[3].GetInt > 1) and (Control is TGraphButtonGI) then
          TGraphButtonGI(Control).SetDisabled(av[3].GetInt = 2);
      end;
    end;
    Exit;
  end;
...
var
  FormName, Path: WideString;
  I: Integer;
  Entry: TInterfaceTextOverride;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script InterfaceText');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  if Galaxy = nil then
  begin
    ...
    begin
      if not (Control is TLabelGI) then
        raise Exception.Create('Object is not a label - ' + Path);
      av[0].SetString(TLabelGI(Control).GetText);
      if High(av) > 2 then
        TLabelGI(Control).SetText(av[3].GetString);
    end;
    Exit;
  end;
...
var
  FormName, Path: WideString;
  I: Integer;
  Entry: TInterfaceImageOverride;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script InterfaceImage');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  if Galaxy = nil then
  begin
    ...
    begin
      ...
      begin
        ...
        av[0].SetString(Control.ConfigPath);
      if High(av) > 2 then
      begin
        if (CountDelimitedPartsW(av[3].GetString, ':') > 1)
            and (ExtractDelimitedPartW(av[3].GetString, 0, ':') = 'Style') then
        ...
        begin
          ...
          TgiGI(Control).SetImagePath(av[3].GetString)
        else if Control is TImageGI then
          TImageGI(Control).SetImagePath(av[3].GetString)
        else
          raise Exception.Create('Object is not an image - ' + Path);
      end;
    end;
    Exit;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Entry.SetImagePath(av[3].GetString);
      end;
      Exit;
    end;
  end;
...
var
  ...
  I: Integer;
  X, Y, Z: Integer;
  Entry: TInterfacePosOverride;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 5 then
    raise Exception.Create('Error.Script InterfacePos');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  X := av[3].GetInt;
  Y := av[4].GetInt;
...
var
  ...
  I: Integer;
  X, Y: Integer;
  Entry: TInterfaceSizeOverride;
  Form: TMessageLoopGI;
  Control: TObjectGI;
begin
  if High(av) < 4 then
    raise Exception.Create('Error.Script InterfaceSize');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  X := av[3].GetInt;
  Y := av[4].GetInt;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ButtonClick');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  if FormName = '' then
    Form := GetInnermostScreenLoop
    ...
    Form := FindMessageLoop(FormName);
  if Form = nil then
    raise Exception.Create('Error.Script ButtonClick - ML not found');
  Control := Form.FindControlByPath(Path);
  if Control = nil then
    raise Exception.Create('Error.Script ButtonClick - Object not found');
  if not (Control is TGraphButtonGI) then
    raise Exception.Create('Error.Script ButtonClick - Object not a button');
  Button := Control as TGraphButtonGI;
  if (High(av) < 3) or (av[3].RealVType <> vkString) or (av[3].GetString = 'AllCode') then
  begin
    if Assigned(Button.UpCallback) then
      Button.UpCallback(Button)
    else if Assigned(Button.DownCallback) then
      Button.DownCallback(Button);
  end;
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script SetFocus');
  FormName := av[1].GetString;
  Path := av[2].GetString;
  if FormName = '' then
    Form := GetInnermostScreenLoop
    ...
    Form := FindMessageLoop(FormName);
  if Form = nil then
    raise Exception.Create('Error.Script SetFocus - ML not found');
  Control := Form.FindControlByPath(Path);
  if Control = nil then
    raise Exception.Create('Error.Script SetFocus - Object not found');
...
begin
  if GetInnermostScreenLoop <> ShipScreen then
    raise Exception.Create('Error.Script FormShipCurItem');
  if High(av) < 1 then
  begin
  ...
  if av[1].RealVType = vkString then
  begin
    if av[1].GetString = 'MoveType' then
      Query := 0
    else if av[1].GetString = 'StackableType' then
      Query := 1
    else if av[1].GetString = 'StackableCount' then
      Query := 2
    else if av[1].GetString = 'StackableCost' then
      Query := 3
    else if av[1].GetString = 'PutBack' then
      Query := 4
    else if av[1].GetString = 'Destroy' then
      Query := 5
    else if av[1].GetString = 'Detach' then
      Query := 6
    else if av[1].GetString = 'Replace' then
      Query := 7
    else
      raise Exception.Create(
          'Error.Script FormShipCurItem - query not recognized: ' + av[1].GetString);
  end
  ...
  case Query of
    ...
    begin
      if ShipScreen.SelectedHoldItem <> nil then
        ShipScreen.SelectedHoldItem.Free;
      ShipScreen.SelectedHoldItem := nil;
      ShipScreen.SelectedHoldKind := phkEmpty;
      PlayerHoldShip.RefreshDerivedStats(True);
      if not ShipScreen.IsCursorImageSelected('Main') then
        ShipScreen.SetCursorByName('Main');
    ...
    7:
    begin
      if (High(av) < 2) or (av[2].GetDword = 0) then
        raise Exception.Create('Error.Script FormShipCurItem no replacing item');
      if (ShipScreen.SelectedHoldItem = nil)
      ...
      if Item is TArtefact then
        ...
        ShipScreen.SelectedHoldKind := phkEquipment;
      PlayerHoldShip.RefreshDerivedStats(True);
    ...
    Exception
        .Create('Error.Script FormShipCurItem - query not recognized: ' + IntToWideString(Query));
  end;
end;
procedure SF_UpdateFormShip(av: array of TVarEC; code: TCodeEC);
begin
...
var
  Id: TGameScreenId;
  Name: WideString;
  Parent, Root, Child: TMessageLoopGI;
  Background, ChildBackground: TObjectGI;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script OpenCustomForm');
...
var
  ResultCode: Integer;
begin
  if CurrentCustomDialog = nil then
    raise Exception.Create('Error.Script CloseCustomForm - no custom form');
  if High(av) > 0 then
    ResultCode := av[1].GetInt
  else
    ResultCode := 1;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomInterfaceState');
  Form := CurrentCustomDialog;
  if Form = nil then
    raise Exception.Create('Error.Script CustomInterfaceState - no custom form');
  Control := Form.FindControlByPath(av[1].GetString);
  if Control = nil then
    raise Exception.Create('Error.Script CustomInterfaceState - object not found');
  av[0].SetInt(Ord(Control.Active));
  if High(av) > 1 then
    Control.SetActive(av[2].GetInt > 0);
  if Control is TGraphButtonGI then
  begin
    if Control.Active then
      av[0].SetInt(Ord(TGraphButtonGI(Control).Disabled) + av[0].GetInt);
    if High(av) > 1 then
      TGraphButtonGI(Control).SetDisabled(av[2].GetInt = 2);
  end;
end;
procedure SF_CustomInterfaceText(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomInterfaceText');
  Form := CurrentCustomDialog;
  if Form = nil then
    raise Exception.Create('Error.Script CustomInterfaceText - no custom form');
  Control := Form.FindControlByPath(av[1].GetString);
  if Control = nil then
    raise Exception.Create(
        'Error.Script CustomInterfaceText - object not found: ' + av[1].GetString);
  if Control is TLabelGI then
    av[0].SetString(TLabelGI(Control).GetText)
  else if Control is TEditGI then
    av[0].SetString(TEditGI(Control).Text)
  else
    raise Exception.Create(
        'Error.Script CustomInterfaceText - object is not a label or edit: ' + av[1].GetString);
  if High(av) > 1 then
  begin
    if Control is TLabelGI then
      TLabelGI(Control).SetText(av[2].GetString)
    else if Control is TEditGI then
      TEditGI(Control).SetText(av[2].GetString);
  end;
end;
procedure SF_CustomInterfaceImage(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script CustomInterfaceImage');
  Form := CurrentCustomDialog;
  if Form = nil then
    raise Exception.Create('Error.Script CustomInterfaceImage - no custom form');
  Control := Form.FindControlByPath(av[1].GetString);
  if Control = nil then
    raise Exception.Create(
        'Error.Script CustomInterfaceImage - object not found: ' + av[1].GetString);
  if Control is TgaiGI then
  begin
    av[0].SetString(TgaiGI(Control).GetImagePath);
    if TgaiGI(Control).GetFirstFrameImagePath <> '' then
  ...
  if High(av) > 1 then
  begin
    if (CountDelimitedPartsW(av[2].GetString, ':') > 1)
        and (ExtractDelimitedPartW(av[2].GetString, 0, ':') = 'Style') then
    ...
    begin
      ...
      TgiGI(Control).SetImagePath(av[2].GetString)
    else if Control is TImageGI then
      TImageGI(Control).SetImagePath(av[2].GetString)
    else
      raise Exception.Create(
          'Error.Script CustomInterfaceImage - object is not a image: ' + av[1].GetString);
  end;
end;
procedure SF_CustomInterfacePos(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 3 then
    raise Exception.Create('Error.Script CustomInterfacePos');
  Form := CurrentCustomDialog;
  if Form = nil then
    raise Exception.Create('Error.Script CustomInterfacePos - no custom form');
  Control := Form.FindControlByPath(av[1].GetString);
  if Control = nil then
    raise Exception.Create('Error.Script CustomInterfacePos - object not found');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script CustomInterfacePosZ');
  Form := CurrentCustomDialog;
  if Form = nil then
    raise Exception.Create('Error.Script CustomInterfacePosZ - no custom form');
  Control := Form.FindControlByPath(av[1].GetString);
  if Control = nil then
    raise Exception.Create('Error.Script CustomInterfacePosZ - object not found');
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script StarMapCenterView');
...
begin
  if High(av) < 10 then
    raise Exception.Create('Error.Script StarMapCustomSelectionMode');
  if GetInnermostScreenLoop = StarMapScreen then
  begin
    if av[1].RealVType = vkString then
    begin
    ...
    StarMapScreen.CustomSelectionColor :=
        CurrentPixelFormat.PackRgb(av[8].GetInt, av[9].GetInt, av[10].GetInt);
...
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script StarMapBlinkingMessage');
  StarMapScreen.ShowLargeHelp(av[1].GetString);
end;
procedure SF_BlinkingWarning(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) <> 1 then
    raise Exception.Create('Error.Script BlinkingWarning');
...
begin
  if High(av) < 1 then
    av[0].SetInt(Galaxy.CustomWeaponTypes.Count)
  else
  begin
    Index := av[1].GetInt;
...
begin
  ...
  for I := 1 to 8 do
    Info.DamageScaleByLevel[I] := Base.DamageScaleByLevel[I];
end;
procedure SF_GetCustomWeaponInfo(av: array of TVarEC; code: TCodeEC);
begin
  if High(av) < 1 then
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script GetCustomWeaponStats');
  if av[1].RealVType = vkString then
    Info := Galaxy.RequireCustomWeaponInfo(av[1].GetString)
  else
  ...
  if Key = 'TechLevel' then
  ...
  else if Key = 'AverageSize' then
    av[0].SetInt(Info.AverageSize)
  else if Key = 'AverageRadius' then
    av[0].SetInt(Info.AverageRange)
  else if Key = 'MissileRadius' then
    av[0].SetInt(Info.MissileRange)
  else if Key = 'SecondaryDamageRadius' then
    av[0].SetFloat(Info.SecondaryDamageRadius)
  else if Key = 'MaxDamage' then
    av[0].SetInt(Info.MaxDamage)
    ...
    av[0].SetInt(Info.AttackCount)
  else if Key = 'ShotCount' then
    av[0].SetInt(Info.ShotCount)
    ...
    case Info.Availability of
      ...
      raise Exception.Create('Error.Script GetCustomWeaponStats - unknown availability type');
    end
  else
    raise Exception.Create(AnsiString('Error.Script GetCustomWeaponStats - unknown stat ' + Key));
end;
procedure SF_GetCustomWeaponPrimaryDamageType(av: array of TVarEC; code: TCodeEC);
var
  Info: PWeaponInfo;
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script GetCustomWeaponPrimaryDamageType');
  if av[1].RealVType = vkString then
    Info := Galaxy.RequireCustomWeaponInfo(av[1].GetString)
  else
    Info := @WeaponInfos[av[1].GetInt and $7F];
...
begin
  ...
  else if Text = 'CoalitionOnly' then
    Info.Availability := waCoalitionOnly
  else if Text = 'PirateOnly' then
    Info.Availability := waPirateOnly
  else if Text = 'NotSold' then
    Info.Availability := waNotSold
  else if Text = 'NotSoldAndNodeRepair' then
    Info.Availability := waNotSoldAndNodeRepair
  else if Text = 'MalocOnly' then
    Info.Availability := waMalocOnly
  else if Text = 'PelengOnly' then
    Info.Availability := waPelengOnly
  else if Text = 'PeopleOnly' then
    Info.Availability := waPeopleOnly
  else if Text = 'FeiOnly' then
    Info.Availability := waFeiOnly
  else if Text = 'GaalOnly' then
    Info.Availability := waGaalOnly
  else if Text = 'SystemOnly' then
    Info.Availability := waSystemOnly
  else
    Info.Availability := waFree;
end;
procedure SF_SetCustomWeaponPrimaryData(av: array of TVarEC; code: TCodeEC);
...
begin
  ...
  begin
    ...
    for I := 1 to Min(8, Count) do
      Info.DamageScaleByLevel[I] :=
          ExtractDecimalToSingleW(ExtractDelimitedPartW(Text, I - 1, ','));
  end
  else
...
begin
  ...
  begin
    if Pos('Splash', Text) > 0 then
      Info.ShotType := wstSplash
    else if Pos('Exploder', Text) > 0 then
      Info.ShotType := wstExploder
    else if Pos('AreaDamage', Text) > 0 then
      Info.ShotType := wstAreaDamage
    else if Pos('Torpedo', Text) > 0 then
      Info.ShotType := wstTorpedo
    else if Pos('Missile', Text) > 0 then
      Info.ShotType := wstMissile
    else if Pos('Rocket', Text) > 0 then
      Info.ShotType := wstRocket
    else if Pos('Chain', Text) > 0 then
      Info.ShotType := wstChain;
  end;
  if Info.ShotType in [wstChain, wstMissile, wstRocket] then
    Info.ShotCount := ExtractDigitsToIntW(Text);
  if High(av) > 2 then
    Info.ShotSpeedPercent := av[3].GetInt;
  if High(av) > 3 then
    Info.AverageRange := av[4].GetInt;
  if High(av) > 4 then
    Info.SecondaryDamageRadius := av[5].GetInt;
  if High(av) > 5 then
    Info.MiningFactor := av[6].GetFloat;
  if High(av) > 6 then
    Info.AttackCount := av[7].GetInt;
end;
procedure SF_SetCustomMissileWeaponStats(av: array of TVarEC; code: TCodeEC);
var
  Info: PWeaponInfo;
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script StarCustomFaction');
  Obj := TStar(av[1].GetDword);
  av[0].SetString(Obj.Status.CustomFaction);
  if High(av) > 1 then
  begin
    Obj.Status.CustomFaction := av[2].GetString;
    if av[2].GetString = '' then
      Obj.ResetControlFaction;
  end;
end;
procedure SF_ShipCustomFaction(av: array of TVarEC; code: TCodeEC);
var
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ShipCustomFaction');
  Ship := TShip(av[1].GetDword);
  if GetPlayer = Ship then
  begin
    if High(av) > 1 then
      raise Exception.Create('Error.Script ShipCustomFaction Player');
    av[0].SetString('');
  end
  else if Ship.ScriptShip = nil then
  begin
    if High(av) > 1 then
      raise Exception.Create('Error.Script ShipCustomFaction non-script ship');
    av[0].SetString('');
  end
  else
  begin
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script EqCustomFaction');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script PlanetCustomFaction');
  Obj := TPlanet(av[1].GetDword);
  av[0].SetString(Obj.CustomFaction);
  if High(av) > 1 then
  begin
...
begin
  if High(av) < 2 then
    raise Exception.Create('Error.Script ImportedFunction');
...
begin
  if High(av) < 1 then
    raise Exception.Create('Error.Script ImportAll');
  if ScriptLibraryCache = nil then
    ScriptLibraryCache := TLibraryCache.Create;
...
begin
  ...
  Scope
      .Add('ChangeGlobalRelationsPlanets', vkExternFun)
      .SetExternFun(@SF_ChangeGlobalRelationsPlanets);
  ...
  Scope
      .Add('ShipStrengthInAverageRanger', vkExternFun)
      .SetExternFun(@SF_ShipStrengthInAverageRanger);
  ...
  Scope
      .Add('GalaxyDominatorResearchPercent', vkExternFun)
      .SetExternFun(@SF_GalaxyDominatorResearchPercent);
  Scope
      .Add('GalaxyDominatorResearchMaterial', vkExternFun)
      .SetExternFun(@SF_GalaxyDominatorResearchMaterial);
  ...
  Scope
      .Add('BuildListOfQuestPossibleLocations', vkExternFun)
      .SetExternFun(@SF_BuildListOfQuestPossibleLocations);
  ...
  Scope
      .Add('StarListToTransitPlanetList', vkExternFun)
      .SetExternFun(@SF_StarListToTransitPlanetList);
  ...
  Scope
      .Add('PlayerPlanetaryBattlesCompleted', vkExternFun)
      .SetExternFun(@SF_PlayerPlanetaryBattlesCompleted);
  ...
  Scope
      .Add('ShipFindCustomShipInfoByType', vkExternFun)
      .SetExternFun(@SF_ShipFindCustomShipInfoByType);
  Scope
      .Add('ShipCustomShipInfoDescription', vkExternFun)
      .SetExternFun(@SF_ShipCustomShipInfoDescription);
  ...
  Scope
      .Add('StarFindCustomStarInfoByType', vkExternFun)
      .SetExternFun(@SF_StarFindCustomStarInfoByType);
  ...
  Scope
      .Add('ItemExtraSpecialsCountByType', vkExternFun)
      .SetExternFun(@SF_ItemExtraSpecialsCountByType);
  Scope.Add('ItemExtraSpecialsAddByType', vkExternFun).SetExternFun(@SF_ItemExtraSpecialsAddByType);
  Scope
      .Add('ItemExtraSpecialsDeleteByType', vkExternFun)
      .SetExternFun(@SF_ItemExtraSpecialsDeleteByType);
  Scope.Add('ExecuteCodeFromString', vkExternFun).SetExternFun(@SF_ExecuteCodeFromString);
  Scope
      .Add('GenerateCodeStringFromBlock', vkExternFun)
      .SetExternFun(@SF_GenerateCodeStringFromBlock);
  ...
  Scope
      .Add('GetCustomWeaponPrimaryDamageType', vkExternFun)
      .SetExternFun(@SF_GetCustomWeaponPrimaryDamageType);
  Scope
      .Add('SetCustomWeaponAvailability', vkExternFun)
      .SetExternFun(@SF_SetCustomWeaponAvailability);
  ...
  Scope
      .Add('SetCustomMissileWeaponStats', vkExternFun)
      .SetExternFun(@SF_SetCustomMissileWeaponStats);
...
begin
  ...
  TMessageLoopGIWithMainPanel.ClassName;
  TMissile.ClassName;
