...
type
  TfRating2 = class;
  {$Z4}
  TRangerRatingSortColumn =
      (rrscName = 1, rrscExperience = 2, rrscRace = 3, rrscRank = 4, rrscCharacter = 5);
  TRangerRatingRow = packed record
    Ranger: TRanger;
    Top: Integer;
    Height: Integer;
  end;
  TfRating2 = class(TMessageLoopGIWithMainPanel)
    RewardWindow: TWindowGI;
    Rows: array of TRangerRatingRow;
    ...
    procedure BackgroundMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure BackgroundMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure AwardsMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure AwardsMouseMove(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure HintMouseLeave(Sender: TObjectGI);
    procedure ShowPartnershipHint(Sender: TObjectGI);
    procedure ShowCareerHint(Sender: TObjectGI);
    procedure ShowAwardHint(Ranger: TRanger; AwardId: Integer);
    procedure HideHint;
    function FindRowByRangerId(RangerId: Integer): Integer;
    procedure SortHeaderMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure FeaturedRangerClicked(Sender: TObjectGI);
    procedure RowMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure SelectRow(Index: Integer);
    procedure ClearRows;
    procedure RebuildTable;
    procedure RebuildRow(Index: Integer);
    procedure CreateRow(Index: Integer);
    procedure RowMouseEnter(Sender: TObjectGI);
    procedure RowMouseLeave(Sender: TObjectGI);
    procedure RefreshFeaturedRangers;
    procedure ShowDominatorKillsHint(Sender: TObjectGI);
  end;
const
  CareerHintColumns: array[1..3] of Integer = (10, 75, 105);
  DominatorHintColumns: array[1..3] of Integer = (10, 75, 95);
...
begin
  ...
  with GetByName('SortCharacter') do
    UserValue := 5;
...
begin
  ...
  begin
    ...
    begin
      if SelectedIndex < High(Rows) then
        SelectRow(SelectedIndex + 1);
    end
...
begin
  ...
  begin
    ...
    if not (GetByName('ImagePanel') as TImageGI).ContainsPoint(Point)
...
begin
  if ClickAutoCloseForm then
    if not (GetByName('ImagePanel') as TImageGI).ContainsPoint(Point)
        and not GetByName('PM_PanelMsg').ContainsPoint(Point)
        and BackgroundClickStarted then
      CloseClicked(nil);
end;
...
begin
  if Delta = WHEEL_DELTA then
    TablePanel.VerticalScrollBar.SetPosition(
        TablePanel.VerticalScrollBar.Position - TablePanel.VerticalScrollBar.SmallChange
    )
...
begin
  ...
  begin
    if Index >= Ranger.AwardIds.Count then
      Index := Ranger.AwardIds.Count - 1;
...
var
  Cursor: TPoint;
  I: Integer;
  Text, PartnerInfo, Names: WideString;
  Ally: TRanger;
  Pirate: TPirate;
  function FormatRangerWingmenHint(
      Ranger: TRanger
  ): WideString; // @addr 0x566378 @ida "void __usercall $name(TRanger *Ranger@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x566f07,0x567068" @note "Nested in TfRating2.ShowPartnershipHint; caller supplies its parent frame."
  var
    I: Integer;
    Names: WideString;
    Ally: TRanger;
  begin
        ...
        if (GetPlayer <> Ally) and (Ally.PartnerShip = Ranger) then
          Result :=
              FormatText2(
                  ...
                  Ally.GetName,
                  '<Date>',
                  Galaxy.FormatTurnDate(Galaxy.CurrentTurn + Ally.PartnershipDaysRemaining)
              );
      end;
    end
    else
    begin
      ...
      Result :=
          FormatText1(LocalizedColorText('FormRating.PartnerBossManyText'), '', '<Names>', Names);
    end;
  end;
begin
  ...
  begin
    ...
    begin
          ...
          Names :=
              Names
                  ...
                  + FormatText2(
                      ...
                      Pirate.GetName,
                      '<Date>',
                      Galaxy.FormatTurnDate(Galaxy.CurrentTurn + Pirate.PartnershipDaysRemaining))
        else
          Names :=
              FormatText2(
                  LocalizedColorText('FormRating.AddInfoAboutPirate'),
      ...
      PartnerInfo :=
          FormatText1(LocalizedColorText('FormRating.PlayerPartnerOk'), '', '<Names>', Names);
    end
    else
      PartnerInfo := LocalizedColorText('FormRating.PlayerPartnerNo');
  ...
  with GetByName('RewardName') as TLabelGI do
    SetSize(
        Classes.Point(
            Self.RewardWindow.ClientSize.X - LocalPosition.X - Self.RewardWindow.WorkSubRect.Right,
            ClientSize.Y
        )
    );
  Cursor := GetCursorPoint;
  RewardWindow.SetPosition(
      Classes.Point(Cursor.X + 100, Max(0, Cursor.Y - RewardWindow.ClientSize.Y - 20))
  );
end;
...
var
  ...
  Text: WideString;
  Ranger: TRanger;
  function FormatCareerHintColumn(
      Column: Integer
  ): WideString; // @addr 0x567654 @ida "void __usercall $name(int Column@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x56786f,0x5678b9,0x567901,0x56791b" @note "Nested in TfRating2.ShowCareerHint; caller supplies its parent frame."
  begin
...
begin
  ...
  begin
    ...
    with GetByName('RewardName') as TLabelGI do
      SetText(Ranger.GetAwardInfo(AwardId).Name);
    with GetByName('RewardText') as TLabelGI do
      SetText(Ranger.GetAwardInfo(AwardId).Text);
    ShipScreen.LayoutItemInfo(
        RewardWindow,
        GetByName('RewardName') as TLabelGI,
        GetByName('RewardText') as TLabelGI,
    ...
    with GetByName('RewardName') as TLabelGI do
      SetSize(
          Classes.Point(
              Self.RewardWindow.ClientSize.X
                  - LocalPosition.X
                  - Self.RewardWindow.WorkSubRect.Right,
              ClientSize.Y
          )
      );
  end;
...
var
  Column: TRangerRatingSortColumn;
begin
  if SelectedIndex >= 0 then
  begin
...
begin
  ...
  begin
    if Self.SortColumn = rrscName then
      SetPosition(
          Classes.Point(
              Self.GetByName('SortName').LocalPosition.X
                  + Self.GetByName('SortName').ClientSize.X
                  - ClientSize.X,
              LocalPosition.Y
          )
      )
    else if Self.SortColumn = rrscExperience then
      SetPosition(
          Classes.Point(
              Self.GetByName('SortScore').LocalPosition.X
                  + Self.GetByName('SortScore').ClientSize.X
                  - ClientSize.X,
              LocalPosition.Y
          )
      )
    else if Self.SortColumn = rrscRace then
      SetPosition(
          Classes.Point(
              Self.GetByName('SortRace').LocalPosition.X
                  + Self.GetByName('SortRace').ClientSize.X
                  - ClientSize.X,
              LocalPosition.Y
          )
      )
    else if Self.SortColumn = rrscRank then
      SetPosition(
          Classes.Point(
              Self.GetByName('SortRank').LocalPosition.X
                  + Self.GetByName('SortRank').ClientSize.X
                  - ClientSize.X,
              LocalPosition.Y
          )
      );
    if Self.SortAscending then
      SetImagePath('GI,Bm.FormRating2.' + GiResourceSuffix + 'SortDown')
    else
      SetImagePath('GI,Bm.FormRating2.' + GiResourceSuffix + 'SortUp');
  end;
  List := TList.Create;
  for I := 0 to Galaxy.Rangers.Count - 1 do
  begin
    A := TRanger(Galaxy.Rangers[I]);
    if not A.ExcludedFromRating then
      List.Add(A);
  end;
  for I := 0 to List.Count - 2 do
    ...
    begin
      ...
      begin
        if SortAscending then
        begin
          if B.TotalExperience < A.TotalExperience then
            List.Exchange(I, J);
        end
        else if B.TotalExperience > A.TotalExperience then
          List.Exchange(I, J);
      end
      ...
      begin
        ...
        begin
          if AnsiStrComp(
                  PChar(AnsiString(B.GetCharacterName)),
                  PChar(AnsiString(A.GetCharacterName)))
              < 0 then
            List.Exchange(I, J);
        end
        else if AnsiStrComp(
  ...
  begin
    ...
    begin
      ...
      Rows[I].Top := 0;
      Rows[I].Height := 0;
      if (SelectedIndex < 0) and (Rows[I].Ranger.Id = SelectedRangerId) then
        SelectedIndex := I;
    end;
  end;
  List.Free;
...
begin
  ...
  begin
    ...
    if Previous.UserState = Index then
      Previous.Free;
  end;
  Delta := Rows[Index].Height;
  CreateRow(Index);
  Delta := Rows[Index].Height - Delta;
  for I := Index + 1 to High(Rows) do
    Inc(Rows[I].Top, Delta);
  Child := TablePanel.FirstChild;
  while Child <> nil do
  begin
    if Cardinal(Child.UserState) > Cardinal(Index) then
      Child.SetPosition(Classes.Point(Child.LocalPosition.X, Child.LocalPosition.Y + Delta));
    Child := Child.NextSibling;
  end;
  TablePanel.UpdateScrollRanges;
...
var
  ...
  function GetRatingRankImagePath(
      Rank: Byte
  ): WideString; // @addr 0x569044 @ida "void __usercall $name(unsigned __int8 Rank@<al>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x56add4" @note "Nested in TfRating2.CreateRow; caller supplies its parent frame."
  begin
    if Rank = 0 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank1'
    else if Rank = 1 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank2'
    else if Rank = 2 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank3'
    else if Rank = 3 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank4'
    else if Rank = 4 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank5'
    else if Rank = 5 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank6'
    else if Rank = 6 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank7'
    else if Rank = 7 then
      Result := 'GI,Bm.FormRating2.' + GiResourceSuffix + 'Rank8'
    else
    ...
    begin
      ...
      for I := 0 to Size - 1 do
        Buffer.GraphBuf.FillRect32(
            ...
            Integer(Round(I / Size * 240) + 10) shl 24 or (250 shl 16) or (250 shl 8) or 50
        );
...
begin
  ...
  begin
    ...
    Caption.SetTextColor(CurrentPixelFormat.PackRgbBytes(0, 0, 0));
    Caption.SetText(
        IntToStr(Ranger.TotalShipKillCount - Ranger.PirateKillCount - Ranger.DominatorKillCount)
    );
    ExtraKills := 0;
    if GetPlayer = Ranger then
    begin
      ExtraKills :=
          (Ranger as TPlayer).HyperspaceKillCount + (Ranger as TPlayer).BlackHoleKillCount;
    ...
    Caption.SetTextAlignY(tayCenterEx);
    Caption.SetText(
        LocalizedText('FormRating.LiberationSystem') + ': ' + IntToStr(Ranger.LiberatedSystemCount)
    );
    Caption.SetTextColor(CurrentPixelFormat.PackRgbBytes(0, 0, 0));
    Caption := TLabelGI.Create(Panel);
    ...
    if GiResourceVariant = 1 then
      BarPanel.SetSize(Classes.Point(82, 10))
    else
      BarPanel.SetSize(Classes.Point(103, 11));
    ...
    if GiResourceVariant = 1 then
      Image.SetPosition(Classes.Point(72, 34))
    else
      Image.SetPosition(Classes.Point(90, 43));
    if GiResourceVariant = 1 then
      Image.SetSize(Classes.Point(77, 86))
    else
      Image.SetSize(Classes.Point(99, 110));
    if AnimCaptain then
    begin
      ...
      if GiResourceVariant = 1 then
        Animation.SetPosition(Classes.Point(72, 34))
      else
        Animation.SetPosition(Classes.Point(90, 43));
      if GiResourceVariant = 1 then
        Animation.SetSize(Classes.Point(77, 86))
      else
        Animation.SetSize(Classes.Point(99, 110));
  ...
  if (GetPlayer = Ranger)
      or Ranger.IsInPrison
      or (Ranger.PartnerShip <> nil)
      or (Ranger.CountWingmen > 0) then
  begin
    Image := TImageGI.Create(Panel);
    if GetPlayer = Ranger then
      Image.SetImagePath('GI,Bm.FormRating2.' + GiResourceSuffix + 'Player')
    else if Ranger.IsInPrison then
      Image.SetImagePath('GI,Bm.FormRating2.' + GiResourceSuffix + 'Prison')
    else
      Image.SetImagePath('GI,Bm.FormRating2.' + GiResourceSuffix + 'Duty');
    Image.SetSize(Image.GetContentSize);
  ...
  Caption := TLabelGI.Create(Panel);
  if GiResourceVariant = 1 then
    Caption.SetPosition(Classes.Point(GiScalePixels(215), GiScalePixels(16) + 1))
  else
    Caption.SetPosition(Classes.Point(GiScalePixels(215), GiScalePixels(16)));
  ...
  Caption := TLabelGI.Create(Panel);
  if GiResourceVariant = 1 then
    Caption.SetPosition(Classes.Point(GiScalePixels(73), GiScalePixels(16) + 1))
  else
    Caption.SetPosition(Classes.Point(GiScalePixels(73), GiScalePixels(16)));
  ...
  Caption := TLabelGI.Create(Panel);
  if GiResourceVariant = 1 then
    Caption.SetPosition(Classes.Point(GiScalePixels(24), GiScalePixels(16) + 1))
  else
    Caption.SetPosition(Classes.Point(GiScalePixels(24), GiScalePixels(16)));
  ...
  Caption := TLabelGI.Create(Panel);
  if GiResourceVariant = 1 then
    Caption.SetPosition(Classes.Point(GiScalePixels(555), GiScalePixels(16) + 1))
  else
    Caption.SetPosition(Classes.Point(GiScalePixels(555), GiScalePixels(16)));
  ...
  TablePanel.VerticalScrollBar.SetSmallChange(
      Min(Rows[Index].Height, TablePanel.VerticalScrollBar.SmallChange)
  );
  if SelectedIndex = Index then
    SelectedRowRect := Classes.Rect(0, Rows[Index].Top, 1, Rows[Index].Top + Rows[Index].Height);
end;
...
begin
  ...
  if RangerRatingScreen.Run = 1 then
    Result := True
  else
    Result := False;
...
var
  ...
  Text: WideString;
  Ranger: TRanger;
  function FormatDominatorKillsHintColumn(
      Column: Integer
  ): WideString; // @addr 0x56C708 @ida "void __usercall $name(int Column@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x56c93f,0x56c99e,0x56ca03,0x56ca20" @note "Nested in TfRating2.ShowDominatorKillsHint; caller supplies its parent frame."
  begin
...
begin
  ...
  begin
    ...
    begin
      Text :=
          '<td='
              + FormatDominatorKillsHintColumn(1)
              + '><align=left>'
