...
uses
  ...
  fPanelMain;
type
  TPlayerHoldUnit = class;
  TfShip2 = class;
  {$Z4}
  TPlayerHoldKind = (phkEmpty = 0, phkGoods = 1, phkEquipment = 2, phkArtefact = 3);
  {$Z4}
  TPlayerHoldSort = (phsType = 0, phsSize = 1, phsPrice = 2);
  ...
  TfShip2 = class(TMessageLoopGIWithMainPanel)
    ...
    ItemDescriptionLabel: TLabelGI;
    ItemSizeLabel: TLabelGI;
    ItemPriceLabel: TLabelGI;
    ItemRaceImage: TImageGI;
    RightOpenImage: TImageGI;
    SkillsPanel: TPanelGI;
    FreeSkillPointsLabel: TLabelGI;
    ExperienceLabel: TLabelGI;
    SkillImages: array[0..5] of TImageGI;
    SkillImagesP: array[0..5] of TImageGI;
    SkillImagesN: array[0..5] of TImageGI;
    SkillPanels: array[0..5] of TPanelGI;
    SkillImageRestTop: array[0..5] of Integer;
    SkillButtons: array[0..5] of TGraphButtonGI;
    SkillValueLabels: array[0..5] of TLabelGI;
    SkillProgressImages: array[0..5] of TImageGI;
    SkillGainImages: array[0..5] of TImageGI;
    HoldSlotZones: array[0..5] of TZoneGI;
    EquipmentSlotZones: array[0..7] of array[0..4] of TZoneGI;
    EquipmentSlotAnimations: array[0..7] of array[0..4] of TgaiGI;
    ArtefactSlotZones: array of TZoneGI;
    BackgroundBuffer: TGraphBufGI;
    ExitButton: TGraphButtonGI;
    RewardsBuffer: TGraphBufGI;
    RewardsWindow: TWindowGI;
    ...
    RightPanelSlideStep: PtrInt;
    PanelSlideWidth: Integer;
    RightPanelRestLeft: Integer;
    DestrPanelSlideWidth: Integer;
    DestrPanelRestLeft: Integer;
    NativeTimer384: PCallbackTimerGI;
    ItemInfoHideTimer: PCallbackTimerGI;
    PropertyInfoHideTimer: PCallbackTimerGI;
    ShipImageCenter: TPoint;
    ItemImageCenter: TPoint;
    ...
    GateSlideTimer: PCallbackTimerGI;
    GateSlideOffset: PtrInt;
    GateLeftRestLeft: Integer;
    GateRightRestLeft: Integer;
    UseSlideTimer: PCallbackTimerGI;
    UseSlideOffset: PtrInt;
    UseLeftRestLeft: Integer;
    UseRightRestLeft: Integer;
    UsePanelSlideTimer: PCallbackTimerGI;
    UsePanelSlideOffset: PtrInt;
    Gap3FC: array[0..7] of Byte;
    SpecialSlot1Timer: PCallbackTimerGI;
    ...
    StorageSlideOffset: PtrInt;
    StoragePanelSlideHeight: Integer;
    StoragePanelRestTop: Integer;
    StorageFirstSlot: Integer;
    PropertyHintRightEdge: Integer;
    ...
    function GetActionParentLoop: TMessageLoopGI; override;
    procedure ExecuteUiCode(Block: TBlockParEC; Key: Cardinal); override;
    constructor Create;
    destructor Destroy; override;
    function CanUseLocalStorage: Boolean;
    function GetLocalStorageOwner: TObject;
    procedure RefreshRewards(Ship: TNormalShip);
    procedure RewardsMouseMove(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    ...
    procedure TrainSkillClicked(Sender: TObjectGI);
    procedure EquipmentSlotMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure ArtefactSlotMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure UseOnArtefactSlot(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure RemoteHoldItemMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure TakeHoldEntry(Entry: TPlayerHoldUnit; Slot: Integer; UsesDisplayOrder: Boolean);
    procedure HullMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    function ConfigureChameleon: Boolean;
    procedure HoldLeftPressed(Sender: TObjectGI);
    ...
    procedure EquipmentConfigurationClicked(Sender: TObjectGI);
    procedure MainKeyDown(Sender: TObjectGI; Key: Cardinal);
    procedure MainKeyUp(Sender: TObjectGI; Key: Cardinal);
    procedure MainLeftButtonUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure MainRightButtonDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    function TryDeployTranclucator(Ship: TShip): Boolean;
    procedure DropSelectedOutside(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    ...
    procedure RepairAllClicked(Sender: TObjectGI);
    procedure RepairAllMouseEnter(Sender: TObjectGI);
    procedure RepairAllMouseLeave(Sender: TObjectGI);
    ...
    procedure LayoutItemInfo(
        ...
        Title: TLabelGI;
        Text: TLabelGI;
        KeepMinimumHeight: Boolean;
        WordWrap: Boolean;
        MinimumWidth: Integer
    );
    procedure LayoutObjectInfo(
        Window: TWindowGI;
        Title: TLabelGI;
        Left1: TLabelGI;
        Right1: TLabelGI;
    ...
    procedure ShowEquipmentInfo(Item: TItem; FromStorage: Boolean);
    procedure ShowHoldGoodsInfo(Good: Byte);
    ...
    function CreateShipInfoImage(Sender: TLabelGI; Item: PFontObjectEC): TObjectGI;
    procedure BuildAdditionalInfoPanel;
    procedure HideSender(Sender: TObjectGI);
    function IsHoldNormalShip: Boolean;
    function CanUsePlayerExperience: Boolean;
    procedure ToggleRemoteHoldClicked(Sender: TObjectGI);
    procedure RemoteHoldUpPressed(Sender: TObjectGI);
    ...
    procedure SellStorageClicked(Sender: TObjectGI);
    procedure ShipToStorageClicked(Sender: TObjectGI);
    procedure SortStorageByTypeClicked(Sender: TObjectGI);
    procedure SortStorageBySizeClicked(Sender: TObjectGI);
    procedure SortStorageByPriceClicked(Sender: TObjectGI);
    procedure LoadHoldRocketsClicked(Sender: TObjectGI);
    procedure SellHoldClicked(Sender: TObjectGI);
    procedure LoadEquippedRocketsClicked(Sender: TObjectGI);
    procedure SortStorage(Sort: TPlayerHoldSort);
    procedure RefreshLoadEquippedRocketsButton;
    procedure SellAllItems(Origin: Integer);
    procedure EnterBridgeClicked(Sender: TObjectGI);
  end;
var
...
function ComparePlayerHoldEntries(
    Left: TPlayerHoldUnit;
    Right: TPlayerHoldUnit;
    Sort: TPlayerHoldSort
): Integer;
...
begin
  if Rank in [0..7] then
    ...
    RaiseWideMessage(
        'No image for rank in function PirateRankToImage(tr: TPirateRank): WideString;'
    );
end;
...
begin
  if Rank in [0..7] then
    ...
    RaiseWideMessage(
        'No image for rank in function PirateRankToImageSmall(tr: TPirateRank): WideString;'
    );
end;
...
var
  Ship: TShip;
  Goods: Byte;
  Entry, Other: TPlayerHoldUnit;
  I, J, Order: Integer;
  Item: TEquipment;
begin
  if PlayerHoldEntries = nil then
    InitializePlayerHoldView;
  if PlayerHoldShip <> nil then
    Ship := PlayerHoldShip
  else
    Ship := GetPlayer;
  ...
  begin
    ...
    begin
      ...
      if (Other.Kind <> phkEmpty) and (Other.DisplayOrder = Entry.DisplayOrder) then
        Break;
      Inc(J);
    end;
    if J < I then
      ...
      begin
        ...
        begin
          ...
          if Other.Kind <> phkEmpty then
            if Other.DisplayOrder = Order then
              Break;
          Inc(J);
        end;
        if J >= PlayerHoldEntries.Count then
          Break;
        Inc(Order);
      end;
...
begin
  if (Index < 0) or (Index >= PlayerHoldEntries.Count) then
    Exit;
...
var
  ...
  LeftType, RightType: Byte;
  LeftName, RightName: WideString;
  NameComparison: Integer;
begin
  Result := 0;
  if Sort = phsType then
  begin
    if Integer(Left.Kind) < Integer(Right.Kind) then
    begin
      Result := -1;
      Exit
    end;
    ...
    if (Left.Kind in [phkEquipment, phkArtefact])
        and (Right.Kind in [phkEquipment, phkArtefact])
        and (Left.Item <> nil)
        and (Right.Item <> nil) then
    begin
...
begin
  for I := PlayerHoldEntries.Count - 1 downto 0 do
    RemoveEmptyPlayerHoldSlot(I);
  for I := 0 to PlayerHoldEntries.Count - 2 do
    for J := I + 1 to PlayerHoldEntries.Count - 1 do
      if TPlayerHoldUnit(PlayerHoldEntries[I]).DisplayOrder
          > TPlayerHoldUnit(PlayerHoldEntries[J]).DisplayOrder then
      begin
        Entry := PlayerHoldEntries[I];
        PlayerHoldEntries[I] := PlayerHoldEntries[J];
        PlayerHoldEntries[J] := Entry;
...
begin
  ...
  begin
    ...
    begin
      Entry := PlayerHoldEntries[I];
      if Entry.Kind <> phkEmpty then
        if Entry.DisplayOrder = Order then
          Break;
      Inc(I);
    end;
    if I >= PlayerHoldEntries.Count then
...
begin
  with GetByName('InfoImage') do
    Self.ItemImageCenter := AddPoints(LocalPosition, HalfPoint(ClientSize));
  with GetByName('InfoSize') do
    Self.ItemSizeLabelPosition :=
        Classes.Point(LocalPosition.X, LocalPosition.Y - Parent.ClientSize.Y);
  with GetByName('InfoPrice') do
    Self.ItemPriceLabelPosition :=
        Classes.Point(LocalPosition.X, LocalPosition.Y - Parent.ClientSize.Y);
  with GetByName('EmRace') do
  ...
  begin
    ...
    with FindByNameRecursive('ADD_WarningMoney') do
      SetPosition(
    ...
    with FindByNameRecursive('UsePanel').Parent do
      SetPosition(
          Classes.Point(
    ...
    with FindByNameRecursive('SC_Panel') do
      SetPosition(Classes.Point(LocalPosition.X, (GameScreenHeight - ClientSize.Y) div 2 - 50));
    ...
    with FindByNameRecursive('CustomBridgeInto') as TGraphButtonGI do
      SetPosition(
          Classes.Point(
              LocalPosition.X + ExtraScreenWidth div 2,
              LocalPosition.Y + ExtraScreenHeight div 2
  ...
  for I := 0 to 5 do
    HoldSlotZones[I] := GetByName('S_' + IntToStr(I) + 'z') as TZoneGI;
  for I := 0 to 5 do
  begin
  ...
  with GetByName('Ship3D') do
    Self.ShipImageCenter := AddPoints(LocalPosition, HalfPoint(ClientSize));
  ...
  with GetByName('SortTypeRH') as TGraphButtonGI do
  begin
    UserValue := 0;
    UpCallback := SortRemoteHoldClicked
  end;
  with GetByName('SortSizeRH') as TGraphButtonGI do
  begin
    UserValue := 1;
    UpCallback := SortRemoteHoldClicked
  end;
  with GetByName('SortMoneyRH') as TGraphButtonGI do
  begin
    UserValue := 2;
    UpCallback := SortRemoteHoldClicked
  end;
...
begin
  ...
  begin
    ...
    if GetPlayer <> PlayerHoldShip then
      GetPlayer.ScriptItemsAct(satOnReEnteringOtherShip, nil, nil, 0);
  end
  else
  begin
  ...
  begin
    ...
    if (Item is TWeapon) and ((Item as TWeapon).Target <> nil) then
      if (Item as TWeapon).EquippedFlag = 0 then
        (Item as TWeapon).Target := nil
      else if (not ((Item as TWeapon).Target is TShip)
              or (Galaxy.IdToShip(((Item as TWeapon).Target as TShip).Id, False) = nil))
          and (not ((Item as TWeapon).Target is TItem)
              or (Galaxy.IdToItem(((Item as TWeapon).Target as TItem).Id, False) = nil))
          and (not ((Item as TWeapon).Target is TAsteroid)
              or (Galaxy.IdToAsteroid(((Item as TWeapon).Target as TAsteroid).Id) = nil))
          and (not ((Item as TWeapon).Target is TMissile)
  ...
  begin
    if PlayerHoldShip is TRuins then
      SetText(TRuins(PlayerHoldShip).GetColoredFullName(''))
    else
      SetText(PlayerHoldShip.GetFullName(#13#10));
    if (PlayerHoldShip is TTranclucator)
        and ((PlayerHoldShip as TTranclucator).OwnerShip = GetPlayer) then
  ...
  begin
    SetActive(True);
    if (PlayerHoldShip is TKling)
        or (PlayerHoldShip is TRuins)
        or (PlayerHoldShip is TTranclucator) then
    begin
  ...
  with GetByName('S_Hull_0z') as TZoneGI do
    ZoneMouseUpCallback := HullMouseDown;
  ...
  begin
    ...
    SetImageKindX(ikxCenter);
    SetImageKindY(ikyCenter);
    if (SavedCaptainFrame >= 0) and (SequenceFrameCount > SavedCaptainFrame) then
      SetSequenceFrame(SavedCaptainFrame)
    else
      SetSequenceFrame(0);
    SetActive(True);
    RestartPlayback;
  ...
  with GetByName('LNewExp1') as TLabelGI do
    if CanUsePlayerExperience then
      SetText(LocalizedColorText('FormShip.LNewExp1b'))
    else
      SetText(LocalizedColorText('FormShip.LNewExp1'));
  with GetByName('LNewExp2') as TLabelGI do
    if CanUsePlayerExperience then
      SetText(LocalizedColorText('FormShip.LNewExp2b'))
    else
      SetText(LocalizedColorText('FormShip.LNewExp2'));
  aCalc.WaitForTurnCalculationUI;
  RefreshShipView;
  InvalidateViewport;
  ...
  begin
    ...
    SetActive(
        not RemoteHoldMode and (GetPlayer = PlayerHoldShip) and (GetPlayer.GetHull.CapitalShip = 1)
    );
  end;
  with GetByName('DestrInto') do
    SetActive(
        (GetPlayer = PlayerHoldShip)
  ...
  begin
    if StorageUpButton.Active then
      ...
      StorageUpClicked(nil);
  end
  else
  begin
    GateSlideOffset := 0;
    with GetByName('GateLeft') do
      SetPosition(Classes.Point(GateLeftRestLeft, LocalPosition.Y));
    with GetByName('GateRight') do
      SetPosition(Classes.Point(GateRightRestLeft, LocalPosition.Y));
    ...
    UseSlideOffset := 0;
    with GetByName('UseLeft') do
      SetPosition(Classes.Point(UseLeftRestLeft, LocalPosition.Y));
    with GetByName('UseRight') do
      SetPosition(Classes.Point(UseRightRestLeft, LocalPosition.Y));
    ...
    with GetByName('UsePanel') do
      SetPosition(Classes.Point(ClientSize.X, LocalPosition.Y));
    ...
    with GetByName('SC_Storage_Panel') do
      SetPosition(Classes.Point(LocalPosition.X, StoragePanelSlideHeight));
  ...
  with GetByName('HullRepair') as TgaiGI do
    if Active then
    begin
      if not PlayServiceAnimations then
        SetActive(False)
      else
      begin
        SetSequenceFrame(0);
...
begin
  ...
  if PlayerHoldShip.Order = soJump then
    if PlayerHoldShip.JumpRange
        < System.Round(
            PointDistance(
                PlayerHoldShip.CurrentStar.Position,
                TStar(PlayerHoldShip.OrderTarget).Position
            )) then
  ...
  begin
    ...
    while I >= 0 do
    begin
      if I >= GetPlayer.ScriptShipBindings.Count then
        I := GetPlayer.ScriptShipBindings.Count - 1
      else
      begin
...
begin
  if GetPlayer.IsOnPlanet then
    Result := GetPlayer.CurrentPlanet
  else if GetPlayer.IsDockedToShip then
    Result := GetPlayer.DockedTo
  else
    Result := nil;
end;
...
var
  Index, I: Integer;
  Icon, Shadow: TGraphBufGR;
  Award: Byte;
  Path: WideString;
  IconSize: Integer;
  Spacing: Single;
  VisibleCount, Count: Integer;
begin
  HideRewardTooltip;
...
begin
  ...
  begin
    ...
    while Index < Count do
    begin
      if (X >= Round(I * Spacing)) and (X < Round((I + 1) * Spacing)) then
        Break;
      Inc(I);
      Inc(Index);
    end;
...
begin
  ...
  begin
    ...
    with GetByName('RewardName') as TLabelGI do
      SetText(Ship.GetAwardInfo(Award).Name);
    with GetByName('RewardText') as TLabelGI do
      SetText(Ship.GetAwardInfo(Award).Text);
    LayoutItemInfo(
        RewardsWindow,
        GetByName('RewardName') as TLabelGI,
        GetByName('RewardText') as TLabelGI,
        True,
        True,
        0
    );
    with GetByName('RewardName') as TLabelGI do
      SetSize(
          Classes.Point(
              Self.RewardsWindow.ClientSize.X
                  - LocalPosition.X
                  - Self.RewardsWindow.WorkSubRect.Right,
              ClientSize.Y
          )
      );
...
begin
  if not RemoteHoldMode then
    Panel := GetByName('PanelRight')
  else
    Panel := GetByName('PanelRH');
  ...
  DestrPanel.SetPosition(
      Classes.Point(DestrPanelRestLeft - RightPanelRestLeft + X, DestrPanel.LocalPosition.Y)
  );
...
begin
  ...
  begin
    ...
    if ShowTextInputDialog(Self, LocalizedColorText('FormShip.EnterShipName'), Name, 30, 0, 0)
        = 1 then
    begin
      aCalc.WaitForTurnCalculationUI;
...
var
  Ship: TNormalShip;
  Path, Title, Description, CustomDescription, CustomName: WideString;
  Skill: TPilotSkill;
  Window: TWindowGI;
  Afterburner: Boolean;
  Position: TPoint;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LoadGiByPathIntoGraphBuf(
            'Bm.FormShip2.' + GiResourceSuffix + 'AI_' + IntToStr(Cardinal(Sender.UserIndex)) + 'L',
            GraphBuf
        );
        SetImageKindX(ikxCenter);
        SetImageKindY(ikyCenter);
      ...
      Description :=
          ExtractDelimitedRangeW(
              Sender.HelpText,
              1,
              CountDelimitedPartsW(Sender.HelpText, '~') - 1,
              '~'
          );
    end
            ...
            Description :=
                Description
                    ...
                    + FormatText2(
                        ...
                        '<WarPoints>',
                        IntToStr(Ship.GetRankPointsToNextRank))
          else
            Description :=
                Description
                    + ' '
      ...
      begin
        ...
        LoadGiByPathIntoGraphBuf(
            'Bm.FormShip2.'
                + GiResourceSuffix
                + 'Skill'
                + IntToStr(ExtractDigitsToIntW(Sender.ControlName) + 1),
            GraphBuf
        );
        SetImageKindX(ikxCenter);
        SetImageKindY(ikyCenter);
    ...
    LayoutItemInfo(
        Window,
        GetByName('RankName') as TLabelGI,
        GetByName('RankText') as TLabelGI,
        True,
        True,
        0
    );
    with GetByName('RankName') as TLabelGI do
      SetSize(
          Classes
              .Point(Window.ClientSize.X - LocalPosition.X - Window.WorkSubRect.Right, ClientSize.Y)
      );
    if not Afterburner then
      Window.SetPosition(
          Classes.Point(PropertyHintRightEdge - Window.ClientSize.X, Window.LocalPosition.Y)
      )
    else
...
var
  Count, MaximumSlots, I, Slot: Integer;
begin
  for I := 0 to 7 do
  begin
    MaximumSlots := 1;
    ...
    begin
      ...
      EquipmentSlotAnimations[I, Slot] :=
          GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'anim') as TgaiGI;
      EquipmentSlotAnimations[I, Slot].UserState := 0;
      GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'off')
          .SetActive(False);
    ...
    begin
      ...
      GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'anim')
          .SetActive(False);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if PlayerHoldShip.HasActiveDisease then
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'LifeRed')
      else if PlayerHoldShip.CountPresentDiseases > 0 then
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'LifeYellow')
      else
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'LifeGreen');
    end
    else
      SetActive(False);
  end;
  ...
  (GetByName('IEmpty') as TLabelGI)
      .SetText(WrapTextInColor(IntToStr(PlayerHoldShip.GetCargoFreeSpace), Text));
  (GetByName('S_Left') as TGraphButtonGI).SetDisabled(HoldFirstIndex <= 0);
  (GetByName('S_Right') as TGraphButtonGI)
      .SetDisabled(HoldFirstIndex + 6 > PlayerHoldEntries.Count);
  (GetByName('UpRH') as TGraphButtonGI).SetDisabled(RemoteHoldFirstOrder <= 0);
  (GetByName('DownRH') as TGraphButtonGI)
      .SetDisabled(RemoteHoldFirstOrder >= GetRemoteHoldScrollLimit);
  RefreshActionPanels(
      SelectedHoldKind,
      SelectedGoodsIndex,
  ...
  begin
    ...
    if Active then
      SetImagePath(
          'GI,'
              + GetMicroModuleBitmapResourceName(PlayerHoldShip.GetHull.MicroModuleIndex - 1)
              + 'Set'
      );
  end;
  for I := 0 to 7 do
  begin
    ...
    begin
      ...
      begin
        ...
        with GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'Set')
            as TImageGI do
        begin
          SetActive(Item.HasMicroModule);
          if Active then
            SetImagePath(
                'GI,' + GetMicroModuleBitmapResourceName(Item.MicroModuleIndex - 1) + 'Set'
            );
        end;
      end;
      ...
      begin
        ...
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'anim')
            .SetActive(False);
      with GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'z') as TZoneGI do
        ZoneMouseDownCallback := EquipmentSlotMouseDown;
      if HighlightRepairableEquipment
          and (Item <> nil)
          and ((Item.EquippedFlag <> 0) or (Item is THull))
          and Item.NeedsRepair then
        Highlight := True
      else if (SelectedHoldKind = phkEquipment)
          and (SelectedHoldItem.ItemType = t_MicroModule)
          and (Item <> nil)
          and (Item.MicroModuleIndex = 0)
          and (SelectedHoldItem as TMicroModule).CanInstallOn(Item) then
        Highlight := True
      else if (SelectedHoldKind in [phkEquipment, phkArtefact])
          and (SelectedHoldItem is TEquipmentWithActCode)
          and (Item <> nil)
          and (RunItemConfigActionCode(
                  SelectedHoldItem,
                  satOnCheckingUsability,
                  PlayerHoldShip,
                  Item,
                  nil,
                  0)
        ...
        Highlight := True
          ...
          and (SelectedHoldItem.ScriptItem <> nil)
          and (Item <> nil)
          and (TScriptItem(SelectedHoldItem.ScriptItem)
                  .RunActionCode(satOnCheckingUsability, PlayerHoldShip, Item, nil, 0)
              > 0) then
        Highlight := True
      else if (SelectedHoldKind in [phkEquipment, phkArtefact])
          and (Item <> nil)
          and (Item is TEquipmentWithActCode)
          and (RunItemConfigActionCode(
                  Item,
                  satOnCheckingUsability2,
                  PlayerHoldShip,
                  SelectedHoldItem,
                  nil,
                  0)
        ...
        Highlight := True
          ...
          and (Item <> nil)
          and (Item.ScriptItem <> nil)
          and (TScriptItem(Item.ScriptItem)
                  .RunActionCode(satOnCheckingUsability2, PlayerHoldShip, SelectedHoldItem, nil, 0)
              > 0) then
        Highlight := True
          ...
          and (TScriptItem(Item.ScriptItem)
                  .RunActionCode(
                      satOnCheckingUsabilityGoods,
                      PlayerHoldShip,
                      TObject(SelectedGoodsIndex),
                      TObject(SelectedGoodsQuantity),
                      0)
              > 0) then
        Highlight := True
      else if (SelectedHoldKind = phkEquipment)
          ...
          and (Item <> nil)
          and (Item.ItemType = t_FuelTanks)
          and ((SelectedHoldItem as TCistern).Fuel > 0)
          and ((Item as TFuelTanks).Fuel < (Item as TFuelTanks).Capacity) then
        Highlight := True
      else
        Highlight :=
            (SelectedHoldKind = phkEquipment)
                and IsCompatibleSlot(SelectedHoldItem.ItemType, EquipmentSlotLayouts[I].ItemType);
      Boost :=
      ...
      GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'a')
          .SetActive(Highlight and not Boost);
      GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'n')
          .SetActive(
              (Item <> nil)
                  and not Highlight
                  and PlayerHoldShip.IsEquipmentUsable(Item)
                  and not Boost);
      GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(Slot) + 'b')
          .SetActive(
              (Item <> nil)
                  and not Highlight
                  and not PlayerHoldShip.IsEquipmentUsable(Item)
                  and not Boost);
  ...
  begin
    ...
    if (DuplicateSlot >= 0) and (Slot <> DuplicateSlot) then
      Highlight := False;
    if not Highlight
        and (Artefact <> nil)
      ...
      Boost := False;
    if Highlight and (Artefact <> nil) and (Artefact.NoDropFlag > 0) then
      Highlight := False;
    GetByName('Art' + IntToStr(Slot) + 'n')
        .SetActive((Artefact <> nil) and not Highlight and (Artefact.BrokenFlag = 0) and not Boost);
    GetByName('Art' + IntToStr(Slot) + 'b')
        .SetActive(
            (Artefact <> nil) and not Highlight and (Artefact.BrokenFlag <> 0) and not Boost);
  ...
  for I := 0 to 5 do
  begin
    if HoldFirstIndex + I >= PlayerHoldEntries.Count then
      Entry := nil
    else
      Entry := TPlayerHoldUnit(PlayerHoldEntries[HoldFirstIndex + I]);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SetImageKindX(ikxCenter);
          SetImageKindY(ikyCenter);
          if Entry.Item = ScriptUseItem then
            SetImagePath('');
        end
  ...
  if (HighlightRepairableEquipment and PlayerHoldShip.GetHull.NeedsRepair) then
    GetByName('HullA').SetActive(True)
  else if ((SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem.ItemType = t_MicroModule)
      and (PlayerHoldShip.GetHull.MicroModuleIndex = 0)
      and (SelectedHoldItem as TMicroModule).CanInstallOn(PlayerHoldShip.GetHull)) then
    GetByName('HullA').SetActive(True)
  else if ((SelectedHoldKind in [phkEquipment, phkArtefact])
      and (SelectedHoldItem is TEquipmentWithActCode)
      and (RunItemConfigActionCode(
    ...
    GetByName('HullA').SetActive(True)
      ...
      and (SelectedHoldItem.ScriptItem <> nil)
      and (TScriptItem(SelectedHoldItem.ScriptItem)
              .RunActionCode(satOnCheckingUsability, PlayerHoldShip, PlayerHoldShip.GetHull, nil, 0)
          > 0)) then
    GetByName('HullA').SetActive(True)
  else if ((SelectedHoldKind in [phkEquipment, phkArtefact])
      and (PlayerHoldShip.GetHull.ScriptItem <> nil)
      and (TScriptItem(PlayerHoldShip.GetHull.ScriptItem)
              .RunActionCode(satOnCheckingUsability2, PlayerHoldShip, SelectedHoldItem, nil, 0)
          > 0)) then
    GetByName('HullA').SetActive(True)
  else if ((SelectedHoldKind = phkGoods)
    ...
    GetByName('HullA')
        .SetActive(
            (SelectedHoldKind = phkEquipment)
                and (SelectedHoldItem.ItemType = t_Hull)
                and IsHoldNormalShip
                and (TItem(PlayerHoldShip.Inventory[0]).NoDropFlag = 0));
  if (SelectedHoldKind = phkArtefact)
    ...
    GetByName('HullEx').SetActive(False);
  GetByName('Forsage').SetActive(PlayerHoldShip.GetSlotCount(sskAfterburner) > 0);
  GetByName('ForsageLight')
      .SetActive(
          PlayerHoldShip.AfterburnerActive
  ...
  begin
    ...
    SetDisabled(
        not PlayerHoldShip.IsEquipmentUsable(PlayerHoldShip.GetEngine)
            or not PlayerHoldShip.InNormalSpace
    );
  end;
  with GetByName('IDestrEnergy') as TLabelGI do
    SetText(
        FormatText2(
            ...
            '<Value2>',
            IntToStr(PlayerHoldShip.GetHull.EnergyMax)
        )
    );
  with GetByName('IDestrShields') as TLabelGI do
  begin
    if PlayerHoldShip.GetHull.ImpulseShieldsEnabled then
      SetText(LocalizedText('FormShip.DestrIShield'))
    else
      SetText(LocalizedText('FormShip.DestrNShield'));
  end;
  with GetByName('IDestrCount') as TLabelGI do
    SetText(
        FormatText2(
            ...
            '<Cost>',
            IntToStr(PlayerHoldShip.GetInterceptorEnergyCost)
        )
    );
  with GetByName('IDestrEnergyOut') as TLabelGI do
    SetText(
        '-'
            + FormatText1(
  ...
  with GetByName('IDestrEnergyIn') as TLabelGI do
    SetText(
        '+'
            + FormatText1(
                LocalizedText('FormShip.DestrPerDay'),
...
procedure TfShip2.RefreshActionPanels(
    Kind: TPlayerHoldKind;
    Good: Byte;
...
var
  I, TotalRepair: Integer;
  Equipment: TEquipment;
  OrdinaryShip: Boolean;
  Unused60, Unused64: Integer; { Two native unused stack slots before managed temporaries. }
  procedure Skill(
      Index, BaseLevel, EffectiveLevel: Integer;
      CanTrain: Boolean
  ); // @addr $6FB6F0 @ida "void __userpurge $name(int Index@<eax>, int BaseLevel@<edx>, int EffectiveLevel@<ecx>, bool CanTrain@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x6FBC2A,0x6FBC66,0x6FBCA2,0x6FBCE1,0x6FBD20,0x6FBD5F" Nested in RefreshActionPanels; captures Self.
  var
    Step, Gap, Height: Integer;
  begin
    Gap := 2;
    Step := Gap + 5;
    Height := 43;
    ...
    begin
      ...
      SetPosition(
          Classes.Point(LocalPosition.X, Self.SkillImageRestTop[Index] + Height - ClientSize.Y)
      );
      SetImageKindY(ikyBottom);
    end;
    ...
    begin
      ...
      begin
        ...
        SetPosition(
            Classes.Point(
                LocalPosition.X,
                Self.SkillImageRestTop[Index] + Height - Step * EffectiveLevel
            )
        );
      end;
      ...
      SkillImagesP[Index].SetActive(False);
    if BaseLevel > EffectiveLevel then
    begin
      with SkillPanels[Index] do
      begin
        ...
        SetPosition(
            Classes
                .Point(LocalPosition.X, Self.SkillImageRestTop[Index] + Height - Step * BaseLevel)
        );
      end;
      ...
      SkillImagesN[Index].SetActive(False);
    with SkillValueLabels[Index] do
      SetText(IntToStr(EffectiveLevel));
    with SkillProgressImages[Index] do
    begin
      if EffectiveLevel > BaseLevel then
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'DSGreen')
      else if EffectiveLevel < BaseLevel then
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'DSRed')
      else
        SetImagePath('GI,Bm.FormShip2.' + GiResourceSuffix + 'DSBlue');
    end;
    with SkillGainImages[Index] do
      SetActive(CanTrain);
  end;
begin
  OrdinaryShip := not (PlayerHoldShip is TRuins) and not (PlayerHoldShip is TTranclucator);
  with FreeSkillPointsLabel do
    SetText(IntToStr(PlayerHoldShip.FreeExperience));
  with ExperienceLabel do
    SetText(IntToStr(PlayerHoldShip.FreeExperience));
  Skill(
      0,
  ...
  begin
    ...
    with GetByName('SC_Slot1_Text') as TLabelGI do
      SetText(
          FormatText1(
              LocalizedText('FormShip.Repair'),
              '',
              '<Money>',
              IntToStr((Item as TEquipment).CalculateRepairCost)
          )
      );
  end
  else
    CloseSpecialSlot1;
  if ((Kind = phkGoods) or (Kind = phkEquipment) or (Kind = phkArtefact))
      and ((Kind = phkGoods)
          or (Item = nil)
          or ((Item.ScriptItem = nil) and (Item.NoDropFlag = 0))
          or ((Item.ScriptItem <> nil) and TScriptItem(Item.ScriptItem).CanSell))
      and ((Kind = phkGoods) or (Kind = phkArtefact) or not (Item is THull) or (Origin <> 0))
      and not PreserveSpaceMusic
      and (GetPlayer.IsDockedToShip
          or (GetPlayer.IsOnPlanet
              and not (GetPlayer.CurrentPlanet.OwnerId
  ...
  begin
    ...
    begin
      if Kind = phkGoods then
        SetText(
            FormatText1(
                LocalizedText('FormShip.Sell'),
                '',
                '<Money>',
                IntToStr(Quantity * GetPlayer.ShopGoodsSellPrice(Good, nil))
            )
        )
      else
        SetText(
            FormatText1(
                ...
                '',
                '<Money>',
                IntToStr(Item.CalculateResaleValue(GetPlayer.GetEffectiveSkillLevel(psTrading)))
            )
        );
    end;
  end
  ...
  if (Kind = phkEquipment)
      ...
      and (TWeapon(Item).Ammo < TWeapon(Item).AmmoCapacity)
      and not PreserveSpaceMusic
      and (GetPlayer.IsDockedToShip
          or (GetPlayer.IsOnPlanet
              and not (GetPlayer.CurrentPlanet.OwnerId
  ...
  begin
    ...
    with GetByName('SC_Slot3_Text') as TLabelGI do
      SetText(
          FormatText1(
              LocalizedText('FormShip.Missile'),
              '',
              '<Money>',
              IntToStr(
                  (TWeapon(Item).AmmoCapacity - TWeapon(Item).Ammo)
                      * Galaxy.ScaleIntByTechLevel(10, 100)
              )
          )
      );
  end
  else if (Kind = phkEquipment)
      and (Item is TCistern)
      and ((Item as TCistern).Fuel < (Item as TCistern).Capacity)
      and not PreserveSpaceMusic
      and (GetPlayer.IsDockedToShip
          or (GetPlayer.IsOnPlanet
              and not (GetPlayer.CurrentPlanet.OwnerId
  ...
  begin
    ...
    begin
      ...
      and (Item is TFuelTanks)
      and ((Item as TFuelTanks).Fuel < (Item as TFuelTanks).Capacity)
      and not PreserveSpaceMusic
      and (GetPlayer.IsDockedToShip
          or (GetPlayer.IsOnPlanet
              and not (GetPlayer.CurrentPlanet.OwnerId
  ...
  begin
    ...
    if (not (Equipment is TWeapon)
        ...
        or GetPlayer.CanRepairArtefactsAtLocation) then
      if PlayerHoldShip.CanRepairEquipmentTech(Equipment)
          and (Equipment <> Item)
          and ((Equipment.EquippedFlag <> 0) or (Equipment is THull))
          and Equipment.NeedsRepair then
        Inc(TotalRepair, Equipment.CalculateRepairCost);
  end;
  with GetByName('FullRepareSum') as TLabelGI do
  begin
    if TotalRepair > 0 then
      SetText(
          FormatText1(LocalizedText('FormShip.RepairAll'), '', '<Money>', IntToStr(TotalRepair))
      )
    else
      SetText(LocalizedText('FormShip.FullRepareNo'));
  end;
...
begin
  ...
  begin
    ...
    SetCursorImage(
        'GI,' + GetShopItemIconName(TItem(SelectedHoldItem)) + 's',
        Classes.Point(16, 16)
    )
...
begin
  ...
  aCalc.WaitForTurnCalculationUI;
  if GetPlayer = PlayerHoldShip then
    GetPlayer.AchievementStats.CheckAllSkillsAchievement;
...
begin
  ...
  if (SelectedHoldKind = phkEmpty)
      and (PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot) = nil) then
    Exit;
  if (GetPlayer.InHyperspace or (QueuedArcadeBattles.Count > 0))
      and not Galaxy.IsArcadeEquipmentChangeEnabled then
  begin
  ...
  begin
    if (SelectedHoldKind = phkEquipment)
        ...
        and (PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot).MicroModuleIndex = 0)
        and (SelectedHoldItem as TMicroModule)
            .CanInstallOn(PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot)) then
    begin
      Item := PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot);
      Text :=
          FormatText2(
    ...
    begin
      ...
      begin
        Quantity :=
            Min(
                (SelectedHoldItem as TCistern).Fuel,
                (Item as TFuelTanks).Capacity - (Item as TFuelTanks).Fuel
            );
    ...
    begin
      ...
      (SelectedHoldItem as TEquipment).AssignedSlotData :=
          ((SelectedHoldItem as TEquipment).AssignedSlotData and EquipmentSecondaryFireFlag)
              or Slot;
      ...
      if (PlayerHoldShip is TTranclucator)
        ...
        ShowMessageBoxGI(
            Self,
            LocalizedColorText('FormShip.TrancMissileWarning'),
            mbgOK or mbgWarning
        );
      SelectedHoldKind := phkEmpty;
      SelectedHoldItem := nil;
    ...
    else if (SelectedHoldKind in [phkGoods])
        and (PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot) <> nil) then
    begin
      Item := PlayerHoldShip.FindEquippedItemInSlot(ItemType, Slot);
      ActionResult := 0;
    ...
    begin
      ...
      if SelectedHoldItem.ScriptItem <> nil then
        ActionResult :=
            TScriptItem(SelectedHoldItem.ScriptItem)
                .RunActionCode(satOnAnotherItem, PlayerHoldShip, Item, nil, ActionResult);
...
var
  ...
  begin
    aCalc.WaitForTurnCalculationUI;
    if PlayerHoldShip.InHyperspace
        or (QueuedArcadeBattles.Count > 0)
        or (RemoteHoldVisible and NeedsRefresh) then
      RefreshShipView
    else if not RemoteHoldVisible then
    begin
      Flag3BC := True;
...
begin
  ...
  begin
    ...
    if (Index < 0) or (PlayerHoldEntries.Count <= Index) then
      Entry := nil
    else
      Entry := TPlayerHoldUnit(PlayerHoldEntries[Index]);
  end
  else
  begin
    ...
    if J < 0 then
      ...
      Entry := TPlayerHoldUnit(PlayerHoldEntries[J]);
  end;
  if (SelectedHoldKind = phkEmpty) and (Entry = nil) then
    Exit;
  NeedsRefresh := False;
  aCalc.WaitForTurnCalculationUI;
  if SelectedHoldKind = phkEmpty then
  ...
  begin
    ...
    begin
      if Entry.Item.ScriptItem <> nil then
        ActionResult :=
            TScriptItem(Entry.Item.ScriptItem)
                .RunActionCode(
                    satOnAnotherGoods,
                    PlayerHoldShip,
                    TObject(SelectedGoodsIndex),
    ...
    Inc(
        PlayerHoldShip.CargoGoods[SelectedGoodsIndex].TotalCost,
        Round(Count / SelectedGoodsQuantity * SelectedGoodsCost)
    );
    SoundManager.PlaySound('Sound.SlotPut');
    if not Expanded then
    begin
      ...
      if SelectedGoodsQuantity <= 0 then
        SelectedHoldKind := phkEmpty
      else
        ReturnSelectedHoldEntry;
    end
  ...
  begin
    ...
    begin
      Available := Max(0, PlayerHoldShip.CargoFreeSpace);
      DialogCount := Min(Count, Available);
      if (SelectedHoldOrigin = 1)
          and (not Expanded
              or (Entry = nil)
              or TCountableItem(SelectedHoldItem).CanMerge(Entry.Item)) then
      begin
        if Count > 1 then
          if ShowCountDialog(
                  Self,
    ...
    if (TCountableItem(SelectedHoldItem).StackCount <> Count)
        or ((Entry <> nil) and TCountableItem(SelectedHoldItem).CanMerge(Entry.Item)) then
    begin
      if not RemoteHoldVisible
          and (SelectedHoldOrigin = 0)
          and (Entry <> nil)
          and (Entry.Item <> nil)
          and (Entry.Kind in [phkEquipment, phkArtefact])
          and not TCountableItem(SelectedHoldItem).CanMerge(Entry.Item) then
      begin
        ActionResult := 0;
        if SelectedHoldItem.ScriptItem <> nil then
          ActionResult :=
              TScriptItem(SelectedHoldItem.ScriptItem)
                  .RunActionCode(satOnAnotherItem, PlayerHoldShip, Entry.Item, nil, ActionResult);
        if Entry.Item.ScriptItem <> nil then
          ActionResult :=
              TScriptItem(Entry.Item.ScriptItem)
                  .RunActionCode(
      ...
      begin
        ...
        if Swap <> nil then
        begin
          if TCountableItem(SelectedHoldItem).StackCount > 0 then
            RaiseWideMessage('move protoplasm');
          TakeHoldEntry(Swap, Index, Expanded);
        end
        else if SelectedHoldItem <> nil then
          ReturnSelectedHoldEntry;
        RestorePlayerHoldDisplayOrder;
      end;
  ...
  begin
    if not RemoteHoldVisible then
    begin
      if (SelectedHoldItem.ItemType = t_Cistern)
          and (Entry <> nil)
          and (Entry.Kind = phkEquipment)
          and (Entry.Item.ItemType = t_Cistern)
          and ((SelectedHoldItem as TCistern).Fuel > 0)
          and ((Entry.Item as TCistern).Fuel < (Entry.Item as TCistern).Capacity) then
      begin
        ...
        begin
          Count :=
              Min(
                  (SelectedHoldItem as TCistern).Fuel,
                  (Entry.Item as TCistern).Capacity - (Entry.Item as TCistern).Fuel
              );
      ...
      if (SelectedHoldItem.ItemType = t_FuelTanks)
          and (Entry <> nil)
          and (Entry.Kind = phkEquipment)
          and (Entry.Item.ItemType = t_FuelTanks) then
      begin
      ...
      if (SelectedHoldItem.ItemType = t_Cistern)
          and (Entry <> nil)
          and (Entry.Kind = phkEquipment)
          and (Entry.Item.ItemType = t_FuelTanks)
          and ((SelectedHoldItem as TCistern).Fuel > 0)
          and ((Entry.Item as TFuelTanks).Fuel < (Entry.Item as TFuelTanks).Capacity) then
      begin
        Text := LocalizedColorText('Items.Cistern.ToFuelTanks');
        if ShowMessageBoxGI(Self, Text, mbgOK or mbgCancel or mbgQuestion) = mbgResultOK then
        begin
          Count :=
              Min(
                  (SelectedHoldItem as TCistern).Fuel,
                  (Entry.Item as TFuelTanks).Capacity - (Entry.Item as TFuelTanks).Fuel
              );
          Dec((SelectedHoldItem as TCistern).Fuel, Count);
      ...
      if (SelectedHoldItem.ItemType = t_FuelTanks)
          ...
          and (Entry.Kind = phkEquipment)
          and (Entry.Item.ItemType = t_Cistern)
          and ((SelectedHoldItem as TFuelTanks).Fuel > 0)
          and ((Entry.Item as TCistern).Fuel < (Entry.Item as TCistern).Capacity) then
      begin
        ...
        begin
          Count :=
              Min(
                  (SelectedHoldItem as TFuelTanks).Fuel,
                  (Entry.Item as TCistern).Capacity - (Entry.Item as TCistern).Fuel
              );
      ...
      if (SelectedHoldItem.ItemType = t_MicroModule)
          and (Entry <> nil)
          and (Entry.Kind = phkEquipment)
          and (TEquipment(Entry.Item).MicroModuleIndex = 0)
          and (SelectedHoldItem as TMicroModule).CanInstallOn(TEquipment(Entry.Item))
          and not PlayerHoldShip.InHyperspace
          and (QueuedArcadeBattles.Count <= 0) then
      begin
        Text :=
            FormatText2(
                LocalizedText('MicroModuls.AddToItem'),
                ...
                (SelectedHoldItem as TMicroModule).GetPlainName,
                '<ItemName>',
                RemoveTextTagsW(Entry.Item.GetDisplayName)
            );
        if ShowMessageBoxGI(Self, Text, mbgOK or mbgCancel or mbgQuestion) = mbgResultOK then
        begin
    ...
    begin
      if (Entry <> nil) and (Entry.Item <> nil) and (Entry.Item.NoDropFlag > 0) then
        Entry := nil;
      Swap := nil;
      if Entry = nil then
      begin
  ...
  begin
    ...
    if SelectedHoldItem is TArtefactTranclucator then
      (TObject((SelectedHoldItem as TArtefactTranclucator).Ship) as TTranclucator).OwnerShip :=
          GetPlayer;
    SoundManager.PlaySound('Sound.SlotPut');
    if not Expanded then
    begin
...
begin
  if (SelectedHoldKind in [phkEquipment, phkArtefact])
      ...
      and (SelectedHoldItem is THull)
      and (PlayerHoldShip.GetHull.NoDropFlag > 0) then
    Exit;
  aCalc.WaitForTurnCalculationUI;
  if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem.ItemType = t_MicroModule)
  ...
  begin
    Text :=
        FormatText2(
            ...
            (SelectedHoldItem as TMicroModule).GetPlainName,
            '<ItemName>',
            RemoveTextTagsW(PlayerHoldShip.GetHull.GetDisplayName)
        );
    if ShowMessageBoxGI(Self, Text, mbgOK or mbgCancel or mbgQuestion) = mbgResultOK then
    begin
      PlayerHoldShip.ScriptItemsAct(satOnPlayerUseMM, PlayerHoldShip.GetHull, SelectedHoldItem, 0);
      ApplyMicroModule(
          (SelectedHoldItem as TMicroModule).MicroModuleIndex - 1,
          PlayerHoldShip.GetHull
      );
      SelectedHoldItem.Free;
      SelectedHoldKind := phkEmpty;
  ...
  begin
    ActionResult := 0;
    if PlayerHoldShip.GetHull.ScriptItem <> nil then
      ActionResult :=
          TScriptItem(PlayerHoldShip.GetHull.ScriptItem)
              .RunActionCode(
                  satOnAnotherGoods,
                  PlayerHoldShip,
                  TObject(SelectedGoodsIndex),
  ...
  if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem.ItemType = t_Hull)
      and (SelectedHoldOrigin = 0) then
    ReturnSelectedHoldEntry
  else if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem.ItemType = t_Hull)
      and (SelectedHoldOrigin = 1)
      and IsHoldNormalShip then
  begin
    PlayerHoldShip
        .ScriptItemsAct(satOnPlayerChangeHull, SelectedHoldItem, PlayerHoldShip.GetHull, 0);
    ...
    if PlayerHoldShip.GetHull.ScriptItem <> nil then
      TScriptItem(PlayerHoldShip.GetHull.ScriptItem)
          .RunActionCode(satOnPlayerChangeHull, PlayerHoldShip, PlayerHoldShip.GetHull, OldHull, 0);
    PlayerHoldShip.RefreshAssignedItemSlots;
    PlayerHoldShip.RebuildEquipmentCache;
    PlayerHoldShip.RefreshDerivedStats(True);
...
var
  ...
  VisualType: Byte;
  Ship: TShip;
  function ChameleonDialogChoiceToSeries(
      Choice: Integer
  ): TDominatorSeries; // @addr $700FFC @ida "unsigned __int8 __usercall $name@<al>(int Choice@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x7011A4" Nested helper; does not access its parent frame.
  begin
    Result := dsBlazer;
    case Choice of
      2: Result := dsBlazer;
...
begin
  Result := False;
  if not PlayerHoldShip.InHyperspace
      and (QueuedArcadeBattles.Count <= 0)
      and not PlayerHoldShip.ScriptChameleon
      and (PlayerHoldShip.HasPlayerChameleonCharges or PlayerHoldShip.ChameleonActive) then
  begin
    aCalc.WaitForTurnCalculationUI;
    ReturnSelectedHoldEntry;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Ship <> PlayerHoldShip)
              and Ship.InNormalSpace
              and not Ship.HasScriptControl
              and (Ship is TNormalShip)
              and (Ship.TypeId <> stPirate) then
  ...
  begin
    ...
    begin
      ...
      if (SelectedHoldKind = phkGoods)
          or (SelectedHoldKind = phkEquipment)
          or (SelectedHoldKind = phkArtefact) then
      begin
        ...
        begin
          ...
          begin
            if PlayerHoldEntries.Count > SelectedHoldSlot then
              Entry := PlayerHoldEntries[SelectedHoldSlot]
            else
            begin
    ...
    begin
      if SelectedHoldKind = phkGoods then
      begin
        if GetPlayer.FindStorageIndexByLocationAndSlot(GetLocalStorageOwner, SelectedHoldSlot)
            < 0 then
          GetPlayer.AddGoodsToPlayerStorage(
              SelectedGoodsIndex,
              SelectedGoodsQuantity,
              SelectedGoodsCost,
              GetLocalStorageOwner,
              SelectedHoldSlot
          )
        else
          GetPlayer.AddGoodsToPlayerStorage(
      ...
      begin
        if GetPlayer.FindStorageIndexByLocationAndSlot(GetLocalStorageOwner, SelectedHoldSlot)
          ...
          GetPlayer.AddItemToPlayerStorage(SelectedHoldItem, GetLocalStorageOwner, -1);
      end;
    end;
    SelectedHoldKind := phkEmpty;
    SelectedHoldItem := nil;
...
begin
  if GetPlayer <> nil then
    ...
    begin
      ...
      begin
        Button.SetDisabled(
            (GetPlayer.SelectedEquipmentConfiguration = I) and (GetPlayer = PlayerHoldShip)
        );
...
begin
  ...
  begin
    ...
    else if not GetPlayer.InHyperspace
        and (QueuedArcadeBattles.Count <= 0)
        and (GetPlayer.SelectedEquipmentConfiguration <> Index) then
    begin
      aCalc.WaitForTurnCalculationUI;
      ...
      begin
        ...
        if GetInnermostScreenLoop = Self then
          RefreshShipView;
      end
      else
        RefreshEquipmentConfigurationButtons;
...
begin
  ...
  begin
    if not PlayerHoldShip.InHyperspace
        and (QueuedArcadeBattles.Count <= 0)
        and IsVirtualKeyDown(VK_CONTROL) then
    begin
      if Key = Ord('I') then
        ShipToStorageClicked(nil)
      else if Key = Ord('O') then
        StorageToShipClicked(nil);
    end;
    ...
    begin
      if (GetPlayer = PlayerHoldShip)
          and (GetPlayer.GetHull.CapitalShip > 0)
    ...
    begin
      ...
      RewardsMouseDown(RewardsBuffer, 0, RewardsBuffer.LocalPosition)
  ...
  begin
    ...
    begin
      with GetByName('PanelAddInfo') as TPanelScrollBarGI do
        VerticalScrollBar
            .SetPosition(VerticalScrollBar.Position - VerticalScrollBar.SmallChange * 5);
    end
...
begin
  ...
  begin
    if GetByName('PanelDestr').ContainsPoint(Point) then
      Exit;
    if GetByName('LoadRocketsInSlots').ContainsPoint(Point) then
      Exit;
    if (GetByName('PanelLeft') as TImageGI).HitTestPixel(Point) then
      Exit;
    if (GetByName('RightOpen') as TImageGI).HitTestPixel(Point) then
      Exit;
    if (GetByName('RHOpen') as TImageGI).HitTestPixel(Point) then
      Exit;
    if (GetByName('S_Left') as TGraphButtonGI).ContainsPoint(Point) then
      Exit;
    if (GetByName('S_Right') as TGraphButtonGI).ContainsPoint(Point) then
      Exit;
    if (GetByName('UseImage') as TImageGI).HitTestPixel(Point) then
      Exit;
    if (GetByName('GateZone') as TZoneGI).ContainsPoint(Point) then
      Exit;
    if (GetByName('UseZone') as TZoneGI).ContainsPoint(Point) then
      Exit;
    if GetByName('CenterNormalImage').ContainsPoint(Point) then
      Exit;
    ...
    if GetByName('Forsage').ContainsPoint(Point) then
      Exit;
    if GetByName('PM_PanelMsg').ContainsPoint(Point) then
      Exit;
    if GetByName('PanelLH').ContainsPoint(Point) then
      Exit;
    if GetByName('PanelDS').ContainsPoint(Point) then
      Exit;
...
var
  ...
  begin
    ...
    begin
      ...
      if (ItemType in [t_Artefact, t_Artefact2])
          and TArtefactCustom(SelectedHoldItem).SharedUse then
      ...
      if (ItemType
          ...
          and (not PlayerHoldShip.HasEquippedArtefactOfSameUseGroup(SelectedHoldItem)
      ...
      begin
        ...
        begin
          ...
          if (ItemType = OtherType)
              and (ItemType in [t_Artefact, t_Artefact2])
              and (Equipment.ConfigBlockName = TEquipment(SelectedHoldItem).ConfigBlockName) then
          begin
            Inc(Slot, Index);
            Break;
          end;
    ...
    begin
      ...
      begin
        ...
        if Index = PlayerHoldShip.GetSlotCount(sskWeapon) then
          Exit;
      end;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if SelectedHoldKind <> phkEmpty then
          begin
            if IsVirtualKeyDown(VK_MENU) then
              StorageItemMouseUp(Zone, KeyState, Zone.LocalPosition)
            else if IsVirtualKeyDown(VK_CONTROL) then
            begin
              if (SelectedHoldItem = nil)
                  or ((SelectedHoldItem.ScriptItem = nil) and (SelectedHoldItem.NoDropFlag = 0))
                  or ((SelectedHoldItem.ScriptItem <> nil)
                      and TScriptItem(SelectedHoldItem.ScriptItem).CanSell) then
  ...
  begin
    ...
    begin
      ...
      RefreshShipView;
      if I > 0 then
        SoundManager.PlaySound(Button.ClickSound);
    end;
  end;
  if not Changed and not RemoteHoldMode then
  begin
...
var
  ...
  ActionResult: Integer;
begin
  if (SelectedHoldKind = phkEquipment) and (SelectedHoldItem is THull) then
    Exit;
  if SelectedHoldKind = phkEmpty then
    Exit;
  ...
  if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem is TCountableItem)
      and (TCountableItem(SelectedHoldItem).StackCount > 1) then
  begin
    ...
    if not RemoteHoldVisible then
      if ShowCountDialog(
          ...
          <> 1 then
        Exit;
    if (Quantity < 1) or (TCountableItem(SelectedHoldItem).StackCount < Quantity) then
      Exit;
    Item := TCountableItem(SelectedHoldItem).Split(Quantity);
    if (not PlayerHoldShip.IsOnPlanet and not PlayerHoldShip.IsDockedToShip)
        or ((GetPlayer.RuinsMode > 0)
            and (GetPlayer.RuinsSavedDockedTo = nil)
            and (GetPlayer.RuinsSavedPlanet = nil)) then
    begin
      ...
      begin
        ...
        Angle :=
            SeededRandomIntRange(
                    0,
                    360,
                    PlayerHoldShip.CurrentStar.GenerationSeed * Galaxy.CurrentTurn * Item.Id)
                * Pi
                / 180;
  ...
  begin
    ...
    begin
      if ((SelectedHoldItem.ItemType = t_FuelTanks) or (SelectedHoldItem.ItemType = t_Engine))
          and PlayerHoldShip.NeedsEquipmentType(SelectedHoldItem.ItemType) then
      begin
        ShowMessageBoxGI(
            Self,
            FormatText1(
    ...
    begin
      ...
      begin
        if PlayerHoldShip is TRuins then
          ...
          Text := LocalizedText('FormShip.ThrowFuelTanksError');
    ...
    begin
      ...
      begin
        if PlayerHoldShip is TRuins then
          ...
          Text := LocalizedText('FormShip.ThrowEngineError');
        ShowMessageBoxGI(Self, Text, mbgCancel or mbgWarning);
        aCalc.WaitForTurnCalculationUI;
        Exit;
      end;
    ...
    if SelectedHoldItem is TWeapon then
      (SelectedHoldItem as TWeapon).Target := nil;
    SoundManager.PlaySound('Sound.Drop');
    if PlayerHoldShip.IsOnPlanet
        or (PlayerHoldShip.IsDockedToShip and (GetPlayer.RuinsMode = 0))
        or ((GetPlayer.RuinsMode > 0)
            and ((GetPlayer.RuinsSavedDockedTo <> nil) or (GetPlayer.RuinsSavedPlanet <> nil))) then
    begin
      SelectedHoldItem.Free;
      SelectedHoldItem := nil;
    end
      ...
      begin
        ...
        Angle :=
            SeededRandomIntRange(
                    0,
                    360,
                    PlayerHoldShip.CurrentStar.GenerationSeed
                        * Galaxy.CurrentTurn
                        * SelectedHoldItem.Id)
                * Pi
                / 180;
        ...
        begin
          ...
          if DamageRadius * DamageRadius
              > Sqr(SelectedHoldItem.Position.X) + Sqr(SelectedHoldItem.Position.Y) then
          begin
            Effect := TWeaponSE.Create('Weapon.NoGraph', Classes.Point(0, 0), 0, -1);
    ...
    begin
      if not RemoteHoldVisible then
        if ShowCountDialog(
            ...
            <> 1 then
          Exit;
      if (Quantity < 1) or (Quantity > SelectedGoodsQuantity) then
        Exit;
    end;
    SoundManager.PlaySound('Sound.Drop');
    if (not PlayerHoldShip.IsOnPlanet and not PlayerHoldShip.IsDockedToShip)
      ...
      begin
        ...
        begin
          ...
          begin
            Item := Items[Index];
            if PointDistanceSquared(Item.Position, Goods.Position) < 100 then
              Break;
            Inc(Index);
          end;
...
begin
  if (SelectedHoldKind in [phkEquipment, phkArtefact])
      and (SelectedHoldItem <> nil)
      and (SelectedHoldItem.NoDropFlag > 0) then
  begin
    ShowNoDropMessage(SelectedHoldItem.NoDropFlag);
    Exit;
  end;
  ...
  begin
    ...
    if (ShowCountDialog(
            ...
            <> 1)
        or (Quantity < 1)
        or (TCountableItem(SelectedHoldItem).StackCount < Quantity) then
      Exit;
    Item := TCountableItem(SelectedHoldItem).Split(Quantity);
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessageBoxGI(
            Self,
            LocalizedText('FormShip.ThrowFuelTanksError'),
            mbgCancel or mbgWarning
        );
        Exit;
      end;
    ...
    begin
      ...
      ab_Item_Drop(PlayerArcadeShip, SelectedHoldItem, 100, 150);
    SelectedHoldKind := phkEmpty;
    SetCursorByName('Main');
    RefreshShipView;
  end
  ...
  begin
    ...
    if SelectedGoodsQuantity > 1 then
      if (ShowCountDialog(
              ...
              <> 1)
          or (Quantity < 1)
          or (Quantity > SelectedGoodsQuantity) then
        Exit;
    SoundManager.PlaySound('Sound.Drop');
    if ArcadeViewMode = 0 then
    begin
...
begin
  ...
  aCalc.WaitForTurnCalculationUI;
  if (SelectedHoldKind in [phkEquipment, phkArtefact])
      and (SelectedHoldItem as TEquipment).NeedsRepair then
  begin
    ...
    else if GetPlayer.CanRepairArtefactsAtLocation
        or (not (SelectedHoldItem is TArtefact)
            and (not (SelectedHoldItem is TWeapon)
                or (TWeapon(SelectedHoldItem).GetWeaponInfo.Availability
                    <> waNotSoldAndNodeRepair))) then
    begin
      ...
      if (SelectedHoldItem is TWeapon)
          and (TWeapon(SelectedHoldItem).GetWeaponInfo.Availability = waNotSoldAndNodeRepair) then
      begin
    ...
    begin
      ...
      begin
        if PlayerHoldShip.DockedTo.TypeNameOverrideKey <> WideString('') then
          Text :=
              LocalizedText(
                  'FormShip.NotLicenseForRepair' + PlayerHoldShip.DockedTo.TypeNameOverrideKey
              );
        if Text = WideString('') then
          Text :=
              LocalizedText(
                  'FormShip.NotLicenseForRepair'
                      + ShipTypeNames[PlayerHoldShip.DockedTo.TypeId].Name
              );
      end
        ...
        Text :=
            LocalizedText(
                'FormShip.NotLicenseForRepair'
                    + OwnerInfo[PlayerHoldShip.CurrentPlanet.OwnerId].InternalName
            );
      if Text = WideString('') then
        Text := LocalizedText('FormShip.NotLicenseForRepair');
...
begin
  ...
  begin
    ...
    else if (PlayerHoldShip is TTranclucator)
        and ((SelectedHoldItem.ItemType = t_FuelTanks) or (SelectedHoldItem.ItemType = t_Engine))
        and PlayerHoldShip.NeedsEquipmentType(SelectedHoldItem.ItemType) then
    begin
      ShowMessageBoxGI(
          Self,
    ...
    if (SelectedHoldItem is TArtefactTranclucator)
        and (ShowMessageBoxGI(
            ...
            = 2) then
      Exit;
    if (SelectedHoldItem is TCountableItem)
        and ((SelectedHoldItem as TCountableItem).StackCount > 1) then
    begin
      ...
      if (ShowCountDialog(
                  ...
                  0,
                  (SelectedHoldItem as TCountableItem).StackCount,
                  (SelectedHoldItem as TCountableItem).StackCount,
                  Round(
                      SelectedHoldItem.CalculateResaleValue(
                              GetPlayer.GetEffectiveSkillLevel(psTrading))
                          / (SelectedHoldItem as TCountableItem).StackCount
                  ),
                  Count,
                  SelectedHoldItem
                      .CalculateResaleValue(GetPlayer.GetEffectiveSkillLevel(psTrading)),
                  Count)
              <> 1)
          or (Count < 1)
          or (Count > (SelectedHoldItem as TCountableItem).StackCount) then
        Exit;
    ...
    begin
      ...
      GetPlayer.SetMoney(GetPlayer.Money + Price);
      if SelectedHoldItem is TCountableItem then
        (SelectedHoldItem as TCountableItem).StackCount := 0;
    end;
    if Price <> 0 then
    ...
    begin
      ...
      Event.AddData(
          SelectedHoldItem.CalculateResaleValue(GetPlayer.GetEffectiveSkillLevel(psTrading))
      );
    ...
    if (SelectedHoldKind = phkEquipment)
        and (SelectedHoldItem.ItemType = t_Hull)
        and ((SelectedHoldItem as THull).HullType = htSpecial) then
    begin
      SelectedHoldItem.Free;
      SelectedHoldItem := nil;
    end
    ...
    begin
      ...
      if GetPlayer.CurrentPlanet <> nil then
        GetPlayer.CurrentPlanet.EquipmentShop.Add(SelectedHoldItem)
      else
        (GetPlayer.DockedTo as TRuins).EquipmentShop.Add(SelectedHoldItem);
      BuildTemporaryShopSlotGrid;
    end
    else if not (SelectedHoldItem is TCountableItem)
        or ((SelectedHoldItem as TCountableItem).StackCount = 0) then
    begin
      SelectedHoldItem.Free;
      SelectedHoldItem := nil;
    end;
    if (SelectedHoldItem = nil)
        or not (SelectedHoldItem is TCountableItem)
        or ((SelectedHoldItem as TCountableItem).StackCount = 0) then
    begin
      SelectedHoldItem := nil;
      SelectedHoldKind := phkEmpty;
  ...
  begin
    if GetPlayer.IsCargoGoodIllegalOnCurrentPlanet(SelectedGoodsIndex)
        and (ShowMessageBoxGI(
                Self,
                LanguageDataConfig.GetParamByPathOrMarker('FormGS.NotPermitGoods'),
                mbgOK or mbgCancel)
            <> mbgResultOK) then
      Exit;
    Count := SelectedGoodsQuantity;
    if SelectedGoodsQuantity > 1 then
      if (ShowCountDialog(
                  ...
                  SelectedGoodsQuantity,
                  SelectedGoodsQuantity,
                  GetPlayer.ShopGoodsSellPrice(SelectedGoodsIndex, nil),
                  Count,
                  1000000000,
                  Count)
              <> 1)
...
begin
  if UsePanelSlideTimer <> nil then
    Exit;
  if (GetPlayer = PlayerHoldShip)
      and (SelectedHoldKind in [phkEquipment, phkArtefact])
      and (SelectedHoldItem.ScriptItem <> nil)
      and (TScriptItem(SelectedHoldItem.ScriptItem).OnUseText <> WideString('')) then
  begin
  ...
  else if (GetPlayer = PlayerHoldShip)
      and (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem.ItemType = t_UselessItem)
      and ((SelectedHoldItem as TUselessItem).GetOnUseCodeText <> WideString('')) then
  begin
    Item := SelectedHoldItem;
    aCalc.WaitForTurnCalculationUI;
    aCalc.WaitForTurnCalculationUI;
  ...
  begin
    ...
    begin
      ...
      if GiResourceVariant = 1 then
        Text := (SelectedHoldItem as TTreasureMap).PreviewTablePage1
      else
    ...
    if (GetPlayer = PlayerHoldShip)
        and (SelectedHoldKind = phkArtefact)
        and (SelectedHoldItem is TArtefact)
        and ((SelectedHoldItem as TArtefact).GetOnUseCodeText <> WideString('')) then
    begin
      Item := SelectedHoldItem;
      aCalc.WaitForTurnCalculationUI;
      aCalc.WaitForTurnCalculationUI;
...
begin
  aCalc.WaitForTurnCalculationUI;
  if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem is TCistern)
      and ((SelectedHoldItem as TCistern).Fuel < (SelectedHoldItem as TCistern).Capacity) then
  begin
    ...
    CloseClicked(nil);
      ...
      and (SelectedHoldItem is TFuelTanks)
      and ((SelectedHoldItem as TFuelTanks).Fuel < (SelectedHoldItem as TFuelTanks).Capacity) then
  begin
    ...
    begin
      ...
      if ShowCountDialogWithFont(
              ...
              GetPlayer.Money,
              Amount,
              GetShopItemIconName(FuelTanks) + 's',
              SmallFontName)
          <> 1 then
        Exit;
    end;
  ...
  begin
    ...
    if (SelectedHoldKind = phkEquipment)
        ...
        and (TWeapon(SelectedHoldItem).Ammo < TWeapon(SelectedHoldItem).AmmoCapacity) then
    begin
...
var
  ...
  Control: TObjectGI;
begin
  if not CanUseLocalStorage then
    Exit;
  if GetPlayer.IsOnPlanet
      and (GetPlayer.CurrentPlanet.GetRelationLevelToShip(GetPlayer) <= rlBad)
      and not GetPlayer.CurrentPlanet.IsMainPiratePlanet then
  ...
  begin
    ...
    if PlayerHoldShip.CanRepairEquipmentTech(Item)
        and (Item <> SelectedHoldItem)
        and ((Item.EquippedFlag <> 0) or (Item is THull))
        and Item.NeedsRepair then
      if (Item is TWeapon)
          and (TWeapon(Item).GetWeaponInfo.Availability = waNotSoldAndNodeRepair) then
        Inc(NodeCost, Item.CalculateRepairCost)
      else
        Inc(Cost, Item.CalculateRepairCost);
  end;
  if Cost + NodeCost <= 0 then
    Exit;
  if not GetPlayer.CanRepairArtefactsAtLocation then
    NodeCost := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PlayerHoldShip.Inventory.Add(SelectedHoldItem);
        (SelectedHoldItem as TEquipment).AssignedSlotData :=
            ((SelectedHoldItem as TEquipment).AssignedSlotData and EquipmentSecondaryFireFlag) or I;
        (SelectedHoldItem as TEquipment).EquippedFlag := 0;
        (SelectedHoldItem as TEquipment).Equip;
        SelectedHoldKind := phkEmpty;
        SelectedHoldItem := nil;
...
begin
  with GetByName('SC_RepareFull_But') as TGraphButtonGI do
    Self.HighlightRepairableEquipment := not Disabled;
  RefreshShipView;
end;
...
begin
  ...
  if Equipped then
    with GetByName('LoadRocketsInSlots') do
      if not Active or not Parent.Active then
        Exit;
  ...
  begin
    Item := PlayerHoldShip.Inventory[I];
    if (Equipped = Boolean(Item.EquippedFlag))
        and (Item <> SelectedHoldItem)
        and (Item is TWeapon)
        and (Item as TWeapon).NeedsAmmo then
  ...
  begin
    ShowMessageBoxGI(
        Self,
        LocalizedColorText('FormShip.ReloadAll.' + Suffix + '.NoItem'),
        mbgCancel or mbgWarning
    );
    Exit;
  end;
...
begin
  ...
  if PlayerHoldShip.AfterburnerActive then
    ShowMessageBoxGI(
        Self,
        LanguageDataConfig.GetParamByPathOrMarker('FormShip.ForsageActivate'),
        mbgOK
    );
end;
...
begin
  ...
  if not Found and not RemoteHoldMode then
    ...
    begin
      if HoldFirstIndex + I >= PlayerHoldEntries.Count then
        ...
        Hold := PlayerHoldEntries[HoldFirstIndex + I];
        ...
        begin
          ...
          begin
            ...
            if (Hold <> nil) and ((Hold.Kind = phkEquipment) or (Hold.Kind = phkArtefact)) then
              ...
              Continue; { Native retries the same column for an unexpected hold entry kind. }
            Found := True;
            CanTake := True;
            Break;
          end;
  ...
  begin
    if ItemInfoWindow.Active then
      InfoWindow := ItemInfoWindow
    else
      InfoWindow := GetByName('InfoHull') as TWindowGI;
    if DynamicTipsPos then
    begin
      if CenterX then
        Dec(Position.X, InfoWindow.ClientSize.X div 2);
      if Position.X < 0 then
        Position.X := Abs(Position.X) - InfoWindow.ClientSize.X;
      if CenterY then
      begin
  ...
  if SelectedHoldKind = phkEmpty then
    ...
    begin
      ...
      SetCursorByName('Main');
  if not Found and (ItemInfoHideTimer = nil) then
    ShowEquipmentInfo(nil, False);
  ...
  begin
    ...
    HoveredEquipmentAnimation.SetSequenceFrame(
        Min(
            Cardinal(HoveredEquipmentAnimation.UserState),
            HoveredEquipmentAnimation.SequenceFrameCount - 1
        )
    );
...
procedure TfShip2.LayoutItemInfo(
    Window: TWindowGI;
    Title, Text: TLabelGI;
    KeepMinimumHeight, WordWrap: Boolean;
    MinimumWidth: Integer
);
var
...
begin
  ...
  begin
    ...
    if KeepMinimumHeight then
      Window.SetSize(
          Classes.Point(
              TextSize.X + Borders.Left + Borders.Right,
              Max(TargetSize.Y, TextSize.Y + Borders.Top + Borders.Bottom)
          )
      )
    else
      Window.SetSize(
          Classes.Point(
              TextSize.X + Borders.Left + Borders.Right,
              TextSize.Y + Borders.Top + Borders.Bottom
          )
      );
    Window.UpdateAutoGeometry;
    WindowSize := Window.ClientSize;
    Ratio := WindowSize.X / WindowSize.Y;
    if Ratio - TargetRatio < -0.1 then
      Inc(TargetSize.X, 10)
    else if Ratio - TargetRatio > 0.1 then
      Inc(TargetSize.Y, 10)
    else
      Break;
    Dec(Attempts);
  ...
  Text.SetSize(
      Classes.Point(
          Window.ClientSize.X - Borders.Left - Borders.Right,
          Window.ClientSize.Y - Borders.Top - Borders.Bottom
      )
  );
...
procedure TfShip2.LayoutObjectInfo(
    Window: TWindowGI;
    Title: TLabelGI;
    Left1,
    Right1,
...
var
  ...
  procedure MeasurePair(
      Left,
      Right: TLabelGI
  ); // @addr $70AC50 @ida "void __usercall $name(TLabelGI *Left@<eax>, TLabelGI *Right@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x70aee7,0x70aef4,0x70af01,0x70af0e,0x70af1b,0x70af28,0x70af35,0x70af42" @note "Nested in LayoutObjectInfo; captures column widths, total height and row count."
  begin
  ...
  procedure PlacePair(
      Left,
      Right: TLabelGI
  ); // @addr $70AD30 @ida "void __usercall $name(TLabelGI *Left@<eax>, TLabelGI *Right@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x70b0a9,0x70b0b6,0x70b0c3,0x70b0d0,0x70b0dd,0x70b0ea,0x70b0f7,0x70b104" @note "Nested in LayoutObjectInfo; captures window, border dimensions and accumulated row position."
  begin
    if (Left <> nil) and (Right <> nil) and Left.Active and Right.Active then
    begin
      CenterX :=
          (Window.ClientSize.X - Borders.Left - Borders.Right) div 2
              - (LeftWidth + RightWidth + EmblemMargin) div 2;
      CenterY := (Window.ClientSize.Y - Borders.Top - Borders.Bottom) div 2 - TotalHeight div 2;
      Left.SetPosition(
          Classes.Point(
              Borders.Left + CenterX + LeftWidth - Left.ClientSize.X,
              RowY + Borders.Top + CenterY
          )
      );
      Right.SetPosition(
          Classes.Point(Borders.Left + CenterX + LeftWidth, RowY + Borders.Top + CenterY)
      );
...
begin
  ...
  EmblemMargin := 0;
  if Emblem <> nil then
    EmblemMargin := Emblem.ClientSize.X div 2;
  Borders := Window.WorkSubRect;
  ...
  if Emblem <> nil then
    Emblem.SetPosition(
        Classes.Point(
            Window.ClientSize.X + ItemRaceImagePosition.X,
            Window.ClientSize.Y + ItemRaceImagePosition.Y - GiScalePixels(5)
        )
    );
end;
...
begin
  ...
  begin
    ...
    if (GetPlayer = PlayerHoldShip)
        ...
        and not PreserveSpaceMusic
        and (SelectedHoldItem.ScriptItem <> nil)
        and (TScriptItem(SelectedHoldItem.ScriptItem).OnUseText <> '') then
      OpenUseSidePanel
    else if (GetPlayer = PlayerHoldShip)
        and (SelectedHoldKind = phkArtefact)
        and (SelectedHoldItem is TArtefact)
        and not PreserveSpaceMusic
        and ((SelectedHoldItem as TArtefact).GetOnUseCodeText <> '') then
      OpenUseSidePanel
    else if (GetPlayer = PlayerHoldShip)
        and (SelectedHoldKind = phkEquipment)
        and not PreserveSpaceMusic
        and (SelectedHoldItem.ItemType = t_UselessItem)
        and ((SelectedHoldItem as TUselessItem).GetOnUseCodeText <> '') then
      OpenUseSidePanel
    else if (SelectedHoldKind = phkEquipment)
        and not PreserveSpaceMusic
        and (SelectedHoldItem.ItemType = t_TreasureMap) then
      OpenUseSidePanel
    else
      CloseUseSidePanel;
  end
  ...
  begin
    ...
    begin
      if Item.ScriptItem <> nil then
        TScriptItem(Item.ScriptItem)
            .RunActionCode(satOnShowingItemInfo, PlayerHoldShip, nil, nil, 0);
      if Item is TEquipmentWithActCode then
        RunItemConfigActionCode(Item, satOnShowingItemInfo, PlayerHoldShip, nil, nil, 0);
    end;
    if (GetPlayer = PlayerHoldShip)
        and (SelectedHoldKind in [phkEquipment, phkArtefact])
        and not PreserveSpaceMusic
    ...
    if (GetPlayer = PlayerHoldShip)
        and (Item.ScriptItem <> nil)
        and (TScriptItem(Item.ScriptItem).OnUseText <> '') then
      OpenUseSidePanel
    else if (GetPlayer = PlayerHoldShip)
        and not PreserveSpaceMusic
        and (Item is TArtefact)
        and ((Item as TArtefact).GetOnUseCodeText <> '') then
      OpenUseSidePanel
    else if (GetPlayer = PlayerHoldShip)
        and not PreserveSpaceMusic
        and (Item.ItemType = t_UselessItem)
        and ((Item as TUselessItem).GetOnUseCodeText <> '') then
      OpenUseSidePanel
    else if (Item.ItemType = t_TreasureMap) and not PreserveSpaceMusic then
      OpenUseSidePanel;
    if ItemInfoHideTimer <> nil then
    begin
      CancelCallbackTimer(ItemInfoHideTimer);
    ...
    if (SelectedHoldKind = phkEmpty) and (Item is TArtefact) then
      RefreshActionPanels(phkArtefact, Byte(Item.ItemType), 0, 0, Item, Origin)
    else if SelectedHoldKind = phkEmpty then
      RefreshActionPanels(phkEquipment, Byte(Item.ItemType), 0, 0, Item, Origin)
    else
      RefreshActionPanels(
          SelectedHoldKind,
          SelectedGoodsIndex,
    ...
    SpecialHull :=
        ((PlayerHoldShip is TRuins)
                or (PlayerHoldShip is TTranclucator)
                or (PlayerHoldShip is TKling))
            and (PlayerHoldShip.GetHull = Equipment);
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            if Cardinal(GraphBuf.Width) >= Cardinal(GraphBuf.Height) then
              GraphBuf.RescaleRgba(
                  ClientSize.X - 5,
                  Round((ClientSize.X - 5) / Cardinal(GraphBuf.Width) * Cardinal(GraphBuf.Height)),
                  5
              )
            else
              GraphBuf.RescaleRgba(
                  Round((ClientSize.Y - 5) / Cardinal(GraphBuf.Height) * Cardinal(GraphBuf.Width)),
                  ClientSize.Y - 5,
                  5
              );
            SetImageKindX(ikxCenter);
      ...
      ItemSizeLabel.SetPosition(
          Classes
              .Point(ItemSizeLabelPosition.X, ItemInfoWindow.ClientSize.Y + ItemSizeLabelPosition.Y)
      );
      ItemPriceLabel.SetPosition(
          Classes.Point(
              ItemPriceLabelPosition.X,
              ItemInfoWindow.ClientSize.Y + ItemPriceLabelPosition.Y
          )
      );
      ItemRaceImage.SetPosition(
          Classes.Point(
              ItemInfoWindow.ClientSize.X + ItemRaceImagePosition.X,
              ItemInfoWindow.ClientSize.Y + ItemRaceImagePosition.Y
          )
      );
    end;
  end;
  ...
  begin
    ...
    if SelectedHoldKind = phkEmpty then
      RefreshActionPanels(
          phkGoods,
          Good,
          PlayerHoldShip.CargoGoods[Good].Count,
          PlayerHoldShip.CargoGoods[Good].TotalCost,
          nil,
          0
      )
    else
    ...
    if not ItemInfoWindow.Active then
      Changed := True;
    ItemInfoWindow.SetActive(True);
    GetByName('InfoHull').SetActive(False);
    with GetByName('InfoImage') as TImageGI do
    begin
      if GetImagePath <> 'GI,' + GetItemTypeBitmapPath(TItemType(Good)) then
        Changed := True;
    ...
    if (GetByName('InfoName') as TLabelGI).GetText
        <> WrapTextInColor(GoodsMarket[Good].DisplayName, InfoNameColorTag) then
      Changed := True;
    if (GetByName('InfoText') as TLabelGI).GetText <> Text then
      Changed := True;
    if (GetByName('InfoSize') as TLabelGI).GetText
        <> IntToStr(PlayerHoldShip.CargoGoods[Good].Count) then
      Changed := True;
    if (GetByName('InfoPrice') as TLabelGI).GetText
        <> IntToStr(PlayerHoldShip.CargoGoods[Good].TotalCost) then
      Changed := True;
    (GetByName('InfoName') as TLabelGI)
        .SetText(WrapTextInColor(GoodsMarket[Good].DisplayName, InfoNameColorTag));
    ...
    (GetByName('InfoPrice') as TLabelGI)
        .SetText(IntToStr(PlayerHoldShip.CargoGoods[Good].TotalCost));
    with GetByName('EmRace') as TImageGI do
    begin
      if GetImagePath <> GetFactionEmblemPath(PlayerHoldShip.GetFactionNameKey) then
        Changed := True;
    ...
    ItemPriceLabel.SetPosition(
        Classes
            .Point(ItemPriceLabelPosition.X, ItemInfoWindow.ClientSize.Y + ItemPriceLabelPosition.Y)
    );
    ItemRaceImage.SetPosition(
        Classes.Point(
            ItemInfoWindow.ClientSize.X + ItemRaceImagePosition.X,
...
begin
  ...
  begin
    ...
    if Goods.Quantity > 0 then
      AverageCost := Goods.Cost / Goods.Quantity
    else
      AverageCost := 0;
    Text :=
        Text
    ...
    (GetByName('InfoName') as TLabelGI)
        .SetText(WrapTextInColor(GoodsMarket[Ord(Goods.ItemType)].DisplayName, InfoNameColorTag));
    (GetByName('InfoText') as TLabelGI).SetText(Text);
...
begin
  if Code = 1 then
    Text := LocalizedColorText('FormShip.NoDrop')
  else
    Text := LocalizedColorText('FormShip.NoDrop' + IntToWideString(Code));
  Options := 1;
...
begin
  if (SelectedHoldKind = phkEquipment) and (TObject(SelectedHoldItem) is THull) then
    CloseGate
  else if SelectedHoldKind <> phkEmpty then
    OpenGate
  else
...
begin
  ...
  with GetByName('GateLeft') do
    SetPosition(Classes.Point(Self.GateLeftRestLeft - Self.GateSlideOffset, LocalPosition.Y));
  with GetByName('GateRight') do
    SetPosition(Classes.Point(Self.GateRightRestLeft + Self.GateSlideOffset, LocalPosition.Y));
end;
...
begin
  ...
  with GetByName('UseLeft') do
    SetPosition(Classes.Point(Self.UseLeftRestLeft - Self.UseSlideOffset, LocalPosition.Y));
  with GetByName('UseRight') do
    SetPosition(Classes.Point(Self.UseRightRestLeft + Self.UseSlideOffset, LocalPosition.Y));
end;
...
begin
  ...
  Panel.SetPosition(
      Classes.Point(Panel.LocalPosition.X, StoragePanelSlideHeight + StorageSlideOffset)
  );
...
begin
  ...
  if (SelectedHoldKind in [phkEquipment, phkArtefact])
      and (SelectedHoldItem <> nil)
      and (SelectedHoldItem.NoDropFlag > 0)
      and (SelectedHoldOrigin <> 1) then
  begin
    ShowNoDropMessage(SelectedHoldItem.NoDropFlag);
    Exit;
  end;
  ...
  if (Index >= 0)
      and (SelectedHoldKind = phkEmpty)
      and (GetPlayer <> PlayerHoldShip)
      and (PStorageEntry(GetPlayer.StorageEntries[Index]).Item.NoDropFlag > 0) then
    Exit;
  aCalc.WaitForTurnCalculationUI;
  if (SelectedHoldKind <> phkEmpty) and (SelectedHoldOrigin = 0) then
    RemoveEmptyPlayerHoldSlots;
  if (SelectedHoldKind = phkEquipment)
      and (SelectedHoldItem is TCountableItem)
      and (TCountableItem(SelectedHoldItem).StackCount >= 1) then
...
var
    ...
    if GiResourceVariant = 1 then
      LabelControl.SetText(
          '<Object='
              ...
              + IntToStr(17)
              + ',0>'
              + ReplaceAllWideString(Text, '<br>', #13#10)
      )
    else
      LabelControl.SetText(
          '<Object='
  ...
  function GetShipInfoColor(
      ColorIndex: Byte
  ): WideString; // @addr $710030 @ida "void __usercall $name(unsigned __int8 ColorIndex@<al>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x710A3A" Nested in BuildAdditionalInfoPanel; does not access its parent frame.
  begin
    Result := '';
    case ColorIndex of
    ...
    for ProgramIndex := Low(ProgramNames) to High(ProgramNames) do
      if GetPlayer.ProgramCounts[ProgramIndex] > 0 then
        AddLine(
            ...
            GetPlayer.GetProgramName(ProgramIndex)
                + '~'
                + GetPlayer.GetProgramInfoText(ProgramIndex),
            0
        );
  end;
...
begin
  if (Message = WM_LBUTTONDOWN)
      ...
      or (Message = WM_KEYDOWN) then
    if StopScriptVideo then
      Exit;
...
begin
  Result := True;
  if not ((PlayerHoldShip is TRuins) and (PlayerHoldShip as TRuins).ModernizationSponsor)
      and not ((PlayerHoldShip is TTranclucator)
          and ((PlayerHoldShip as TTranclucator).OwnerShip = GetPlayer)) then
    Result := False;
end;
...
begin
  if not PlayerHoldShip.InHyperspace
      and (QueuedArcadeBattles.Count <= 0)
      and ((GetPlayer = PlayerHoldShip) or (PlayerHoldShip is TTranclucator))
      and (GetPlayer.RuinsMode <= 0) then
  begin
    aCalc.WaitForTurnCalculationUI;
                ...
                if Item is TGoods then
                  Inc(
                      ...
                      TGoods(Item).Quantity
                          * GetPlayer.ShopGoodsSellPrice(Byte(TGoods(Item).ItemType), Location)
                  )
...
begin
  if ((GetPlayer = PlayerHoldShip) or (PlayerHoldShip is TTranclucator))
      and (GetPlayer.RuinsMode <= 0)
      and not GetPlayer.InHyperspace
      and (QueuedArcadeBattles.Count <= 0) then
  begin
    ...
    begin
      ...
      begin
        ...
        if (Item.EquippedFlag = 0)
            ...
            and (Item.ScriptItem = nil)
            and GetPlayer.CanAccessStoredItem(Item)
            and (Item.NoDropFlag <= 0)
            and ((Item.ScriptItem = nil) or TScriptItem(Item.ScriptItem).CanSell)
...
var
  LeftValue, RightValue: Integer;
  LeftType, RightType: Byte;
  LeftClass, RightClass: Integer;
  LeftName, RightName: WideString;
  NameComparison: Integer;
  function Classify(
      ItemType: Byte
  ): Integer; // @addr $7129B4 @ida "int __usercall $name@<eax>(unsigned __int8 ItemType@<al>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x712A81,0x712A8E" Nested helper; does not read the parent frame.
  begin
    Result := 0;
    if ItemType in [Ord(t_Food)..Ord(t_Narcotics)] then
      Result := 1
    else if ItemType in [Ord(t_Artefact)..Ord(t_ArtFastRacks)] then
      Result := 3
    else if ItemType in [Ord(t_Hull)..Ord(t_CustomWeapon)] then
      Result := 2
    else if ItemType in [Ord(t_Protoplasm)..Ord(t_UselessCountableItem)] then
      Result := 4;
  end;
begin
  Result := 0;
  if Left.Item = nil then
  begin
    Result := -1;
    Exit
  end;
  if Right.Item = nil then
  begin
    Result := 1;
    Exit
  end;
  ...
  begin
    ...
    if (LeftClass = 4)
        and (RightClass = 4)
        and (Left.Item.ItemType = t_MicroModule)
        and (Right.Item.ItemType = t_MicroModule) then
    begin
      LeftValue :=
          GetMicroModulePriorityColorTier((Left.Item as TMicroModule).MicroModuleIndex - 1);
      RightValue :=
  ...
  begin
    if LeftClass = 1 then
      LeftValue := TGoods(Left.Item).Quantity
    else
      LeftValue := Left.Item.Weight;
    if RightClass = 1 then
      RightValue := TGoods(Right.Item).Quantity
    else
      RightValue := Right.Item.Weight;
    if LeftValue < RightValue then
    begin
  ...
  for I := 0 to Count - 2 do
    ...
    begin
      ...
      if (LeftIndex >= 0) and (RightIndex >= 0) then
        if CompareStoredItems(
                GetPlayer.StorageEntries[LeftIndex],
                GetPlayer.StorageEntries[RightIndex],
                Sort)
            > 0 then
        begin
...
begin
  ...
  if ((PlayerHoldShip.CurrentPlanet <> nil)
...
var
  ...
  function SellItem(
      Item: TItem;
      IgnoreRequiredEquipment: Boolean
  ): Boolean; // @addr $712F80 @ida "bool __usercall $name@<al>(TItem *Item@<eax>, bool IgnoreRequiredEquipment@<dl>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x713540,0x713661,0x7139EB" Nested helper captures sale/change flags.
  var
  ...
  begin
    ...
    begin
      if PlayerHoldShip is TRuins then
      begin
        if PlayerHoldShip.NeedsEquipmentType(Item.ItemType) then
          Exit;
      end
      else if ((Item.ItemType = t_FuelTanks) or (Item.ItemType = t_Engine))
          and PlayerHoldShip.NeedsEquipmentType(Item.ItemType) then
        Exit;
    end;
    ...
    begin
      ...
      begin
        ...
        if GetPlayer.CurrentPlanet <> nil then
          ...
          (GetPlayer.DockedTo as TRuins).EquipmentShop.Add(Item);
        BuildTemporaryShopSlotGrid;
      end
      else
        Item.Free;
  ...
  begin
    ...
    begin
      ...
      if (Item.NoDropFlag = 0)
          ...
          and (TEquipment(Item).EquippedFlag = 0)
          and ((Item.ScriptItem = nil) or TScriptItem(Item.ScriptItem).CanSell)
          and GetPlayer.CanAccessStoredItem(Item) then
        if SellItem(Item, False) then
          PlayerHoldShip.Inventory.Delete(I);
      Dec(I);
    end;
    ...
    for Good := 0 to 7 do
      if PlayerHoldShip.CargoGoods[Good].Count > 0 then
        ...
        begin
          if GetPlayer.IsCargoGoodIllegalOnCurrentPlanet(Good)
              and not AllowIllegal
              and not DeclineIllegal then
            if ShowMessageBoxGI(
                    Self,
                    LanguageDataConfig.GetParamByPathOrMarker('FormGS.NotPermitGoods'),
                    mbgOK or mbgCancel)
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Item := Entry.Item;
        if (Item.NoDropFlag > 0)
            or ((Item.ScriptItem <> nil) and not TScriptItem(Item.ScriptItem).CanSell) then
        begin
          Dec(I);
          Continue;
        end;
        ...
        if Item is TGoods then
          if GetPlayer.IsCargoGoodIllegalOnCurrentPlanet(Byte(Item.ItemType))
              and not AllowIllegal
              and not DeclineIllegal then
            if ShowMessageBoxGI(
                    Self,
              ...
              AllowIllegal := True;
        if (Item is TGoods)
            and GetPlayer.IsCargoGoodIllegalOnCurrentPlanet(Byte(Item.ItemType))
            and DeclineIllegal then
        begin
          Dec(I);
          Continue;
        end;
