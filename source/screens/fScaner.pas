...
type
  ...
  TfScaner = class(TMessageLoopGI)
    ...
    SkillPositiveImages: array[0..5] of TImageGI;
    SkillNegativeImages: array[0..5] of TImageGI;
    ...
    RewardWindow: TWindowGI;
    HoveredItemAnimation: TgaiGI;
    VisibleCargoCount: Integer;
    CargoOffset: Integer;
    CargoEntryCount: Integer;
    PanelSlideTimer: PCallbackTimerGI;
    PanelSlideStep: PtrInt;
    PanelSlideStartX: Integer;
    PanelSlideEndX: Integer;
    ItemHoverTimer: PCallbackTimerGI;
    HideItemTimer: PCallbackTimerGI;
    PropertyHintTimer: PCallbackTimerGI;
    ShipImageCenter: TPoint;
    ArtefactZones: array of TZoneGI;
    EquipmentAnimations: array[0..7] of array[0..4] of TgaiGI;
    HoveredItem: TItem;
    ...
    procedure BuildRewardStrip(Ship: TShip);
    ...
    procedure Update;
    procedure ScrollCargoLeft(Sender: TObjectGI);
    procedure ScrollCargoRight(Sender: TObjectGI);
    ...
    function CreateAdditionalInfoIcon(Sender: TLabelGI; Item: PFontObjectEC): TObjectGI;
...
begin
  ...
  with GetByName('Ship3D') do
    ShipImageCenter := AddPoints(LocalPosition, HalfPoint(ClientSize));
  for I := 0 to 5 do
  begin
...
var
  ...
  Stage: Integer;
  ReservedText:
      WideString; // Native frame initializes and finalizes this unreferenced managed slot.
begin
  Stage := 0;
  try
    ...
    begin
      ...
      begin
        EquipmentAnimations[I, J] :=
            GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(J) + 'anim') as TgaiGI;
      ...
      begin
        ...
        begin
          GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(J) + 'off')
              .SetActive(True);
          GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(J) + 'Set')
              .SetActive(False);
        end;
      end
      else
      ...
      begin
        ...
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(J) + 'anim')
            .SetActive(False);
      end;
    end;
    Stage := 9;
    ...
    begin
      Stage := 14;
      if (ShipToInspect is TKling)
          or (ShipToInspect is TRuins)
          or (ShipToInspect is TTranclucator) then
      begin
    ...
    with GetByName('RankI2') as TImageGI do
      ...
      begin
        ...
        MouseLeaveCallback := HidePropertyInfo;
        SetImagePath(
            'GI,Bm.FormShip2.PRank' + IntToStr((ShipToInspect as TNormalShip).PirateRank + 1)
        );
      end
      else
        SetActive(False);
    ...
    Stage := 24;
    GetByName('PanelRight')
        .SetPosition(Classes.Point(PanelSlideStartX, GetByName('PanelRight').LocalPosition.Y));
    PanelSlideStep := 20;
    ...
    GetByName('ForsageLight')
        .SetActive(
            ShipToInspect.AfterburnerActive
                and ShipToInspect.IsEquipmentUsable(ShipToInspect.GetEngine));
    Stage := 28;
...
begin
  ...
  if Ship.AwardIds.Count <= VisibleCount then
    Spacing := Size
  else
  begin
  ...
  begin
    ...
    GraphBuf.AllocateRgbaTight(
        Max(ClientSize.X, Round(VisibleCount * Spacing + Size - Spacing)) + 2,
        Size + 2
    );
  ...
  begin
    ...
    if AwardId < 10 then
      Path := 'Bm.FormRewards.' + GiResourceSuffix + '_0' + IntToStr(AwardId)
    else
      Path := 'Bm.FormRewards.' + GiResourceSuffix + '_' + IntToStr(AwardId);
    ...
    if Cardinal(Image.Width) >= Cardinal(Image.Height) then
      ...
      Image.RescaleRgba(Round(Size / Cardinal(Image.Height) * Cardinal(Image.Width)), Size, 5);
    ...
    if (Image.Height <= Size)
        and (Image.Width + Round(DrawIndex * Spacing) <= RewardsBuffer.GraphBuf.Width) then
    begin
      RewardsBuffer.GraphBuf.BlendRect32(
          Classes.Point(Round(DrawIndex * Spacing) + 2, 2),
          Shadow,
          Classes.Rect(0, 0, Image.Width, Image.Height)
      );
      RewardsBuffer.GraphBuf.BlendRect32(
          Classes.Point(Round(DrawIndex * Spacing), 0),
          Image,
          Classes.Rect(0, 0, Image.Width, Image.Height)
      );
    end;
...
begin
  ...
  while I < Ship.AwardIds.Count do
  begin
    if (X >= Round(DrawIndex * Spacing)) and (X < Round((DrawIndex + 1) * Spacing)) then
      Break;
    Inc(DrawIndex);
    Inc(I);
  end;
  if I >= Ship.AwardIds.Count then
    I := Ship.AwardIds.Count - 1;
  AwardId := Byte(Ship.AwardIds[I]);
  if Ship is TNormalShip then
    ShowRewardInfo(Ship as TNormalShip, AwardId);
end;
...
begin
  ...
  RewardWindow.SetPosition(
      Classes.Point(
          CursorPoint.X - RewardWindow.ClientSize.X - 50,
  ...
  with GetByName('RewardName') as TLabelGI do
    SetText(Ship.GetAwardInfo(AwardId).Name);
  with GetByName('RewardText') as TLabelGI do
    SetText(Ship.GetAwardInfo(AwardId).Text);
  ...
  with GetByName('RewardName') as TLabelGI do
    SetSize(
        Classes.Point(
            RewardWindow.ClientSize.X - LocalPosition.X - RewardWindow.WorkSubRect.Right,
            ClientSize.Y
        )
    );
...
begin
  ...
  begin
      ...
      begin
        ...
        begin
          if ShipToInspect is TTranclucator then
            ImagePath := 'Bm.FormShip.' + GiResourceSuffix + 'Rank4'
          else if (ShipToInspect as TNormalShip).Rank = 0 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank1'
          else if (ShipToInspect as TNormalShip).Rank = 1 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank2'
          else if (ShipToInspect as TNormalShip).Rank = 2 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank3'
          else if (ShipToInspect as TNormalShip).Rank = 3 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank4'
          else if (ShipToInspect as TNormalShip).Rank = 4 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank5'
          else if (ShipToInspect as TNormalShip).Rank = 5 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank6'
          else if (ShipToInspect as TNormalShip).Rank = 6 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank7'
          else if (ShipToInspect as TNormalShip).Rank = 7 then
            ImagePath := 'Bm.FormShip2.' + GiResourceSuffix + 'Rank8';
        end;
        with GetByName('RankImage') as TGraphBufGI do
        begin
          SetActive(ImagePath <> '');
    ...
    if not UseAnchor then
      Window.SetPosition(
          Classes
              .Point(ShipScreen.PropertyHintRightEdge - Window.ClientSize.X, Window.LocalPosition.Y)
      )
    else
...
begin
  CargoEntryCount := 0;
  for Kind := 0 to 7 do
    if ShipToInspect.CargoGoods[Kind].Count > 0 then
      Inc(CargoEntryCount);
  for I := 0 to ShipToInspect.Inventory.Count - 1 do
  begin
    Item := TEquipment(ShipToInspect.Inventory[I]);
    if (Item.EquippedFlag = 0) and (ShipToInspect.GetHull <> Item) then
      Inc(CargoEntryCount);
  end;
...
begin
  ...
  try
    ...
    (GetByName('IEmpty') as TLabelGI)
        .SetText(WrapTextInColor(IntToStr(ShipToInspect.GetCargoFreeSpace), Text));
    (GetByName('S_Left') as TGraphButtonGI).SetDisabled(not (CargoOffset > 0));
    (GetByName('S_Right') as TGraphButtonGI)
        .SetDisabled(not (CargoOffset + VisibleCargoCount <= CargoEntryCount));
    ...
    begin
      ...
      if Active then
        SetImagePath(
            'GI,'
                + GetMicroModuleBitmapResourceName(ShipToInspect.GetHull.MicroModuleIndex - 1)
                + 'Set'
        );
    end;
    Stage := 6;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Image.SetImagePath('');
          GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'Set')
              .SetActive(False);
        end
        else
        begin
          ...
          with GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'Set')
              as TImageGI do
          begin
            ...
            if Active then
              SetImagePath(
                  'GI,'
                      + GetMicroModuleBitmapResourceName(TEquipment(Item).MicroModuleIndex - 1)
                      + 'Set'
              );
          end;
        end;
        ...
        begin
          Stage := 15;
          GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'anim')
              .SetActive(False);
        end;
        Stage := 16;
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'a')
            .SetActive(False);
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'n')
            .SetActive((Item <> nil) and ShipToInspect.IsEquipmentUsable(TEquipment(Item)));
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'b')
            .SetActive((Item <> nil) and not ShipToInspect.IsEquipmentUsable(TEquipment(Item)));
        GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'Ex')
            .SetActive(False);
        Stage := 17;
      end;
      if EquipmentSlotLayouts[I].ItemType = t_Weapon1 then
      begin
        ...
        for SlotIndex := SlotCount to 4 do
          (GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'z')
                      as TZoneGI)
    ...
    begin
      ...
      GetByName('Art' + IntToStr(SlotIndex) + 'b')
          .SetActive((Artefact <> nil) and Boolean(Artefact.BrokenFlag) and not CanBoost);
      ...
      Stage := 25;
      with GetByName('Art' + IntToStr(SlotIndex) + 'z') as TZoneGI do
    ...
    for I := 0 to VisibleCargoCount - 1 do
      with GetByName('S_' + IntToStr(I) + 'i') as TImageGI do
        if not GetCargoEntry(CargoOffset + I, CargoKind, Item) then
          SetImagePath('')
...
begin
  if (Key = VK_ESCAPE)
      or ((Key = Ord('I'))
          ...
          and not IsVirtualKeyDown(VK_SHIFT)) then
  begin
    AuxRenderBuffer.Clear;
    RequestedScreenId := ScannerReturnScreenId;
    RequestClose(1);
...
begin
      ...
      begin
        ...
        with GetByName('S_' + EquipmentSlotLayouts[I].Name + '_' + IntToStr(SlotIndex) + 'z')
          ...
          begin
            ...
            Position :=
                Classes.Point(
                    (ClientSize.X div 2) + HitTestBounds.Left,
                    HitTestBounds.Top + ClientSize.Y
                );
            ShowItemInfo(Item);
            Found := True;
    ...
    for I := 0 to VisibleCargoCount - 1 do
      with GetByName('S_' + IntToStr(I) + 'z') as TZoneGI do
        ...
        begin
          ...
          begin
            if CargoKind in [t_Food..t_Narcotics] then
  ...
  begin
    if ItemInfoWindow.Active then
      ...
      Window := GetByName('InfoHull') as TWindowGI;
    if DynamicTipsPos then
    begin
      if CenterX then
        Position.X := Position.X - Window.ClientSize.X div 2;
      if Position.X < 0 then
        Position.X := Abs(Position.X) - Window.ClientSize.X;
      if CenterY then
      begin
        Position.Y := Position.Y - Window.ClientSize.Y div 2;
        if Window.ClientSize.Y + Position.Y + 10 > GameScreenHeight then
          Position.Y := GameScreenHeight - Window.ClientSize.Y - 10;
      end
      else if Window.ClientSize.Y + Position.Y + 10 > GameScreenHeight then
  ...
  begin
    ...
    HoveredItemAnimation.SetSequenceFrame(
        Min(Cardinal(HoveredItemAnimation.UserState), HoveredItemAnimation.SequenceFrameCount - 1)
    );
...
begin
  ...
  begin
    ...
    if (Item.ItemType = t_Hull)
        ...
        and (ShipToInspect.TypeId <> stKling)
        and not CompactHullInfo then
    begin
...
begin
  ...
  (GetByName('InfoName') as TLabelGI)
      .SetText(WrapTextInColor(GoodsMarket[Ord(ItemType)].DisplayName, InfoNameColorTag));
  (GetByName('InfoText') as TLabelGI)
      .SetText(LocalizedText('Items.Goods.Text.' + IntToStr(Ord(ItemType) + 1)));
  (GetByName('InfoSize') as TLabelGI)
      .SetText(IntToStr(ShipToInspect.CargoGoods[Ord(ItemType)].Count));
  if GetPlayer = ShipToInspect then
    (GetByName('InfoPrice') as TLabelGI)
        .SetText(IntToStr(ShipToInspect.CargoGoods[Ord(ItemType)].TotalCost))
  else
    (GetByName('InfoPrice') as TLabelGI).SetText('-');
  with GetByName('EmRace') as TImageGI do
  begin
...
var
  ...
  ); // @addr $6EEF34 @ida "void __usercall $name(int Index@<eax>, int BaseLevel@<edx>, int EffectiveLevel@<ecx>, void *ParentFrame@<^0>);"
  var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SetPosition(
            Classes
                .Point(LocalPosition.X, SkillImageRestTop[Index] + Bottom - Step * EffectiveLevel)
        );
      end;
      ...
      SkillPositiveImages[Index].SetActive(False);
    if BaseLevel > EffectiveLevel then
    begin
      with SkillPanels[Index] do
      begin
        ...
        SetPosition(
            Classes.Point(LocalPosition.X, SkillImageRestTop[Index] + Bottom - Step * BaseLevel)
        );
      end;
      ...
      SkillNegativeImages[Index].SetActive(False);
  end;
begin
  with FreeSkillPointsLabel do
    SetText(IntToStr(ShipToInspect.FreeExperience));
  UpdateOne(
      0,
      ...
      if GiResourceVariant = 1 then
        SetText(
            '<Object='
                ...
                + IntToStr(17)
                + ',0>'
                + ReplaceAllWideString(Caption, '<br>', #13#10)
        )
      else
        SetText(
            '<Object='
