...
type
  TfJournal = class;
  TfJournal = class(TMessageLoopGIWithMainPanel)
    ...
    procedure RefreshTelevisionAnimation(Sender: TObjectGI);
    procedure ClearEntries;
    procedure FinishEntries;
    procedure AddEntrySpacing(Height: Integer);
    procedure AddEntryHeading(
        Text: WideString;
        MessageText: WideString;
        Compact: Integer;
        RecordIndex: Integer
    );
    procedure AddEntryText(Text: WideString; Align: TTextAlignXGI; FontName: WideString);
    procedure ToggleJournalNews(Sender: TObjectGI);
    ...
    function HasTelevisionReception: Boolean;
    procedure PinEntryClicked(Sender: TObjectGI);
    procedure DeleteEntryClicked(Sender: TObjectGI);
    procedure ClearEntriesConfirmed;
    procedure ExportEntriesConfirmed;
    procedure RebuildNewsEntries;
    procedure RebuildJournalEntries;
    procedure AddRecordClicked(Sender: TObjectGI);
    procedure CloseClicked(Sender: TObjectGI);
    procedure ClearInputClicked(Sender: TObjectGI);
    procedure CopyInputClicked(Sender: TObjectGI);
    procedure PasteInputClicked(Sender: TObjectGI);
  end;
...
begin
  ...
  if GetPlayer <> nil then
    GetPlayer.RefreshNewsAtLocation;
  with GetByName('MainPanel') do
  begin
...
var
  ...
  TelevisionClipNames: array[0..19] of WideString = (
      'a',
      'b',
      'c',
      'd',
...
var
  ...
  TelevisionClipWeights: array[0..19] of Integer = (
      9,
      5,
      9,
      9,
      ...
      5,
      3
  ); // @addr $87AFBC Selection uses each weight plus one. @indexrefs "$561935,$561965"
...
begin
  Choice := 0;
  for I := 0 to 19 do
    Inc(Choice, TelevisionClipWeights[I] + 1);
  ...
  begin
    ...
    if HasTelevisionReception then
      RestartPlayback
    else
      StopAutoPlayback;
  end;
...
procedure TfJournal.AddEntryHeading(
    Text, MessageText: WideString;
    Compact: Integer;
    RecordIndex: Integer
);
var
...
begin
  ...
  if Compact = 0 then
    Image.SetPosition(
        Types.Point(InfoPanel.ClientSize.X div 2 - Image.ClientSize.X div 2, ContentHeight)
    )
  else
    Image.SetPosition(Types.Point(InfoPanel.ClientSize.X - Image.ClientSize.X, ContentHeight));
  Image.SetDepth(1);
  ...
  begin
    ...
    if Compact <> 0 then
      SetSize(Types.Point(ClientSize.X - GiScalePixels(20), ClientSize.Y));
    SetPosition(
        Types
            .Point(Image.LocalPosition.X + Image.ClientSize.X - ClientSize.X, Image.LocalPosition.Y)
    );
  ...
  if MessageText <> '' then
    ...
    begin
      ...
      if Compact = 0 then
        SetPosition(
            Types.Point(
                Image.LocalPosition.X + Image.ClientSize.X - ClientSize.X - GiScalePixels(10),
          ...
          SetPosition(
              Types.Point(
                  Image.LocalPosition.X
                      ...
                      - GiScalePixels(15)
                      - PinWidth,
                  Image.LocalPosition.Y - GiScalePixels(3)
              )
          );
          SetKind(gbkDisable);
          SetPositionModeW(True);
...
begin
  ...
  begin
    if Key = Ord('C') then
      CopyInputClicked(nil)
    else if Key = Ord('V') then
      PasteInputClicked(nil)
    else if Key = VK_BACK then
      ClearInputClicked(nil);
  end;
  if not IsVirtualKeyDown(VK_SHIFT) and not IsVirtualKeyDown(VK_MENU) then
  begin
    if IsVirtualKeyDown(VK_CONTROL) and (Key = VK_DELETE) then
      ClearEntriesConfirmed
    else if IsVirtualKeyDown(VK_CONTROL) and (Key = VK_INSERT) then
      ExportEntriesConfirmed
    else if not IsVirtualKeyDown(VK_CONTROL) then
    begin
      Position := InfoPanel.VerticalScrollBar.Position;
      if Key = VK_PRIOR then
        Dec(Position, InfoPanel.VerticalScrollBar.LargeChange);
      if Key = VK_NEXT then
        Inc(Position, InfoPanel.VerticalScrollBar.LargeChange);
      if Key = VK_UP then
        Dec(Position, InfoPanel.VerticalScrollBar.SmallChange);
      if Key = VK_DOWN then
        Inc(Position, InfoPanel.VerticalScrollBar.SmallChange);
      if Position < InfoPanel.VerticalScrollBar.Minimum then
        Position := InfoPanel.VerticalScrollBar.Minimum;
      if Position > InfoPanel.VerticalScrollBar.Maximum then
      ...
      else if (Key = VK_RETURN) and JournalSelected then
        AddRecordClicked(nil)
      else if (Key = VK_ESCAPE) or (Key = VK_F1) then
        CloseClicked(nil);
    end;
  end;
end;
...
begin
  if Delta = WHEEL_DELTA then
    InfoPanel.VerticalScrollBar.SetPosition(
        InfoPanel.VerticalScrollBar.Position - InfoPanel.VerticalScrollBar.SmallChange
    )
...
begin
  if ShowMessageBoxGI(
          Self,
          LookupLocalizedTextByKey('FormInfo.DelRecord'),
          mbgOK or mbgCancel or mbgQuestion)
      = mbgResultOK then
  begin
    if GetPlayer <> nil then
      GetPlayer.DeleteJournalRecord(Sender.UserValue);
    Position := InfoPanel.VerticalScrollBar.Position;
    RebuildJournalEntries;
    if Position < InfoPanel.VerticalScrollBar.Minimum then
      Position := InfoPanel.VerticalScrollBar.Minimum;
...
begin
  ...
  begin
    ...
    begin
      if ShowMessageBoxGI(
              Self,
              LookupLocalizedTextByKey('FormInfo.ClearRecord'),
              mbgOK or mbgCancel or mbgQuestion)
          = mbgResultOK then
      begin
    ...
    begin
      if ShowMessageBoxGI(
              Self,
              LookupLocalizedTextByKey('FormInfo.ClearNews'),
              mbgOK or mbgCancel or mbgQuestion)
          = mbgResultOK then
      begin
  ...
  begin
    ...
    begin
      if GetPlayer.JournalRecords.Count > 0 then
        if ShowMessageBoxGI(
                Self,
                LookupLocalizedTextByKey('FormInfo.ExtractRecord'),
                mbgOK or mbgCancel or mbgQuestion)
            = mbgResultOK then
        begin
      ...
      if ShowMessageBoxGI(
              Self,
              LookupLocalizedTextByKey('FormInfo.ExtractNews'),
              mbgOK or mbgCancel or mbgQuestion)
          = mbgResultOK then
      begin
