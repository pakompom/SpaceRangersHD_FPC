...
type
  TfStarMap = class;
  {$Z1}
  TStarMapMode = (smmInactive = 0, smmOrders = 1, smmTurnFilm = 2);
  {$Z1}
  TStarMapResumeMode = (smrNormal = 0, smrOrders = 1, smrTurnFilm = 2, smrWaitForTurn = 3);
  {$Z4}
  TStarMapPathKind = (smpNone = 0, smpAsteroid = 1, smpShip = 2);
  TStarMapReservedEntry = packed record
  ...
  TfStarMap = class(TMessageLoopGIWithMainPanel)
    ...
    LargeHelpBuffer: TGraphBufGI;
    LargeHelpText: WideString;
    LargeHelpTimer: PCallbackTimerGI;
    LargeHelpProgress: Single;
    PendingHoleRefresh: THole;
    ...
    PlayerPathTimer: PCallbackTimerGI;
    ScannerSelectionActive: Boolean;
    TalkSelectionActive: Boolean;
    ...
    CustomSelectionColor: Cardinal;
    CustomSelectionAllowedCursor: WideString;
    CustomSelectionDeniedCursor: WideString;
    CustomSelectionSuccessText: WideString;
    CustomSelectionOutOfRangeText: WideString;
    CustomSelectionFailureText: WideString;
    Gap150: array[0..3] of Byte;
    MinimapPathKind: TStarMapPathKind;
    WeaponPanel: TPanelGI;
    WeaponPanelRestTop: Integer;
    WeaponPanelProgress: Single;
    WeaponPanelTarget: Integer;
    WeaponPanelTimer: PCallbackTimerGI;
    SpacePanel: TPanelGI;
    SpacePanelRestTop: Integer;
    SpacePanelProgress: Single;
    ...
    FilmStepIndex: PtrInt;
    NextFilmCommand: PEFilmCommand;
    FilmProgressTimer: PCallbackTimerGI;
    ContinueTurnCalculation: Boolean;
    BreakRequested: Boolean;
    BreakOnNextFilm: Boolean;
    Flag19F: Boolean;
    ReservedFilmState1A0: Integer;
    FilmFrameIntervalMs: Single;
    FilmFrameIntervalDelta: Single;
    FilmCameraPosition: TPointF;
    FilmCameraTarget: TPointF;
    FilmCameraTargetUntilStep: Integer;
    FilmCameraEventIndex: Integer;
    FilmCameraTargetKind: Integer;
    FilmCameraShakeAngle: Single;
    ...
    Gap1E8: array[0..3] of Byte;
    TrailingEffectSteps: Integer;
    DisplayedFilmObject: TObjectSE;
    MapControls: TPanelGI;
    PartnerPanel: TPanelGI;
    SecondaryPartnerPanel: TPanelGI;
    InfoWindow: TWindowGI;
    InfoTextLabel: TLabelGI;
    ItemInfoWindow: TWindowGI;
    ShipInfoPanel: TPanelGI;
    PlanetInfoPanel: TPanelGI;
    StarInfoWindow: TWindowGI;
    StandardInfoPanel: TPanelGI;
    StarField: TStarFieldGI;
    ActionCircle: TCircleGI;
    ActionColorCircle: TSpaceCircleGI;
    WeaponColorCircles: array[0..3] of TSpaceCircleGI;
    WeaponButtons: array[0..4] of TGraphButtonGI;
    WeaponImages: array[0..4] of TImageGI;
    AllWeaponsButton: TGraphButtonGI;
    HideSpacePanelButton: TGraphButtonGI;
    ShowSpacePanelButton: TGraphButtonGI;
    ScannerButton: TGraphButtonGI;
    TalkButton: TGraphButtonGI;
    TurnFilmButton: TGraphButtonGI;
    WeaponBackgroundImage: TImageGI;
    SpaceBackgroundImage: TImageGI;
    ShipToInspect: TShip;
    HitObjectPosition: TPoint;
    HitObjectSize: TPoint;
    BattleMusicSelected: Boolean;
    EndTurnAfterOpen: Boolean;
    Gap296: array[0..1] of Byte;
    PendingSceneObjects: TList;
    StarThumbnailCache: TStringList;
    StarThumbnailTurn: Integer;
    StarThumbnailSystem: TStar;
    procedure ClearStarThumbnails;
    procedure LoadStarThumbnail(Obj: TObject; Buffer: TGraphBufGR; Size: TPoint);
    ...
    function GetMapCenter: TPoint;
    procedure SetMapCenterManually(Point: TPoint);
    procedure SetMapCenter(Center: TPoint);
    procedure CenterMapForTalk(Position: TPointF);
    procedure RestorePendingSceneObjects;
    procedure BuildSpaceBackground(
        StarField: TStarFieldGI;
        SpaceImage: TSpaceImgGI;
        Seed: Cardinal;
    ...
    procedure OpenFilmHistoryClicked(Sender: TObjectGI);
    procedure RefreshScoreModsLabel;
    procedure BuildShipPathOverlay(
        Ship: TShip;
    ...
    procedure ClearPartnerButtons;
    procedure RebuildPartnerButtons;
    procedure PartnerClicked(Sender: TObjectGI);
    procedure PartnerRightButtonDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure PartnerMouseEnter(Sender: TObjectGI);
    procedure PartnerMouseLeave(Sender: TObjectGI);
    procedure AddMapAnimation(Position: TPointF; ImagePath: WideString; DelayMs: Integer);
    procedure ClearMapAnimations;
    procedure MapAnimationFinished(Sender: TObjectGI);
    function FindObjectAtCursor: TObject;
    ...
    // FPC host boundary: a local world can finish playback without campaign jobs.
    function CompleteHostedTurnFilm: Boolean; virtual;
    procedure MapRightButtonDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure MapMouseMove(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure OrderKeyDown(Sender: TObjectGI; Key: Cardinal);
    procedure OrderKeyUp(Sender: TObjectGI; Key: Cardinal);
    procedure SelectAllUsableWeapons;
    procedure SelectUntargetedWeapons;
    procedure ShowObjectInfo(Obj: TObject);
    procedure MapScrollChanged;
    function GetPriceSnapshotKey(Obj: TObject): WideString;
    procedure SaveVisiblePriceSnapshots;
    procedure CenterOnShip(Ship: TShip);
    procedure CenterOnDominator(Selection: Integer);
    procedure CenterShipClicked(Sender: TObjectGI);
    procedure CenterShipMouseEnter(Sender: TObjectGI);
    procedure CenterShipMouseLeave(Sender: TObjectGI);
    procedure AllWeaponsClicked(Sender: TObjectGI);
    procedure ScannerClicked(Sender: TObjectGI);
    procedure TalkClicked(Sender: TObjectGI);
    procedure SelectInterceptorTarget;
    procedure BeginCustomSelection;
    procedure ToggleWeaponPanelClicked(Sender: TObjectGI);
    procedure WeaponButtonDown(Sender: TObjectGI);
    procedure WeaponButtonUp(Sender: TObjectGI);
    procedure RefreshWeaponButtons;
    procedure RefreshActionRanges;
    procedure HideActionRanges;
    procedure RebuildTargetMarkers;
    ...
    procedure CenterFilmShipMouseEnter(Sender: TObjectGI);
    procedure CenterFilmShipMouseLeave(Sender: TObjectGI);
    procedure FilmMouseMove(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    function FindFilmObjectAtCursor(out ObjectId: Cardinal): TObjectSE;
    procedure ShowFilmObjectInfo(Obj: TObjectSE; ObjectId: Cardinal);
    procedure PrepareTalkDisplay;
    procedure WaitForTurnOrTalk;
    procedure UpdateTerronTransformation;
    procedure TerronTransformationStarted(Sender: TObjectGI);
    procedure TerronTransformationFrame(Sender: TObjectGI);
    procedure TerronTransformationFinished(Sender: TObjectGI);
  end;
var
  PanelSlideCurve: array[0..15] of Single = (
      0,
      0.033,
      0.112,
      0.269,
  ...
  FilmCameraLookAheadSteps: Integer = 30;
...
// Cache only images. Rebuild names, ownership, relations and script-provided
// information on every opening. Buffers copied into controls remain owned by
// those controls, so invalidation cannot leave dangling UI references.
procedure TfStarMap.ClearStarThumbnails;
var
  I: Integer;
begin
  if StarThumbnailCache <> nil then
  begin
    for I := 0 to StarThumbnailCache.Count - 1 do
      StarThumbnailCache.Objects[I].Free;
    StarThumbnailCache.Clear;
  end;
  StarThumbnailSystem := nil;
end;
// CHANGE: PERFORMANCE - Reuse rendered thumbnails within the current system and turn.
procedure TfStarMap.LoadStarThumbnail(Obj: TObject; Buffer: TGraphBufGR; Size: TPoint);
var
  Key, Path: WideString;
  Index, Y: Integer;
  Thumbnail: TGraphBufGR;
begin
  if StarThumbnailCache = nil then
  begin
    StarThumbnailCache := TStringList.Create;
    StarThumbnailCache.CaseSensitive := True;
  end;
  if not CacheInfoThumbnails
      or (StarThumbnailSystem <> GetPlayer.CurrentStar)
      or (StarThumbnailTurn <> Galaxy.CurrentTurn)
      or (StarThumbnailCache.Count >= 128) then
  begin
    ClearStarThumbnails;
    StarThumbnailSystem := GetPlayer.CurrentStar;
    StarThumbnailTurn := Galaxy.CurrentTurn;
  end;
  Path := '';
  if Obj is TStar then
    Path := ExtractDelimitedPartW(TStarSE(TStar(Obj).Graphic).StaticImagePath, 1, ',')
  else if Obj is TPlanet then
    Path :=
        TPlanetSE(TPlanet(Obj).Graphic).ImagePath
            + '|'
            + TPlanetSE(TPlanet(Obj).Graphic).RuinsImagePath
  else if Obj is TRuins then
  begin
    if TShip(Obj).Graphic is TRuinsSE then
      Path := ExtractDelimitedPartW(TRuinsSE(TShip(Obj).Graphic).StaticImagePath, 1, ',')
    else
      Path := ExtractDelimitedPartW(TShip2SE(TShip(Obj).Graphic).AlternateImagePath, 1, ',');
  end
  else
    Path := TCustomSystemInfo(Obj).Icon;
  Key :=
      IntToHex(PtrUInt(Obj), SizeOf(Pointer) * 2)
          + ':'
          + IntToStr(Size.X)
          + ':'
          + IntToStr(Size.Y)
  ...
  if Obj is TPlanet then
    Key := Key + ':' + IntToStr(TPlanetSE(TPlanet(Obj).Graphic).Radius);
  Index := StarThumbnailCache.IndexOf(UTF8Encode(Key));
  if Index >= 0 then
    Thumbnail := TGraphBufGR(StarThumbnailCache.Objects[Index])
  else
  begin
    Thumbnail := TGraphBufGR.Create(False);
    try
      if Obj is TPlanet then
        TPlanetSE(TPlanet(Obj).Graphic).RenderToBuffer(Self, Thumbnail, True)
      else
        LoadGiByPathIntoGraphBuf(Path, Thumbnail);
      if (Thumbnail.Width > 0)
          and (Thumbnail.Height > 0)
          and ((Size.X < Thumbnail.Width)
              or (Size.Y < Thumbnail.Height)
              or (Obj is TCustomSystemInfo)) then
      begin
        if Thumbnail.Width >= Thumbnail.Height then
          Thumbnail
              .RescaleRgba(Size.X, Max(1, Round(Size.X / Thumbnail.Width * Thumbnail.Height)), 5)
        else
          Thumbnail
              .RescaleRgba(Max(1, Round(Size.Y / Thumbnail.Height * Thumbnail.Width)), Size.Y, 5);
      end;
      StarThumbnailCache.AddObject(UTF8Encode(Key), Thumbnail);
    except
      Thumbnail.Free;
      raise;
    end;
  end;
  Buffer.AllocateRgbaTight(Thumbnail.Width, Thumbnail.Height);
  for Y := 0 to Thumbnail.Height - 1 do
    Move(
        PByte(Thumbnail.GetPixels)[Y * Thumbnail.PitchBytes],
        PByte(Buffer.GetPixels)[Y * Buffer.PitchBytes],
        Thumbnail.Width * 4
    );
end;
...
begin
  ...
  if PendingSceneObjects <> nil then
    PendingSceneObjects.Free;
...
begin
  ...
  if SpaceProcess.IsSpaceOpen then
...
begin
  if (GetPlayer <> nil) and (Byte(GetPlayer.RuinsMode) > 0) then
    GetPlayer.ExitRuinsMode;
  ...
  if RobotAutoWin and (PlanetBattleState = 1) then
    PlanetBattleState := AutoWinRobotBattle;
  ...
  begin
    ...
    try
      BattleFailed := False;
      PlanetBattleState :=
          FRun(RobotMapDefinitions[MapIndex].Map, StartText, WinText, LossText, TerronName);
    except
      on E: Exception do
      begin
  ...
  begin
    ...
    BuildSpaceBackground(
        GetByName('StarField') as TStarFieldGI,
        GetByName('SpaceImg') as TSpaceImgGI,
        PlayerStar.GenerationSeed,
        PlayerStar.BackgroundImage
    );
      ...
      begin
        ...
        begin
          ...
          if Entry.RelatedObject1 <> nil then
            Entry.RelatedObject1.AttachToSpace(SpaceProcess.Space);
          Entry := Entry.Next;
        end;
      end;
...
begin
  Stage := 0;
  try
    if (Galaxy <> nil) and (Mode = smmOrders) then
      aCalc.WaitForTurnCalculationUI;
    Stage := 1;
    ...
    ClearPartnerButtons;
    Stage := 5;
    if (Galaxy <> nil) and not MemorySnapshotActive then
      SaveSpaceBackground;
    Stage := 6;
    ...
    if SecondaryFilm <> nil then
      SecondaryFilm.ReleaseWeaponSceneObjects;
    Stage := 9;
    ...
    for Index := 0 to Galaxy.Holes.Count - 1 do
      ...
      begin
        with PendingHoleRefresh do
          ...
          begin
            if GetPlayer.CurrentStar = Star1 then
              Graphic.SetPosition(Position1)
            else
              Graphic.SetPosition(Position2);
...
procedure TfStarMap.BuildSpaceBackground(
    StarField: TStarFieldGI;
    SpaceImage: TSpaceImgGI;
    Seed: Cardinal;
    BackgroundIndex: Integer
...
begin
  ...
  for Index := 0 to 3 do
    Colors[Index] :=
        CurrentPixelFormat.PackRgbBytes(100 + 16 * Index, 100 + 16 * Index, 100 + 16 * Index);
  for Index := 0 to 3 do
    Colors[Index + 4] := CurrentPixelFormat.PackRgbBytes(100 + 40 * Index, 1, 1);
  for Index := 0 to 3 do
    Colors[Index + 8] := CurrentPixelFormat.PackRgbBytes(1, 100 + 40 * Index, 1);
  for Index := 0 to 3 do
    Colors[Index + 12] := CurrentPixelFormat.PackRgbBytes(1, 1, 100 + 40 * Index);
  ...
  if (GlobalsV.SpaceImage = 1) and (Count > 1) then
    Count := 1;
  if BackgroundIndex < 10 then
    Text :=
        GameDataConfig
            .GetBlockByPath('StyleNebula')
            .GetParam(WideString('0' + IntToStr(BackgroundIndex)))
  else
    Text :=
        GameDataConfig
            .GetBlockByPath('StyleNebula')
            .GetParam(WideString(IntToStr(BackgroundIndex)));
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        Seed := StepRandomSeed(Seed);
        Inc(Attempts);
      until (SpaceImage.NearestImageDistance(X, Y)
              > RemapClamped(Radius, 2500.0, 4000.0, 3000.0, 6000.0))
          or (Attempts > 100);
    ...
    begin
      ...
      repeat
        ...
        Inc(Attempts);
      until (SpaceImage.NearestImageDistance(X, Y)
              > RemapClamped(Radius, 1500.0, 2000.0, 1500.0, 3000.0))
          or (Attempts > 100);
      SpaceImage.AddImage(SelectSpaceImageTemplate(Galaxy.SpaceEffectKind + 10), X, Y, Depth);
    end;
    if High(Galaxy.SpaceBackgroundEntries) + 1 <= 0 then
      Galaxy.GenerateSpaceBackground(BackgroundIndex);
    for Index := 0 to High(Galaxy.SpaceBackgroundEntries) do
    begin
      Image :=
          SpaceImage.AddImage(
              Galaxy.SpaceBackgroundEntries[Index].ImageIndex,
              Galaxy.SpaceBackgroundEntries[Index].Position.X,
              Galaxy.SpaceBackgroundEntries[Index].Position.Y,
              Galaxy.SpaceBackgroundEntries[Index].Position.Z
          );
  ...
  if BackgroundIndex < 10 then
    StarField.SetBackgroundImage(WideString('Bm.BGO.bg0' + IntToStr(BackgroundIndex)))
  else
    StarField.SetBackgroundImage(WideString('Bm.BGO.bg' + IntToStr(BackgroundIndex)));
...
begin
  ...
  begin
    ...
    aCalc.WaitForTurnCalculationUI;
    if not IsTurnCalculationRunningUI
        and (TurnCalculationPhase <> tcpGalaxyRunning)
        and (TurnCalculationPhase <> tcpPlayerStarRunning) then
    begin
      if GetPlayer.CalculateSpeed = 0 then
        GetPlayer.OrderNone(False);
      if PendingPlayerFollowTarget <> nil then
        GetPlayer.ProcessPendingPlayerFollowTargeting;
      ...
      if not PlayerStarDayPrepared then
        AppendLogLineThreadSafe('Not calc NextDay header');
      GetPlayer.NextDay;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not MainPanel.RemoveDismissibleMessages('GOODS') then
          MainPanel.RemoveDismissibleMessages('');
      end
      else if Key = VK_F1 then
        MainPanel.JournalClicked(nil);
    end;
    if Key = VK_LEFT then
    begin
      ScrollLeftHeld := True;
    ...
    begin
      if Mode = smmOrders then
        CenterShipClicked(nil)
      else
        CenterFilmShipClicked(nil);
    end
    ...
    begin
      if not (GetByName('PM_Break') as TGraphButtonGI).Disabled then
        BreakTurnClicked(nil);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Text := Galaxy.FinalizationName;
      if (Length(Text) = 0) and Galaxy.EditableStateApplied then
        Text := LookupLocalizedTextByKey('Cheat.Warning')
      else if (Length(Text) > 0) and (Galaxy.GetCheatPoints <> 0) then
        Text := Text + ' + ' + LookupLocalizedTextByKey('Cheat.Warning');
      if (Galaxy.DominatorModLevel > 0) and (Length(Text) > 0) then
        Text := Text + ' + ';
      if Galaxy.DominatorModLevel = 1 then
        Text := Text + LookupLocalizedTextByKey('Cheat.Mod_DomikHorrible')
      else if Galaxy.DominatorModLevel = 2 then
        Text := Text + LookupLocalizedTextByKey('Cheat.Mod_DomikNightmare')
      else if Galaxy.DominatorModLevel = 3 then
        Text := Text + LookupLocalizedTextByKey('Cheat.Mod_DomikHellish');
...
procedure TfStarMap.BuildShipPathOverlay(
    Ship: TShip;
    DelayEndImage: Boolean;
    InitialImagePath: WideString
);
...
begin
  ...
  Owner := MapControls;
  LandingTurns := -1;
  if Ship = TerronShip then
    Exit;
  if (Ship.Order = soFollowShip) or ((Ship = GetPlayer) and (PendingPlayerFollowTarget <> nil)) then
  begin
    if (Ship = GetPlayer) and (PendingPlayerFollowTarget <> nil) then
      TargetShip := PendingPlayerFollowTarget
    else
      TargetShip := Ship.OrderTarget as TShip;
    if TargetShip.IsOnPlanet then
    ...
    begin
      ...
      begin
        Angle :=
            HeadingDegreesToRadians(SeededRandomIntRange(0, 259, Ship.Seed * TargetShip.Seed))
                + AngleOffset;
      ...
      begin
        Angle :=
            ArcTan2(Ship.Position.X - TargetPosition.X, -(Ship.Position.Y - TargetPosition.Y))
                + AngleOffset;
        Point.X := TargetPosition.X + Sin(Angle) * Radius;
        Point.Y := TargetPosition.Y - Cos(Angle) * Radius;
      end;
      ...
      begin
        ...
        if OtherShip.InNormalSpace and (PointDistanceSquared(Point, OtherShip.Position) < 900) then
          Break;
        Inc(Index);
      end;
      AngleOffset := AngleOffset + 1.5707963;
      if Index >= Ship.CurrentStar.Ships.Count then
        Break;
    end;
    aCalc.WaitForTurnCalculationUI;
  ...
  begin
    ...
    begin
      ...
      Ship.BuildFullPathTo(
          AddPointsF((Ship.OrderTarget as TPlanet).GetPosition, Ship.OrderDestination)
      );
    end
    else
    begin
      if Ship.OrderTarget is TShip then
        Ship.BuildFullPathTo(
            AddPointsF((Ship.OrderTarget as TShip).Position, Ship.OrderDestination)
        )
      else
        Ship.BuildFullPathTo(
            AddPointsF((Ship.OrderTarget as TPlanet).GetPosition, Ship.OrderDestination)
  ...
  begin
    if Ship = GetPlayer then
      PathImages := GetByName('PlayerPath') as TMultiImageGI
    else
      PathImages := GetByName('ShipPath') as TMultiImageGI;
    ...
    begin
      ...
      begin
        ...
        begin
          if (Point.X <> 1e10) and (Ship = GetPlayer) then
            PathLength := PathLength + PointDistance(Point, Cursor.Position);
          ...
          begin
            ...
            begin
              if Node = Ship.MovementPath.ActiveHead then
    ...
    if (Ship.Order = soLand)
        and not ((Ship = GetPlayer) and (PendingPlayerFollowTarget <> nil)) then
    begin
      if Ship.OrderTarget is TShip then
        EndPosition :=
            TruncatePointF(AddPointsF((Ship.OrderTarget as TShip).Position, Ship.OrderDestination))
      else
        EndPosition :=
            TruncatePointF(
                AddPointsF((Ship.OrderTarget as TPlanet).GetPosition, Ship.OrderDestination)
    ...
    begin
      ...
      begin
        ...
        EndImage.SetImagePath('Bm.PI.PathEndLanding')
      else if Ship.Order = soJumpHole then
        EndImage.SetImagePath('Bm.PI.PathEndJumpHole')
      else
        EndImage.SetImagePath('Bm.PI.PathEndMove');
    end
    else
    begin
      if (PendingPlayerFollowTarget <> nil) and (Ship = GetPlayer) then
    ...
    if Ship = GetPlayer then
      EndImage.SetDepth(UnitPathEndDepth)
    else
      EndImage.SetDepth(ShipPathEndDepth);
    ...
    if Ship = GetPlayer then
      ...
      LabelGI.SetDepth(ShipPathEndDepth);
    ...
    begin
      if ViewPathLength and (Ship = GetPlayer) and (Ship.Order = soMove) then
        LabelGI.SetText(
            IntToStr(Ship.GetMovementPathTurnCount) + ' (' + IntToStr(Round(PathLength)) + ')'
        )
      else
        LabelGI.SetText(IntToStr(Ship.GetMovementPathTurnCount));
    end
    else
...
begin
  ...
  with MapControls do
    NextControl := FirstChild;
  while NextControl <> nil do
  begin
    Control := NextControl;
    NextControl := NextControl.NextSibling;
    if (PlayerPath and (Control.Depth = UnitPathEndDepth))
        or (not PlayerPath and (Control.Depth = ShipPathEndDepth)) then
    begin
  ...
  if PlayerPath then
    with GetByName('PlayerPath') as TMultiImageGI do
      ClearUnits
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if I < 200 then
        Image.SetImagePath('GI,Bm.PI.Path1')
      else
        Image.SetImagePath('GI,Bm.PI.Path2');
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Portrait := '';
            if (Ship.Graphic <> nil)
                and (Ship.Graphic is TShip2SE)
                and (TShip2SE(Ship.Graphic).PanelPartnerImage <> '') then
              Portrait := TShip2SE(Ship.Graphic).PanelPartnerImage
            else if (Ship.Graphic <> nil)
                and (Ship.Graphic is TRuinsSE)
                and (TRuinsSE(Ship.Graphic).PanelPartnerImage <> '') then
              Portrait := TRuinsSE(Ship.Graphic).PanelPartnerImage
            else if Ship.IsFemaleHumanPilot
                and (Ship.TypeId = stRanger)
                and (Ship.GetHull.HullType = htSpecial)
                and (Ship.GetHull.GetSpecialKindGraph = 'J') then
              Portrait := 'Female'
            ...
            if PartnerCount <= 5 then
              SetPosition(
                  Classes.Point(
                      GiScalePixelsEx(23, 20)
                          + PlacedCount * GiScalePixels(25)
                          - ClientSize.X div 2,
            ...
            PartnerPanel.SetSize(
                Classes.Point(
                    Math.Min(
                        GiScalePixelsEx(12, 10)
                            + PlacedCount * GiScalePixels(25)
                            + GiScalePixelsEx(22, 18),
                        PartnerPanel.Parent.ClientSize.X - 10
                    ),
                    PartnerPanel.ClientSize.Y
                )
            );
            PartnerPanel.SetPosition(
                Classes.Point(PartnerPanel.Parent.ClientSize.X - PartnerPanel.ClientSize.X, 0)
            );
            with GetByName('MapPartnerBG2') do
              SetPosition(Classes.Point(PartnerPanel.ClientSize.X - ClientSize.X, 0));
            with GetByName('MapPartnerDuty') do
              SetPosition(Classes.Point(PartnerPanel.LocalPosition.X - 5, LocalPosition.Y));
          end;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    if (FilmObject = nil) or (FilmObject.SceneObject = nil) then
      Exit;
  ...
  begin
    ...
    if (GetPlayer.CurrentStar <> Ship.CurrentStar) or Ship.InHyperspace then
      Exit;
    if Ship.InNormalSpace then
      SetMapCenterManually(TruncatePointF(Ship.Position))
    else if Ship.IsOnPlanet then
      SetMapCenterManually(TruncatePointF(Ship.CurrentPlanet.GetPosition))
    else if Ship.IsDockedToShip then
      SetMapCenterManually(TruncatePointF(Ship.DockedTo.Position))
    else
      Exit;
  end;
...
begin
  ...
  begin
    ...
    if (Missile <> nil)
        and (Missile.GetGraphObject <> nil)
        and Missile.GetGraphObject.HitTestCursor then
    begin
      HitObjectPosition :=
          Classes.Point(
              Round(Missile.GetGraphObject.Position.X) - GetMapCenter.X,
              Round(Missile.GetGraphObject.Position.Y) - GetMapCenter.Y
          );
  ...
  begin
    ...
    if not (Ship is TRuins)
        and (Ship <> nil)
        and (Ship.Graphic <> nil)
        and Ship.Graphic.HitTestCursor then
    begin
      HitObjectPosition :=
          Classes.Point(
              Round(Ship.Graphic.Position.X) - GetMapCenter.X,
              Round(Ship.Graphic.Position.Y) - GetMapCenter.Y
          );
  ...
  begin
    ...
    begin
      HitObjectPosition :=
          Classes.Point(
              Round(Item.GetGraphObject.Position.X) - GetMapCenter.X,
              Round(Item.GetGraphObject.Position.Y) - GetMapCenter.Y
          );
  ...
  begin
    ...
    if (Asteroid <> nil)
        and (Asteroid.GraphObject <> nil)
        and Asteroid.GraphObject.HitTestCursor then
    begin
      HitObjectPosition :=
          Classes.Point(
              Round(Asteroid.GraphObject.Position.X) - GetMapCenter.X,
              Round(Asteroid.GraphObject.Position.Y) - GetMapCenter.Y
          );
  ...
  begin
    ...
    begin
      ...
      begin
        HitObjectPosition :=
            Classes.Point(
                Round(Planet.Graphic.Position.X) - GetMapCenter.X,
                Round(Planet.Graphic.Position.Y) - GetMapCenter.Y
            );
    ...
    else if Sqr(Point.X - Planet.GetPosition.X) + Sqr(Point.Y - Planet.GetPosition.Y)
        < Sqr(Planet.GraphicRadius) then
    begin
      HitObjectPosition :=
          Classes.Point(
              Round(Planet.Graphic.Position.X) - GetMapCenter.X,
  ...
  begin
    HitObjectPosition :=
        Classes.Point(
            Round(PlayerStar.Graphic.Position.X) - GetMapCenter.X,
            Round(PlayerStar.Graphic.Position.Y) - GetMapCenter.Y
        );
  ...
  begin
    ...
    begin
      if PointDistanceSquared(Hole.Graphic.Position, PointToPointF(Point))
          < Sqr(THoleSE(Hole.Graphic).HitRadius) then
      begin
        HitObjectPosition :=
            Classes.Point(
                Round(Hole.Graphic.Position.X) - GetMapCenter.X,
                Round(Hole.Graphic.Position.Y) - GetMapCenter.Y
            );
...
begin
  ...
  begin
    ...
    begin
      ...
      Color :=
          $FF000000
              or (Cardinal(Round(Intensity * 224.0)) shl 16)
              or (Cardinal(Round(Intensity * 225.0)) shl 8)
              or Cardinal(Round(Intensity * 183.0));
      RenderLabelTextToBuffer(
          LargeHelpBuffer.GraphBuf,
          Right - Left + 1,
          0,
          0,
          LargeHelpText,
...
begin
  ...
  try
    ...
    Stage := 11;
    if not ShipScreen.FlagD4
        and ((TalkScreen = nil) or (TalkScreen.Flag128 = 0))
        and ((GoodsShopScreen = nil) or not GoodsShopScreen.FlagEC) then
    begin
...
begin
  ...
  if StarMapWeaponPanelOpen then
    WeaponPanelProgress := 1.0
  else
    WeaponPanelProgress := 0.0;
  ...
  ScannerButton.SetDisabled(
      (not GetPlayer.IsEquipmentUsable(GetPlayer.GetRadar)
              or not GetPlayer.IsEquipmentUsable(GetPlayer.GetScanner))
          and (Galaxy.UltraScanModEnabled = 0)
  );
...
begin
  ...
  Point := OldPoint;
  if ScrollLeftHeld then
    Dec(Point.X, ScrollStep);
  if ScrollRightHeld then
    Inc(Point.X, ScrollStep);
  if ScrollUpHeld then
...
begin
  Result := True;
  if (Sender <> nil) and Sender.IsOccludedAtPoint(Point) then
    Exit;
  if MainPanel.BackgroundImage.HitTestPixel(Point) then
    Exit;
  if WeaponPanel.Active and WeaponBackgroundImage.HitTestPixel(Point) then
    Exit;
  if SpaceBackgroundImage.HitTestPixel(Point) then
    Exit;
  if WeaponPanel.Active and WeaponButtons[0].HitTest(Point) then
    Exit;
  if WeaponPanel.Active and WeaponButtons[1].HitTest(Point) then
    Exit;
  if WeaponPanel.Active and WeaponButtons[2].HitTest(Point) then
    Exit;
  if WeaponPanel.Active and WeaponButtons[3].HitTest(Point) then
    Exit;
  if WeaponPanel.Active and WeaponButtons[4].HitTest(Point) then
    Exit;
  if WeaponPanel.Active and AllWeaponsButton.HitTest(Point) then
    Exit;
  if HideSpacePanelButton.HitTest(Point) then
    Exit;
  if ShowSpacePanelButton.HitTest(Point) then
    Exit;
  if ScannerButton.HitTest(Point) then
    Exit;
  if TalkButton.HitTest(Point) then
    Exit;
  if TurnFilmButton.HitTest(Point) then
    Exit;
  if MainPanel.ShipButton.HitTest(Point) then
    Exit;
  if MainPanel.GalaxyButton.HitTest(Point) then
    Exit;
  if MainPanel.QuestButton.HitTest(Point) then
    Exit;
  if MainPanel.EndTurnButton.HitTest(Point) then
    Exit;
  if MainPanel.MenuButton.HitTest(Point) then
    Exit;
  Result := False;
end;
...
begin
  ...
  begin
    TargetPosition.X := 0;
    TargetPosition.Y := 0;
    if CursorObject is TShip then
      TargetPosition := TShip(CursorObject).Position
    else if CursorObject is TMissile then
      TargetPosition := TMissile(CursorObject).Position
    else if CursorObject is TAsteroid then
      TargetPosition := TAsteroid(CursorObject).Position
    else if CursorObject is TItem then
      TargetPosition := TItem(CursorObject).Position
    else if CursorObject is TPlanet then
      TargetPosition := TPlanet(CursorObject).GetPosition
    else if CursorObject is THole then
    begin
      // Kept by the native routine despite the outer hole exclusion.
      if THole(CursorObject).Star1 = PlayerStar then
        TargetPosition := THole(CursorObject).Position1
      else
        TargetPosition := THole(CursorObject).Position2;
    end;
    if PointDistanceSquared(GetPlayer.Position, TargetPosition)
        > CustomSelectionRadius * CustomSelectionRadius then
  ...
  begin
    if ((ScannerSelectionActive or TalkSelectionActive) and not (CursorObject is TShip))
        or ((CursorObject is TShip)
            and ((ScannerSelectionActive
                    and ((CursorObject as TShip).NoScan or GetPlayer.ScanLocked))
                or (TalkSelectionActive
                    and ((CursorObject as TShip).NoTalk or GetPlayer.TalkLocked))
                or ((HadWeapons or InterceptorSelectionActive)
                    and not GetPlayer.CanSelectShipTarget(CursorObject as TShip)))) then
    begin
      if ScannerSelectionActive then
        ShowLargeHelp(LookupLocalizedTextByKey('Help.ScanImpossible'))
      else if TalkSelectionActive then
        ShowLargeHelp(LookupLocalizedTextByKey('Help.TalkImpossible'))
      else if HadWeapons then
  ...
  if (GetPlayer.CalculateSpeed <= 0)
      and not HadWeapons
      and not ScannerSelectionActive
      and not TalkSelectionActive
      and not InterceptorSelectionActive
      and not (CursorObject is TItem) then
  begin
    SetCursorActive(False);
    DrawFrame;
    SetCursorActive(True);
  ...
  begin
    if (CursorObject is TPlanet) and (CursorObject as TPlanet).NoLanding then
      CursorObject := nil
    else if CursorObject is TRuins then
    begin
      if HadWeapons or InterceptorSelectionActive then
        if not GetPlayer.CanSelectShipTarget(CursorObject as TRuins) then
          CursorObject := nil;
      ...
      if GetPlayer.NoJump or ((CursorObject as THole).ArcadeMapName = 'NoEntry') then
        CursorObject := nil;
  end;
  if CursorObject = nil then
  begin
    ...
    begin
      ...
      begin
        ...
        if FollowingNode = nil then
          Break;
        if FollowingNode.Next = nil then
          Break;
      ...
      begin
        ...
        while (FollowingNode <> nil)
            and (PointDistanceSquared(FollowingNode.Position, Node.Position) < 25) do
  ...
  else if (CursorObject is TRuins)
      or ((TerronShip = CursorObject)
          and (Galaxy.TerronLandingLockTurn > 0)
          and not TalkSelectionActive) then
  begin
    ...
    begin
      ...
      aCalc.WaitForTurnCalculationUI;
      if Mode = 1 then
        ShowLargeHelp(LookupLocalizedTextByKey('Help.ShotSet'))
      else if Mode = 2 then
    ...
    begin
      if (PointDistance(GetPlayer.Position, (CursorObject as TShip).Position) <= 1000)
          and ((CursorObject as TRuins).InterceptorPassesRemaining = 0) then
      begin
    ...
    else if not (((GetPlayer.Order = soLand) and (GetPlayer.OrderTarget = Ship))
        or ((Byte(Ship.GetRelationLevelToShip(GetPlayer)) <= 0)
            and ((Ship <> TerronShip) or (Galaxy.TerronLandingLockTurn <= 0)))) then
    begin
  ...
  begin
    ...
    if ScannerSelectionActive then
    begin
      if (PointDistance(GetPlayer.Position, Ship.Position) <= GetPlayer.GetRadarRange)
          and (GetPlayer <> CursorObject)
          and GetPlayer.CanScanShip(Ship)
          and ((Galaxy.UltraScanModEnabled <> 0)
              or GetPlayer.CanResolveObjectWithScanner(Ship)
      ...
      else if PointDistance(GetPlayer.Position, (CursorObject as TShip).Position)
      ...
      begin
        ...
        if not GetPlayer.CanResolveObjectWithScanner(CursorObject)
            and ((CursorObject as TShip).TypeId <> stTranclucator) then
        begin
          ScanUnresolved := False;
    ...
    begin
      if (PointDistance(GetPlayer.Position, (CursorObject as TShip).Position)
          ...
          and (GetPlayer <> CursorObject)
          and not ((CursorObject is TKling)
              and (CursorObject as TKling).IsProgramActive(prgDisconnection)) then
      begin
        HideLargeHelp;
    ...
    begin
      if (PointDistance(GetPlayer.Position, (CursorObject as TShip).Position) <= 1000)
          and ((CursorObject as TShip).InterceptorPassesRemaining = 0) then
      begin
    ...
    begin
      ...
      aCalc.WaitForTurnCalculationUI;
      GetPlayer.OrderNone(False);
      if GetPlayer.CanSelectShipTarget(CursorObject as TShip) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Byte(Ship.GetRelationLevelToShip(GetPlayer)) <= 0 then
              Mode := 1
            else
              Mode := 2;
          end;
        ...
        begin
          ...
          begin
            if Mode = 1 then
              InitialImage := 'Bm.PI.PathEndAutoBattle'
            else if Mode = 2 then
              InitialImage := 'Bm.PI.PathEndFollowNear'
            else if Mode = 3 then
              InitialImage := 'Bm.PI.PathEndFollowMin'
            else
              InitialImage := 'Bm.PI.PathEndFollowMax';
          end;
          Inc(Mode);
          if Mode > 3 then
            Mode := 1;
        ...
        aCalc.WaitForTurnCalculationUI;
        if Mode = 1 then
          if not GetPlayer.CanSelectShipTarget(CursorObject as TShip) then
            Mode := 2;
        if Mode = 1 then
        begin
          GetPlayer.OrderNone(False);
        ...
        aCalc.WaitForTurnCalculationUI;
        if AnimatePath then
          BuildShipPathOverlay(GetPlayer, True, InitialImage)
        else
          BuildShipPathOverlay(GetPlayer, False, '');
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        aCalc.WaitForTurnCalculationUI;
        if GetPlayer.HasPickupTarget(TItem(CursorObject)) then
          GetPlayer.RemovePickupTarget(TItem(CursorObject))
        else
          GetPlayer.AddPickupTarget(TItem(CursorObject), False);
        aCalc.WaitForTurnCalculationUI;
        RebuildTargetMarkers;
      end;
  end
  ...
  if HadWeapons and (SelectedCount <= 0) and not OutOfRange then
    ShowLargeHelp(LookupLocalizedTextByKey('Help.ShotCancel'));
  if ScanUnresolved then
    ShowLargeHelp(LookupLocalizedTextByKey('Help.ScanImpossible'));
  if TalkUnresolved then
    ShowLargeHelp(LookupLocalizedTextByKey('Help.TalkImpossible'));
  RefreshActionRanges;
  RefreshWeaponButtons;
  UpdateActionCursor(False);
...
begin
  ...
  begin
    ...
    if (CursorObject is THole)
        and not GetPlayer.NoJump
        and ((CursorObject as THole).ArcadeMapName <> 'NoEntry') then
    begin
      Hole := CursorObject as THole;
      ClearPathOverlay(True);
      aCalc.WaitForTurnCalculationUI;
      ...
      aCalc.WaitForTurnCalculationUI;
        ...
        and (((CursorObject is TRuins) and (CursorObject as TRuins).CanDock(GetPlayer))
            or ((CursorObject = TerronShip) and (Galaxy.TerronLandingLockTurn > 0))) then
    begin
      Ship := CursorObject as TShip;
      ClearPathOverlay(True);
      aCalc.WaitForTurnCalculationUI;
      ...
      aCalc.WaitForTurnCalculationUI;
        ...
        and (CursorObject is TShip)
        and (GetPlayer <> CursorObject)
        and (RightClickOnShip <> 2) then
    begin
      ClearPathOverlay(True);
      Ship := CursorObject as TShip;
      FollowMode := 0;
      ...
      begin
        ...
        if (FollowMode < 0) or (FollowMode > 3) then
          FollowMode := 0;
        if DefaultOrder = 0 then
        begin
          if Byte(Ship.GetRelationLevelToShip(GetPlayer)) <= 0 then
      ...
      aCalc.WaitForTurnCalculationUI;
        ...
        and (CursorObject is TPlanet)
        and not (CursorObject as TPlanet).NoLanding then
    begin
    ...
    begin
      CameraVector.X := GetPlayer.Position.X - TItem(CursorObject).Position.X;
      CameraVector.Y := GetPlayer.Position.Y - TItem(CursorObject).Position.Y;
      Scale :=
          GetPlayer.GetCargoHookRange
              / 4
              * (1 / Sqrt(CameraVector.X * CameraVector.X + CameraVector.Y * CameraVector.Y));
      CameraVector.X := Round(TItem(CursorObject).Position.X + Scale * CameraVector.X);
      CameraVector.Y := Round(TItem(CursorObject).Position.Y + Scale * CameraVector.Y);
      aCalc.WaitForTurnCalculationUI;
      PendingPlayerFollowTarget := nil;
      GetPlayer.OrderMove(CameraVector, False);
      if CanCargoHookHandleItem(TItem(CursorObject), GetPlayer)
          and not GetPlayer.IsRecentlyDroppedItem(TItem(CursorObject)) then
      begin
        if GetPlayer.HasPickupTarget(TItem(CursorObject)) then
          GetPlayer.RemovePickupTarget(TItem(CursorObject));
...
begin
  ...
  begin
    ...
    else if (CursorObject is TTranclucator)
        and ((CursorObject as TTranclucator).OwnerShip = GetPlayer) then
    begin
    ...
    begin
      ...
      begin
        if (RightClickOnShip = 0)
            and ((GetPlayer.IsEquipmentUsable(GetPlayer.GetScanner)
                    and GetPlayer.IsEquipmentUsable(GetPlayer.GetRadar))
                or (Galaxy.UltraScanModEnabled = 1)) then
        begin
          if (PointDistance(GetPlayer.Position, Ship.Position) <= GetPlayer.GetRadarRange)
              and (Ship <> GetPlayer)
              and GetPlayer.CanScanShip(Ship)
              and ((Galaxy.UltraScanModEnabled <> 0)
                  ...
                  or (Ship.TypeId = stTranclucator)) then
          begin
            if Ship.NoScan or GetPlayer.ScanLocked then
              ShowLargeHelp(LookupLocalizedTextByKey('Help.ScanImpossible'))
            else
            begin
              HideLargeHelp;
          ...
          begin
            if PointDistance(GetPlayer.Position, Ship.Position) > GetPlayer.GetRadarRange then
              ShowLargeHelp(LookupLocalizedTextByKey('Help.ScanOutRange'))
            else if (CursorObject <> GetPlayer)
                and not (CursorObject is TRuins)
                and not GetPlayer.CanResolveObjectWithScanner(CursorObject)
                and (Ship.TypeId <> stTranclucator) then
              ShowLargeHelp(LookupLocalizedTextByKey('Help.ScanPowerLow'));
          end;
        end
        ...
        begin
          if (PointDistance(GetPlayer.Position, Ship.Position) <= GetPlayer.GetRadarRange)
              and (CursorObject <> GetPlayer)
              and not ((CursorObject is TKling)
                  and (CursorObject as TKling).IsProgramActive(prgDisconnection)) then
          begin
            if Ship.NoTalk or GetPlayer.TalkLocked then
              ShowLargeHelp(LookupLocalizedTextByKey('Help.TalkImpossible'))
            else
            begin
              HideLargeHelp;
        ...
        begin
          ...
          begin
            Inc(FollowMode);
            if FollowMode > 3 then
              FollowMode := 1;
          end;
          if not GetPlayer.CanSelectShipTarget(Ship) and (FollowMode = 3) then
            FollowMode := 1;
          aCalc.WaitForTurnCalculationUI;
          if FollowMode = 1 then
            if not GetPlayer.CanSelectShipTarget(CursorObject as TShip) then
          ...
          BuildShipPathOverlay(GetPlayer, False, '');
            ...
            and (PointDistance(GetPlayer.Position, (CursorObject as TPlanet).GetPosition)
                <= GetPlayer.GetRadarRange))
        or ((CursorObject is TRuins)
            and (CursorObject as TRuins).CanDock(GetPlayer)
            and not (CursorObject as TRuins).NoTalk
            and (PointDistance(GetPlayer.Position, (CursorObject as TRuins).Position)
                <= GetPlayer.GetRadarRange)) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not ScrollLeftHeld
            and not ScrollRightHeld
            and not ScrollUpHeld
            and not ScrollDownHeld then
          ShowObjectInfo(FindObjectAtCursor)
...
begin
  ...
  begin
    ...
    begin
      if (GetPlayer <> nil) and GetPlayer.GetHull.InterceptorsEnabled then
    ...
    begin
      ...
      MainPanel.QuestClicked(nil)
    else if Key = VK_ESCAPE then
      MainPanel.MenuClicked(nil)
    else if Key = Ord('H') then
      OpenFilmHistoryClicked(nil)
    else if Key = Ord('W') then
      ToggleWeaponPanelClicked(nil)
    else if (Key = Ord('F')) and (Mode = smmOrders) then
    begin
      CanAfterburn :=
          (GetPlayer.GetSlotCount(sskAfterburner) > 0)
              and GetPlayer.IsEquipmentUsable(GetPlayer.GetEngine)
      ...
      begin
        ...
        begin
          ...
          while Index >= 0 do
          begin
            if Index >= GetPlayer.ScriptShipBindings.Count then
              Index := GetPlayer.ScriptShipBindings.Count - 1
            else
            begin
...
begin
  ...
  if (Obj <> nil) and (Obj is TAsteroid) then
    ShowAsteroidPath(Obj as TAsteroid)
  else
    ClearAsteroidPath;
  if (Obj = nil) or (DisplayedObject <> Obj) then
  begin
    if (Obj <> nil) and (Obj is TShip) then
      TShip(Obj).ScriptItemsAct(satOnShowingShipInfo, nil, nil, 0);
    if (Obj <> nil) and (Obj is TStar) and (GetPlayer <> nil) then
      GetPlayer.ScriptItemsAct(satOnShowingStarInfo, Obj, nil, 0);
    if Obj = nil then
    begin
      InfoWindow.SetActive(False);
      ItemInfoWindow.SetActive(False);
      ...
      DisplayedObject := nil;
        ...
        or ((Obj is TItem)
            and (PointDistance(TItem(Obj).Position, GetPlayer.Position) > GetPlayer.GetRadarRange))
        or ((Obj is TShip)
            and ((GetPlayer.CurrentStar <> TShip(Obj).CurrentStar)
                or (TShip(Obj).InHyperspace and (Obj <> TerronShip))
                or (PointDistance(TShip(Obj).Position, GetPlayer.Position)
                    > GetPlayer.GetRadarRange)))
        or ((Obj is TAsteroid)
            and (PointDistance(TAsteroid(Obj).Position, GetPlayer.Position)
                > GetPlayer.GetRadarRange))
        or ((Obj is TMissile)
            and (PointDistance(TMissile(Obj).Position, GetPlayer.Position)
                > GetPlayer.GetRadarRange))
        or (Obj is THole)
        or (Obj is TPlanet) then
    begin
      InfoWindow.SetActive(False);
      ItemInfoWindow.SetActive(False);
      ShipInfoPanel.SetActive(False);
      ...
      begin
        ...
        begin
          ...
          if ItemObject.ScriptItem <> nil then
            TScriptItem(ItemObject.ScriptItem)
                .RunActionCode(satOnShowingItemInfo, nil, PlayerStar, nil, 0);
          if ItemObject is TEquipmentWithActCode then
            RunItemConfigActionCode(ItemObject, satOnShowingItemInfo, nil, PlayerStar, nil, 0);
        end;
        ...
        begin
          ...
          if Obj is TGoods then
            SetImagePath('GI,' + GetItemTypeBitmapPath(TItem(Obj).ItemType))
          else
            SetImagePath('GI,' + TItem(Obj).GetBitmapResourceName + 's');
          SetImageKindX(ikxCenter);
          SetImageKindY(ikyCenter);
        ...
        begin
          (GetByName('InfoStdName') as TLabelGI)
              .SetText(
                  WrapTextInColor(
                      GoodsMarket[Ord(TItem(Obj).ItemType)].DisplayName,
                      InfoNameColorTag
                  ));
          (GetByName('InfoStdText') as TLabelGI)
              .SetText(LocalizedText('Items.Goods.Text.' + IntToStr(Ord(TItem(Obj).ItemType) + 1)));
        end
        else
        begin
          (GetByName('InfoStdName') as TLabelGI)
              .SetText(WrapTextInColor(LocalizedText('FormInfo.ContainerName'), InfoNameColorTag));
        ...
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
            GetByName('InfoStdText') as TLabelGI,
            True,
            True,
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            LoadGiByPathIntoGraphBuf(
                ExtractDelimitedPartW(((Obj as TShip).Graphic as TRuinsSE).StaticImagePath, 1, ','),
                GraphBuf
            );
            if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
            begin
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor((Obj as TShip).GetFullName(' '), InfoNameColorTag));
        if Obj is TShip then
          if (Obj as TShip).PartnerShip = GetPlayer then
            (GetByName('InfoStdName') as TLabelGI)
                .SetText(
                    (GetByName('InfoStdName') as TLabelGI).GetText
                        + #13#10
                        + WrapTextInColor(
      ...
      begin
        ...
        begin
          SetActive(True);
          SourceHasPerPixelAlpha := True;
          LoadGaiFrameToGraphBuf(
              TAsteroidSE(TAsteroid(Obj).GraphObject).ImagePath,
              GraphBuf,
              TAsteroid(Obj).Id
          );
          if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor((Obj as TAsteroid).GetDisplayName, InfoNameColorTag));
        (GetByName('InfoStdText') as TLabelGI).SetText(LocalizedText('FormInfo.ObjOutOfRange'));
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
      ...
      begin
        ...
        begin
          ...
          SourceHasPerPixelAlpha := True;
          LoadGiByPathIntoGraphBuf(
              'Bm.Missile.w' + (Obj as TMissile).GetGraphSuffix + '_' + GiResourceSuffix + 'i',
              GraphBuf
          );
          SetPosition(SubtractPoints(ShipScreen.ItemImageCenter, GetVisualCenter));
        end;
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor((Obj as TMissile).GetDisplayName, InfoNameColorTag));
        (GetByName('InfoStdText') as TLabelGI).SetText(LocalizedText('FormInfo.ObjOutOfRange'));
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
      ...
      begin
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(
                WrapTextInColor(
                    LocalizedText(THoleSE(THole(Obj).Graphic).NameTextPath),
                    InfoNameColorTag
                ));
        (GetByName('InfoStdText') as TLabelGI)
            .SetText(LocalizedText(THoleSE(THole(Obj).Graphic).InfoTextPath));
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
            GetByName('InfoStdText') as TLabelGI,
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            GetByName('InfoPlanetEmRace').SetActive(False);
          ...
          if (Obj as TPlanet).IsMainPiratePlanet then
            (GetByName('InfoPlanetOwner') as TLabelGI)
                .SetText(OwnerInfo[(Obj as TPlanet).OwnerId].DisplayName)
          else
    ...
    begin
      ...
      if not ((Byte((Obj as TItem).ItemType) in WearableItemTypes)
          or ((Obj as TItem).ItemType = t_Hull)) then
      begin
      ...
      begin
          ...
          BarWidth :=
              Round(
                  Sqrt(
                          (Obj as TItem).Weight
                              / HullBaseSize
                              / Max(0.1, (Obj as THull).GetFragilityFactor([])))
                      * 64
              )
        else
          BarWidth := Round(64 / Max(0.1, (Obj as TEquipment).GetFragilityFactor([])));
        ...
        begin
          ...
          if (Obj as TItem).ItemType = t_Hull then
            SetPosition(
                Classes.Point(
                    Round((Obj as THull).HullPoints / (Obj as THull).Weight * BarWidth)
                        - (GetContentSize.X - 5),
                    LocalPosition.Y
                )
            )
            ...
            SetPosition(
                Classes.Point(
                    Round((Obj as TEquipment).ConditionPercent / 100 * BarWidth)
                        - (GetContentSize.X - 5),
                    LocalPosition.Y
                )
            );
    ...
    begin
      ...
      begin
        ...
        if Obj is TRanger then
          (GetByName('InfoShipType') as TLabelGI).SetText((Obj as TRanger).GetCharacterName)
        else
          (GetByName('InfoShipType') as TLabelGI).SetText((Obj as TShip).GetLocalizedTypeName);
      end;
      ...
      if GetPlayer.CanResolveObjectWithScanner(Obj)
          ...
          or ((Obj as TShip).TypeId = stTranclucator) then
      begin
        Text :=
            WrapTextInColor(IntToStr((Obj as TShip).GetHull.HullPoints), ColorTag)
                + '/'
                + IntToStr((Obj as TShip).GetHull.Weight);
      ...
      begin
        ...
        if GetPlayer.HasScannerArtefact(Obj as TShip) then
          Text := (Obj as TShip).GetManeuverabilitySummary + Text;
      end;
      ...
      BarWidth :=
          Round(
              Sqrt(
                      (Obj as TShip).GetHull.Weight
                          / HullBaseSize
                          / Max(0.1, (Obj as TShip).GetHull.GetFragilityFactor([])))
                  * 64
          );
      BarWidth := Min(192, Max(32, BarWidth));
      ...
      begin
        if GetPlayer.CanResolveObjectWithScanner(Obj)
          ...
          SetPosition(
              Classes.Point(
                  Round(
                          (Obj as TShip).GetHull.HullPoints
                              / (Obj as TShip).GetHull.Weight
                              * BarWidth)
                      - (GetContentSize.X - 5),
                  LocalPosition.Y
      ...
      begin
        ...
        begin
          if GetPlayer.HasScannerArtefact(Obj as TShip) then
            SetText((Obj as TShip).GetCombatStatusDescription(StatusCount, True))
          else
            SetText(Text);
      ...
      DisplayedObject := Obj;
      if Obj is TShip then
        BuildShipPathOverlay(Obj as TShip, False, '')
      else
        ClearPathOverlay(False);
    end
    ...
    begin
      ...
      begin
        SourceHasPerPixelAlpha := True;
        LoadStarThumbnail(Obj, GraphBuf, ClientSize);
        SetImageKindX(ikxCenter);
        SetImageKindY(ikyCenter);
      end;
      ...
      Panel.SetSize(
          Classes.Point(
              StarInfoWindow.ClientSize.X
                  - StarInfoWindow.WorkSubRect.Left
                  - StarInfoWindow.WorkSubRect.Right,
              Panel.ClientSize.Y
          )
      );
      Objects := TList.Create;
      for I := 0 to GetPlayer.CurrentStar.Planets.Count - 1 do
        Objects.Add(GetPlayer.CurrentStar.Planets[I]);
      for I := 0 to GetPlayer.CurrentStar.Ships.Count - 1 do
        if TObject(GetPlayer.CurrentStar.Ships[I]) is TRuins then
          if (TObject(GetPlayer.CurrentStar.Ships[I]) as TRuins).InNormalSpace then
            if ((TObject(GetPlayer.CurrentStar.Ships[I]) as TRuins).Graphic is TRuinsSE)
                or (((TObject(GetPlayer.CurrentStar.Ships[I]) as TRuins).Graphic is TShip2SE)
                    and (((TObject(GetPlayer.CurrentStar.Ships[I]) as TRuins).Graphic as TShip2SE)
                            .AlternateImagePath
                        <> '')) then
            begin
              Distance :=
                  PointDistanceSquared(
                      TShip(GetPlayer.CurrentStar.Ships[I]).Position,
                      MakePointF(0, 0)
                  );
              J := 0;
              ...
              begin
                ...
                begin
                  if PointDistanceSquared(TPlanet(Objects[J]).GetPosition, MakePointF(0, 0))
                      > Distance then
                    Break;
                end
                else if PointDistanceSquared(TShip(Objects[J]).Position, MakePointF(0, 0))
                    > Distance then
                  Break;
                Inc(J);
              end;
      ...
      begin
        ...
        begin
          ...
          begin
            if PointDistanceSquared(TShip(Objects[J]).Position, MakePointF(0, 0)) > Distance then
              Break;
          end
          else if Sqr(TCustomSystemInfo(Objects[J]).Distance) > Distance then
            Break;
          Inc(J);
        end;
      ...
      for I := 0 to Objects.Count - 1 do
        ...
        begin
          ...
          else if TObject(Objects[I]) is TShip then
            SetText(TShip(Objects[I]).Name)
          else
            SetText(TCustomSystemInfo(Objects[I]).Name);
      ...
      begin
        ...
        begin
          ...
          if TObject(Objects[I]) is TPlanet then
          begin
            if (TObject(Objects[I]) as TPlanet).Radius < 70 then
              IconInset := 4
            else if (TObject(Objects[I]) as TPlanet).Radius < 80 then
              IconInset := 3
            else if (TObject(Objects[I]) as TPlanet).Radius < 90 then
              IconInset := 2
            else if (TObject(Objects[I]) as TPlanet).Radius < 100 then
              IconInset := 1
            else
              IconInset := 0;
          end;
          ...
          SetPosition(
              Classes.Point(
                  NameWidth + 5 + 1 + (IconInset shr 1),
                  RowHeight * I + 1 + (IconInset shr 1)
              )
          );
          SetSize(Classes.Point(RowHeight - 2 - IconInset, RowHeight - 2 - IconInset));
          if (TObject(Objects[I]) is TPlanet) or (TObject(Objects[I]) is TRuins) then
            LoadStarThumbnail(TObject(Objects[I]), GraphBuf, ClientSize)
        ...
        if TObject(Objects[I]) is TPlanet then
          OwnerId := TPlanet(Objects[I]).OwnerId
        ...
        begin
          ...
          if (CountDelimitedPartsW(CustomInfo.Info, ':') > 1)
              and (ExtractDelimitedPartW(CustomInfo.Info, 0, ':') = 'Image') then
          begin
          ...
          else if (CountDelimitedPartsW(CustomInfo.Info, ':') > 1)
              and (ExtractDelimitedPartW(CustomInfo.Info, 0, ':') = 'RGBA') then
          begin
            Images := ExtractDelimitedPartW(CustomInfo.Info, 1, ':');
            RowX := NameWidth + 5 + RowHeight + 5 + 1;
            for J := 0 to CountDelimitedPartsW(Images, ',') - 1 do
              ...
              LoadBitmapPathAsRgba(
                  ExtractDelimitedPartW(
                          GetFactionEmblemPath(
                              (TObject(Objects[I]) as TPlanet).GetFactionResourceName
                          ),
                          1,
                          ',')
                      + '?RGBA'
        ...
        begin
          ...
          begin
            case (TObject(Objects[I]) as TPlanet).GetRelationLevelToShip(GetPlayer) of
              ...
              SetImagePath('GI,Bm.FormGalaxy2.Face2');
            end;
            SetSize(Classes.Point(RowHeight - 2, RowHeight - 2));
        ...
        else if (TObject(Objects[I]) is TPlanet)
            and (TObject(Objects[I]) as TPlanet).IsMainPiratePlanet then
        begin
          with TLabelGI.Create(Panel) do
          begin
            if GiResourceVariant = 2 then
      ...
      StarInfoWindow.SetSize(
          Classes.Point(
              Panel.ClientSize.X
                  + StarInfoWindow.WorkSubRect.Left
                  + StarInfoWindow.WorkSubRect.Right,
              StarInfoWindow.WorkSubRect.Top
                  + StarInfoWindow.WorkSubRect.Bottom
                  + RowHeight * Objects.Count
          )
      );
    ...
    begin
      ...
      InfoWindow.SetSize(
          Classes.Point(
              InfoTextLabel.ClientSize.X
                  + InfoWindow.WorkSubRect.Left
                  + InfoWindow.WorkSubRect.Right,
              InfoTextLabel.ClientSize.Y
                  + InfoWindow.WorkSubRect.Top
                  + InfoWindow.WorkSubRect.Bottom
          )
      );
      InfoWindow.UpdateAutoGeometry;
      InfoTextLabel
          .SetPosition(Classes.Point(InfoWindow.WorkSubRect.Left, InfoWindow.WorkSubRect.Top));
      DisplayedObject := Obj;
      if Obj is TAsteroid then
        ShowAsteroidPath(Obj as TAsteroid)
      else
        ClearAsteroidPath;
      if Obj is TShip then
  ...
  if InfoWindow.Active then
    ActivePanel := InfoWindow
  else if ItemInfoWindow.Active then
    ActivePanel := ItemInfoWindow
  else if ShipInfoPanel.Active then
    ActivePanel := ShipInfoPanel
  else if PlanetInfoPanel.Active then
    ActivePanel := PlanetInfoPanel
  else if StarInfoWindow.Active then
    ActivePanel := StarInfoWindow
  else if StandardInfoPanel.Active then
  ...
  begin
    ...
    if HitObjectPosition.X <= I then
      Inc(HitObjectPosition.X, HitObjectSize.X div 2)
    ...
    begin
      ...
      begin
        ...
        if HitObjectPosition.Y < 10 then
          HitObjectPosition.Y := 10;
      end
      else
        HitObjectPosition.Y :=
            HitObjectPosition.Y - HitObjectSize.Y div 2 - ActivePanel.ClientSize.Y;
    end
    else
    begin
      I := Cardinal(GameScreenHeight) div 3;
      if HitObjectPosition.Y <= I then
        Inc(HitObjectPosition.Y, HitObjectSize.Y div 2)
      else if HitObjectPosition.Y >= 2 * I then
        HitObjectPosition.Y :=
            HitObjectPosition.Y - HitObjectSize.Y div 2 - ActivePanel.ClientSize.Y
      else
        Dec(HitObjectPosition.Y, ActivePanel.ClientSize.Y div 2);
    end;
    ...
    ActivePanel.SetPosition(
        Classes.Point(10 - Cardinal(GameScreenWidth) div 2, 10 - Cardinal(GameScreenHeight) div 2)
    );
end;
...
begin
  if Obj is TPlanet then
    Result := (Obj as TPlanet).Name
  else if Obj is TRuins then
    Result := (Obj as TRuins).Name
  else if Obj is TShip then
    Result := (Obj as TShip).Name
  else
    Result := '';
...
begin
  ...
  if (Selection = 1) and (NearestShip <> nil) then
    CenterOnShip(NearestShip)
  else if Selection = 2 then
    if FarthestShip <> nil then
...
begin
  ...
  if IsTurnCalculationRunning
      and ((TurnCalculationThread.Job = tcjPreparePlayerStar)
          or (Integer(TurnCalculationThread.Job) = 5)) then
    Exit;
  if GetPlayer <> nil then
    ShowObjectInfo(GetPlayer);
end;
...
begin
  ...
  begin
    ...
    InterceptorSelectionActive := False;
    CustomSelectionActive := False;
    if (not GetPlayer.IsEquipmentUsable(GetPlayer.GetScanner)
            or not GetPlayer.IsEquipmentUsable(GetPlayer.GetRadar))
        and (Galaxy.UltraScanModEnabled = 0) then
      ScannerSelectionActive := False
    else
      ScannerSelectionActive := not ScannerSelectionActive;
    RefreshActionRanges;
    RefreshWeaponButtons;
    UpdateActionCursor(False);
  end;
...
begin
  ...
  begin
    ...
    InterceptorSelectionActive := False;
    CustomSelectionActive := False;
    if not GetPlayer.IsEquipmentUsable(GetPlayer.GetRadar) then
      TalkSelectionActive := False
    else
      TalkSelectionActive := not TalkSelectionActive;
    RefreshActionRanges;
    RefreshWeaponButtons;
    UpdateActionCursor(False);
  end;
...
begin
  if (Mode = smmOrders)
      and (GetPlayer <> nil)
      and GetPlayer.GetHull.InterceptorsEnabled
  ...
  begin
    ...
    if GetPlayer.GetSlotCount(sskWeapon) <= Slot then
      Button.SetImageDisabledPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Disabled'
      )
    else if Weapon = nil then
      Button.SetImageDisabledPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Empty'
      )
    else if not GetPlayer.IsEquipmentUsable(Weapon) then
      Button.SetImageDisabledPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Red'
      );
    if SelectedWeapons[Slot] then
    begin
      Button.SetImageNormalPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Down'
      );
      Button.SetImageNormalActivePath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Down'
      );
      Button.SetImageDownPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Normal'
      );
    end
    else if SelectedWeapons[Slot] or ((Weapon <> nil) and (Weapon.Target <> nil)) then
    begin
      Button.SetImageNormalPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Select'
      );
      Button.SetImageNormalActivePath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Select'
      );
    ...
    begin
      ...
      Button.SetImageNormalActivePath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Active'
      );
      Button.SetImageDownPath(
          'GI,Bm.PanelMain2.' + GiResourceSuffix + 'W' + IntToStr(Slot + 1) + 'Down'
      );
    ...
    if SelectedWeapons[Slot] then
      ...
      Image.SetPosition(Classes.Point(Image.LocalPosition.X, -2));
    if Weapon <> nil then
...
begin
    ...
    for Slot := 0 to 4 do
      ...
      begin
        ...
        if GetPlayer.GetWeaponActionRange(Weapon) > MaxRange then
          MaxRange := GetPlayer.GetWeaponActionRange(Weapon);
      end;
  ...
  if (MainCircle.Center.X <> Point.X) or (MainCircle.Center.Y <> Point.Y) then
    MainCircle.SetCenter(Point);
  if MainCircle.Radius <> MainRadius then
    MainCircle.SetRadius(MainRadius);
  if MainCircle.Color <> MainColor then
    MainCircle.Color := MainColor;
  if MainCircle.Active <> MainVisible then
  ...
  if (InnerCircle.Center.X <> Point.X) or (InnerCircle.Center.Y <> Point.Y) then
    InnerCircle.SetCenter(Point);
  if InnerCircle.Radius <> InnerRadius then
    InnerCircle.SetRadius(InnerRadius);
  if InnerCircle.Color <> InnerColor then
    InnerCircle.Color := InnerColor;
  if InnerCircle.Active <> InnerVisible then
  ...
  if (DirectRange <> 0)
      ...
      and (not InnerVisible or (InnerRadius <> DirectRange)) then
    ExtraIndex := 0;
  for Slot := 0 to 2 do
  begin
    ExtraCircle := WeaponColorCircles[Slot + 1];
    if Slot = ExtraIndex then
      ExtraCircle.UpdateAbsolutePosition;
    Point := ExtraCircle.ToLocalPoint(MapControls.ToAbsolutePoint(MainCenter));
    if (ExtraCircle.Center.X <> Point.X) or (ExtraCircle.Center.Y <> Point.Y) then
      ExtraCircle.SetCenter(Point);
    if ExtraCircle.Radius <> DirectRange then
      ExtraCircle.SetRadius(DirectRange);
    if ExtraCircle.Color <> MainColor then
      ExtraCircle.Color := MainColor;
    if (Slot = ExtraIndex) <> ExtraCircle.Active then
  ...
  begin
    ...
    if (SimpleCircle.Center.X <> Point.X) or (SimpleCircle.Center.Y <> Point.Y) then
      SimpleCircle.SetCenter(Point);
    if SimpleCircle.Radius <> MainRadius then
      SimpleCircle.SetRadius(MainRadius);
    if SimpleCircle.Active <> MainVisible then
      SimpleCircle.SetActive(MainVisible);
  end;
  RadarChanged := False;
  if (SpaceProcess.RadarCenter.X <> RadarCenter.X)
      or (SpaceProcess.RadarCenter.Y <> RadarCenter.Y) then
  begin
...
begin
  with ActionColorCircle do
    SetActive(False);
  with WeaponColorCircles[0] do
    SetActive(False);
  with WeaponColorCircles[1] do
    SetActive(False);
  with WeaponColorCircles[2] do
    SetActive(False);
  with WeaponColorCircles[3] do
    SetActive(False);
  with ActionCircle do
    SetActive(False);
end;
...
begin
  ...
  begin
    ...
    if Weapon.Target <> nil then
      if (Weapon.Target is TItem)
          or (Weapon.Target is TAsteroid)
          or (Weapon.Target is TMissile)
          or ((Weapon.Target is TShip) and (Weapon.Target as TShip).InNormalSpace) then
      begin
        if Weapon.Target is TItem then
          Point := (Weapon.Target as TItem).Position
        else if Weapon.Target is TAsteroid then
          Point := (Weapon.Target as TAsteroid).Position
        else if Weapon.Target is TMissile then
  ...
  begin
    ...
    if Target = nil then
      Target := GetPlayer.SelectInterceptorTarget;
    if Target <> nil then
    begin
...
begin
  ...
  begin
    ...
    begin
      if Sqr(GetPlayer.GetRadarRange)
          >= PointDistanceSquared((Obj as TShip).Position, GetPlayer.Position) then
      begin
        if not IsCursorImageSelected('ScanFull') then
          SetCursorByName('ScanFull');
      end
      else if not IsCursorImageSelected('ScanSmall') then
        SetCursorByName('ScanSmall');
    end
    else if not IsCursorImageSelected('ScanSmall') then
      SetCursorByName('ScanSmall');
  end
  ...
  begin
    ...
    begin
      if Sqr(GetPlayer.GetRadarRange)
          >= PointDistanceSquared((Obj as TShip).Position, GetPlayer.Position) then
      begin
        if not IsCursorImageSelected('TalkFull') then
          SetCursorByName('TalkFull');
      end
      else if not IsCursorImageSelected('TalkSmall') then
        SetCursorByName('TalkSmall');
    end
    else if not IsCursorImageSelected('TalkSmall') then
      SetCursorByName('TalkSmall');
  end
    ...
    begin
      ...
      begin
        if not IsCursorImageSelected('InterceptorsFull') then
          SetCursorByName('InterceptorsFull');
      end
      else if not IsCursorImageSelected('InterceptorsSmall') then
        SetCursorByName('InterceptorsSmall');
    end
    else if not IsCursorImageSelected('InterceptorsSmall') then
      SetCursorByName('InterceptorsSmall');
  end
  ...
  begin
    ...
    if CursorObject is TShip then
      Point := TShip(Obj).Position
    else if CursorObject is TMissile then
      Point := TMissile(Obj).Position
    else if CursorObject is TAsteroid then
      Point := TAsteroid(Obj).Position
    else if CursorObject is TItem then
      Point := TItem(Obj).Position
    else if CursorObject is TPlanet then
      Point := TPlanet(Obj).GetPosition
    else if CursorObject is THole then
    begin
      if THole(Obj).Star1 = PlayerStar then
        Point := THole(Obj).Position1
      else
        Point := THole(Obj).Position2;
    end;
    if PointDistanceSquared(GetPlayer.Position, Point)
        > CustomSelectionRadius * CustomSelectionRadius then
    begin
      if not IsCursorImageSelected(CustomSelectionDeniedCursor) then
    ...
    begin
      ...
      if ActionResult > 0 then
      begin
        if not IsCursorImageSelected(CustomSelectionAllowedCursor) then
          SetCursorByName(CustomSelectionAllowedCursor);
      end
      else if not IsCursorImageSelected(CustomSelectionDeniedCursor) then
        SetCursorByName(CustomSelectionDeniedCursor);
    end;
  end
  else if AnyWeapon then
  begin
    ...
    begin
      ...
      if Obj is TShip then
        ...
        Point := (Obj as TItem).Position
      else if Obj is TAsteroid then
        Point := (Obj as TAsteroid).Position
      else if Obj is TMissile then
        Point := (Obj as TMissile).Position;
      if PointDistanceSquared(Point, GetPlayer.Position) <= Sqr(Range) then
      begin
        if not IsCursorImageSelected('FireFull') then
          SetCursorByName('FireFull');
      end
      else if not IsCursorImageSelected('FireSmall') then
        SetCursorByName('FireSmall');
    end
    else if not IsCursorImageSelected('FireSmall') then
      SetCursorByName('FireSmall');
  end
  ...
  begin
    Obj := FindObjectAtCursor;
    if (Obj is TItem)
        and CanCargoHookHandleItem(Obj as TItem, GetPlayer)
        and not GetPlayer.IsRecentlyDroppedItem(TItem(Obj)) then
    begin
...
begin
  if (Mode = smmOrders)
      and ((GetPlayer.Order = soFollowShip) or (PendingPlayerFollowTarget <> nil)) then
  begin
    ClearPathOverlay(True);
    if PendingPlayerFollowTarget <> nil then
    begin
    ...
    aCalc.WaitForTurnCalculationUI;
    if FollowMode = 1 then
      if not GetPlayer.CanSelectShipTarget(Ship) then
        FollowMode := 2;
    if FollowMode = 1 then
    begin
      GetPlayer.OrderNone(False);
...
begin
  if Activity = 0 then
    Result := 18
  else if Activity = 1 then
    Result := 14
  else
    Result := 10;
  if FilmSpeed = 0 then
    Inc(Result, 2)
  else if FilmSpeed = 2 then
    Dec(Result, 2)
  else if FilmSpeed = 3 then
...
begin
  ...
  if SecondaryFilm.PlayerCombatRecorded
      and not BattleMusicSelected
      and not DoNotChangeMusicInBattle then
  begin
    if not MusicInSpaceEnabled then
      MusicManager.RequestFadeOut
    else
      MusicManager.PlayCategory('Battle');
  end;
  ...
  if SecondaryFilm.InitialActivity = SecondaryFilm.FinalActivity then
    FilmFrameIntervalDelta := 0
  else
    FilmFrameIntervalDelta :=
        (GetTurnFilmFrameInterval(SecondaryFilm.FinalActivity)
                - GetTurnFilmFrameInterval(SecondaryFilm.InitialActivity))
            / 180.0;
...
begin
  ...
  if StopTurnProcessing and ContinueTurnCalculation then
...
function TfStarMap.CompleteHostedTurnFilm: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  try
    ...
    begin
      Stage := 3;
      if NextFilmCommand = nil then
        RaiseWideMessage('film step')
    ...
    if MainPanel.DateSlideProgress > 1 then
      MainPanel.DateSlideProgress := 1;
    MainPanel.RefreshDate;
    UpdateFilmCamera;
    Stage := 8;
    ...
    begin
      if CompleteHostedTurnFilm then
        Exit;
      Stage := 11;
      if (GetPlayer = nil) and (SecondaryFilm.Turn >= Galaxy.CurrentTurn) then
        TrailingEffectSteps := 200
      else if (GetPlayer <> nil)
          and GetPlayer.InHyperspace
          and not GetPlayer.Graphic.IsAttachedToSpace then
      begin
        ...
        begin
          Stage := 15;
          if (Galaxy <> nil)
              and Galaxy.IsOldHyperspaceEnabled
              and (GetPlayer.Order <> soTeleport) then
            RequestedScreenId := screenArcadeBattle
          else
            RequestedScreenId := screenJump;
        end;
        ...
        begin
          if GetPlayer.GetEngine <> nil then
            GetPlayer.ApplyItemDegradation(
                GetPlayer.GetEngine,
                idkUse,
                NextRandomUnitFloat(GetPlayer.RandomState) * 15
            );
      ...
      begin
        ...
        begin
          ...
          if PlanetBattleMapId < 0 then
            RaiseWideMessage('terron map');
          PlanetBattleState := 1;
          RequestedScreenId := screenStarMap;
          LoadPanel.SelectBackgroundStyle(3);
          LoadPanel.RefreshBackgroundImages;
...
begin
  if BreakRequested and (GetPlayer <> nil) and not GetPlayer.IsOnPlanet then
    ContinueTurnCalculation := False
      ...
      and (((BeginCalcNextTurn < 1.0) and (FilmStepIndex > Round(BeginCalcNextTurn * 200.0)))
          or ((BeginCalcNextTurn >= 1.0)
              and (FilmStepIndex > Round(AdaptiveBeginCalcNextTurn * 200.0)))) then
  begin
  ...
  begin
    ...
    FilmCameraMoving :=
        (FilmCameraPosition.X - HalfSize.X > SecondaryFilm.CameraAnchor.X)
            ...
            or (FilmCameraPosition.Y - HalfSize.Y > SecondaryFilm.CameraAnchor.Y)
            or (FilmCameraPosition.Y + HalfSize.Y <= SecondaryFilm.CameraAnchor.Y);
  end;
  if FilmCameraMoving
      or SecondaryFilm.ForceCameraMovement
      or (SecondaryFilm.CameraEventCount <> 0) then
  begin
    ...
    begin
      ...
      begin
        if PEFilmCameraEventArray(SecondaryFilm.CameraEvents)[FilmCameraEventIndex].StepIndex
            >= FilmStepIndex + FilmCameraLookAheadSteps then
          Break;
        if PEFilmCameraEventArray(SecondaryFilm.CameraEvents)[FilmCameraEventIndex].Priority
            > BestPriority then
        begin
          BestPriority :=
              PEFilmCameraEventArray(SecondaryFilm.CameraEvents)[FilmCameraEventIndex].Priority;
      ...
      begin
        ...
        begin
          if (Left > StartPosition.X)
              or (Right <= StartPosition.X)
              or (Top > StartPosition.Y)
              or (Bottom <= StartPosition.Y)
              or (Left > EndPosition.X)
              or (Right <= EndPosition.X)
    ...
    begin
      ...
      begin
        ...
        FilmCameraTarget := SpaceProcess.RadarCenter;
    end;
    if (FilmStepIndex <> 0) and FilmCameraFollow then
    begin
      Center := GetMapCenter;
      if (Center.X <> Round(FilmCameraPosition.X - FilmCameraShakeOffset.X))
          or (Center.Y <> Round(FilmCameraPosition.Y - FilmCameraShakeOffset.Y)) then
      begin
      ...
      SetMapCenter(
          Classes.Point(
              Round(FilmCameraPosition.X + FilmCameraShakeOffset.X),
...
begin
  if IsTurnCalculationRunning
    ...
    Exit;
  if GetPlayer <> nil then
    ShowFilmObjectInfo(GetPlayer.Graphic, GetPlayer.Id);
end;
...
begin
  ...
  while Obj <> nil do
  begin
    if (Obj.SceneObject <> nil)
        and (Obj.SceneObject is TMissileSE)
        and Obj.SceneObject.HitTestCursor then
    begin
  ...
  begin
    if (Obj.SceneObject <> nil)
        and (Obj.SceneObject is TShip2SE)
        and Obj.SceneObject.HitTestCursor then
    begin
      Result := Obj.SceneObject;
      ObjectId := Obj.ObjectId;
  ...
  begin
    if (Obj.SceneObject <> nil)
        and (Obj.SceneObject is TContainerSE)
        and Obj.SceneObject.HitTestCursor then
    begin
      Result := Obj.SceneObject;
      ObjectId := Obj.ObjectId;
  ...
  begin
    if (Obj.SceneObject <> nil)
        and (Obj.SceneObject is TRuinsSE)
        and Obj.SceneObject.HitTestCursor then
    begin
      Result := Obj.SceneObject;
      ObjectId := Obj.ObjectId;
  ...
  begin
    if (Obj.SceneObject <> nil)
        and (Obj.SceneObject is TAsteroidSE)
        and Obj.SceneObject.HitTestCursor then
    begin
      Result := Obj.SceneObject;
      ObjectId := Obj.ObjectId;
  ...
  begin
    ...
    begin
      ...
      else if Sqr(Point.X - Obj.SceneObject.Position.X) + Sqr(Point.Y - Obj.SceneObject.Position.Y)
          < Sqr((Obj.SceneObject as TPlanetSE).Radius) then
      begin
        Result := Obj.SceneObject;
        ObjectId := Obj.ObjectId;
        Exit;
  ...
  if Sqr((SecondaryFilm.ObjectInfo as TEObjInfo).StarRadius)
      > Point.X * Point.X + Point.Y * Point.Y then
  begin
    Obj := SecondaryFilm.FirstObject;
    while Obj <> nil do
    begin
...
begin
  ...
  begin
    if Obj is TPlanetSE then
      Planet := (SecondaryFilm.ObjectInfo as TEObjInfo).FindPlanet(ObjectId);
    if (Obj is TShip2SE) or (Obj is TRuinsSE) then
      Ship := (SecondaryFilm.ObjectInfo as TEObjInfo).FindShip(ObjectId);
    if Obj is TContainerSE then
      Item := (SecondaryFilm.ObjectInfo as TEObjInfo).FindItem(ObjectId);
    if Obj is TAsteroidSE then
      Asteroid := (SecondaryFilm.ObjectInfo as TEObjInfo).FindAsteroid(ObjectId);
    if Obj is TMissileSE then
      Missile := (SecondaryFilm.ObjectInfo as TEObjInfo).FindMissile(ObjectId);
  end;
  if (Obj = nil)
      or ((Obj is TPlanetSE) and (Planet = nil))
      or ((Obj is TShip2SE) and (Ship = nil))
      or ((Obj is TRuinsSE) and (Ship = nil))
      or ((Obj is TContainerSE) and (Item = nil))
      or ((Obj is TAsteroidSE) and (Asteroid = nil))
      or ((Obj is TMissileSE) and (Missile = nil)) then
  begin
    InfoWindow.SetActive(False);
    ItemInfoWindow.SetActive(False);
    ShipInfoPanel.SetActive(False);
  ...
  if (Obj is TPlanetSE)
      or (Obj is THoleSE)
      or ((Obj is TContainerSE)
          and (PointDistance(Obj.Position, SpaceProcess.RadarCenter) > SecondaryFilm.RadarRange))
      or (((Obj is TShip2SE) or (Obj is TRuinsSE))
          and (PointDistance(Obj.Position, SpaceProcess.RadarCenter) > SecondaryFilm.RadarRange))
      or ((Obj is TAsteroidSE)
          and (PointDistance(Obj.Position, SpaceProcess.RadarCenter) > SecondaryFilm.RadarRange))
      or ((Obj is TMissileSE)
          and (PointDistance(Obj.Position, SpaceProcess.RadarCenter)
              > SecondaryFilm.RadarRange)) then
  begin
    HitObjectPosition :=
        Classes
            .Point(Round(Obj.Position.X) - GetMapCenter.X, Round(Obj.Position.Y) - GetMapCenter.Y);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SourceHasPerPixelAlpha := True;
            LoadGiByPathIntoGraphBuf(
                ExtractDelimitedPartW((Obj as TRuinsSE).StaticImagePath, 1, ','),
                GraphBuf
            );
            if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
            begin
        ...
        begin
          ...
          begin
            ...
            if IsCivilized then
              ...
              Text := LocalizedText('Planet.' + Planet^.Faction + '.Info.TextAboutPlanet');
            ReplaceTextToken(
                Text,
                '<Race>',
      ...
      begin
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor(Asteroid^.Name, InfoNameColorTag));
        (GetByName('InfoStdText') as TLabelGI).SetText(LocalizedText('FormInfo.ObjOutOfRange'));
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
      ...
      begin
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor(Missile^.Name, InfoNameColorTag));
        (GetByName('InfoStdText') as TLabelGI).SetText(LocalizedText('FormInfo.ObjOutOfRange'));
        ShipScreen.LayoutItemInfo(
            StandardInfoPanel as TWindowGI,
            GetByName('InfoStdName') as TLabelGI,
      ...
      begin
        ...
        (GetByName('InfoStdName') as TLabelGI)
            .SetText(WrapTextInColor(LocalizedText(THoleSE(Obj).NameTextPath), InfoNameColorTag));
    ...
    begin
      ...
      begin
        if Item^.ItemType = t_Hull then
          ...
          BarWidth := Round(64 / Max(0.1, Item^.Fragility));
        BarWidth := Min(192, Max(32, BarWidth));
        with GetByName('InfoDurableLeft') as TImageGI do
        begin
          CapWidth := GetContentSize.X;
        ...
        begin
          ...
          SetPosition(
              Classes.Point(
                  Round(BarWidth * (Item^.ConditionPercent / 100)) - (GetContentSize.X - 5),
                  LocalPosition.Y
              )
          );
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        Text :=
            WrapTextInColor(IntToStr(Ship^.HullPoints), ColorTag)
                + '/'
                + IntToStr(Ship^.HullCapacity);
        if Ship^.RepairPoints >= 0 then
          Text := Text + ' + ' + WrapTextInColor(IntToStr(Ship^.RepairPoints), '');
        (GetByName('InfoShipSize') as TLabelGI).SetText(Text);
      end
      else
        (GetByName('InfoShipSize') as TLabelGI).SetText(WrapTextInColor('???', ColorTag));
      ...
      BarWidth :=
          Round(Sqrt(Ship^.HullCapacity / HullBaseSize / Max(0.1, Ship^.HullFragility)) * 64);
      BarWidth := Min(192, Max(32, BarWidth));
      with GetByName('InfoShipDurableLeft') as TImageGI do
      begin
        CapWidth := GetContentSize.X;
      ...
      begin
        if Ship^.ScannerResolved then
          SetPosition(
              Classes.Point(
                  Round(Ship^.HullPoints / Ship^.HullCapacity * BarWidth) - (GetContentSize.X - 5),
                  LocalPosition.Y
              )
          )
        else
  ...
  begin
    ...
    begin
      ...
      (GetByName('InfoStarName') as TLabelGI)
          .SetText(
              WrapTextInColor((SecondaryFilm.ObjectInfo as TEObjInfo).StarName, InfoNameColorTag));
      with GetByName('InfoStarImage') as TGraphBufGI do
      begin
        SourceHasPerPixelAlpha := True;
        LoadGiByPathIntoGraphBuf(
            ExtractDelimitedPartW((Obj as TStarSE).StaticImagePath, 1, ','),
            GraphBuf
        );
        if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
        begin
      ...
      begin
        if FilmObject.SceneObject <> nil then
          if ((FilmObject.SceneObject is TRuinsSE)
                  and not TRuinsSE(FilmObject.SceneObject).HideOnStarInfo)
              or ((FilmObject.SceneObject is TShip2SE)
                  and ((FilmObject.SceneObject as TShip2SE).AlternateImagePath <> '')) then
            if (FilmObject.SceneObject.GraphKey <> 'Ruins.Blazer')
                and (FilmObject.SceneObject.GraphKey <> 'Ruins.Keller')
                and (FilmObject.SceneObject.GraphKey <> 'Ruins.Terron')
                and (FilmObject.SceneObject.GraphKey <> 'Ruins.FighterSwarm') then
            begin
              ...
              if Ship <> nil then
                ...
                begin
                  Distance :=
                      PointDistanceSquared(FilmObject.SceneObject.Position, MakePointF(0, 0));
                  J := 0;
                  while J < Objects.Count do
                  begin
                    if PointDistanceSquared(TObjectSE(Objects[J]).Position, MakePointF(0, 0))
                        > Distance then
                      Break;
                    Inc(J);
                  end;
        ...
        begin
          if Objects[J] <> nil then
            if PointDistanceSquared(TObjectSE(Objects[J]).Position, MakePointF(0, 0))
              ...
              Break;
          if Objects[J] = nil then
            if PECustomSystemInfo(Records[J])^.Distance > CustomInfo^.Distance then
              Break;
          Inc(J);
        end;
      ...
      for I := 0 to Objects.Count - 1 do
        ...
        begin
          ...
          if Objects[I] = nil then
            SetText(PECustomSystemInfo(Records[I])^.Name)
          else if TObject(Objects[I]) is TPlanetSE then
            SetText(PEPlanetInfo(Records[I])^.Name)
          else
            SetText(PEShipInfo(Records[I])^.Name);
          NameWidth := Max(NameWidth, ClientSize.X);
        end;
      Child := Panel.FirstChild;
      while Child <> nil do
      ...
      begin
        ...
        begin
          ...
          begin
            if (TObject(Objects[I]) as TPlanetSE).Radius < 70 then
              IconInset := 4
            else if (TObject(Objects[I]) as TPlanetSE).Radius < 80 then
              IconInset := 3
            else if (TObject(Objects[I]) as TPlanetSE).Radius < 90 then
              IconInset := 2
            else if (TObject(Objects[I]) as TPlanetSE).Radius < 100 then
              IconInset := 1
            else
              IconInset := 0;
          end;
          ...
          begin
            LoadGiByPathIntoGraphBuf(
                ExtractDelimitedPartW(TShip2SE(Objects[I]).AlternateImagePath, 1, ','),
                GraphBuf
            );
            if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
            begin
          ...
          begin
            LoadGiByPathIntoGraphBuf(
                ExtractDelimitedPartW(TRuinsSE(Objects[I]).StaticImagePath, 1, ','),
                GraphBuf
            );
            if (ClientSize.X < GraphBuf.Width) or (ClientSize.Y < GraphBuf.Height) then
            begin
        ...
        if Objects[I] = nil then
          ...
          OwnerId := PEShipInfo(Records[I])^.OwnerId;
        if Objects[I] = nil then
        begin
          CustomInfo := Records[I];
          if (CountDelimitedPartsW(CustomInfo^.Text, ':') > 1)
              and (ExtractDelimitedPartW(CustomInfo^.Text, 0, ':') = 'Image') then
          begin
          ...
          else if (CountDelimitedPartsW(CustomInfo^.Text, ':') > 1)
              and (ExtractDelimitedPartW(CustomInfo^.Text, 0, ':') = 'RGBA') then
          begin
            Images := ExtractDelimitedPartW(CustomInfo^.Text, 1, ':');
            RowX := NameWidth + 5 + RowHeight + 5 + 1;
            for J := 0 to CountDelimitedPartsW(Images, ',') - 1 do
        ...
        begin
          if PEPlanetInfo(Records[I])^.OwnerId = Byte(oiPirate) then
            ...
            IsCivilized :=
                PEPlanetInfo(Records[I])^.Faction
                    = OwnerInfo[PEPlanetInfo(Records[I])^.OwnerId].InternalName;
        end;
        if IsCivilized then
        begin
          RowX := NameWidth + 5 + RowHeight + 5 + 1;
          ...
          begin
            case PEPlanetInfo(Records[I])^.Relation of
              ...
              rlNormal:
                SetImagePath(
                    'GI,Bm.FormGalaxy2.Face2'
                ); { Native explicit case, also repeated by the default. }
              rlGood: SetImagePath('GI,Bm.FormGalaxy2.Face1');
              rlExcellent: SetImagePath('GI,Bm.FormGalaxy2.Face0');
            else
              SetImagePath('GI,Bm.FormGalaxy2.Face2');
          ...
          if PEPlanetInfo(Records[I])^.Economy in [peAgricultural, peIndustrial] then
            ...
            and (MainPiratePlanet <> nil)
            and (PEPlanetInfo(Records[I])^.Id = MainPiratePlanet.Id) then
        begin
          with TLabelGI.Create(Panel) do
          begin
            if GiResourceVariant = 2 then
...
begin
  ...
  ShowObjectInfo(nil);
  if TalkShip <> nil then
    CenterMapForTalk(TalkShip.Position)
  else if (TalkPlanet <> nil) and (TalkPlanet.CurrentStar = GetPlayer.CurrentStar) then
...
begin
  if (TerronShip <> nil)
      and ((Sender as TgaiGI).SequenceFrame <> 0)
      and (TerronFadeImage <> nil) then
  begin
...
begin
  ...
  TShip.ClassName;
  TShip2SE.ClassName;
  TSpaceCircleGI.ClassName;
  TSpaceImgGI.ClassName;
  TStar.ClassName;
  TStarFieldGI.ClassName;
  TStarFieldImgGI.ClassName;
  TStarSE.ClassName;
