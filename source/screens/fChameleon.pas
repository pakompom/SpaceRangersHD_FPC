...
var
  ...
  WorkRect: TRect;
  function ChameleonChargeUnavailable(
      Count: Integer
  ): Boolean; // @addr $522588 @ida "bool __usercall $name@<al>(int Count@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x522D48" Nested OnOpen helper; does not access its parent frame.
  begin
    if Count > 0 then
      Result := False
    else
  ...
  function FormatChameleonChargeCount(
      Count: Integer
  ): WideString; // @addr $5225A8 @ida "void __usercall $name(int Count@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x522D9D"
  begin
    Result :=
        ' ('
            + FormatText1(
  ...
  function ChameleonSeriesColor(
      Series: Byte
  ): WideString; // @addr $5226C4 @ida "void __usercall $name(unsigned __int8 Series@<al>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x522D67"
  begin
    Result := '';
    case Series of
