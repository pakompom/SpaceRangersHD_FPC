...
type
  ...
  TfGalaxy2 = class(TMessageLoopGIWithMainPanel)
    ...
    GalaxyExtent: TPointF;
    SelectedJumpStar: TStar;
    RouteStars: TList;
    StarLinks: TPolyLineGI;
    StarInfoHideTimer: PCallbackTimerGI;
    JumpAnimationTimer: PCallbackTimerGI;
    JumpHintAnimationState: PtrInt;
    JumpLightTick: Integer;
    JumpButton: TGraphButtonGI;
    JumpDestinationLabel: TLabelGI;
    ...
    function CanShowExtendedRadarInfo(Star: TStar): Boolean;
    function GetRadarSummaryRadius: Integer;
    function MapPointToGalaxyPoint(Point: TPoint): TPointF;
    function GalaxyPointToMapPoint(Point: TPointF): TPoint;
    function GalaxyDistanceToMapDistance(Distance: Double): Integer;
    procedure RebuildJumpPath;
    procedure ClearJumpPath;
    procedure ConfigureReadOnlyMap;
    procedure ClearReadOnlyMapCallbacks;
    procedure ConfigureJumpSelection;
    procedure ClearJumpSelectionCallbacks;
...
const
  GalaxyMapFriendlyShipOrder: array[1..7] of Byte = (0, 1, 10, 2, 3, 4, 5);
...
begin
  ...
  if (GetPlayer <> nil) and not CapturePreviewOnOpen then
  ...
  if GiResourceVariant = 2 then
    MapPixelBounds := Classes.Rect(50, 50, 974, 718)
  else
    MapPixelBounds := Classes.Rect(39, 39, 761, 561);
  MapPixelBounds.TopLeft := Classes.Point(0, 0);
  MapPixelBounds.BottomRight := MapPanel.ClientSize;
  BufferOffset :=
      HalfPoint(
          SubtractPoints(
              Classes.Point(HideBuffer.GraphBuf.Width, HideBuffer.GraphBuf.Height),
              MapPanel.ClientSize
          )
      );
  ...
  if GameDataConfig.CountBlocks('StyleConstellation') > 0 then
    Block := GameDataConfig.GetBlock('StyleConstellation')
  else
    Block := nil;
  for I := 0 to Galaxy.Constellations.Count - 1 do
  begin
    ...
    if Constellation.Visible then
      if (Block = nil)
          or (Block.CountParams('DrawLines') <= 0)
          or ParseEnabledNameGI(Block.GetParam('DrawLines')) then
        for J := 0 to Constellation.StarLinks.Count - 1 do
        begin
          Segment := Constellation.StarLinks[J];
          if (Block = nil) or (Block.CountParams('LinesColor') <= 0) then
            Color := CurrentPixelFormat.PackNormalizedRgb(1, 1, 0)
          else
          begin
          ...
          StarLinks
                  .AddParentLine(
                      GalaxyPointToMapPoint(Segment.StartPoint),
                      GalaxyPointToMapPoint(Segment.EndPoint),
                      Color,
                      Constellation.Id)
                  .Animated :=
              True;
  ...
  begin
    ...
    if Constellation.Visible then
      ...
      begin
        Segment := Constellation.OutlineSegments[J];
        if Galaxy.CountVisibleConstellationsWithBoundaryPoints(Segment.StartPoint, Segment.EndPoint)
            <= 1 then
          HideBuffer.GraphBuf.DrawAntialiasedLine(
              AddPoints(GalaxyPointToMapPoint(Segment.StartPoint), BufferOffset),
              AddPoints(GalaxyPointToMapPoint(Segment.EndPoint), BufferOffset),
              $FFFFFF00
          )
        else
          HideBuffer.GraphBuf.DrawAntialiasedLine(
  ...
  begin
    ...
    begin
      ...
      NameLabel.SetPosition(
          Classes.Point(
              StarImage.LocalPosition.X - NameLabel.ClientSize.X div 2,
      ...
      if Star.Status.CustomFaction <> '' then
        ColoredName := WrapTextInColor(Text, LookupNamedColorTag(Star.Status.CustomFaction))
      else
      begin
        ColoredName := '';
        J := 1;
        ...
        if Text <> '' then
          for OwnerId := Byte(oiMaloc) to 7 do
            ...
            begin
              K := Length(Star.Name) div Star.CountDistinctInhabitedPlanetOwners;
              Inc(J);
              if J = Star.CountDistinctInhabitedPlanetOwners then
                K := Length(Text);
              ColoredName :=
                  ColoredName + WrapTextInColor(Copy(Text, 1, K), OwnerInfo[OwnerId].ColorTag);
              Delete(Text, 1, K);
            end;
      end;
      ...
      begin
        ...
        begin
          if CanRevealBossPresence(BlazerShip)
              and GetPlayer.CanResolveObjectWithScanner(BlazerShip) then
          begin
        ...
        begin
          if CanRevealBossPresence(KellerShip)
              and GetPlayer.CanResolveObjectWithScanner(KellerShip) then
          begin
            if Length(BossText) > 0 then
              BossText := BossText + '-';
            BossText :=
        ...
        begin
          if CanRevealBossPresence(TerronShip)
              and GetPlayer.CanResolveObjectWithScanner(TerronShip) then
          begin
            if Length(BossText) > 0 then
              BossText := BossText + '-';
            BossText :=
        ...
        begin
          ...
          ForceText :=
              ForceText
                  + WrapTextInColor(IntToStr(CustomCount), LookupNamedColorTag(CustomFaction));
        end;
      ...
      ForceLabel.SetPosition(
          Classes.Point(
              NameLabel.LocalPosition.X,
              NameLabel.LocalPosition.Y + ForceLabel.ClientSize.Y + 2
          )
      );
      Star.GetControlPresence(
          PlayerPartyPresent,
          CoalitionPresent,
          DominatorsPresent,
          PiratesPresent,
      ...
      if CustomPresent then
        DominatorsPresent := True;
      if (Star.Battle <> 0)
          and (not PlayerPartyPresent
              or ((CoalitionPresent or PiratesPresent)
                  and (PiratesPresent or DominatorsPresent)
                  and (CoalitionPresent or DominatorsPresent))) then
      begin
        ...
        BattleRing
            .SetOrigin(Classes.Point(BattleRing.ClientSize.X div 2, BattleRing.ClientSize.Y div 2));
        ...
        StarImage := TgaiGI.Create(MapPanel);
        if (CoalitionPresent and PiratesPresent) and DominatorsPresent then
          StarImage.SetImagePath('Bm.FormGalaxy.BattleRoyale')
        else if CoalitionPresent and PiratesPresent then
          StarImage.SetImagePath('Bm.FormGalaxy.BattleNormalsVsPirates')
        else if CoalitionPresent and DominatorsPresent then
          StarImage.SetImagePath('Bm.FormGalaxy.BattleNormalsVsDominators')
        else if PiratesPresent and DominatorsPresent then
          StarImage.SetImagePath('Bm.FormGalaxy.BattlePiratesVsDominators')
        else
          StarImage.SetImagePath('Bm.FormGalaxy.Defend');
        StarImage.SetSize(StarImage.GetContentSize);
        StarImage
            .SetOrigin(Classes.Point(StarImage.ClientSize.X div 2, StarImage.ClientSize.Y div 2));
        StarImage.SetPosition(
            AddPoints(
                GalaxyPointToMapPoint(Star.Position),
                Classes.Point(BattleRing.ClientSize.X div 2 - 5, -BattleRing.ClientSize.Y div 2 + 5)
            )
        );
      ...
      begin
        ...
        if (Hole.ArcadeMapName <> 'NoEntry')
            and (Hole.Star1 = Star)
            and ((SelectedHole = nil)
                or (THoleSE(SelectedHole.Graphic).GalaxyPriority
                    < THoleSE(Hole.Graphic).GalaxyPriority)) then
          SelectedHole := Hole;
      end;
      ...
      begin
        ...
        HoleImage
            .SetOrigin(Classes.Point(HoleImage.ClientSize.X div 2, HoleImage.ClientSize.Y div 2));
        HoleImage.SetPosition(
            AddPoints(
                GalaxyPointToMapPoint(Star.Position),
                Classes.Point(HoleImage.ClientSize.X div 2 + 3, HoleImage.ClientSize.Y div 2 + 3)
            )
        );
  ...
  begin
    ...
    if (GetPlayer.GetFuelTanks = nil)
        or not GetPlayer.CanUseEquipmentTech(GetPlayer.GetFuelTanks) then
      SetRadius(1)
    else
      SetRadius(GalaxyDistanceToMapDistance(GetPlayer.JumpRange));
    if ViewMode in [0, 3] then
      SetRadius(1000);
  end;
  ...
  else if ViewMode in [0, 2, 3] then
  ...
  begin
    with GetByName('PathCurPos') as TgaiGI do
      SetSequenceFrame(SequenceFrameCount div 2);
    ClearJumpPath;
    (GetByName('ImagePanel') as TImageGI).SetActive(False);
    DrawQueuedUpdateRects;
    First :=
        SubtractPoints(
            MapPanel.ToAbsolutePoint(GalaxyPointToMapPoint(GetPlayer.CurrentStar.Position)),
            Classes.Point(
                SecondarySavePreviewGraph.Width shr 1,
                SecondarySavePreviewGraph.Height shr 1
            )
        );
    if First.X < MapPanel.HitTestBounds.Left then
      First.X := MapPanel.HitTestBounds.Left;
    if First.X + SecondarySavePreviewGraph.Width >= MapPanel.HitTestBounds.Right then
      First.X := MapPanel.HitTestBounds.Right - SecondarySavePreviewGraph.Width;
    if First.Y < MapPanel.HitTestBounds.Top then
      First.Y := MapPanel.HitTestBounds.Top;
    if First.Y + SecondarySavePreviewGraph.Height >= MapPanel.HitTestBounds.Bottom then
      First.Y := MapPanel.HitTestBounds.Bottom - SecondarySavePreviewGraph.Height;
    ...
    begin
      ...
      CopyBgraToRgb24(
          ...
          AddPointerOffset(
              CaptureBuffer.GetPixels,
              CaptureBuffer.PitchBytes * First.Y + First.X * 4
          ),
          CaptureBuffer.PitchBytes,
      ...
      Ex_OKGF_Convert565toRGB(
          PAnsiChar(ScreenRenderBuffer.GetPixels)
              + First.X * 2
              + First.Y * ScreenRenderBuffer.PitchBytes,
...
begin
  ...
  begin
    ...
    begin
      if not JumpButton.Disabled then
        JumpClicked(nil);
    end
...
begin
  if ClickAutoCloseForm then
    if not (GetByName('ImagePanel') as TImageGI).HitTestPixel(Point)
        ...
        and not (GetByName('ButExit') as TGraphButtonGI).ContainsPoint(Point)
        and not JumpButton.ContainsPoint(Point)
        and not GetByName('PM_PanelMsg').ContainsPoint(Point) then
      CloseClicked(nil);
end;
...
begin
  ...
  begin
    ...
    if GetRadarSummaryRadius < Round(Sqrt(Sqr(First.X - Second.X) + Sqr(First.Y - Second.Y))) then
      Exit;
    Result := True;
  end;
end;
...
begin
  Result.X :=
      (Point.X - MapPixelBounds.Left)
              / (MapPixelBounds.Right - MapPixelBounds.Left + 1)
              * GalaxyExtent.X
          + GalaxyOrigin.X;
  Result.Y :=
      (Point.Y - MapPixelBounds.Top)
              / (MapPixelBounds.Bottom - MapPixelBounds.Top + 1)
              * GalaxyExtent.Y
          + GalaxyOrigin.Y;
end;
...
begin
  Result.X :=
      Round(
              (Point.X - GalaxyOrigin.X)
                  / GalaxyExtent.X
                  * (MapPixelBounds.Right - MapPixelBounds.Left + 1))
          + MapPixelBounds.Left;
  Result.Y :=
      Round(
              (Point.Y - GalaxyOrigin.Y)
                  / GalaxyExtent.Y
                  * (MapPixelBounds.Bottom - MapPixelBounds.Top + 1))
          + MapPixelBounds.Top;
end;
...
begin
  ...
  begin
    ...
    if Abs(First.X - Second.X) < Abs(First.Y - Second.Y) then
      UseY := True
    else
      UseY := False;
    Length :=
    ...
    begin
      ...
      begin
        ...
        if GetPlayer.GetJumpRange < JumpDistance then
          Reachable := ViewMode in [0, 3];
    ...
    begin
      ...
      if GetPlayer.GetJumpRange < TotalDistance then
        Reachable := ViewMode in [0, 3];
      First := Second;
      for I := 0 to RouteStars.Count - 1 do
      begin
    ...
    with GetByName('Distance') as TLabelGI do
      if Reachable then
        ...
        SetTextColor(CurrentPixelFormat.PackRgbBytes(255, 0, 0));
  end;
end;

procedure TfGalaxy2.ClearJumpPath;
var
  Control, Current: TObjectGI;
begin
...
begin
  ...
  if (GetPlayer.Order = soJump) and (GetPlayer.OrderTarget is TStar) then
    SelectedJumpStar := GetPlayer.OrderTarget as TStar
  else
    SelectedJumpStar := GetPlayer.CurrentStar;
  RebuildJumpPath;
  JumpButton.SetDisabled(
      (SelectedJumpStar = nil)
          or (GetPlayer.CurrentStar = SelectedJumpStar)
          or (ViewMode = 1)
          or GetPlayer.NoJump
  );
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      GetPlayer.OrderJump(SelectedJumpStar, False);
  end
...
begin
  ...
  JumpDestinationLabel.SetText(
      ReplaceAllWideString(LocalizedText('FormGalaxy.JumpTo'), '<Name>', SelectedJumpStar.Name)
  );
...
begin
  if JumpHintAnimationState = 1 then
    JumpHintAnimationState := 0
...
begin
  ...
  Owner.SetSize(
      Classes.Point(
          InfoPanel.ClientSize.X - InfoPanel.WorkSubRect.Left - InfoPanel.WorkSubRect.Right,
          Owner.ClientSize.Y
      )
  );
  Objects := TList.Create;
  for I := 0 to Star.Planets.Count - 1 do
    Objects.Add(Star.Planets[I]);
  for I := 0 to Star.Ships.Count - 1 do
    if TObject(Star.Ships[I]) is TRuins then
      if (TObject(Star.Ships[I]) as TRuins).InNormalSpace then
        if ((TObject(Star.Ships[I]) as TRuins).Graphic is TRuinsSE)
            or (((TObject(Star.Ships[I]) as TRuins).Graphic is TShip2SE)
                and (((TObject(Star.Ships[I]) as TRuins).Graphic as TShip2SE).AlternateImagePath
                    <> '')) then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        if PointDistanceSquared(TShip(Objects[J]).Position, MakePointF(0, 0)) > ObjectDistance then
          Break;
      end
      else if Sqr(TCustomSystemInfo(Objects[J]).Distance) > ObjectDistance then
        Break;
      Inc(J);
    end;
  ...
  begin
    ...
    begin
      ...
      SetPosition(
          Classes
              .Point(NameWidth + 5 + 1 + (IconInset shr 1), RowHeight * I + 1 + (IconInset shr 1))
      );
        ...
        if TRuins(Objects[I]).Graphic is TRuinsSE then
          LoadGiByPathIntoGraphBuf(
              ExtractDelimitedPartW(
                  (TRuins(Objects[I]).Graphic as TRuinsSE).StaticImagePath,
                  1,
                  ','
              ),
              GraphBuf
          ...
          LoadGiByPathIntoGraphBuf(
              ExtractDelimitedPartW(
                  (TRuins(Objects[I]).Graphic as TShip2SE).AlternateImagePath,
                  1,
                  ','
              ),
              GraphBuf
  ...
  InfoPanel.SetSize(
      Classes.Point(
          Owner.ClientSize.X + InfoPanel.WorkSubRect.Left + InfoPanel.WorkSubRect.Right,
  ...
  begin
    if I = 0 then
      InfoPanel.SetPosition(
          AddPoints(
              MapPanel.ToAbsolutePoint(GalaxyPointToMapPoint(Star.Position)),
              Classes.Point(
                  -InfoPanel.ClientSize.X - GiScalePixels(50),
                  -InfoPanel.ClientSize.Y - GiScalePixels(50)
              )
          )
      )
    else if I = 1 then
      InfoPanel.SetPosition(
          AddPoints(
              MapPanel.ToAbsolutePoint(GalaxyPointToMapPoint(Star.Position)),
              Classes.Point(GiScalePixels(50), -InfoPanel.ClientSize.Y - GiScalePixels(50))
          )
      )
    else if I = 2 then
      InfoPanel.SetPosition(
          AddPoints(
              MapPanel.ToAbsolutePoint(GalaxyPointToMapPoint(Star.Position)),
              Classes.Point(GiScalePixels(50), GiScalePixels(50))
          )
      )
    else
      InfoPanel.SetPosition(
          AddPoints(
              MapPanel.ToAbsolutePoint(GalaxyPointToMapPoint(Star.Position)),
              Classes.Point(-InfoPanel.ClientSize.X - GiScalePixels(50), GiScalePixels(50))
          )
      );
    if (InfoPanel.LocalPosition.X >= 0)
        and (InfoPanel.LocalPosition.Y >= 0)
        and (GameScreenWidth - GiScalePixels(100)
            >= InfoPanel.LocalPosition.X + InfoPanel.ClientSize.X)
        and (GameScreenHeight - GiScalePixels(100)
            >= InfoPanel.LocalPosition.Y + InfoPanel.ClientSize.Y) then
      Break;
  end;
end;
...
begin
  ...
  if (GetPlayer.GetScanner = nil) or (GetPlayer.GetScanner.BrokenFlag <> 0) then
  begin
    Result := False;
    Exit;
  end;
  ...
  Result := True;
  if (BlazerShip = Ship) and (TechLevel < 7) then
  begin
    Result := False;
    Exit;
  end;
  if (KellerShip = Ship) and (TechLevel < 5) then
  begin
    Result := False;
    Exit;
  end;
  if (TerronShip = Ship) and (TechLevel < 3) then
  begin
    Result := False;
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Faction = StarFaction then
          Inc(StarFactionCount)
        else
        begin
          if OtherFactionCount = 0 then
            OtherFaction := Faction
          else if OtherFaction <> Faction then
            OtherFaction := '';
          Inc(OtherFactionCount);
        end;
      end;
    ...
    begin
      ...
      case Ship.TypeId of
        ...
        stPirate:
          ...
          begin
            if TPirate(Ship).PirateType <> 0 then
              Inc(RoleCounts[12])
            else
              Inc(RoleCounts[13]);
          end;
        stWarrior:
          if Ship.HasScriptStateText then
            Inc(ScriptedCoalition)
          else if (Ship as TWarrior).WarriorType = wtFlagship then
            Inc(RoleCounts[10])
          else
            Inc(RoleCounts[1]);
        stTransport:
        ...
        stTranclucator:
          ...
          else if (Ship as TTranclucator).OwnerShip = nil then
          ...
          case Star.ControlFaction of
            sfCoalition:
              if Ship.CurrentStanding in [ssCoalitionMilitary..ssPiratePassive] then
                Inc(CoalitionStations)
              else if Ship.CurrentStanding in [ssPirateActive..ssPirateMilitary] then
                Inc(PirateStations);
            sfPirates:
              if Ship.CurrentStanding in [ssCoalitionPassive..ssPirateMilitary] then
                Inc(PirateStations)
              else if Ship.CurrentStanding in [ssCoalitionMilitary..ssCoalitionActive] then
                Inc(CoalitionStations);
          else
            if Ship.CurrentStanding in [ssCoalitionMilitary..ssNeutral] then
              Inc(CoalitionStations)
            else if Ship.CurrentStanding in [ssPiratePassive..ssPirateMilitary] then
              Inc(PirateStations);
          end;
      end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Ship.CurrentStar = Star) and (Star.Ships.IndexOf(Ship) < 0) then
        AccumulateShip;
    end;
  end;
  Line := '';
...
begin
  ...
  if not JumpButton.Disabled
      and (GetPlayer.CurrentStar <> SelectedJumpStar)
      and (SelectedJumpStar <> nil)
      and GetPlayer.HasPositiveSpeed then
  begin
    First := SelectedJumpStar.Position;
    Second := GetPlayer.CurrentStar.Position;
    if GetPlayer.JumpRange < Round(Sqrt(Sqr(First.X - Second.X) + Sqr(First.Y - Second.Y))) then
