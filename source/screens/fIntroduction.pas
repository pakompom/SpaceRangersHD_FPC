...
type
  TfIntroduction = class;
  TfIntroduction = class(TMessageLoopGI)

    GenerationProgressTimer: PCallbackTimerGI;
    TextScrollTimer: PCallbackTimerGI;
    BackgroundTimer: PCallbackTimerGI;
    BackgroundScrollOffset: PtrInt;
    TextPanelTop: Integer;
    TextPanelHeight: Integer;
    ProgressPulsePhase: Single;
    ContinueBlinkTimer: PCallbackTimerGI;
...
var
  IntroductionPulseCounter: Integer = 0;
  NewGameGenerationStage: Integer;
  IntroductionBlinkColorA: Cardinal;
  IntroductionBlinkColorB: Cardinal;
  IntroductionPulseRed: Byte;
  IntroductionPulseGreen: Byte;
  IntroductionPulseBlue: Byte;
  DisplayedGenerationStage: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      with FindByNameRecursive(AnsiString('ICW') + IntToStr(I)) do
        SetPosition(
            Classes.Point(
      ...
      with FindByNameRecursive(AnsiString('MCW') + IntToStr(I)) do
        SetPosition(
            Classes.Point(
                LocalPosition.X + ExtraScreenWidth div 2,
                LocalPosition.Y + ExtraScreenHeight
...
begin
  ...
  if IntroductionPulseCounter > 50 then
    Delta := -3
  else
    Delta := 3;
  ...
  begin
    ...
    begin
      ...
      if (DisplayedGenerationStage + 0 >= 1) and (DisplayedGenerationStage + 0 <= 8) then
        (GetByName(AnsiString('MCW') + IntToStr(DisplayedGenerationStage)) as TLabelGI)
            .SetTextColor(IntroductionBlinkColorA);
    end;
  end;
  if ProgressPulsePhase < 0.5 then
    Amount := ProgressPulsePhase * 2
  else
    Amount := 1 - (ProgressPulsePhase - 0.5) * 2;
  if (DisplayedGenerationStage + 1 >= 1) and (DisplayedGenerationStage + 1 <= 8) then
    (GetByName(AnsiString('MCW') + IntToStr(DisplayedGenerationStage + 1)) as TLabelGI)
        .SetTextColor(
            CurrentPixelFormat
                .InterpolateRgb(IntroductionBlinkColorA, IntroductionBlinkColorB, Amount));
  if (NewGameGenerationThread = nil)
      or (not NewGameGenerationThread.IsRunning and (DisplayedGenerationStage = 8)) then
  begin
    if GenerationProgressTimer <> nil then
    begin
      CancelCallbackTimer(GenerationProgressTimer);
...
begin
  if not IsVirtualKeyDown(VK_CONTROL)
      ...
      and ((Key = VK_SPACE) or (Key = VK_RETURN) or (Key = VK_RIGHT)) then
