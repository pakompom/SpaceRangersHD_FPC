...
type
  ...
  TfLoadRobot = class(TMessageLoopGI)
    ...
    function InsertEntry(Access: Integer): Integer;
    procedure RebuildEntries;
    procedure BuildEntryPanel(Panel: TObjectGI);
    procedure SelectEntry(Index: Integer);
    procedure UpdateEntryImage(Index: Integer);
    ...
    procedure EntryDoubleClick(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure CategoryClick(Sender: TObjectGI);
    procedure DifficultyClick(Sender: TObjectGI);
    procedure StartClick(Sender: TObjectGI);
    procedure UpdateSelectionDetails;
    procedure LoadCompletionData;
    procedure SaveCompletionData;
    procedure RecordCompletion(MapId: Integer; Score: Integer; Level: Integer);
    function GetUnlockedAccess: Integer;
    function GetCompletionCounts: TPoint;
    function GetCompletionSummary: WideString;
  end;
var
  RobotMapNameColor: Cardinal;
...
begin
  ...
  if ExtraScreenHeight < 0 then
    Panel.SetPosition(
        Classes.Point(
            Panel.LocalPosition.X + ExtraScreenWidth div 2,
    ...
    Panel.SetPosition(
        Classes.Point(
            Panel.LocalPosition.X + ExtraScreenWidth div 2,
            Panel.LocalPosition.Y + ExtraScreenHeight div 2
        )
    );
  ...
  begin
    UpCallback := CategoryClick;
    DownCallback := CategoryClick;
  end;
  ...
  begin
    UpCallback := DifficultyClick;
    DownCallback := DifficultyClick;
  end;
...
begin
  ...
  begin
    ...
    begin
      KeyHistory := '';
      if (SelectedIndex >= 0)
          and (SelectedIndex <= High(Entries))
          and (Entries[SelectedIndex].MapIndex >= 0) then
      begin
        LoadCompletionData;
        while (Entries[SelectedIndex].MapIndex >= 0)
            and (Entries[SelectedIndex].MapIndex <= High(RobotMapDefinitions)) do
        begin
          MapId := RobotMapDefinitions[Entries[SelectedIndex].MapIndex].Id;
          if (MapId >= 0)
              and (MapId < (High(CompletionData) + 1) div 2)
              and (CompletionData[MapId * 2 + 1] <> 0) then
            Break;
  ...
  begin
    if (High(Entries) + 1 > 0) and (Entries[0].Access <= UnlockedAccess) then
      SelectEntry(0);
  end
  ...
  begin
    ...
    else if (High(Entries) + 1 > 0) and (Entries[0].Access <= UnlockedAccess) then
      SelectEntry(0);
  end
  ...
  begin
    ...
    if SelectedIndex >= 0 then
    begin
      if (SelectedIndex < High(Entries))
          and (Entries[SelectedIndex + 1].Access <= UnlockedAccess) then
        SelectEntry(SelectedIndex + 1);
    end
    else if (High(Entries) + 1 > 0) and (Entries[0].Access <= UnlockedAccess) then
      SelectEntry(0);
  ...
  begin
    EntryMouseLeave(nil);
    if Category = 0 then
      CategoryClick(GetByName('ButGroup1'))
    else if Category = 1 then
      CategoryClick(GetByName('ButGroup2'))
    else if Category = 2 then
      CategoryClick(GetByName('ButGroup0'));
  end;
end;
...
begin
  I := 0;
  while (I <= High(Entries)) and (Entries[I].Access <= Access) do
    Inc(I);
  SetLength(Entries, High(Entries) + 1 + 1);
  for J := High(Entries) downto I + 1 do
...
begin
    ...
    begin
      repeat
        ...
        begin
          ...
          begin
            if Path = TrimWideString(LowerCaseWideString(RobotMapDefinitions[I].Map)) then
              Break;
            Inc(I);
          end;
  ...
  if (SelectedIndex < 0) or (SelectedIndex >= High(Entries) + 1) then
    SelectedIndex := -1;
...
var
  Index, MapIndex, RightEdge, MapId, Hours, Minutes, Seconds: Integer;
  TimeText: WideString;
begin
  ...
  begin
    ...
    begin
      if not Entries[Index].AlternateBackground then
        ...
        SetImagePath('GI,Bm.FormLoadRobot.' + GiResourceSuffix + 'SlotNormal2');
    end
    else
      SetImagePath('GI,Bm.FormLoadRobot.' + GiResourceSuffix + 'SlotDisabled');
  ...
  if (Entries[Index].Side and 4) <> 0 then
    ...
    begin
      ...
      if Entries[Index].Access <= UnlockedAccess then
        ...
        SetImagePath('GI,Bm.FormLoadRobot.' + GiResourceSuffix + 'IconKellerD');
      SetSize(GetContentSize);
      SetPosition(
          Classes.Point(RightEdge - ClientSize.X, Panel.ClientSize.Y div 2 - ClientSize.Y div 2)
      );
      SetActive(True);
  ...
  if (Entries[Index].Side and 2) <> 0 then
    ...
    begin
      ...
      if Entries[Index].Access <= UnlockedAccess then
        ...
        SetImagePath('GI,Bm.FormLoadRobot.' + GiResourceSuffix + 'IconTerronD');
      SetSize(GetContentSize);
      SetPosition(
          Classes.Point(RightEdge - ClientSize.X, Panel.ClientSize.Y div 2 - ClientSize.Y div 2)
      );
  ...
  if (Entries[Index].Side and 1) <> 0 then
    ...
    begin
      ...
      if Entries[Index].Access <= UnlockedAccess then
        ...
        SetImagePath('GI,Bm.FormLoadRobot.' + GiResourceSuffix + 'IconBlazerD');
      SetSize(GetContentSize);
      SetPosition(
          Classes.Point(RightEdge - ClientSize.X, Panel.ClientSize.Y div 2 - ClientSize.Y div 2)
      );
    ...
    begin
      ...
      if MapIndex <> 0 then
        ...
        begin
          ...
          SetDepth(9);
          if MapIndex = 1 then
            SetImagePath('GI,Bm.FormRewards.' + GiResourceSuffix + '_06')
          else
            SetImagePath('GI,Bm.FormRewards.' + GiResourceSuffix + '_13');
          SetSize(GetContentSize);
          SetPosition(
              Classes.Point(
                  Panel.ClientSize.X - ClientSize.X - GiScalePixels(10),
                  Panel.ClientSize.Y div 2 - ClientSize.Y div 2
              )
          );
          SetActive(True);
        end;
  ...
  begin
    ...
    if Entries[Index].Access <= UnlockedAccess then
      SetTextColor(RobotMapNameColor)
    else
      SetTextColor(CurrentPixelFormat.PackRgbBytes(112, 112, 112));
  end;
  with TLabelGI.Create(Panel) do
  begin
    if GiResourceVariant = 1 then
    ...
    repeat
      ...
      if Entries[Index].Length < 0 then
        SetText('')
      else
        SetText(
            LocalizedText(WideString('FormLoadRobot.Length' + IntToStr(Entries[Index].Length)))
        );
    until True;
    if Entries[Index].Access <= UnlockedAccess then
      SetTextColor(RobotMapNameColor)
...
begin
  ...
  SelectedIndex := Index;
  if SelectedIndex >= High(Entries) + 1 then
    SelectedIndex := -1;
  if (SelectedIndex >= 0) and (Entries[SelectedIndex].Access > UnlockedAccess) then
    SelectedIndex := -1;
...
begin
  ...
  if Difficulty <> NewDifficulty then
...
begin
  if BattleResult <> 0 then
    Exit;
  if (SelectedIndex < 0) or (SelectedIndex > High(Entries)) then
    Exit;
  BattleResult := 1;
  RequestedScreenId := screenLoadRobot;
  LoadPanel.SelectBackgroundStyle(3);
...
var
  Text, MapName, Extension: WideString;
  F: TFileEC;
  Success: Boolean;
  Buffer: TBufEC;
begin
  ...
  begin
    ...
    if (SelectedIndex >= 0)
        and (SelectedIndex <= High(Entries))
        and (Entries[SelectedIndex].MapIndex >= 0)
        and (Entries[SelectedIndex].MapIndex <= High(RobotMapDefinitions)) then
    begin
      SetActive(True);
      ...
      if Text <> '' then
        SetText(GetText + #13#10 + ' ' + #13#10 + Text);
    end;
  end;
  ...
  if Success then
    (GetByName('Dif1') as TGraphButtonGI).SetDown(True);
end;
...
begin
  ...
  begin
    ...
    begin
      if Entries[I + Count].Access <> Access then
        Break;
      ...
      begin
        MapId := RobotMapDefinitions[MapIndex].Id;
        if (MapId >= 0) and (MapId < (High(CompletionData) + 1) div 2) then
          if (MapId < (High(CompletionData) + 1) div 2)
              and (CompletionData[MapId * 2 + 1] <> 0) then
            Inc(Completed);
      end;
      Inc(Count);
    ...
    Result := Access;
    if (Access > 0) and (Completed < Count - 1) then
      Break;
    Inc(I, Count);
  end;
  if Result = 0 then
...
begin
  ...
  for I := 0 to High(RobotMapDefinitions) do
  begin
    if RobotMapDefinitions[I].Group > -1 then
      Inc(Result.Y);
...
begin
  ...
  Total := 0;
  for I := 0 to High(RobotMapDefinitions) do
    if RobotMapDefinitions[I].Group in [0, 1, 2] then
      Inc(Total);
