...
type
  TfInfo = class;
  TfInfo = class(TMessageLoopGIWithMainPanel)
    PlanetPanel: TfPanelPlanet;
    StationPanel: TfPanelRuins;
    LoadPanel: TfPanelLoad;
    InfoPanel: TPanelScrollBarGI;
    InfoContentHeight: Integer;
    SearchMode: Boolean;
    GapE9: array[0..2] of Byte;
    SelectedSearchCategory: Integer;
    PreviousSearchCategory: Integer;
    ...
    procedure RefreshNewsAnimation(Sender: TObjectGI);
    procedure EndTurnClicked(Sender: TObjectGI);
    procedure ShipClicked(Sender: TObjectGI);
    procedure ClearInfoContents;
    procedure FinishInfoLayout;
    procedure AddInfoSpacing(Pixels: Integer);
    procedure AddInfoSeparator;
    procedure AddInfoHeading(
        Title: WideString;
        BookmarkText: WideString;
        LayoutKind: Integer;
        BookmarkIndex: Integer;
        GoodsReference: Integer
    );
    procedure AddSearchPriceLabel(Text: WideString);
    procedure AddInfoText(Text: WideString; Alignment: TTextAlignXGI; Font: WideString);
    procedure AddInfoImageText(ImagePath: WideString; Text: WideString);
    procedure AddPlanetInfoText(Planet: TPlanet; Text: WideString);
    procedure AddItemInfoText(Item: TItem; Text: WideString);
    procedure AddStarInfoText(Star: TStar; Text: WideString);
    procedure FocusSearchField(Force: Boolean);
    procedure ToggleSearchMode(Sender: TObjectGI);
    ...
    procedure MainPanelKeyUp(Sender: TObjectGI; Key: Cardinal);
    procedure BookmarkClicked(Sender: TObjectGI);
    procedure ShowNews;
    procedure ShowSearch;
    procedure SearchClicked(Sender: TObjectGI);
    procedure RunSearch(Sender: TObjectGI);
    procedure AddEquipmentInfoText(Item: TItem; Text: WideString);
    procedure ToggleVisibleBookmark;
    function IsAtBusinessCenter: Boolean;
    procedure CategoryStateChanged(Sender: TObjectGI);
    procedure CategoryClicked(Sender: TObjectGI);
    procedure NextSearchPageClicked(Sender: TObjectGI);
    procedure BindFilterLabels(Parent: TObjectGI);
    procedure FilterLabelMouseEnter(Sender: TObjectGI);
    procedure FilterLabelMouseLeave(Sender: TObjectGI);
    procedure FilterLabelMouseDown(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure FilterLabelMouseUp(Sender: TObjectGI; KeyState: Cardinal; Point: TPoint);
    procedure ClearSearchText(Sender: TObjectGI);
    procedure CopySearchText(Sender: TObjectGI);
    procedure PasteSearchText(Sender: TObjectGI);
    procedure ClearSearch12Name(Sender: TObjectGI);
    procedure CopySearch12Name(Sender: TObjectGI);
    procedure PasteSearch12Name(Sender: TObjectGI);
    procedure ClearSearch15Name(Sender: TObjectGI);
    procedure CopySearch15Name(Sender: TObjectGI);
    procedure PasteSearch15Name(Sender: TObjectGI);
    procedure ClearSearchField(Name: WideString);
    procedure ClearSearch01Filters(Sender: TObjectGI);
    procedure ClearSearch02Filters(Sender: TObjectGI);
    procedure ClearSearch03Filters(Sender: TObjectGI);
    procedure ClearSearch04Filters(Sender: TObjectGI);
    procedure ClearSearch05Filters(Sender: TObjectGI);
    procedure ClearSearch06Filters(Sender: TObjectGI);
    procedure ClearSearch07Filters(Sender: TObjectGI);
    procedure ClearSearch09Filters(Sender: TObjectGI);
    procedure ClearSearch10Filters(Sender: TObjectGI);
    procedure ClearSearch11Filters(Sender: TObjectGI);
    procedure ClearSearch12Filters(Sender: TObjectGI);
    procedure ClearSearch13Filters(Sender: TObjectGI);
    procedure ClearSearch15Filters(Sender: TObjectGI);
    function GetSearchResultLimit: Integer;
  end;
function FindLowercaseInfoTextFrom(
    const Search: WideString;
    const Text: WideString;
    StartPosition: Integer
): Integer;
function FindLowercaseInfoText(const Search: WideString; const Text: WideString): Integer;
...
var
  I, Count: Integer;
  function ItemMatchesInfoSearchTerm(
      Term: WideString
  ): Boolean; // @addr $597890 @ida "bool __usercall $name@<al>(unsigned __int16 *Term@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x597A66" Nested helper captures Item.
  begin
    Result := True;
    if FindTextOffsetW(WideLowerCase(RemoveTextTagsW(Item.GetDisplayName)), Term) >= 0 then
      Exit;
    if (Item is THull) and ((Item as THull).HullSeries <> -1) then
      if FindTextOffsetW(
              WideLowerCase(RemoveTextTagsW(HullSeriesDefinitions[THull(Item).HullSeries].Name)),
              Term)
          >= 0 then
        Exit;
    if (Item is TWeapon) and ((Item as TWeapon).SpecialModuleIndex <> 0) then
      if FindTextOffsetW(
              WideLowerCase(RemoveTextTagsW((Item as TWeapon).GetSpecialModuleName)),
              Term)
          >= 0 then
        Exit;
    Result := False;
...
begin
  ...
  for I := 0 to Count - 1 do
    if not ItemMatchesInfoSearchTerm(ExtractDelimitedPartW(Search, I, ' ')) then
      Exit;
  Result := True;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        with FindByNameRecursive('ButSearch') do
          SetPosition(Types.Point(LocalPosition.X, LocalPosition.Y + ExtraHeight));
        with FindByNameRecursive('ButNews') do
          SetPosition(Types.Point(LocalPosition.X, LocalPosition.Y + ExtraHeight));
        ...
        begin
          ...
          VerticalScrollBar.SetSize(
              Types.Point(
                  ...
                  VerticalScrollBar.ClientSize.Y + ExtraHeight
              )
          );
        end;
      end;
...
var
  ReservedInfoText: WideString =
      ''; // @addr $87B350 Retained empty managed variable, cleared before the names array.
...
var
  InfoNewsAnimationWeights: array[0..19] of Integer = (
      ...
      5,
      3
  ); // @addr $87B3A4 Native selection adds one to each weight.
  ...
  InfoQualityGrades: array[0..6] of Single =
      (0.7, 0.8, 0.9, 1.0, 1.2, 1.4, 1.6); // @addr $87B3F4 @indexrefs "$59C6A4"
...
begin
  Weight := 0;
  for I := 0 to 19 do
    Inc(Weight, InfoNewsAnimationWeights[I] + 1);
...
begin
  ...
  InfoPanel.VerticalScrollBar.SetSmallChange(
      (GovernmentScreen.GetByName('TalkText') as TLabelGI).GetLineHeight
  );
...
begin
  ...
  Image.SetPosition(
      Types.Point(InfoPanel.ClientSize.X div 2 - Image.ClientSize.X div 2, InfoContentHeight)
  );
  Image.SetDepth(1);
...
procedure TfInfo.AddInfoHeading(
    Title, BookmarkText: WideString;
    LayoutKind, BookmarkIndex, GoodsReference: Integer
);
var
  Image: TImageGI;
  Caption: TLabelGI;
...
begin
  ...
  Image := TImageGI.Create(InfoPanel);
  if LayoutKind = 0 then
    Image.SetImagePath('GI,Bm.FormInfo2.' + GiResourceSuffix + 'CaptionL')
  else
    Image.SetImagePath('GI,Bm.FormInfo2.' + GiResourceSuffix + 'Caption');
  Image.SetSize(Image.GetContentSize);
  if LayoutKind = 0 then
    Image.SetPosition(
        Types.Point(InfoPanel.ClientSize.X div 2 - Image.ClientSize.X div 2, InfoContentHeight)
    ...
    Image.SetPosition(Types.Point(InfoPanel.ClientSize.X - Image.ClientSize.X, InfoContentHeight));
  Image.SetDepth(1);
  Image.SetImageKindX(ikxLeftFill);
  Image.SetImageKindY(ikyCenter);
  Inc(InfoContentHeight, Image.ClientSize.Y);
  ...
  if (FontDialog = 0) or SearchMode then
    Caption.SetFontName(NormalFontName)
  else if FontDialog = 1 then
    Caption.SetFontName(SmoothBigFontName)
  else if FontDialog = 2 then
    Caption.SetFontName(SmoothHugeFontName)
  else if FontDialog >= 3 then
    Caption.SetFontName(SmoothIntroFontName);
  Caption.SetDepth(-1);
  Caption.SetSize(Image.ClientSize);
  if LayoutKind <> 0 then
    Caption.SetSize(Types.Point(Caption.ClientSize.X - GiScalePixels(20), Caption.ClientSize.Y));
  Caption.SetPosition(
      Types.Point(
          Image.LocalPosition.X + Image.ClientSize.X - Caption.ClientSize.X,
  ...
  begin
    ...
    if LayoutKind = 0 then
      Button.SetPosition(
          Types.Point(
              Image.LocalPosition.X + Image.ClientSize.X - Button.ClientSize.X - GiScalePixels(10),
              Image.LocalPosition.Y
          )
      )
    else
      Button.SetPosition(
          Types.Point(
              Image.LocalPosition.X + Image.ClientSize.X - Button.ClientSize.X - GiScalePixels(10),
              Image.LocalPosition.Y - GiScalePixels(5)
          )
      );
...
begin
  ...
  Image.SetPosition(
      Types
          .Point(InfoPanel.ClientSize.X - Image.ClientSize.X - GiScalePixels(40), InfoContentHeight)
  );
  Image.SetDepth(1);
  Image.SetImageKindX(ikxLeftFill);
  Image.SetImageKindY(ikyCenter);
  ...
  Caption.SetSize(
      Types.Point(
          InfoPanel.ClientSize.X
              - (InfoPanel.ClientSize.X - Image.LocalPosition.X)
              - GiScalePixels(10),
          Image.ClientSize.Y
      )
  );
...
begin
  ...
  if Cardinal(Image.GraphBuf.Width) >= Cardinal(Image.GraphBuf.Height) then
    Image.GraphBuf.RescaleRgba(
        Image.ClientSize.X,
        Round(
            Image.ClientSize.X / Cardinal(Image.GraphBuf.Width) * Cardinal(Image.GraphBuf.Height)
        ),
        5
    )
  else
    Image.GraphBuf.RescaleRgba(
        Round(
            Image.ClientSize.Y / Cardinal(Image.GraphBuf.Height) * Cardinal(Image.GraphBuf.Width)
        ),
        Image.ClientSize.Y,
        5
    );
...
begin
  ...
  begin
    ...
    Emblem.SetPosition(
        Types.Point(
            InfoPanel.ClientSize.X - Emblem.ClientSize.X - GiScalePixels(10),
            InfoContentHeight
        )
    );
...
var
  ...
  Emblem: TImageGI;
  Caption: TLabelGI;
  function GetInfoStarFactionName(
      Star: TStar
  ): WideString; // @addr $59AEA4 @ida "void __usercall $name(TStar *Star@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x59B12B" Nested in AddStarInfoText; static link unused.
  begin
    if Star.Status.CustomFaction <> WideString('') then
      Result := Star.Status.CustomFaction
    else if Star.ControlFaction = sfDominators then
      Result := DominatorSeriesNames[Ord(Star.DominatorSeries)]
    else if Star.ControlFaction = sfPirates then
      Result := OwnerInfo[Ord(oiPirate)].InternalName
    else
  ...
  begin
    ...
    begin
      if SelectedSearchCategory = 1 then
        SetFocusedControl(GetByName('M01Speed'))
      else if SelectedSearchCategory = 2 then
        SetFocusedControl(GetByName('M02Capacity'))
      else if SelectedSearchCategory = 3 then
        SetFocusedControl(GetByName('M03Range'))
      else if SelectedSearchCategory = 4 then
        SetFocusedControl(GetByName('M04Power'))
      else if SelectedSearchCategory = 5 then
        SetFocusedControl(GetByName('M05Power'))
      else if SelectedSearchCategory = 6 then
        SetFocusedControl(GetByName('M06ObjSize'))
      else if SelectedSearchCategory = 7 then
        SetFocusedControl(GetByName('M07Block'))
      else if SelectedSearchCategory = 8 then
        SetFocusedControl(GetByName('M08Range'))
      else if SelectedSearchCategory = 9 then
        SetFocusedControl(GetByName('M09Const'))
      else if SelectedSearchCategory = 10 then
        SetFocusedControl(GetByName('M10Const'))
      else if SelectedSearchCategory = 11 then
        SetFocusedControl(GetByName('M11Size'))
      else if SelectedSearchCategory = 12 then
        SetFocusedControl(GetByName('M12DamageMin'))
      else if SelectedSearchCategory = 13 then
        SetFocusedControl(GetByName('M13Range'))
      else if SelectedSearchCategory = 15 then
        SetFocusedControl(GetByName('M15Const'));
    end;
  end
  else
    SetFocusedControl(nil);
...
begin
  ...
  begin
    if Key = Ord('C') then
    begin
      if SelectedSearchCategory = 12 then
        CopySearch12Name(nil)
      else if SelectedSearchCategory = 15 then
        CopySearch15Name(nil)
      else
        CopySearchText(nil);
    end
    ...
    begin
      if SelectedSearchCategory = 12 then
        PasteSearch12Name(nil)
      else if SelectedSearchCategory = 15 then
        PasteSearch15Name(nil)
      else
        PasteSearchText(nil);
    end
    ...
    begin
      if SelectedSearchCategory = 12 then
        ClearSearch12Name(nil)
      else if SelectedSearchCategory = 15 then
        ClearSearch15Name(nil)
      else
        ClearSearchText(nil);
    end;
  end
  else if not IsVirtualKeyDown(VK_SHIFT)
      and not IsVirtualKeyDown(VK_MENU)
      and (not SearchMode or (Key < Ord('A')) or (Key > Ord('Z'))) then
  begin
    if IsAtBusinessCenter and (Key = VK_INSERT) then
      ToggleVisibleBookmark;
    NewPosition := InfoPanel.VerticalScrollBar.Position;
    if Key = VK_PRIOR then
      Dec(NewPosition, InfoPanel.VerticalScrollBar.LargeChange);
    if Key = VK_NEXT then
      Inc(NewPosition, InfoPanel.VerticalScrollBar.LargeChange);
    if Key = VK_UP then
      Dec(NewPosition, InfoPanel.VerticalScrollBar.SmallChange);
    if Key = VK_DOWN then
      Inc(NewPosition, InfoPanel.VerticalScrollBar.SmallChange);
    if NewPosition < InfoPanel.VerticalScrollBar.Minimum then
      NewPosition := InfoPanel.VerticalScrollBar.Minimum;
    if NewPosition > InfoPanel.VerticalScrollBar.Maximum then
    ...
    begin
      if not SearchMode and GetByName('PM_EndTurn').Active then
        EndTurnClicked(nil);
    end
    else if (Key = VK_RETURN) and SearchMode then
      SearchClicked(nil)
    else if (Key = Ord('F')) and not SearchMode then
      ToggleSearchMode(nil)
...
begin
  if Delta = WHEEL_DELTA then
    InfoPanel.VerticalScrollBar.SetPosition(
        InfoPanel.VerticalScrollBar.Position - InfoPanel.VerticalScrollBar.SmallChange * 5
    )
...
// CHANGE: ENHANCEMENT - Save searchable results with their destination.
...
begin
  ...
  if Sender.UserValue <> 0 then
    Key := 'GOODS ' + IntToStr(Cardinal(Sender.UserValue))
  else
    Key := '';
...
begin
  ...
  with GetByName('ButNext') as TGraphButtonGI do
    SetActive(False);
  for I := 1 to 15 do
    with GetByName('PanelM' + IntToFixedWidthWideString(I, 2)) do
      SetActive(False);
  ClearInfoContents;
  AddInfoSpacing(10);
  ...
  begin
    ...
    AddInfoHeading(
        WrapTextInColor(Galaxy.FormatTurnDate(Entry.Turn) + Source, '<color=255,240,100>'),
...
begin
  ...
  begin
    with GetByName('PanelM' + IntToFixedWidthWideString(I, 2)) do
      SetActive(False);
    if I <> 14 then
      with GetByName('M' + IntToFixedWidthWideString(I, 2) + 'Search') as TGraphButtonGI do
        UpCallback := SearchClicked;
  end;
...
var
  ...
  Good: Byte;
  function GetInfoQualityGrade(
      Quality: Single
  ): WideString; // @addr $59C664 @ida "void __userpurge $name(unsigned __int16 **Result@<eax>, float Quality@<^0>, void *ParentFrame@<^4>);" @stackpop 4 @calls "0x59CDCC" Nested in RunSearch.
  var
    I, Grade: Integer;
  ...
  function GetInfoEquipmentSummary(
      Item: TItem
  ): WideString; // @addr $59C760 @ida "void __usercall $name(TItem *Item@<eax>, unsigned __int16 **Result@<edx>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x59eada" Nested in RunSearch.
  var
    Detail, Text: WideString;
  begin
    Text := '';
    Text :=
        Text
            + FormatText1(
                LocalizedText('FormInfo.Equipment.Cost'),
                '<color=255,240,100>',
                '<val>',
                IntToStr(Item.GetConditionAdjustedCost));
    ...
    begin
      Detail :=
          FormatText1(LocalizedText('FormInfo.Equipment.Speed'), '', '<val>', '<Speed>')
              + ', '
              + FormatText1(LocalizedText('FormInfo.Equipment.Jump'), '', '<val>', '<Parsec>');
      TEquipment(Item).ReplaceInfoTokens(Detail, '<color=255,240,100>', nil);
      Text := Text + Detail;
    end
    ...
    begin
      Detail :=
          FormatText1(LocalizedText('FormInfo.Equipment.Droid'), '', '<val>', '<RecoverHitPoints>');
      TEquipment(Item).ReplaceInfoTokens(Detail, '<color=255,240,100>', nil);
      Text := Text + Detail;
    end
    ...
    begin
      Detail :=
          FormatText1(LocalizedText('FormInfo.Equipment.Hook'), '', '<val>', '<PickUpSize>')
              + ', '
              + FormatText1(
                  LocalizedText('FormInfo.Equipment.HookRadius'),
                  '',
                  '<val>',
                  '<Radius>');
      TEquipment(Item).ReplaceInfoTokens(Detail, '<color=255,240,100>', nil);
    ...
    begin
      if (Cardinal(TWeapon(Item).GetDamageFlags) and $100000) <> 0 then
        Detail :=
            FormatText1(
                    LocalizedText('FormInfo.Equipment.Damage'),
                    '',
                    '<min>-<max>',
                    '<MaxDamage><Bonus>')
                + ', '
      ...
      TEquipment(Item).ReplaceInfoTokens(Detail, '<color=255,240,100>', nil);
      Text := Text + Detail;
      if ((Item as TWeapon).SpecialModuleIndex <> 0)
          and (MicroModuleTemplates[(Item as TWeapon).SpecialModuleIndex - 1].TextReplace
              = WideString('')) then
        Text :=
            Text
                + #13#10
    ...
    begin
      Detail :=
          FormatText1(LocalizedText('FormInfo.Equipment.Protect'), '', '<val>', '<HitProtect>')
              + #13#10
      ...
      if THull(Item).HullSeries <> -1 then
        Text :=
            Text
                + #13#10
                + FormatText1(
                    LocalizedText('FormInfo.Equipment.Series'),
                    '<color=255,240,100>',
                    '<val>',
    ...
    if (Item is TEquipment) and (Item.ItemType in [t_FuelTanks..t_CustomWeapon, t_Satellite]) then
      Text :=
          Text
              + #13#10
              + FormatText1(
                  LocalizedText('FormInfo.Equipment.Reliability'),
                  '<color=255,240,100>',
                  '<val>',
                  GetInfoQualityGrade(TEquipment(Item).GetFragilityFactor([])));
    Result := Text;
  end;
  procedure AddInfoSearchResult(
      Value: TObject
  ); // @addr $59D3F4 @ida "void __usercall $name(TObject *Value@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x59f5c5,0x59f67f,0x59f738,0x59f7f2,0x59f8ac,0x59f965,0x59fa2d,0x59fb38,0x59fd60,0x59fecc,0x5a010e,0x5a038b,0x5a065f,0x5a2a95,0x5a2b79,0x5a2c1a,0x5a2cf4,0x5a2da9,0x5a2e6f,0x5a2efb,0x5a2ff1" Nested in RunSearch; captures location, count, and displayed objects.
  var
  ...
  begin
    if (Value is TShip)
        and (Ship <> nil)
        and not (Ship.OwnerId in [Ord(oiMaloc)..Ord(oiGaal), Ord(oiPirate)]) then
      Exit;
    ...
    begin
      ...
      begin
        AddInfoText(
            FormatText1(
                LocalizedText('FormInfo.ObjectFoundStart'),
                '<color=255,240,100>',
                '<Count>',
                IntToStr(GetSearchResultLimit)
            ),
            taxCenter,
            ''
        );
        ...
        begin
          ...
          GoodsText :=
              GoodsText
                  + #13#10
                  + '<td='
                  + IntToStr(GiScalePixels(5))
                  + '>'
                  + '<align=center>'
                  + WrapTextInColor(IntToStr(GoodIndex + 1), '')
                  + '.'
                  + '</align>';
          Color := '';
          GoodsText :=
              GoodsText
                  + '<td='
                  + IntToStr(GiScalePixels(15))
                  + '>'
                  + WideString('')
                  + WrapTextInColor(GoodsMarket[Good].DisplayName, Color)
                  + WideString('');
          GoodsText :=
              GoodsText
                  + '<td='
                  + IntToStr(GiScalePixels(160))
                  + '>'
                  + '<align=right>'
                  + WrapTextInColor(IntToStr(Station.ShopGoods[Good].Count), '')
                  + '</align>';
          GoodsText :=
              GoodsText
                  + '<td='
                  + IntToStr(GiScalePixels(205))
                  + '><align=right>'
                  + WrapTextInColor(IntToStr(GetPlayer.ShopGoodsPurchasePrice(Good, Station)), '')
                  + '</align>';
          GoodsText :=
              GoodsText
                  + '<td='
                  + IntToStr(GiScalePixels(215))
                  + '><align=center>'
                  + WrapTextInColor('/', '')
                  + '</align>';
          GoodsText :=
              GoodsText
                  + '<td='
                  + IntToStr(GiScalePixels(255))
                  + '><align=right>'
                  + WrapTextInColor(IntToStr(GetPlayer.ShopGoodsSellPrice(Good, Station)), '')
                  + '</align>';
        end;
        Description := Description + GoodsText;
        Heading :=
            WrapTextInColor(
                '- ' + WrapTextInColor(Station.GetFullName(' '), '<color=255,240,100>') + ' -',
                '<color=255,240,100>'
            );
        AddInfoHeading(
            Heading,
            Heading + #13#10 + Description,
            0,
            0,
            Integer(Cardinal(Station.Id) or $80000000)
        );
        AddInfoText(' .', taxCenter, '');
        AddInfoImageText(
            ExtractDelimitedPartW(Station.GetShipPortraitImagePath, 1, ','),
            Description
        );
      end
      ...
      begin
        Description :=
            FormatText1(
                ...
                '<color=255,240,100>',
                '<SectorName>',
                Ship.CurrentStar.Constellation.GetName
            );
        Description :=
            Description
                + #13#10
                + FormatText1(
                    ...
                    '<color=255,240,100>',
                    '<StarName>',
                    Ship.CurrentStar.Name);
        if Ship.CurrentPlanet <> nil then
          Description :=
              Description
                  + #13#10
                  + FormatText1(
                      LocalizedText('FormInfo.Planet'),
                      '<color=255,240,100>',
                      '<PlanetName>',
                      Ship.CurrentPlanet.Name);
        if Ship.IsInPrison then
          Description := Description + #13#10 + LocalizedText('FormInfo.InPrison');
        Ship.DaysSincePlayerSeen := 0;
        Heading :=
            WrapTextInColor(
                '- ' + WrapTextInColor(Ship.GetFullName(' '), '<color=255,240,100>') + ' -',
                '<color=255,240,100>'
            );
      ...
      begin
        Description :=
            FormatText1(
                ...
                '<color=255,240,100>',
                '<SectorName>',
                Planet.CurrentStar.Constellation.GetName
            );
        Description :=
            Description
                + #13#10
        ...
        if (Planet.IsCoalitionOwned or (Planet.OwnerId = Byte(oiPirate)))
            and (Planet.CurrentStar.Status.CustomFaction = WideString('')) then
        begin
          for GoodIndex := 0 to 7 do
          begin
            Good := GoodsTextOrder[GoodIndex];
            GoodsText :=
                GoodsText
                    ...
                    + '.'
                    + '</align>';
            if GoodsLegalOnPlanet[Good, Planet.RaceId, Ord(Planet.Government)]
            ...
            GoodsText :=
                GoodsText
                    ...
                    + '>'
                    + '<align=right>'
                    + WrapTextInColor(IntToStr(Planet.Goods[Good].Count), '')
                    + '</align>';
            GoodsText :=
                GoodsText
                    + '<td='
                    + IntToStr(GiScalePixels(205))
                    + '><align=right>'
                    + WrapTextInColor(IntToStr(GetPlayer.ShopGoodsPurchasePrice(Good, Planet)), '')
                    + '</align>';
            GoodsText :=
                GoodsText
                    + '<td='
            ...
            GoodsText :=
                GoodsText
                    ...
                    + IntToStr(GiScalePixels(255))
                    + '><align=right>'
                    + WrapTextInColor(IntToStr(GetPlayer.ShopGoodsSellPrice(Good, Planet)), '')
                    + '</align>';
          end;
        end;
        Description := Description + GoodsText;
        Heading :=
            WrapTextInColor(
                '- ' + WrapTextInColor(Planet.GetFullName(' '), '<color=255,240,100>') + ' -',
                '<color=255,240,100>'
            );
      ...
      begin
        ...
        Heading :=
            FormatText1(
                '- ' + LocalizedText('FormInfo.StarInfo.StarName') + ' -',
                '<color=255,240,100>',
                '<StarName>',
                (Value as TStar).Name
            );
        if TStar(Value).Status.CustomFaction <> WideString('') then
          Description :=
              Description
                  + #13#10
                  + LocalizedText(
                      'FormInfo.StarInfo.ControlledBy' + TStar(Value).Status.CustomFaction)
        else
          case TStar(Value).ControlFaction of
            sfCoalition:
              Description :=
                  Description + #13#10 + LocalizedText('FormInfo.StarInfo.ControlledByCoalition');
            sfPirates:
              Description :=
                  Description + #13#10 + LocalizedText('FormInfo.StarInfo.ControlledByPirates');
            sfDominators:
              Description :=
                  Description + #13#10 + LocalizedText('FormInfo.StarInfo.ControlledByDominators');
          end;
        ...
        begin
          ...
          Description :=
              Description
                  + #13#10
                  + FormatText1(
                      LocalizedText('FormInfo.StarInfo.RelativeLocationDist'),
                      '<color=255,240,100>',
                      '<Dist>',
                      IntToStr(
                          Round(
                              PointDistance(
                                  (Value as TStar).Position,
                                  GetPlayer.CurrentStar.Position
                              )
                          )
                      ));
          Bearing :=
              Round(PointBearingDegrees((Value as TStar).Position, GetPlayer.CurrentStar.Position));
          ...
          if Bearing = 1 then
            Description :=
                Description
                    + #13#10
                    + FormatText1(
                        LocalizedText('FormInfo.StarInfo.RelativeLocationAngle0'),
                        '<color=255,240,100>',
                        '<Angle>',
                        IntToStr(Bearing));
          if (Bearing > 1) and (Bearing <= 4) then
            Description :=
                Description
                    + #13#10
                    + FormatText1(
                        LocalizedText('FormInfo.StarInfo.RelativeLocationAngle1'),
                        '<color=255,240,100>',
                        '<Angle>',
                        IntToStr(Bearing));
          if Bearing > 4 then
            Description :=
                Description
                    + #13#10
                    + FormatText1(
                        LocalizedText('FormInfo.StarInfo.RelativeLocationAngle2'),
                        '<color=255,240,100>',
                        '<Angle>',
                        IntToStr(Bearing));
        end;
        if (Value as TStar).Battle <> 0 then
          Description := Description + #13#10 + LocalizedText('FormInfo.StarInfo.BattleInSystem');
        AddInfoHeading(Heading, Heading + #13#10 + Description, 0, 0, 0);
        AddInfoText(' .', taxCenter, '');
      ...
      begin
          ...
          Description :=
              Description
                  ...
                  + FormatText1(
                      ...
                      Ship.CurrentStar.Name);
          Description :=
              Description + #13#10 + WrapTextInColor(Ship.GetFullName(' '), '<color=255,240,100>');
        end;
        Heading :=
            WrapTextInColor(
                '- '
                    + WrapTextInColor(
                        RemoveTextTagsW(TItem(Value).GetDisplayName),
                        '<color=255,240,100>')
                    + ' ['
                    + WrapTextInColor(IntToStr(TItem(Value).Weight), '<color=0,255,0>')
                    + ']'
                    + ' -',
                '<color=255,240,100>'
            );
        ...
        if Planet <> nil then
          ...
          AddInfoHeading(
              ...
              0,
              0,
              Integer(Cardinal(Ship.Id) or $80000000)
          );
        AddInfoText(' .', taxCenter, '');
        if Value is THull then
          AddEquipmentInfoText(Value as TItem, Description)
        else
          AddItemInfoText(Value as TItem, Description);
      end;
      Found := True;
      Inc(ResultCount);
    end;
  end;
  procedure CheckInfoSearchResult(
      Value: TObject
  ); // @addr $59F4D8 @ida "void __usercall $name(TObject *Value@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x5a2aa1,0x5a2b85,0x5a2c29,0x5a2d03,0x5a2db5,0x5a2e7e,0x5a2f0a,0x5a2ffd" Nested in RunSearch; applies the active category filters.
  var
    Engine: TEngine;
    Fuel: TFuelTanks;
    Radar: TRadar;
    ...
    Droid: TRepairRobot;
    Hook: TCargoHook;
    Defense: TDefGenerator;
    CandidateStar: TStar;
    CandidatePlanet: TPlanet;
    CandidateStation: TRuins;
    Hull: THull;
    Weapon: TWeapon;
    CandidateShip: TShip;
    I, EffectiveRange: Integer;
    Entry: PExtraSpecial;
  begin
    if SelectedSearchCategory = 1 then
    begin
      ...
      begin
        ...
        if ((MinSpeed = 0) or (MinSpeed <= Engine.Speed))
            and ((RangeFilter = 0) or (Engine.JumpRange >= RangeFilter))
            and ((SizeFilter = 0) or (SizeFilter >= Engine.Weight))
            and ((MaxCost = 0) or (Engine.GetConditionAdjustedCost <= MaxCost))
            and (Engine.OwnerId in Owners) then
    ...
    begin
      ...
      begin
        ...
        if ((MinFuel = 0) or (Fuel.Capacity >= MinFuel))
            and ((SizeFilter = 0) or (Fuel.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Fuel.GetConditionAdjustedCost <= MaxCost))
            and (Fuel.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 3 then
    begin
      ...
      begin
        ...
        if ((RangeFilter = 0) or (Radar.Range >= RangeFilter))
            and ((SizeFilter = 0) or (Radar.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Radar.GetConditionAdjustedCost <= MaxCost))
            and (Radar.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 4 then
    begin
      ...
      begin
        ...
        if ((MinPower = 0) or (Scanner.ScanPower >= MinPower))
            and ((SizeFilter = 0) or (Scanner.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Scanner.GetConditionAdjustedCost <= MaxCost))
            and (Scanner.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 5 then
    begin
      ...
      begin
        ...
        if ((MinPower = 0) or (Droid.RepairPoints >= MinPower))
            and ((SizeFilter = 0) or (Droid.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Droid.GetConditionAdjustedCost <= MaxCost))
            and (Droid.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 6 then
    begin
      ...
      begin
        ...
        if ((MinPickup = 0) or (Hook.PickupPower >= MinPickup))
            and ((SizeFilter = 0) or (Hook.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Hook.GetConditionAdjustedCost <= MaxCost))
            and (Hook.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 7 then
    begin
      ...
      begin
        ...
        if ((MinDefense = 0)
                or ((Integer(DefenseDamageFactorToPercent(Defense.DamageFactor)) and $7F)
                    >= MinDefense))
            and ((SizeFilter = 0) or (Defense.Weight <= SizeFilter))
            and ((MaxCost = 0) or (Defense.GetConditionAdjustedCost <= MaxCost))
            and (Defense.OwnerId in Owners) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 8 then
    begin
      ...
      begin
        ...
        if (CandidateStar.ControlFaction = sfDominators) and not IncludeDominators then
          Exit;
        if (CandidateStar.ControlFaction = sfCoalition) and not IncludeCoalition then
          Exit;
        if (CandidateStar.ControlFaction = sfPirates) and not IncludePirates then
          Exit;
        if (CandidateStar.Status.CustomFaction <> WideString(''))
            and (not IncludeDominators or not IncludeCoalition or not IncludePirates) then
          Exit;
        if (RangeFilter <> 0)
            and (RangeFilter
                < Round(PointDistance(CandidateStar.Position, GetPlayer.CurrentStar.Position))) then
          Exit;
        AddInfoSearchResult(Value);
      end;
    end
    ...
    begin
      ...
      begin
        ...
        if (CandidatePlanet.OwnerId = Byte(oiDominator))
            and (CandidatePlanet.CurrentStar.Status.CustomFaction = WideString(''))
            and not IncludeDominators then
          Exit;
        if (CandidatePlanet.OwnerId in [Ord(oiMaloc)..Ord(oiGaal), Ord(oiPirate)])
            and (CandidatePlanet.CurrentStar.Status.CustomFaction = WideString(''))
            and not IncludeCoalition then
          Exit;
        if (CandidatePlanet.CurrentStar.Status.CustomFaction <> WideString(''))
            and (not IncludeDominators or not IncludeCoalition) then
          Exit;
        if (CandidatePlanet.OwnerId = Byte(oiUninhabited)) and not IncludeUninhabited then
          Exit;
        if (((CandidatePlanet.OwnerId = Byte(oiPirate))
                and (CandidatePlanet.CurrentStar.Status.CustomFaction = WideString(''))
                and (CandidatePlanet.OwnerId in Owners)
                and IncludeCoalition)
            or (RaceToOwner(CandidatePlanet.RaceId) in Owners)
            or (CandidatePlanet.CurrentStar.Status.CustomFaction <> WideString(''))
            or (not IncludeDominators and not IncludeCoalition)) then
        begin
          if (RangeFilter <> 0)
              and (RangeFilter
                  < Round(
                      PointDistance(
                          CandidatePlanet.CurrentStar.Position,
                          GetPlayer.CurrentStar.Position
                      ))) then
            Exit;
          if (ConstellationFilter <> WideString(''))
              and (FindLowercaseInfoText(
                      ConstellationFilter,
                      WideLowerCase(CandidatePlanet.CurrentStar.Constellation.GetName))
                  <= 0) then
            Exit;
          if (StarFilter <> WideString(''))
              and (FindLowercaseInfoText(
                      StarFilter,
                      WideLowerCase(CandidatePlanet.CurrentStar.Name))
                  <= 0) then
            Exit;
          AddInfoSearchResult(Value);
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          if StationTypes <> [] then
            Exit;
        end
        else if not (CandidateStation.TypeId in StationTypes) then
          Exit;
        if CandidateStation.OwnerId = Byte(oiDominator) then
          Exit;
        if CandidateStation.HasIndependentScriptFaction then
          Exit;
        if (RangeFilter <> 0)
            and (RangeFilter
                < Round(
                    PointDistance(
                        CandidateStation.CurrentStar.Position,
                        GetPlayer.CurrentStar.Position
                    ))) then
          Exit;
        if (ConstellationFilter <> WideString(''))
            and (FindLowercaseInfoText(
                    ConstellationFilter,
                    WideLowerCase(CandidateStation.CurrentStar.Constellation.GetName))
                <= 0) then
          Exit;
        if (StarFilter <> WideString(''))
            and (FindLowercaseInfoText(StarFilter, WideLowerCase(CandidateStation.CurrentStar.Name))
                <= 0) then
          Exit;
        AddInfoSearchResult(Value);
      end;
    ...
    begin
      ...
      begin
        Hull := Value as THull;
        if (Hull.OwnerId in Owners)
            and ((Hull.HullType <> htRanger) or IncludeRangerType)
            and ((Hull.HullType <> htWarrior) or IncludeWarriorType)
            and ((Hull.HullType <> htPirate) or IncludePirateType)
            and ((Hull.HullType <> htTransport) or IncludeTransportType)
            and ((Hull.HullType <> htLiner) or IncludeLinerType)
            and ((Hull.HullType <> htDiplomat) or IncludeDiplomatType)
            and ((SizeFilter = 0) or (Hull.Weight >= SizeFilter))
            and ((MinArmor = 0) or (Hull.Armor >= MinArmor))
            and ((MaxCost = 0) or (Hull.GetConditionAdjustedCost <= MaxCost))
            and ((WeaponSlots = 0) or (Hull.GetSlotCount(sskWeapon) >= WeaponSlots))
            and ((ScannerSlots = 0) or (Hull.GetSlotCount(sskScanner) >= ScannerSlots))
            and ((RadarSlots = 0) or (Hull.GetSlotCount(sskRadar) >= RadarSlots))
            and ((DroidSlots = 0) or (Hull.GetSlotCount(sskRepairRobot) >= DroidSlots))
            and ((HookSlots = 0) or (Hull.GetSlotCount(sskCargoHook) >= HookSlots))
            and ((DefenseSlots = 0) or (Hull.GetSlotCount(sskDefGenerator) >= DefenseSlots))
            and ((ArtifactSlots = 0) or (Hull.GetSlotCount(sskArtefact) >= ArtifactSlots))
            and ((AfterburnerSlots = 0)
                or (Hull.GetSlotCount(sskAfterburner) >= AfterburnerSlots)) then
          AddInfoSearchResult(Value);
      end;
    end
    else if SelectedSearchCategory = 12 then
    begin
      ...
      begin
        ...
        if not (Weapon.OwnerId in Owners) then
          Exit;
        ...
        begin
          ...
          if Weapon.GetWeaponInfo.ShotType in [wstTorpedo..wstRocket] then
            EffectiveRange :=
                Min(
                    EffectiveRange
                        - Weapon.CalculateGeneratedRange
                        + Weapon.GetWeaponInfo.MissileRange,
                    Weapon.GetWeaponInfo.MissileRange
                );
          if RangeFilter > EffectiveRange then
            Exit;
        end;
        if (SizeFilter <> 0) and (Weapon.Weight > SizeFilter) then
          Exit;
        if (MaxCost <> 0) and (Weapon.GetConditionAdjustedCost > MaxCost) then
          Exit;
        if ((Cardinal(Weapon.GetDamageFlags) and 1) <> 0) and not IncludeEnergy then
          Exit;
        if ((Cardinal(Weapon.GetDamageFlags) and 2) <> 0) and not IncludeSplinter then
          Exit;
        if (Weapon.GetWeaponInfo.ShotType in [wstTorpedo..wstRocket]) and not IncludeMissile then
          Exit;
        if MinDamage <> 0 then
        begin
          if (Cardinal(Weapon.GetDamageFlags) and $100000) <> 0 then
          begin
            if MinDamage > Max(Weapon.MaxDamage, Weapon.MinDamage) then
              Exit;
          end
          else if Weapon.MinDamage < MinDamage then
            Exit;
        end;
        if (MaxDamage <> 0) and (Weapon.MaxDamage < MaxDamage) then
          Exit;
        if (NameFilter <> WideString('')) and not ItemMatchesInfoSearch(Weapon, NameFilter) then
          Exit;
        AddInfoSearchResult(Value);
      end;
    end
    ...
    begin
      ...
      begin
        ...
        if (CandidateShip.OwnerId in Owners)
            and ((RaceToOwner(CandidateShip.PilotRace) in Owners) or (Owners = [Ord(oiPirate)]))
            and not CandidateShip.HasScriptStateText
            and (CandidateShip.TypeId in [stRanger..stWarrior])
            and ((CandidateShip.TypeNameOverrideKey = WideString(''))
                or (IncludeRangerType
                    and IncludeWarriorType
                    and IncludePirateType
                    and IncludeTransportType
                    and IncludeLinerType
                    and IncludeDiplomatType))
            and ((CandidateShip.TypeId <> stRanger) or IncludeRangerType)
            and ((CandidateShip.TypeId <> stWarrior) or IncludeWarriorType)
            and ((CandidateShip.TypeId <> stPirate) or IncludePirateType)
            and ((CandidateShip.TypeId <> stTransport)
                or ((CandidateShip as TTransport).TransportType <> ttTransport)
                or IncludeTransportType)
            and ((CandidateShip.TypeId <> stTransport)
                or ((CandidateShip as TTransport).TransportType <> ttLiner)
                or IncludeLinerType)
            and ((CandidateShip.TypeId <> stTransport)
                or ((CandidateShip as TTransport).TransportType <> ttDiplomat)
                or IncludeDiplomatType)
            and ((ConstellationFilter = WideString(''))
                or (FindLowercaseInfoText(
                        ConstellationFilter,
                        WideLowerCase(CandidateShip.CurrentStar.Constellation.GetName))
                    > 0))
            and ((StarFilter = WideString(''))
                or (FindLowercaseInfoText(StarFilter, WideLowerCase(CandidateShip.CurrentStar.Name))
                    > 0))
            and ((NameFilter = WideString(''))
                or (FindLowercaseInfoText(NameFilter, WideLowerCase(CandidateShip.GetFullName(' ')))
                    > 0)
                or (FindLowercaseInfoText(
                        NameFilter,
                        WideLowerCase(CandidateShip.GetLocalizedTypeName))
                    > 0)) then
          AddInfoSearchResult(Value);
      end;
    end;
  end;
  procedure ReadInfoSearchOwners(
      Category: Integer
  ); // @addr $5A06A0 @ida "void __usercall $name(int Category@<eax>, void *ParentFrame@<^0>);" @stackpop 0 @calls "0x5a0ccd,0x5a0d60,0x5a0df3,0x5a0e86,0x5a0f19,0x5a0fac,0x5a103f,0x5a11f5,0x5a1770,0x5a18fb,0x5a29f9" Nested in RunSearch; captures the owner filter set.
  begin
    Owners := [];
    if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'Maloc') as TGraphButtonGI)
        .Down then
      Include(Owners, Ord(oiMaloc));
    if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'Peleng') as TGraphButtonGI)
        .Down then
      Include(Owners, Ord(oiPeleng));
    if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'People') as TGraphButtonGI)
        .Down then
      Include(Owners, Ord(oiHuman));
    if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'Fei') as TGraphButtonGI)
        .Down then
      Include(Owners, Ord(oiFeyan));
    if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'Gaal') as TGraphButtonGI)
        .Down then
      Include(Owners, Ord(oiGaal));
    if FindControlByPath('M' + IntToFixedWidthWideString(Category, 2) + 'Pirate') <> nil then
      if not (GetByName('M' + IntToFixedWidthWideString(Category, 2) + 'Pirate') as TGraphButtonGI)
          .Down then
        Include(Owners, Ord(oiPirate));
    if Owners = [] then
      Owners := [Ord(oiPirate)];
  end;
begin
  ...
  begin
    ...
    AddInfoText(
        FormatText1(
            LocalizedText('FormInfo.NotMoney'),
            '<color=255,240,100>',
            '<Money>',
            IntToStr(3)
        ),
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Control <> nil) and (Control as TGraphButtonGI).Down then
          Include(StationTypes, StationType);
      end;
    ...
    begin
      ...
      WeaponSlots := 0;
      if (GetByName('M11S01') as TGraphButtonGI).Down then
        Inc(WeaponSlots);
      if (GetByName('M11S02') as TGraphButtonGI).Down then
        Inc(WeaponSlots);
      if (GetByName('M11S03') as TGraphButtonGI).Down then
        Inc(WeaponSlots);
      if (GetByName('M11S04') as TGraphButtonGI).Down then
        Inc(WeaponSlots);
      if (GetByName('M11S05') as TGraphButtonGI).Down then
        Inc(WeaponSlots);
      ScannerSlots := 0;
      if (GetByName('M11S08') as TGraphButtonGI).Down then
        Inc(ScannerSlots);
      RadarSlots := 0;
      if (GetByName('M11S09') as TGraphButtonGI).Down then
        Inc(RadarSlots);
      DroidSlots := 0;
      if (GetByName('M11S10') as TGraphButtonGI).Down then
        Inc(DroidSlots);
      HookSlots := 0;
      if (GetByName('M11S11') as TGraphButtonGI).Down then
        Inc(HookSlots);
      DefenseSlots := 0;
      if (GetByName('M11S12') as TGraphButtonGI).Down then
        Inc(DefenseSlots);
      ArtifactSlots := 0;
      if (GetByName('M11S13') as TGraphButtonGI).Down then
        Inc(ArtifactSlots);
      if (GetByName('M11S14') as TGraphButtonGI).Down then
        Inc(ArtifactSlots);
      if (GetByName('M11S15') as TGraphButtonGI).Down then
        Inc(ArtifactSlots);
      if (GetByName('M11S16') as TGraphButtonGI).Down then
        Inc(ArtifactSlots);
      ...
      AfterburnerSlots := 0;
      if (GetByName('M11S17') as TGraphButtonGI).Down then
        Inc(AfterburnerSlots);
    ...
    begin
      ...
      begin
        if ResultCount >= GetSearchResultLimit then
          Break;
        Star := TObject(GetPlayer.CurrentStar.StarDistances[I].Star) as TStar;
        if (Star.Id = 71) or (Star.Id = 72) then
          Continue;
        if (RangeFilter <> 0)
            and (RangeFilter
                < Round(PointDistance(Star.Position, GetPlayer.CurrentStar.Position))) then
          Continue;
        if Star.Status.CustomFaction = WideString('') then
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if ((MinGoodsCount = 0) or (Planet.Goods[Good].Count >= MinGoodsCount))
                    and GoodsSelected[Good]
                    and ((MaxBuyPrice = 0)
                        or (GetPlayer.ShopGoodsPurchasePrice(Good, Planet) <= MaxBuyPrice))
                    and ((MinSellPrice = 0)
                        or (GetPlayer.ShopGoodsSellPrice(Good, Planet) >= MinSellPrice)) then
                begin
                  Heading :=
                      Heading
                          + #13#10
                          ...
                          + '>'
                          + '<align=center>'
                          + WrapTextInColor(IntToStr(GoodsIndex + 1), '')
                          + '.'
                          + '</align>';
                  if GoodsLegalOnPlanet[Good, Planet.RaceId, Ord(Planet.Government)]
                      or (Planet.OwnerId = Byte(oiPirate)) then
                  ...
                  Heading :=
                      Heading
                          ...
                          + '>'
                          + WideString('')
                          + WrapTextInColor(GoodsMarket[Good].DisplayName, SearchText)
                          + WideString('');
                  Heading :=
                      Heading
                          + '<td='
          ...
          begin
            ...
            begin
              ...
              // Native exits here, bypassing the later list release.
              if (Station.OwnerId = Byte(oiDominator)) or Station.HasIndependentScriptFaction then
                Exit;
              ...
              begin
                ...
                begin
                  Good := GoodsTextOrder[GoodsIndex];
                  if ((MinGoodsCount = 0) or (Station.ShopGoods[Good].Count >= MinGoodsCount))
                      and GoodsSelected[Good]
                      and ((MaxBuyPrice = 0)
                          or (GetPlayer.ShopGoodsPurchasePrice(Good, Station) <= MaxBuyPrice))
                      and ((MinSellPrice = 0)
                          or (GetPlayer.ShopGoodsSellPrice(Good, Station) >= MinSellPrice)) then
                  begin
                    Heading :=
                        Heading
                            + #13#10
                    ...
                    Heading :=
                        Heading
                            ...
                            + '><align=right>'
                            + WrapTextInColor(
                                IntToStr(GetPlayer.ShopGoodsPurchasePrice(Good, Station)),
                                '')
                            + '</align>';
                    Heading :=
                        Heading
                    ...
                    Heading :=
                        Heading
                            ...
                            + '><align=right>'
                            + WrapTextInColor(
                                IntToStr(GetPlayer.ShopGoodsSellPrice(Good, Station)),
                                '')
                            + '</align>';
                  end;
                end;
    ...
    begin
      ...
      begin
        if SelectedSearchCategory = 0 then
        begin
          if FindLowercaseInfoText(SearchText, WideLowerCase(Star.Name)) > 0 then
            AddInfoSearchResult(Star);
        end
        else
        ...
        begin
          ...
          begin
            if SelectedSearchCategory = 0 then
            begin
              if (FindLowercaseInfoText(SearchText, WideLowerCase(Ship.GetFullName(' '))) > 0)
                  or (FindLowercaseInfoText(SearchText, WideLowerCase(Ship.GetLocalizedTypeName))
                      > 0) then
                AddInfoSearchResult(Ship);
            end
            else
            ...
            begin
              ...
              begin
                ...
                begin
                  if SelectedSearchCategory = 0 then
                  begin
                    if ItemMatchesInfoSearch(Item, SearchText) then
                      AddInfoSearchResult(Item);
                  end
                  else
        ...
        begin
          ...
          if SelectedSearchCategory = 0 then
          begin
            if FindLowercaseInfoText(SearchText, WideLowerCase(Planet.GetFullName(' '))) > 0 then
              AddInfoSearchResult(Planet);
          end
          else
          ...
          begin
            if (GetPlayer.CurrentPlanet <> nil)
                and (GetPlayer.CurrentPlanet = Planet)
                and (TemporaryShopSlots <> nil) then
            begin
              for ItemIndex := 0 to TemporaryShopSlots.Count - 1 do
              begin
                Item := TShopSlot(TemporaryShopSlots[ItemIndex]).Item;
              ...
              begin
                ...
                begin
                  if (FindLowercaseInfoText(SearchText, WideLowerCase(Ship.GetFullName(' '))) > 0)
                      or (FindLowercaseInfoText(
                              SearchText,
                              WideLowerCase(Ship.GetLocalizedTypeName))
                          > 0) then
                    AddInfoSearchResult(Ship);
                end
                else
    ...
    begin
      ...
      AddInfoText(
          FormatText1(
              LocalizedText('FormInfo.ObjectFoundEnd'),
              '<color=255,240,100>',
              '<Count>',
              IntToStr(ResultCount)
          ),
          taxCenter,
          ''
      );
...
begin
  ...
  begin
    ...
    EquipmentShopScreen.BuildHullSlotOverlays(
        InfoPanel,
        Item as THull,
        InfoPanel.ClientSize.X - 80,
        InfoContentHeight
    );
    Height := 90;
  end
  ...
  Caption := TLabelGI.Create(InfoPanel);
  if (GiResourceVariant = 2) or (Item is THull) then
    Caption.SetFontName(NormalFontName)
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if Button = nil then
        Exit;
      if Button.LocalPosition.Y >= ScrollPosition then
        Break;
      Inc(I);
    end;
    if Button <> nil then
...
begin
  Result := False;
  if (GetPlayer <> nil)
      and (GetPlayer.DockedTo <> nil)
      and (GetPlayer.DockedTo.TypeId = Byte(rstRangerCenter)) then
    Result := True;
end;
...
begin
  ...
  begin
    ...
    if (SelectedSearchCategory <> 0) and (SelectedSearchCategory <> PreviousSearchCategory) then
      HasSearchResults := False;
  end;
  if SelectedSearchCategory <> 0 then
    PreviousSearchCategory := SelectedSearchCategory;
  InfoPanel.SetActive(False);
  InfoPanel.VerticalScrollBar.SetActive(False);
  with GetByName('PanelSearch') do
    SetActive(SelectedSearchCategory = 0);
  for I := 1 to 15 do
    with GetByName('PanelM' + IntToFixedWidthWideString(I, 2)) do
      SetActive(SelectedSearchCategory = I);
  with GetByName('ButPrev') as TGraphButtonGI do
    SetDisabled(SelectedSearchCategory = 0);
  with GetByName('ButNext') as TGraphButtonGI do
    SetDisabled(
        not (((SelectedSearchCategory = 0) and ((PreviousSearchCategory <> 0) or HasSearchResults))
            or ((SelectedSearchCategory <> 0) and HasSearchResults))
    );
  FocusSearchField(True);
  ...
  begin
    ...
    AddInfoText(
        FormatText1(
            LocalizedColorText('FormInfo.SearchInfo3'),
            '<color=0,50,200>',
            '<Count>',
            IntToStr(GetSearchResultLimit)
        ),
...
begin
  if (SelectedSearchCategory = 0)
      and (PreviousSearchCategory <> 0)
      and (PreviousSearchCategory <> 14) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Button.ImageNormal <> nil)
          and (FindTextOffsetW(Button.ImageNormal.GetImagePath, 'Check') >= 0) then
      begin
        ...
        begin
          if (Caption <> Child)
              and (Caption is TLabelGI)
              and (X >= Caption.LocalPosition.X)
              and (X < Caption.LocalPosition.X + Caption.ClientSize.X)
              and (Y >= Caption.LocalPosition.Y)
              and (Y < Caption.LocalPosition.Y + Caption.ClientSize.Y) then
            Break;
          Caption := Caption.NextSibling;
        end;
        if Caption <> nil then
        begin
          Caption.UserValue := PtrInt(Button);
...
begin
  with TGraphButtonGI(Sender.UserValue) do
    SetHovered(True);
end;
...
begin
  with GetByName('TextSearch') as TEditGI do
    SetText('');
end;
...
begin
  with GetByName('M15Name') as TEditGI do
    SetText('');
end;
